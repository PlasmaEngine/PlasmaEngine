...
LightningDefineType(HeightMapEvent, builder, type)
{
  PlasmaBindDocumented();
}

// HeightPatch
LightningDefineType(HeightPatch, builder, type)
{
}
...
LightningDefineType(HeightMap, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDocumented();
  PlasmaBindDependency(Transform);
  LightningBindGetterProperty(Source);
  LightningBindGetterSetterProperty(UnitsPerPatch);
}
...
void HeightMap::Initialize(CogInitializer& initializer)
{
  if (PL::gRuntimeEditor)
    PL::gRuntimeEditor->Visualize(this, cHeightMapIcon);
...
void HeightMap::ApplyNoiseToPatch(HeightPatch* patch, float baseHeight, float frequency, float amplitude)
{
  ...
  for (size_t y = 0; y < HeightPatch::Size; ++y)
  {
    for (size_t x = 0; x < HeightPatch::Size; ++x)
    {
      ...
      float noiseHeight = Math::PerlinNoise(0.5, 1.0, 1.0, (float)absoluteIndex.x * frequency * FrequencyScale, (float)absoluteIndex.y * frequency * FrequencyScale);
...
void HeightMap::SaveToHeightMapSource(Serializer& stream)
{
  ...
  if (PL::gRuntimeEditor)
    mSource = (HeightMapSource*)PL::gRuntimeEditor->NewResourceOnWrite(HeightMapSourceManager::GetInstance(),
                                                                      LightningTypeId(HeightMap),
                                                                      "Source",
                                                                      GetSpace(),
                                                                      mSource,
                                                                      archetype,
...
Vec2 CellRayRange::IntersectPlane(Vec2Param planeDistance, Vec2Param rayStart, Vec2Param rayDir)
{
  ...
  // safeguard from plasma divisions, if we would plasma divide leave the distance
