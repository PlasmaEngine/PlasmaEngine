...
LightningDefineType(CogPathMetaSerialization, builder, type)
{
}
...
LightningDefineType(CogPath, builder, type)
{
  LightningBindDefaultCopyDestructor();
  LightningBindConstructor(StringParam);
  ...
  LightningBindGetterSetter(Path);
  LightningBindGetterSetter(Cog);

  // METAREFACTOR This should be a property, but we need a custom Cog display
  LightningBindGetterSetter(DirectCog);

  LightningBindGetterSetter(RelativeTo);
  LightningBindGetterSetterProperty(ErrorOnResolveToNull);
  LightningBindGetterSetterProperty(ErrorOnPathCantCompute);
  LightningBindGetterSetterProperty(ErrorOnDirectLinkFail);
  LightningBindGetterSetterProperty(UpdateCogOnPathChange);
  LightningBindGetterSetterProperty(UpdateCogOnInitialize);
  LightningBindGetterSetterProperty(UpdatePathOnCogChange);

  LightningBindMethod(Refresh);
  LightningBindMethod(RefreshIfNull);
  LightningBindMethod(Clone);

  PlasmaBindEvent(Events::CogPathCogChanged, CogPathEvent);

  LightningBindOverloadedMethod(Resolve, (Cog * (*)(Cog*, StringParam)));
  LightningBindOverloadedMethod(ComputePath, (String(*)(Cog*, Cog*, CogPathPreference::Enum)));

  LightningBindGetterSetterProperty(PathPreference0);
  LightningBindGetterSetterProperty(PathPreference1);
  LightningBindGetterSetterProperty(PathPreference2);
}
...
bool ValidateMetaIsCog(Cog* cog, Status& status)
{
  ...
  BoundType* meta = LightningVirtualTypeId(cog);
  if (meta != LightningTypeId(Cog))
  {
...
LightningDefineType(CogPathEvent, builder, type)
{
  LightningBindFieldProperty(mCogPath);
}
...
bool Policy<CogPath>::Serialize(Serializer& stream, cstr fieldName, CogPath& path)
{
  ...
  if (stream.Start("CogPath", fieldName, StructureType::Object))
  {
    ...
    if (!stream.EnumField("CogPathPreference",
                          ...
                          LightningTypeId(CogPathPreference::Enum)))
...
LightningDefineType(CogPathMetaComposition, builder, type)
{
}

CogPathMetaComposition::CogPathMetaComposition() : MetaComposition(LightningTypeId(Component))
{
...
Handle CogPathMetaComposition::GetComponent(HandleParam owner, BoundType* componentType)
{
  ...
  if (Cog* cog = cogPath->GetCog())
  {
    ...
    // problem when building a Lightning::Call. Don't do this if the component is
