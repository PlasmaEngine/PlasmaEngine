...
LightningDefineType(GamepadEvent, builder, type)
{
  PlasmaBindDocumented();

  LightningBindFieldProperty(mGamepad);
  LightningBindFieldProperty(mButton);
  LightningBindFieldProperty(mFlickDirection);
  LightningBindFieldProperty(mFlickedStick);
}
...
LightningDefineType(Gamepad, builder, type)
{
  type->HandleManager = LightningManagerId(PointerManager);
  PlasmaBindDocumented();

  LightningBindFieldProperty(mIsActive);
  LightningBindFieldProperty(mGamepadIndex);

  LightningBindFieldProperty(mLeftStick);
  LightningBindFieldProperty(mRightStick);
  LightningBindFieldProperty(mLeftStickDelta);
  LightningBindFieldProperty(mRightStickDelta);

  LightningBindFieldProperty(mLeftTrigger);
  LightningBindFieldProperty(mRightTrigger);

  LightningBindMethod(IsButtonPressed);
  LightningBindMethod(IsButtonHeld);
  LightningBindMethod(IsButtonReleased);
  LightningBindMethod(TimeButtonHeld);

  LightningBindMethod(Vibrate);

  PlasmaBindEvent(Events::ButtonDown, GamepadEvent);
  PlasmaBindEvent(Events::ButtonUp, GamepadEvent);
  PlasmaBindEvent(Events::GamepadUpdated, ObjectEvent);
  PlasmaBindEvent(Events::GamepadStickFlicked, GamepadEvent);
}
...
LightningDefineType(Gamepads, builder, type)
{
  type->HandleManager = LightningManagerId(PointerManager);
  PlasmaBindDocumented();
  LightningBindMethod(PauseVibration);
  LightningBindMethod(ResumeVibration);
  LightningBindMethod(GetGamePad);
  LightningBindGetterProperty(MaxGamepadCount);
  PlasmaBindEvent(Events::GamepadsUpdated, ObjectEvent);
}
...
void Gamepads::Startup()
{
  ...
  ConnectThisTo(PL::gEngine, Events::EngineUpdate, OnUpdate);
  ConnectThisTo(PL::gEngine, Events::OsDeviceChanged, OnDeviceChanged);
