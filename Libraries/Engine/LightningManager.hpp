...
public:
  ...
  void OnDebuggerPause(Lightning::DebuggerEvent* event);

  // When the debugger resumes execution
  void OnDebuggerResume(Lightning::DebuggerEvent* event);

  // When the debugger is paused on a line and updates
  void OnDebuggerPauseUpdate(Lightning::DebuggerEvent* event);

  // When the debugger skips a breakpoint
  void OnDebuggerBreakNotAllowed(Lightning::DebuggerTextEvent* event);

  // The last library we properly built (set inside
  // CompileLoadedScriptsIntoLibrary) Once this library becomes in use by an
  // executable state, we CANNOT update it, or any LightningMeta types
