...
LightningDefineType(GameEvent, builder, type)
{
  PlasmaBindDocumented();
  LightningBindFieldProperty(LevelName);
  LightningBindFieldProperty(mGame);
  LightningBindFieldProperty(mSpace);
  LightningBindFieldProperty(mHandled);
}

LightningDefineType(GameSession, builder, type)
{
  PlasmaBindDocumented();
  ...
  LightningBindMethod(Start);
  LightningBindMethod(Quit);
  LightningBindMethod(RequestQuit);
  LightningBindMethod(CreateSpace);
  LightningBindMethod(CreateNamedSpace);
  LightningBindMethod(Pause);

  LightningBindMethod(FindSpaceByName);
  LightningBindMethod(FindAllSpacesByName);
  LightningBindMethod(IsEditorMode);
  LightningBindGetter(AllSpaces);

  LightningBindGetter(Focused);
  LightningBindGetter(Resolution);
  LightningBindGetter(FullScreen);

  LightningBindField(mPaused);

  PlasmaBindEvent(Events::GameSetup, GameEvent);
  PlasmaBindEvent(Events::GameLoad, GameEvent);

  PlasmaBindEvent(Events::GameFocusIn, GameEvent);
  PlasmaBindEvent(Events::GameFocusOut, GameEvent);

  PlasmaBindEvent(Events::GameQuit, GameEvent);
  PlasmaBindEvent(Events::GameRequestQuit, GameEvent);
  PlasmaBindEvent(Events::GameStarted, GameEvent);

  PlasmaBindEvent(Events::LevelStarted, GameEvent);
  PlasmaBindEvent(Events::EditSpaces, Event);
}
...
GameSession::GameSession()
{
  ...
  mStarted = false;
  mMainWindow = nullptr;
  PL::gEngine->mGameSessions.PushBack(this);
}

GameSession::~GameSession()
{
  Mouse::GetInstance()->SetTrapped(false);
  PL::gEngine->mGameSessions.EraseValue(this);
}
...
void GameSession::Initialize(CogInitializer& initializer)
{
  ...
  if (!IsEditorMode())
    PlasmaPrintFilter(Filter::DefaultFilter, "---------------- Begin Game ----------------\n");
...
Space* GameSession::CreateSpaceFlags(Archetype* archetype, CreationFlags::Type flags)
{
  ...
  Space* space = PL::gFactory->CreateSpace(archetype->ResourceIdName, flags, this);
...
Space* GameSession::CreateNamedSpace(StringParam name, Archetype* archetype)
{
  ...
  Space* space = PL::gFactory->CreateSpace(archetype->ResourceIdName, CreationFlags::Default, this);
  if (space && !space->GetMarkedForDestruction())
  {
    InternalAdd(space);
    space->SetName(name);
...
bool GameSession::GetFocused()
{
  if (PL::gRuntimeEditor)
    return PL::gRuntimeEditor->HasFocus(this);
  else if (mMainWindow)
...
{
  if (!PL::gRuntimeEditor || IsEditorMode() || mTrackedSpaces.Empty())
    return;

  Level* editLevel = PL::gRuntimeEditor->GetEditingLevel();
  Space* focusSpace = nullptr;
  forRange (Space* space, mTrackedSpaces.AllValues())
  {
    PL::gRuntimeEditor->OpenEditorViewport(space);
  ...
  // first space
  if (focusSpace)
    PL::gRuntimeEditor->SetFocus(focusSpace);
  else
    PL::gRuntimeEditor->SetFocus(mTrackedSpaces.Front().second);
