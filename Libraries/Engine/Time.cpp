...
LightningDefineType(UpdateEvent, builder, type)
{
  PlasmaBindDocumented();
  LightningBindFieldProperty(Dt);
  LightningBindFieldProperty(TimePassed);
  LightningBindFieldProperty(RealTimePassed);
}
...
LightningDefineType(TimeSpace, builder, type)
{
  ...
  PlasmaBindEvent(Events::LogicUpdate, UpdateEvent);
  PlasmaBindEvent(Events::FrameUpdate, UpdateEvent);
  PlasmaBindEvent(Events::PreviewUpdate, UpdateEvent);

  PlasmaBindDependency(Space);

  LightningBindMethod(TogglePause);
  LightningBindMethod(Step);
  LightningBindGetter(GloballyPaused);
  LightningBindGetter(DtOrZero);
  LightningBindGetterSetterProperty(TimeMode);
  LightningBindFieldProperty(mMinDt);
  LightningBindFieldProperty(mMaxDt);
  LightningBindGetterSetterProperty(TimeScale);
  LightningBindFieldProperty(mPaused);
  LightningBindFieldGetterAs(mScaledClampedDt, "Dt");
  LightningBindFieldGetter(mRealDt);
  LightningBindFieldAs(mScaledClampedTimePassed, "TimePassed");
...
{
  mTimeSystem = PL::gEngine->has(TimeSystem);
...
void TimeSpace::Update(float dt)
{
  ...
  Array<NotificationCallback>& cb = PL::gNotifyCallbackStack;
...
LightningDefineType(TimeSystem, builder, type)
{
}
