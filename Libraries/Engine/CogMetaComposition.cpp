...
LightningDefineType(CogMetaComposition, builder, type)
{
}

CogMetaComposition::CogMetaComposition() : MetaComposition(LightningTypeId(Component))
{
}
...
bool CogMetaComposition::CanAddComponent(HandleParam owner, BoundType* typeToAdd, AddInfo* info)
{
  ...
  if (typeToAdd == LightningTypeId(Transform))
  {
...
{
  Component* component = LightningAllocate(Component, typeToCreate);
...
bool CogMetaComposition::CanRemoveComponent(HandleParam owner, HandleParam component, String& reason)
{
  ...
  // pointer.
  if (typeToRemove->IsA(LightningTypeId(Transform)))
  {
