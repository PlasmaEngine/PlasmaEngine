...
LightningDefineType(Factory, builder, type)
{
}

Factory* Factory::StaticInitialize(Engine* engine, Tracker* tracker)
{
  ErrorIf(PL::gFactory != nullptr, "Factory already created.");
  PL::gFactory = new Factory(engine, tracker);
  return PL::gFactory;
}
...
{
  BoundType* boundType = LightningTypeId(type);
  if (node.RuntimeType != nullptr)
  {
...
void ComponentPropertyPatched(cstr fieldName, void* clientData)
{
  ...
  if (*fieldName == 'm')
    ++fieldName;
  Property* property = LightningVirtualTypeId(component)->GetProperty(fieldName);
  if (property == nullptr)
    return;
...
Cog* Factory::BuildFromStream(CogCreationContext* context, Serializer& stream)
{
  ...
  if (stream.GetPolymorphic(cogNode))
  {
    ...
    {
      gameObject = LightningAllocate(Cog, compositionMeta, HeapFlags::NonReferenceCounted);
    ...
    while (stream.GetPolymorphic(componentNode))
    {
      ...
      else
      {
        ...
        if (componentMeta != nullptr && !componentMeta->IsA(LightningTypeId(Component)))
        {
        ...
        {
          // If Lightning hasn't fully compiled due to script errors or
          // because it's waiting on plugins then don't emit proxy errors.
          bool compileSuccess = LightningManager::GetInstance()->mLastCompileResult == CompileResult::CompilationSucceeded;
        ...
        if (component == nullptr && !subtractiveNode)
        {
          ...
          if (canAdd)
          {
            ...
            component = LightningAllocate(Component, componentMeta, HeapFlags::NonReferenceCounted);
    ...
    BoundType* expectedType, Space* space, StringParam filename, uint flags, GameSession* gameSession)
{
  ...
  if (cog != nullptr)
  {
    ...
    if (expectedType == LightningTypeId(GameSession))
    {
...
{
  return (Space*)CreateCheckedType(LightningTypeId(Space), nullptr, filename, flags, gameSession);
}
...
Cog* Factory::Create(Space* space, StringParam filename, uint flags, GameSession* gameSession)
{
  ...
  Cog* cog = BuildAndSerialize(LightningTypeId(Cog), filename, &context);
  if (cog != nullptr)
  {
    cog->Initialize(initializer);
  }
