...
LightningDefineType(ResourceEvent, builder, type)
{
}
...
void AddIdAndCheckConflicts(ResourceManager::ResourceIdMapType& resourceIdMap, ResourceEntry& entry, Resource* resource)
{
  ...
  if (!idrange.Empty())
  {
    if (PL::gRuntimeEditor)
      PL::gRuntimeEditor->OnResourceIdConflict(entry, idrange.Front().second);
    else
      PlasmaPrint("Resource Id Conflict %s\n", entry.Name.c_str());
  }
  else
  {
...
ResourceManager::ResourceManager(BoundType* resourceType)
{
  ...
  PL::gResources->RegisterManager(this);
  mAddSortWeight = 100;
}
...
Resource* ResourceManager::CreateRuntimeInternal(StringParam name)
{
  ...
  AddIdAndCheckConflicts(PL::gResources->ResourceIdMap, entry, resource);
...
void ResourceManager::AddResource(ResourceEntry& entry, Resource* resource)
{
  ...
  resource->mBuilderType = LightningVirtualTypeId(entry.mBuilder);
  ...
  PL::gResources->DispatchEvent(Events::ResourceAdded, &event);
}

void ResourceManager::AddLoader(StringParam name, ResourceLoader* loader)
{
  PL::gResources->AddLoader(name, loader);
}
...
void ResourceManager::Remove(Resource* resource, RemoveMode::Enum removeMode)
{
  ...
  PL::gResources->DispatchEvent(Events::ResourceRemoved, &event);
  ...
  PL::gResources->ResourceIdMap.Erase(resource->mResourceId);
}
...
Resource* ResourceManager::GetResourceNameOrId(StringRange resourceName, ResourceId resourceId)
{
  ...
  // Either name was not found or the id is plasma which is never used
