...
LightningDefineType(OsWindow, builder, type)
{
  LightningBindGetterSetterProperty(MonitorClientPosition);
  LightningBindSetter(MinClientSize);
  LightningBindGetterSetterProperty(ClientSize);
  LightningBindGetterProperty(Parent);
  // Seems to be problematic to expose because they can set some dangerous stuff
  // LightningBindGetterSetterProperty(Style);
  LightningBindGetterSetterProperty(Visible);
  LightningBindSetter(Title);
  LightningBindGetterSetterProperty(State);

  LightningBindMethod(HasFocus);
  // Currently behaves weird when called from script when the window doesn't
  // have focus
  // LightningBindMethod(TakeFocus);
  LightningBindSetter(MouseCapture);
  LightningBindGetterSetter(MouseTrap);

  LightningBindMethod(MonitorToClient);
  LightningBindMethod(ClientToMonitor);
}
...
{
  OsWindow* self = (OsWindow*)window->mUserData;
  PL::gMouse->mRawMovement += ToVec2(movement);
}
    ...
    PlatformInputDevice& device, uint buttons, const Array<uint>& axes, const DataBlock& data, ShellWindow* window)
{
  ...
  Joystick* joystick = PL::gJoysticks->GetJoystickByDevice(device.mDeviceHandle);
...
LightningDefineType(OsWindowEvent, builder, type)
{
}
...
LightningDefineType(OsMouseEvent, builder, type)
{
}
...
LightningDefineType(OsWindowBorderHitTest, builder, type)
{
}
...
LightningDefineType(OsMouseDropEvent, builder, type)
{
}
