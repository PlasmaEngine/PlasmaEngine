...
PatchResolveMethod::Enum ObjectLoader::ResolveInheritedData(StringRange inheritId, DataNode*& result)
{
  ...
  Resource* resource = PL::gResources->GetResourceByName(inheritId);
  if (!resource)
...
void AddDependencies(DataNode* parent, BoundType* type, DataNode* addLocation)
{
  ...
  forRange (CogComponentMeta* metaComponent, type->HasAll<CogComponentMeta>())
  {
    forRange (BoundType* dependency, metaComponent->mDependencies.All())
    {
      ...
      if (dependency == LightningTypeId(Cog) || dependency == LightningTypeId(Space) || dependency == LightningTypeId(GameSession))
      {
        continue;
      }
...
CachedModifications::ObjectNode* CachedModifications::FindChildNode(Object* rootObject, Object* childObject)
{
  ...
  {
    ++pathSize;
    BoundType* currType = LightningVirtualTypeId(currObject);
...
{
  MetaComposition* composition = LightningVirtualTypeId(instance)->Has<MetaComposition>();
...
void CachedModifications::ApplyModificationsToObjectInternal(Object* object, ObjectNode* objectNode, bool combine)
{
  ...
  BoundType* objectType = LightningVirtualTypeId(object);
