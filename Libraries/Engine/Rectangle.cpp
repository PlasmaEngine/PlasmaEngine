...
LightningDefineType(Thickness, builder, type)
{
  type->CreatableInScript = true;

  LightningBindDestructor();
  LightningBindConstructor(float, float, float, float);
  LightningBindConstructor(Vec4);
  LightningBindConstructor(float, float);
  LightningBindConstructor(Vec2);

  LightningBindMethod(All);

  LightningBindFieldProperty(Left);
  LightningBindFieldProperty(Top);
  LightningBindFieldProperty(Right);
  LightningBindFieldProperty(Bottom);
  LightningBindFieldGetter(cZero);

  LightningBindMethod(Size);
  LightningBindMethod(TopLeft);
}
...
LightningDefineType(Rectangle, builder, type)
{
  type->CreatableInScript = true;
  ...
  LightningBindMethod(SetSize);
  LightningBindOverloadedMethod(ResizeToPoint, LightningInstanceOverload(void, Location::Enum, float));
  LightningBindOverloadedMethod(ResizeToPoint, LightningInstanceOverload(void, Location::Enum, Vec2Param));
  LightningBindOverloadedMethod(ResizeToPoint, LightningInstanceOverload(void, Location::Enum, Vec2Param, Vec2Param));
  LightningBindMethod(Expand);

  LightningBindOverloadedMethod(Transform, LightningInstanceOverload(void, Mat2Param));
  LightningBindOverloadedMethod(Transform, LightningInstanceOverload(void, Mat3Param));
  LightningBindOverloadedMethod(Transform, LightningInstanceOverload(void, Mat4Param));
  LightningBindOverloadedMethod(Transformed, LightningConstInstanceOverload(Rectangle, Mat2Param));
  LightningBindOverloadedMethod(Transformed, LightningConstInstanceOverload(Rectangle, Mat3Param));
  LightningBindOverloadedMethod(Transformed, LightningConstInstanceOverload(Rectangle, Mat4Param));

  LightningBindGetterSetterProperty(TopLeft);
  LightningBindGetterSetterProperty(TopRight);
  LightningBindGetterSetterProperty(BottomLeft);
  LightningBindGetterSetterProperty(BottomRight);
  LightningBindGetterSetterProperty(Center);

  LightningBindGetterSetterProperty(Left);
  LightningBindGetterSetterProperty(Right);
  LightningBindGetterSetterProperty(Top);
  LightningBindGetterSetterProperty(Bottom);

  LightningBindOverloadedMethod(Contains, LightningConstInstanceOverload(bool, Vec2Param));
  LightningBindOverloadedMethod(Contains, LightningConstInstanceOverload(bool, RectangleParam));
  LightningBindMethodAs(Overlap, "Overlaps");
  ...
  LightningBindMethod(RemoveThickness);

  LightningBindMethod(GetCardinalLocation);
  LightningBindOverloadedMethod(SetLocation, LightningInstanceOverload(void, Location::Enum, float));
  LightningBindMethod(GetLocation);
  LightningBindOverloadedMethod(SetLocation, LightningInstanceOverload(void, Location::Enum, Vec2Param));

  type->ToStringFunction = Lightning::BoundTypeToGlobalToString<Rectangle>;
}
