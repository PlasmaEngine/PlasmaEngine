...
LightningDefineType(CogInitializerEvent, builder, type)
{
  PlasmaBindDocumented();
  LightningBindFieldProperty(mCogInitializer);
}

LightningDefineType(CogInitializer, builder, type)
{
  LightningBindGetterProperty(Space);
  LightningBindGetterProperty(Parent);
  LightningBindGetterProperty(GameSession);
  LightningBindMethod(DispatchEvent);
  PlasmaBindEvent(Events::AllObjectsInitialized, CogInitializerEvent);
}

CogInitializer::CogInitializer(Plasma::Space* space, Plasma::GameSession* gameSession)
{
  mSpace = space;
