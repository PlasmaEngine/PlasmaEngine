...
LightningDefineType(CogSerializationFilter, builder, type)
{
}

bool CogSerializationFilter::ShouldSerialize(Object* object)
{
  bool isCog = !LightningVirtualTypeId(object)->IsA(LightningTypeId(Cog));
...
// Cog Meta Operations
LightningDefineType(CogMetaOperations, builder, type)
{
}
...
LightningDefineType(CogMetaDataInheritance, builder, type)
{
}

String CogMetaDataInheritance::GetInheritId(HandleParam instance, InheritIdContext::Enum context)
{
  ReturnIf(!instance.StoredType->IsA(LightningTypeId(Cog)), "", "Expected Cog");
...
// Cog Meta Transform
LightningDefineType(CogMetaTransform, builder, type)
{
}
...
MetaTransformInstance CogMetaTransform::GetInstance(HandleParam object)
{
  ...
  if (transform != nullptr)
  {
    ...
    type = LightningTypeId(Transform);
  }
  else
  {
    ...
    if (objectLink != nullptr)
    {
      ...
      type = LightningTypeId(ObjectLink);
    }
  }
...
// Archetype Extension
LightningDefineType(CogArchetypeExtension, builder, type)
{
}

// Cog Meta Display
LightningDefineType(CogMetaDisplay, builder, type)
{
}
...
String CogMetaDisplay::GetName(HandleParam object)
{
  ...
  if (Archetype* archetype = cog->GetArchetype())
    ...
    return String::Format("(%s)[%d]", LightningTypeId(Cog)->Name.c_str(), cog->GetId().Id);
}
...
LightningDefineType(CogMetaSerialization, builder, type)
{
}
...
LightningDefineType(CogArchetypePropertyFilter, builder, type)
{
}
