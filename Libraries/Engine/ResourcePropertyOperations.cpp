...
void GetResourcesFromProperties(HandleParam object, HashSet<Resource*>& resources)
{
  ...
  // Originally the range class allocated to store values safely for Lightning...
  // (does it actually matter for Lightning, since all types are safe?)
  ...
  forRange (Property* metaProperty, objectType->GetProperties())
  {
    ...
    if (propType->IsA(LightningTypeId(Resource)) == false)
      continue;
  ...
  if (MetaComposition* composition = objectType->has(MetaComposition))
  {
    ...
    for (uint i = 0; i < componentCount; ++i)
    {
      ...
      if (fromResource != nullptr && fromResource->mResourceId != 0)
      {
        ...
        Resource* resource = PL::gResources->GetResource(fromResource->mResourceId);
        if (resource)
          resources.Insert(resource);
      }
