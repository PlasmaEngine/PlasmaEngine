...
{
public:
  LightningDeclareType(SimpleResourceFactory, TypeCopyMode::ReferenceType);
  SimpleResourceFactory() : MetaComposition(LightningTypeId(BlockType)), mDeleteMemory(false){};

  SimpleResourceFactory(bool deleteMemory) : MetaComposition(LightningTypeId(BlockType))
  {
  ...
  {
    Handle handle = LightningAllocate(BlockType, blockMeta);
