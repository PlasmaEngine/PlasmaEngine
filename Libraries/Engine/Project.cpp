...
LightningDefineType(ProjectSettings, builder, type)
{
  PlasmaBindComponent();
  ...
  LightningBindFieldGetter(ProjectName);
  LightningBindFieldGetter(ProjectFolder);
  LightningBindFieldGetter(ContentFolder);
  LightningBindFieldGetter(EditorContentFolder);

  LightningBindFieldProperty(ProjectOwner);
  LightningBindFieldProperty(AutoTakeProjectScreenshot);
}
...
void ProjectSettings::Save()
{
  if (ProjectFile.Empty())
    ProjectFile = FilePath::CombineWithExtension(ProjectFolder, ProjectName, ".plasmaproj");
...
LightningDefineType(ContentLibraryReference, builder, type)
{
  LightningBindFieldProperty(mContentLibraryName);
}
...
LightningDefineType(SharedContent, builder, type)
{
  PlasmaBindComponent();
  // PlasmaBindSetup(SetupMode::DefaultSerialization);

  LightningBindFieldProperty(ExtraContentLibraries);
}
...
LightningDefineType(ProjectDescription, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  LightningBindFieldProperty(ShortDescription);
  LightningBindFieldProperty(LongDescription);
  LightningBindFieldProperty(Tags);
}
...
LightningDefineType(WindowLaunchSettings, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindDocumented();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  ...
  // LightningBindFieldProperty(mUseLaunchOptionsPopup)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindFieldProperty(mLaunchFullscreen);   //->PlasmaFilterEquality(mUseLaunchOptionsPopup,
                                               // bool, false);
...
LightningDefineType(FrameRateSettings, builder, type)
{
  ...
  LightningBindFieldProperty(mVerticalSync);
  LightningBindFieldProperty(mLimitFrameRate);
  LightningBindGetterSetterProperty(FrameRate);
}
...
LightningDefineType(LauncherProjectInfoProxy, builder, type)
{
  PlasmaBindComponent();
  type->AddAttribute(ObjectAttributes::cCore);
}

LightningDefineType(DebugSettings, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindDocumented();
  PlasmaBindSetup(SetupMode::DefaultSerialization);

  LightningBindGetterSetterProperty(MaxDebugObjects);
}
...
LightningDefineType(ExportSettings, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindDocumented();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
