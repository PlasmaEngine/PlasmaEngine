...
LightningDefineTemplateType(ProxyObject<Component>, builder, type)
{
}

// Component
LightningDefineType(Component, builder, type)
{
  LightningBindDefaultConstructor();
  type->HandleManager = LightningManagerId(ComponentHandleManager);
...
void ComponentHandleManager::ObjectToHandle(const byte* object, BoundType* type, Handle& handleToInitialize)
{
  ...
  ComponentHandleData& data = *(ComponentHandleData*)(handleToInitialize.Data);
  data.mComponentType = LightningVirtualTypeId(component);
