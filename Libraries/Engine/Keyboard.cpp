...
LightningDefineType(Keyboard, builder, type)
{
  PlasmaBindDocumented();
  type->HandleManager = LightningManagerId(PointerManager);

  LightningBindMethod(IsAnyKeyDown);
  LightningBindMethod(IsAnyNonModifierDown);
  LightningBindMethod(KeyIsDown);
  LightningBindMethod(KeyIsUp);
  LightningBindMethod(KeyIsPressed);
  LightningBindMethod(KeyIsReleased);

  LightningBindMethod(GetKeyName);

  LightningBindOverloadedMethod(Valid, LightningInstanceOverload(bool, Keys::Enum));
  LightningBindOverloadedMethod(Valid, LightningInstanceOverload(bool, StringParam));

  LightningBindMethod(ToKey);
  LightningBindOverloadedMethod(ToSymbol, LightningInstanceOverload(String, Keys::Enum));
  LightningBindOverloadedMethod(ToSymbol, LightningInstanceOverload(String, StringParam));

  PlasmaBindEvent(Events::KeyUp, KeyboardEvent);
  PlasmaBindEvent(Events::KeyDown, KeyboardEvent);
  PlasmaBindEvent(Events::KeyRepeated, KeyboardEvent);
  PlasmaBindEvent(Events::TextTyped, KeyboardTextEvent);
}
...
LightningDefineType(KeyboardEvent, builder, type)
{
  PlasmaBindDocumented();
  LightningBindFieldProperty(Key);
  LightningBindFieldProperty(State);
  LightningBindFieldProperty(ShiftPressed);
  LightningBindFieldProperty(AltPressed);
  LightningBindFieldProperty(CtrlPressed);
  LightningBindFieldProperty(SpacePressed);
  LightningBindFieldPropertyAs(HandledEventScript, "HandledEvent");
  LightningBindGetterProperty(Keyboard);
  LightningBindFieldProperty(OsKey);
  LightningBindGetterProperty(ModifierPressed);
}
...
// KeyboardTextEvent
LightningDefineType(KeyboardTextEvent, builder, type)
{
  PlasmaBindDocumented();
  LightningBindFieldProperty(mRune);
}
