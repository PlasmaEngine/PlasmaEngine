...
// Archetype
LightningDefineType(Archetype, builder, type)
{
  PlasmaBindDocumented();
  LightningBindField(mStoredType);
}
                                                  ...
                                                  Archetype* inheritedArchetype)
{
  ...
  if (archetype)
  {
    ...
    if (archetype->mStoredType != LightningVirtualTypeId(cog))
    {
...
bool ArchetypeManager::SaveToContent(Cog* object, Archetype* archetype, ResourceId addResourceId)
{
  ...
  archetype->mStoredType = LightningVirtualTypeId(object);
  ...
  if (archetype->mContentItem == nullptr)
  {
    ...
    PL::gRuntimeEditor->AddResource(this, resourceAdd);
