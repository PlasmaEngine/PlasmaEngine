...
LightningDefineType(ObjectLinkEdge, builder, type)
{
  ...
  LightningBindGetterProperty(SelfCog);
  LightningBindGetterProperty(OtherCog);
  LightningBindGetterProperty(ObjectLink);
}
...
LightningDefineType(ObjectLinkAnchor, builder, type)
{
  PlasmaBindComponent();
  type->AddAttribute(ObjectAttributes::cHidden);

  PlasmaBindDocumented();
  LightningBindGetterProperty(ObjectLinks);
}
...
LightningDefineType(ObjectLink, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDocumented();

  LightningBindGetterSetterProperty(ObjectAPath)->AddAttribute(PropertyAttributes::cLocalModificationOverride);
  LightningBindGetterSetterProperty(ObjectBPath)->AddAttribute(PropertyAttributes::cLocalModificationOverride);

  LightningBindGetterSetter(ObjectA);
  LightningBindGetterSetter(ObjectB);

  LightningBindGetterSetterProperty(WorldPointA);
  LightningBindGetterSetterProperty(WorldPointB);

  LightningBindGetterSetterProperty(LocalPointA)->PlasmaSerialize(Vec3::cZero);
  LightningBindGetterSetterProperty(LocalPointB)->PlasmaSerialize(Vec3::cZero);

  PlasmaBindEvent(Events::ObjectLinkChanged, ObjectLinkEvent);
  PlasmaBindEvent(Events::ObjectLinkPointChanged, ObjectLinkPointChangedEvent);
...
void ObjectLink::Initialize(CogInitializer& initializer)
{
  ...
  if (PL::gRuntimeEditor)
    PL::gRuntimeEditor->Visualize(this, "Connection");
}
...
LightningDefineType(ObjectLinkEvent, builder, type)
{
  PlasmaBindDocumented();

  LightningBindFieldProperty(NewCog);
  LightningBindFieldProperty(OldCog);
  LightningBindFieldProperty(EdgeId);
}
...
LightningDefineType(ObjectLinkPointChangedEvent, builder, type)
{
  PlasmaBindDocumented();

  LightningBindFieldProperty(mOldLocalPoint);
  LightningBindFieldProperty(mNewLocalPoint);
  LightningBindFieldProperty(mEdgeId);
}
