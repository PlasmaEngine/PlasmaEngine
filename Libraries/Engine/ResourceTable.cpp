...
LightningDefineType(ResourceTableEntry, builder, type)
{
  ...
  LightningBindGetterSetterProperty(Value);
  LightningBindGetterSetterProperty(Weight);
  LightningBindGetterSetterProperty(Resource);
  LightningBindFieldProperty(mResourceType);
...
{
  Resource* resource = PL::gResources->GetResourceByTypeAndName(mResourceType, mValue);
  return resource;
}
...
LightningDefineType(ResourceTable, builder, type)
{
  PlasmaBindDocumented();
  ...
  LightningBindMethod(AddOrError);
  LightningBindMethod(AddOrIgnore);
  LightningBindMethod(AddOrOverwrite);
  LightningBindMethod(GetOrDefault);
  LightningBindMethod(GetOrError);
  LightningBindMethod(GetOrNull);
  // Array Get/Set
  LightningBindOverloadedMethod(Get, LightningInstanceOverload(ResourceTableEntry*, int));
  LightningBindOverloadedMethod(Set, LightningInstanceOverload(void, int, ResourceTableEntry*));
  // HashSet Get/Set
  LightningBindOverloadedMethod(Get, LightningInstanceOverload(ResourceTableEntry*, ResourceTableEntry*));
  LightningBindOverloadedMethod(Set, LightningInstanceOverload(void, ResourceTableEntry*));
  // HashMap Get/Set
  LightningBindOverloadedMethod(Get, LightningInstanceOverload(ResourceTableEntry*, StringParam));
  LightningBindOverloadedMethod(Set, LightningInstanceOverload(void, StringParam, ResourceTableEntry*));
  LightningBindMethod(RemoveAt);
  LightningBindMethod(RemoveOrError);
  LightningBindMethod(RemoveOrIgnore);
  LightningBindMethod(Contains);
  LightningBindGetterProperty(Count);
  LightningBindGetterProperty(All);
  LightningBindMethod(Clear);
  LightningBindGetterSetterProperty(ResourceType);
  LightningBindGetterSetterProperty(MaxWeight);
  LightningBindOverloadedMethod(SampleIndex, LightningInstanceOverload(uint, float, float));
  LightningBindOverloadedMethod(Sample, LightningInstanceOverload(ResourceTableEntry*, float, float));
...
bool ResourceTable::AddNewEntry(const ValueType& name, const ValueType& value, const WeightType& prob)
{
  ...
  ResourceManager* manager = PL::gResources->Managers.FindValue(mResourceType, nullptr);
  if (manager != nullptr)
  {
...
ResourceTableManager::ResourceTableManager(BoundType* resourceType) : ResourceManager(resourceType)
{
  ...
  ConnectThisTo(PL::gResources, Events::PackagedFinished, OnValidateTables);
}
