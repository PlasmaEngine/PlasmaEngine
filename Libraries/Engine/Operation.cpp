...
LightningDefineType(OperationQueueEvent, builder, type)
{
  LightningBindFieldProperty(mOperation);
...
LightningDefineType(Operation, builder, type)
{
  LightningBindFieldGetter(mParent);
  LightningBindField(mName);
  LightningBindField(mInvalidReason);

  LightningBindGetter(Children);
  LightningBindMethod(FindRoot);

  PlasmaBindEvent(Events::OperationQueued, OperationQueueEvent);
  PlasmaBindEvent(Events::OperationUndo, OperationQueueEvent);
  PlasmaBindEvent(Events::OperationRedo, OperationQueueEvent);
}
...
LightningDefineType(OperationBatch, builder, type)
{
}
...
LightningDefineType(OperationQueue, builder, type)
{
  LightningBindOverloadedMethod(Undo, LightningInstanceOverload(void));
  LightningBindOverloadedMethod(Undo, LightningInstanceOverload(bool, Operation*));
  LightningBindOverloadedMethod(Redo, LightningInstanceOverload(void));
  LightningBindOverloadedMethod(Redo, LightningInstanceOverload(bool, Operation*));
  LightningBindMethod(ClearUndo);
  LightningBindMethod(ClearRedo);
  LightningBindMethod(ClearAll);

  LightningBindGetterSetterProperty(ActiveBatchName);
  LightningBindGetterProperty(Commands);
  LightningBindGetterProperty(RedoCommands);

  LightningBindOverloadedMethod(BeginBatch, LightningInstanceOverload(void, StringParam));
  LightningBindOverloadedMethod(BeginBatch, LightningInstanceOverload(void));
  LightningBindMethod(EndBatch);

  LightningBindMethod(SaveObjectState);
  LightningBindMethod(ObjectCreated);
  LightningBindMethod(DestroyObject);

  LightningBindDefaultDestructor();

  type->CreatableInScript = true;

  LightningBindMethod(StartListeningForSideEffects);
  LightningBindMethod(IsListeningForSideEffects);
  LightningBindMethodAs(RegisterSideEffectProperty, "RegisterSideEffect");
  LightningBindMethod(QueueRegisteredSideEffects);
  LightningBindMethod(PopSubPropertyContext);
  LightningBindMethod(MarkPropertyAsModified);
}
...
void OperationQueue::ObjectCreated(Cog* object)
{
  ...
  Plasma::ObjectCreated(this, object);
}
...
void OperationQueue::DestroyObject(Cog* object)
{
  ...
  Plasma::DestroyObject(this, object);
}

void OperationQueue::DetachObject(Cog* object, bool relative)
{
  Plasma::DetachObject(this, object, relative);
}

void OperationQueue::AttachObject(Cog* object, Cog* parent, bool relative)
{
  Plasma::AttachObject(this, object, parent, relative);
}

void AddProperties(MetaProxy* proxy, Object* object)
{
  forRange (Property* prop, LightningVirtualTypeId(object)->GetProperties())
  {
...
String SaveObjectToString(Object* object)
{
  ...
  saver.StartPolymorphic(LightningVirtualTypeId(object));
...
{
  proxy->mType = LightningVirtualTypeId(object);
  ...
  forRange (Component* component, object->GetComponents())
  {
    ...
    componentProxy->mType = LightningVirtualTypeId(component);
...
void OperationQueue::QueueChanges(MetaProxy* proxy)
{
  ...
  for (uint i = 0; i < proxy->mComponents.Size(); ++i)
  {
    ...
    {
      Property* prop = LightningVirtualTypeId(component)->GetProperty(propertyState.mName);
  ...
  forRange (Component* component, newCog->GetComponents())
  {
    ...
    {
      if (proxyComponent->mType == LightningVirtualTypeId(component))
      {
        found = true;
        break;
      }
...
{
  mUndoId = PL::gUndoMap->GetUndoId(object);
...
{
  if (mUndoId != cInvalidUndoObjectId)
    return PL::gUndoMap->FindObject(mUndoId);
  return mRawObject;
}

void UndoHandle::UpdateObject(HandleParam newObject)
{
  mUndoId = PL::gUndoMap->UpdateUndoId(mUndoId, newObject);
}

// Undo Map
void UndoMap::Initialize()
{
  PL::gUndoMap = new UndoMap();
}
