...
LightningDefineType(RaycastProvider, builder, type)
{
...
LightningDefineType(Raycaster, builder, type)
{
  PlasmaBindSetup(SetupMode::DefaultSerialization);
...
{
  Handle handle = LightningAllocate(RaycastProvider, blockMeta);
...
LightningDefineType(RaycasterMetaComposition, builder, type)
{
}

RaycasterMetaComposition::RaycasterMetaComposition() : MetaComposition(LightningTypeId(RaycastProvider))
{
...
Handle RaycasterMetaComposition::GetComponent(HandleParam owner, BoundType* componentType)
{
  ...
  {
    if (LightningVirtualTypeId(provider)->IsA(componentType))
      return provider;
  }
  return Handle();
}
