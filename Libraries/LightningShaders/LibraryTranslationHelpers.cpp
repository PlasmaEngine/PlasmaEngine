...
Lightning::Function* GetFunction(Array<String>& params, const Lightning::FunctionArray* functions)
{
  ...
  for (uint i = 0; i < functions->Size(); ++i)
  {
    ...
    Lightning::DelegateType* d = fn->FunctionType;
    ...
    {
      const Lightning::DelegateParameter& param = d->Parameters[j];
...
Lightning::Function*
GetFunction(Lightning::Type* type, StringParam fnName, Array<String>& params, const Lightning::FunctionArray* functions)
{
  return GetFunction(params, functions);
}

// simple (not pretty or efficient) function to get a lightning
// function by name and parameter types, only in the Math module for static
// functions
Lightning::Function* GetMemberOverloadedFunction(Lightning::Type* type, StringParam fnName, Array<String>& params)
{
  // get all of the overloaded static functions by this name
  Lightning::BoundType* boundType = Lightning::Type::GetBoundType(type);
  if (boundType == nullptr)
    return nullptr;

  const Lightning::FunctionArray* functions = boundType->GetOverloadedInstanceFunctions(fnName);
  return GetFunction(type, fnName, params, functions);
}

Lightning::Function* GetStaticFunction(Lightning::Type* type, StringParam fnName, Array<String>& params)
{
  // get all of the overloaded static functions by this name
  Lightning::BoundType* boundType = Lightning::Type::GetBoundType(type);
  if (boundType == nullptr)
  ...
  const Lightning::FunctionArray* functions = boundType->GetOverloadedStaticFunctions(fnName);
  return GetFunction(type, fnName, params, functions);
}
...
Lightning::Function* GetConstructor(Lightning::Type* type, Array<String>& params)
{
  ...
  const Lightning::FunctionArray* constructors = boundType->GetOverloadedInheritedConstructors();
...
Lightning::Function*
GetStaticFunction(Lightning::Type* type, StringParam fnName, StringParam p0, StringParam p1, StringParam p2, StringParam p3)
{
  Array<String> params;
  params.PushBack(p0);
  params.PushBack(p1);
