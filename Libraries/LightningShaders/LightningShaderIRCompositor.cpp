...
LightningShaderIRCompositor::FragmentDescriptionMap::~FragmentDescriptionMap()
{
  Clear();
}

void LightningShaderIRCompositor::FragmentDescriptionMap::Clear()
{
...
LightningShaderIRCompositor::ShaderStageDescription::~ShaderStageDescription()
{
  Clear();
}

void LightningShaderIRCompositor::ShaderStageDescription::Clear()
{
}

LightningShaderIRCompositor::ShaderDefinition::ShaderDefinition()
{
                                        ...
                                        LightningShaderSpirVSettingsRef& settings)
{
  ...
  // Now generate the actual lightning composite string
  for (size_t i = 1; i < compositeInfo.mActiveStages.Size() - 1; ++i)
  {
    StageLinkingInfo* currentStage = compositeInfo.mActiveStages[i];
    GenerateLightningComposite(currentStage, shaderDef.mResults[currentStage->mFragmentType], shaderDef.mExtraAttributes);
  }
...
bool LightningShaderIRCompositor::CompositeCompute(ShaderDefinition& shaderDef, ComputeShaderProperties* computeProperties, const ShaderCapabilities& capabilities, LightningShaderSpirVSettingsRef& settings)
{
  ...
  GenerateLightningComposite(&computeStageInfo, stageDesc, shaderDef.mExtraAttributes);
  ...
  for (size_t i = 0; i < compositeInfo.mActiveStages.Size(); ++i)
  {
    ...
    for (size_t i = 0; i < stageInfo->mFragmentTypes.Size(); ++i)
    {
      ...
      if (!fragmentType->mHasMainFunction)
      {
        ...
        ReturnIf(lightningType == nullptr, false, "Cannot composite a fragment with no backing lightning type");
        ...
        if (stageInfo->mFragmentType == FragmentType::Geometry)
          msg = String::Format("Geometry fragment '%s' must have a 'Main' function of "
                               ...
                               lightningType->Name.c_str());
        else
          msg = String::Format("Fragment '%s' must have a a function of signature 'Main()'.", lightningType->Name.c_str());
        SendTranslationError(lightningType->Location, msg);
        return false;
      }
    }
    ...
    if (stageInfo->mFragmentType == FragmentType::Geometry)
    {
      ...
      {
        Lightning::BoundType* lightningType = fragmentType->mMeta->mLightningType;
        SendTranslationError(lightningType->Location, "Geometry shader stage only supports one fragment at a time.");
        return false;
      }
    }
  }
...
void LightningShaderIRCompositor::CollectExpectedOutputs(CompositedShaderInfo& compositeInfo)
{
...
void LightningShaderIRCompositor::CreateCpuStage(CompositedShaderInfo& compositeInfo)
{
...
void LightningShaderIRCompositor::ResolveInputs(StageLinkingInfo* previousStage, StageLinkingInfo* currentStage)
{
      ...
      if (fieldMeta->mLightningType->HasAttribute(nameSettings.mNonCopyableAttributeName))
      {
        ...
        for (size_t attributeIndex = 0; attributeIndex < fieldMeta->mAttributes.Size(); ++attributeIndex)
        {
              ...
              attribute->mAttributeName == nameSettings.mStageOutputAttribute)
          {
            ...
            fieldName = fieldMeta->mLightningName;

            String propertyName = MakePropertyName(fieldName, fieldMeta->mOwner->mLightningName);
      ...
      for (size_t attributeIndex = 0; attributeIndex < fieldMeta->mAttributes.Size(); ++attributeIndex)
      {
        ...
        if (attribute->mAttributeName == nameSettings.mPropertyInputAttribute)
        {
          ...
          fieldLinkInfo.mFieldPropertyName =
              MakePropertyName(fieldLinkInfo.mOutputFieldDependencyName, fieldMeta->mOwner->mLightningName);
          break;
        }
      ...
      for (size_t attributeIndex = 0; attributeIndex < fieldMeta->mAttributes.Size(); ++attributeIndex)
      {
        ...
        if (attribute->mAttributeName == nameSettings.mHardwareBuiltInOutputAttribute)
        {
          ...
          if (builtInFieldMeta != nullptr)
          {
            ...
            ResolvedFieldOutputInfo resolvedFieldOutput(builtInFieldMeta->mLightningName, fieldMeta);
...
void LightningShaderIRCompositor::AddStageInput(ExpectedOutputData* previousStageOutputData,
                                            ...
                                            StringParam fieldAttributeName)
{
  ...
  String fieldType = previousStageOutputMeta->mLightningType->ToString();
...
ShaderIRFieldMeta* LightningShaderIRCompositor::FindHardwareBuiltInInput(ShaderFieldKey& fieldKey,
...
ShaderIRFieldMeta* LightningShaderIRCompositor::FindHardwareBuiltInOutput(ShaderFieldKey& fieldKey,
                                                                      FragmentType::Enum fragmentType)
{
  LightningShaderSpirVSettings* settings = mSettings;
  BuiltInStageDescription& builtInStageDescription = settings->mBuiltIns[fragmentType];
...
ShaderFieldKey LightningShaderIRCompositor::MakeFieldKey(ShaderIRFieldMeta* fieldMeta, ShaderIRAttribute* attribute)
{
  String fieldName = GetFieldInOutName(fieldMeta, attribute);
  return ShaderFieldKey(fieldName, fieldMeta->mLightningType->ToString());
}

String LightningShaderIRCompositor::MakePropertyName(StringParam fieldName, StringParam ownerType)
{
  return GenerateSpirVPropertyName(fieldName, ownerType);
}

String LightningShaderIRCompositor::GetFieldInOutName(ShaderIRFieldMeta* fieldMeta, ShaderIRAttribute* attribute)
{
  return fieldMeta->GetFieldAttributeName(attribute);
}

void LightningShaderIRCompositor::GetStageFieldName(ShaderIRFieldMeta* fieldMeta,
                                                ...
                                                String& fieldAttributeNameOut)
{
  ...
  fieldVarNameOut = BuildString("Stage_", fieldAttributeNameOut, "_", fieldMeta->mLightningType->ToString());
}

void LightningShaderIRCompositor::ResolveGpuStage(CompositedShaderInfo& compositeInfo)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
  Lightning::BoundType* renderTargetType = mSettings->mRenderTargetType;
...
void LightningShaderIRCompositor::ResolveStageLinkOrder(CompositedShaderInfo& compositeInfo)
{
  ...
  {
    LightningShaderIRCompositor::StageLinkingInfo* currStage = compositeInfo.mActiveStages[i];
    LightningShaderIRCompositor::StageLinkingInfo* nextStage = compositeInfo.mActiveStages[i + 1];
...
void LightningShaderIRCompositor::GenerateLightningComposite(StageLinkingInfo* currentStage,
                                                     ...
                                                     ShaderIRAttributeList& extraAttributes)
{
  ...
  case ShaderStage::Pixel:
    GenerateBasicLightningComposite(currentStage, stageResults, extraAttributes);
    break;
  case ShaderStage::Geometry:
    GenerateGeometryLightningComposite(currentStage, stageResults, extraAttributes);
    break;
  case ShaderStage::Compute:
    GenerateComputeLightningComposite(currentStage, stageResults, extraAttributes);
    break;
  default:
  {
...
void LightningShaderIRCompositor::GenerateBasicLightningComposite(StageLinkingInfo* currentStage,
                                                          ShaderStageDescription& stageResults,
                                                          ShaderIRAttributeList& extraAttributes)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
  ...
  for (; !outputRange.Empty(); outputRange.PopFront())
  {
    ...
    builder << MakeFragmentVarName(fieldMeta->mOwner) << "." << fieldMeta->mLightningName << ";"
...
void LightningShaderIRCompositor::GenerateGeometryLightningComposite(StageLinkingInfo* currentStage,
                                                             ShaderStageDescription& stageResults,
                                                             ShaderIRAttributeList& extraAttributes)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
  ...
  String compositeInputStreamType = BuildString(lightningInputStreamType->TemplateBaseName, "[", inputTypeName, "]");
  String compositeOutputStreamType = BuildString(lightningOutputStreamType->TemplateBaseName, "[", outputTypeName, "]");
  ...
  builder.WriteLocalVariableDefaultConstruction("fragmentInput", lightningInputStreamType->Name);
  ...
  for (size_t i = 0; i < inputVertexLinkInfo.mFieldList.Size(); ++i)
  {
    ...
    {
      builder << builder.EmitIndent() << "fragmentInput[i]." << linkInfo.mFieldMeta->mLightningName;
  ...
  builder.WriteLocalVariableDefaultConstruction("fragmentOutput", lightningOutputStreamType->Name);
...
void LightningShaderIRCompositor::GenerateComputeLightningComposite(StageLinkingInfo* currentStage,
                                                            ShaderStageDescription& stageResults,
                                                            ShaderIRAttributeList& extraAttributes)
{
                                                          ...
                                                          StringParam fragmentVarName)
{
  ...
  String fragmentVarType = fragmentType->mLightningType->ToString();
  ...
  for (size_t i = 0; i < linkInfo.mFieldList.Size(); ++i)
  {
    ...
    {
      builder << builder.EmitIndent() << fragmentVarName << "." << fieldMeta->mLightningName << " = ";
    ...
    else if (fieldLinkInfo.mLinkedType == LinkedFieldType::Fragment)
    {
      ...
      builder << builder.EmitIndent() << fragmentVarName << "." << fieldMeta->mLightningName << " = ";
      builder << fragmentDependencyVarName << "." << outputFieldDependency->mLightningName;
      builder << ";" << builder.EmitLineReturn();
    }
    ...
    else if (fieldLinkInfo.mLinkedType == LinkedFieldType::AppBuiltIn)
    {
      ShaderIRFieldMeta* outputFieldDependency = fieldLinkInfo.mOutputFieldDependency;
      builder << builder.EmitIndent() << fragmentVarName << "." << fieldMeta->mLightningName << " = ";
      builder << "this." << outputFieldDependency->mLightningName;
      builder << ";" << builder.EmitLineReturn();
    }
...
void LightningShaderIRCompositor::DeclareFieldsInOrder(ShaderCodeBuilder& builder,
                                                   StageAttachmentLinkingInfo& linkingInfo,
...
void LightningShaderIRCompositor::DeclareFieldsWithAttribute(ShaderCodeBuilder& builder,
                                                         StageAttachmentLinkingInfo& linkingInfo,
...
String LightningShaderIRCompositor::MakeFragmentVarName(ShaderIRTypeMeta* typeMeta)
{
  // Lowercase the first letter of the type name
  String name = typeMeta->mLightningName;
...
void LightningShaderIRCompositor::GenerateStageDescriptions(CompositedShaderInfo& compositeInfo,
...
LightningShaderIRCompositor::FieldLinkingInfo::FieldLinkingInfo()
{
...
void LightningShaderIRCompositor::FragmentLinkingInfo::CopyTo(ShaderFragmentDescription& fragDesc)
{
...
void LightningShaderIRCompositor::FragmentLinkingInfo::CopyFieldTo(FieldLinkingInfo& fieldInfo,
...
LightningShaderIRCompositor::ResolvedFieldOutputInfo::ResolvedFieldOutputInfo()
{
  mOutputFieldDependency = nullptr;
}

LightningShaderIRCompositor::ResolvedFieldOutputInfo::ResolvedFieldOutputInfo(StringParam fieldName,
...
LightningShaderIRCompositor::StageAttachmentLinkingInfo::StageAttachmentLinkingInfo()
{
...
void LightningShaderIRCompositor::StageAttachmentLinkingInfo::AddResolvedField(ShaderIRFieldMeta* fieldMeta,
                                                                           StringParam attributeName)
{
  AddResolvedField(fieldMeta->mLightningName, fieldMeta->mLightningType->ToString(), attributeName);
}

void LightningShaderIRCompositor::StageAttachmentLinkingInfo::AddResolvedFieldProperty(ShaderIRFieldMeta* fieldMeta,
                                                                                   ShaderIRAttribute* attribute)
{
  String baseFieldName = LightningShaderIRCompositor::GetFieldInOutName(fieldMeta, attribute);
  String fieldName = LightningShaderIRCompositor::MakePropertyName(baseFieldName, fieldMeta->mOwner->mLightningName);
  AddResolvedField(fieldName, fieldMeta->mLightningType->ToString(), attribute->mAttributeName);
}

void LightningShaderIRCompositor::StageAttachmentLinkingInfo::AddResolvedStageField(SpirVNameSettings& nameSettings,
                                                                                ...
                                                                                StringParam attributeParameter)
{
  ...
  ShaderIRAttribute* fieldAttribute = AddResolvedField(fieldName, fieldMeta->mLightningType->ToString(), attributeName);
  ...
  ShaderFieldKey fieldKey(fieldName, fieldMeta->mLightningType->ToString());
...
ShaderIRAttribute* LightningShaderIRCompositor::StageAttachmentLinkingInfo::AddResolvedField(StringParam fieldName,
...
LightningShaderIRCompositor::ResolvedFieldInfo*
LightningShaderIRCompositor::StageAttachmentLinkingInfo::CreateResolvedField(StringParam fieldName, StringParam fieldType)
{
...
LightningShaderIRCompositor::StageLinkingInfo::StageLinkingInfo()
{
}

void LightningShaderIRCompositor::StageLinkingInfo::SetFragmentType(FragmentType::Enum fragType)
{
  mFragmentType = fragType;
  mShaderStage = FragmentTypeToShaderStage(mFragmentType);
}

void LightningShaderIRCompositor::StageLinkingInfo::Clear()
{
...
void LightningShaderIRCompositor::ApiPerspectivePositionCallback(CompositorCallbackData& callbackData, void* userData)
{
  LightningShaderSpirVSettings* settings = callbackData.mSettings;
