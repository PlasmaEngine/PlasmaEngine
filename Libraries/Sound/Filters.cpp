...
float Delay::ReadDelayAt(const float mSec)
{
  ...
  // If still plasma or if larger than buffer size, bad value passed in
...
bool DelayLine::IsDataInBuffer()
{
  ...
  // No values other than plasma were found, so return false
  return false;
}
...
{
  Plasma::Array<float> resultSamples(numChannels);
...
bool FFTConvolver::Initialize(int blockSize, const float* impulseResponse, int irLength)
{
  ...
  //// Remove plasmas at end of impulse response
