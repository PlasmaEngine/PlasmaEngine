...
LightningDefineType(SoundNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindMethod(AddInputNode);
  LightningBindMethod(InsertNodeAfter);
  LightningBindMethod(InsertNodeBefore);
  LightningBindMethod(ReplaceWith);
  LightningBindMethod(RemoveInputNode);
  LightningBindMethod(RemoveAllInputs);
  LightningBindMethod(RemoveAllOutputs);
  LightningBindMethod(RemoveAndAttachInputsToOutputs);
  LightningBindGetterSetter(AutoCollapse);
  LightningBindGetter(HasInputs);
  LightningBindGetter(HasOutputs);
  LightningBindGetter(InputCount);
  LightningBindGetter(OutputCount);
  LightningBindGetterSetter(BypassPercent)->AddAttribute(DeprecatedAttribute);
  LightningBindGetterSetter(BypassValue);

  PlasmaBindEvent(Events::AudioInterpolationDone, SoundEvent);
  PlasmaBindEvent(Events::SoundNodeDisconnected, SoundEvent);
  PlasmaBindEvent(Events::SoundListenerRemoved, SoundEvent);
}
...
SoundNode::SoundNode(StringParam name, int ID, bool listenerDependent, bool generator) :
    ...
    mMixedVersionThreaded(PL::gSound->Mixer.mMixVersionThreaded - 1),
...
{
  ConnectThisTo(&(PL::gSound->Mixer), Events::SoundListenerRemoved, RemoveListenerThreaded);
}
...
void SoundNode::AddInputNode(SoundNode* newNode)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&SoundNode::AddInputNodeThreaded, this, handle), this);
}
...
void SoundNode::RemoveInputNode(SoundNode* node)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&SoundNode::RemoveInputNodeThreaded, this, handle), this);
...
bool SoundNode::Evaluate(BufferType* outputBuffer, const unsigned numberOfChannels, ListenerNode* listener)
{
  ...
  if (PL::gSound->Mixer.mMixVersionThreaded == mMixedVersionThreaded)
  {
    // Check for errors
    String message;
    ...
    if (!message.Empty())
    {
      ...
      PL::gSound->Mixer.AddTaskThreaded(CreateFunctor(&SoundNode::WarningFromMixThread, this, title, message), this);

      PL::gSound->Mixer.AddTaskThreaded(CreateFunctor(&SoundNode::RemoveAndAttachInputsToOutputs, this), this);
  ...
  {
    mInProcessThreaded = true;
    mMixedVersionThreaded = PL::gSound->Mixer.mMixVersionThreaded;
...
LightningDefineType(SimpleCollapseNode, builder, Type)
{
}
...
LightningDefineType(OutputNode, builder, Type)
{
}
...
LightningDefineType(CombineNode, builder, Type)
{
}

CombineNode::CombineNode(Plasma::StringParam name, unsigned ID) : SimpleCollapseNode(name, ID, false, false)
{
}
...
LightningDefineType(CombineAndPauseNode, builder, Type)
{
}

CombineAndPauseNode::CombineAndPauseNode(Plasma::StringParam name, unsigned ID) :
...
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&CombineAndPauseNode::SetPausedThreaded, this, paused), this);
}
...
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&CombineAndPauseNode::SetMutedThreaded, this, muted), this);
}
