...
LightningDefineType(NodePrintInfo, builder, type)
{
  type->HandleManager = LightningManagerId(PointerManager);

  LightningBindGetter(Connections);
  LightningBindField(mPosition);
  LightningBindField(mName);
  LightningBindField(mHasOutput);
  LightningBindField(mID);
}
...
NodeInfoListType::range SoundNodeGraph::GetNodeInfoList()
{
  ...
  CreateInfo(PL::gSound->mOutputNode, nullptr, 0);
