...
#define SerializeEnumName(enumName, variable)                                                                          \
  stream.EnumField(enumName::EnumName, #variable, (uint&)variable, LightningTypeId(enumName::Enum))

#define SerializeEnumCustomName(enumName, variableName, variable)                                                      \
  stream.EnumField(enumName::EnumName, variableName, (uint&)variable, LightningTypeId(enumName::Enum))

#define SerializeEnumNameDefault(enumName, variable, defaultValue)                                                     \
  if (!stream.EnumField(enumName::EnumName, #variable, (uint&)variable, LightningTypeId(enumName::Enum)))                  \
