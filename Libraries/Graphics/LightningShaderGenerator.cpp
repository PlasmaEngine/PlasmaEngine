...
void LightningShaderGenerator::InitializeSpirV()
{
  ...
  Lightning::ShaderIntrinsicsLibrary::GetInstance().GetLibrary();
 ...
 // miscData.AddField(sampledImage2dType, "HeightMapWeights");
  ...
  EventConnect(&mFragmentsProject,
               ...
               this);
  EventConnect(&mFragmentsProject, Lightning::Events::TypeParsed, &LightningShaderGenerator::OnLightningFragmentTypeParsed, this);
  EventConnect(
      &mFragmentsProject, Events::TranslationError, &LightningShaderGenerator::OnLightningFragmentTranslationError, this);
  EventConnect(
      &mFragmentsProject, Events::ValidationError, &LightningShaderGenerator::OnLightningFragmentValidationError, this);
}
    ...
    if (shaderTypeMeta->mFragmentType != FragmentType::None)
    {
      ...
      forRange (ShaderIRFieldMeta* field, shaderTypeMeta->mFields.All())
      {
        ...
        if (field->ContainsAttribute("PropertyInput"))
        {
          ...
          String propertyTypeName = lookupMap.FindValue(field->mLightningType->Name, field->mLightningType->Name);
      ...
      forRange (ShaderPropertyInfo& shaderProperty, shaderProperties.All())
      {
        ...
        forRange (ShaderIRAttribute& shaderAttribute, shaderProperty.mShaderField->mAttributes.All())
        {
          if (plasmaAttributes->IsValidPropertyAttribute(shaderAttribute.mAttributeName))
          {
            ...
            {
              Lightning::AttributeParameter& param = attributeParam.GetLightningAttributeParameter();
                                                       ...
                                                       StringParam libraryName)
{
  ...
  forRange (Resource* resource, fragments.All())
  {
    ...
    LightningFragment* fragment = (LightningFragment*)resource;
...
bool LightningShaderGenerator::Commit(LightningCompileEvent* e)
{
  ...
  forRange (ResourceLibrary* library, e->mModifiedLibraries.All())
  {
    ...
    if (pendingLibrary)
    {
      LightningShaderIRLibraryRef internalPendingLibrary = mPendingToPendingInternal.FindValue(pendingLibrary, nullptr);
                                        ...
                                        Array<ShaderDefinition>* compositeShaderDefs)
{
  ...
  for (size_t startIndex = 0; startIndex < totalShaderCount; startIndex += compositeBatchCount)
  {
    ...
    for (size_t i = startIndex; i < endIndex; ++i)
    {
      ...
      LightningShaderIRCompositor::ShaderStageDescription& vertexInfo = shaderDef.mResults[FragmentType::Vertex];
      LightningShaderIRCompositor::ShaderStageDescription& geometryInfo = shaderDef.mResults[FragmentType::Geometry];
    ...
    EventConnect(
        &shaderProject, Lightning::Events::CompilationError, &LightningShaderGenerator::OnLightningFragmentCompilationError, this);
    LightningShaderIRLibraryRef shaderLibrary = shaderProject.CompileAndTranslate(shaderDependencies, mFrontEndTranslator);
...
void LightningShaderGenerator::OnLightningFragmentCompilationError(Lightning::ErrorEvent* event)
{
  String shortMessage = event->ExactError;
  String fullMessage = event->GetFormattedMessage(Lightning::MessageFormat::Python);
  LightningFragmentManager::GetInstance()->DispatchScriptError(
      Events::SyntaxError, shortMessage, fullMessage, event->Location);
}

void LightningShaderGenerator::OnLightningFragmentTypeParsed(Lightning::ParseEvent* event)
{
