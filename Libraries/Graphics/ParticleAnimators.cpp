...
LightningDefineType(LinearParticleAnimator, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDependency(Transform);
  PlasmaBindDependency(ParticleSystem);
  PlasmaBindDocumented();

  LightningBindFieldProperty(mForce);
  LightningBindFieldProperty(mRandomForce);

  LightningBindFieldProperty(mTorque);
  LightningBindFieldProperty(mGrowth);
  LightningBindFieldProperty(mDampening);
  LightningBindFieldProperty(mTwist);
}
...
LightningDefineType(ParticleWander, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDependency(ParticleSystem);
  PlasmaBindDocumented();
  LightningBindFieldProperty(mWanderAngle);
  LightningBindFieldProperty(mWanderAngleVariance);
  LightningBindFieldProperty(mWanderStrength);
}
...
LightningDefineType(ParticleColorAnimator, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDependency(ParticleSystem);
  ...
  LightningBindFieldProperty(mTimeGradient)->Add(new MetaEditorResource(false, true));
  LightningBindFieldProperty(mVelocityGradient)->Add(new MetaEditorResource(false, true));
  LightningBindFieldProperty(mMaxParticleSpeed);
}
...
LightningDefineType(ParticleAttractor, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDependency(ParticleSystem);
  ...
  LightningBindFieldProperty(mPositionSpace);
  LightningBindFieldProperty(mAttractPosition);
  LightningBindFieldProperty(mStrength);
  LightningBindFieldProperty(mMinDistance);
  LightningBindFieldProperty(mMaxDistance);
}
...
LightningDefineType(ParticleTwister, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDependency(ParticleSystem);
...
LightningDefineType(ParticleCollisionPlane, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDependency(ParticleSystem);
  PlasmaBindDocumented();
  LightningBindFieldProperty(mPlaneSpace);
  LightningBindFieldProperty(mPlanePosition);
  LightningBindFieldProperty(mPlaneNormal);
  LightningBindGetterSetterProperty(Restitution)->Add(new EditorSlider(0.0f, 1.0f, 0.01f));
  LightningBindGetterSetterProperty(Friction)->Add(new EditorSlider(0.0f, 1.0f, 0.01f));
}
...
// Heightmap
LightningDefineType(ParticleCollisionHeightmap, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDependency(ParticleSystem);
  PlasmaBindDocumented();
  LightningBindFieldProperty(mHeightMap);
  LightningBindGetterSetterProperty(Restitution)->Add(new EditorSlider(0.0f, 1.0f, 0.01f));
  LightningBindGetterSetterProperty(Friction)->Add(new EditorSlider(0.0f, 1.0f, 0.01f));
}
