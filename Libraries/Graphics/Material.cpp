...
LightningDefineType(CompositionLabelExtension, builder, type)
{
}

LightningDefineType(Material, builder, type)
{
  ...
  LightningBindFieldGetterPropertyAs(mSerializedList, "RenderGroups");
  LightningBindFieldGetterProperty(mReferencedByList);

  LightningBindGetterProperty(CompositionLabel)->Add(new CompositionLabelExtension());
}
...
void Material::SerializeMaterialBlocks(Serializer& stream)
{
  ...
  else
  {
    ...
    BoundType* materialBlockType = LightningTypeId(MaterialBlock);
    while (stream.GetPolymorphic(materialNode))
    {
      ...
      // for it (for lightning fragments that fail to compile and because resources
      ...
      if (materialNodeType == nullptr)
      {
        ...
        block = LightningAllocate(MaterialBlock, proxyType);
...
MaterialManager::MaterialManager(BoundType* resourceType) : ResourceManager(resourceType)
{
  ...
  DefaultResourceName = "PlasmaMaterial";
  mCanCreateNew = true;
  mCanDuplicate = true;
  mCanReload = false;
...
void MaterialManager::ReInitializeRemoveComponents()
{
  ...
  forRange (Resource* resource, AllResources())
  {
    ...
    while (!range.Empty())
    {
      ...
      BoundType* blockType = LightningVirtualTypeId(materialBlock);
