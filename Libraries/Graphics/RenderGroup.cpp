...
LightningDefineType(RenderGroup, builder, type)
{
  PlasmaBindDocumented();

  LightningBindMethod(IsSubRenderGroup);
  LightningBindMethod(IsSubRenderGroupOf);

  LightningBindFieldGetterPropertyAs(mSerializedList, "Materials");
  LightningBindFieldGetterProperty(mReferencedByList);

  LightningBindFieldProperty(mGraphicalSortMethod);
  ...
  LightningBindGetterSetterProperty(ParentRenderGroup)->Add(metaEditor);
  LightningBindFieldGetterProperty(mChildRenderGroups);
}
