...
LightningDefineType(VertexBuffer, builder, type)
{
  PlasmaBindDocumented();

  LightningBindMethod(AddAttribute);
  LightningBindMethod(GetAttributes);

  LightningBindOverloadedMethod(AddByte, LightningInstanceOverload(void, int));
  LightningBindOverloadedMethod(AddByte, LightningInstanceOverload(void, IntVec2));
  LightningBindOverloadedMethod(AddByte, LightningInstanceOverload(void, IntVec3));
  LightningBindOverloadedMethod(AddByte, LightningInstanceOverload(void, IntVec4));
  LightningBindOverloadedMethod(AddShort, LightningInstanceOverload(void, int));
  LightningBindOverloadedMethod(AddShort, LightningInstanceOverload(void, IntVec2));
  LightningBindOverloadedMethod(AddShort, LightningInstanceOverload(void, IntVec3));
  LightningBindOverloadedMethod(AddShort, LightningInstanceOverload(void, IntVec4));
  LightningBindOverloadedMethod(AddReal, LightningInstanceOverload(void, real));
  LightningBindOverloadedMethod(AddReal, LightningInstanceOverload(void, Vec2));
  LightningBindOverloadedMethod(AddReal, LightningInstanceOverload(void, Vec3));
  LightningBindOverloadedMethod(AddReal, LightningInstanceOverload(void, Vec4));
  LightningBindOverloadedMethod(GetVertexData, LightningInstanceOverload(Vec4, uint, VertexSemantic::Enum));
  LightningBindOverloadedMethod(GetVertexData,
                            LightningInstanceOverload(Vec4, uint, VertexSemantic::Enum, VertexElementType::Enum, uint));
  LightningBindMethod(IsValidVertexData);

  LightningBindMethod(ClearAttributes);
  LightningBindMethod(ClearData);

  LightningBindMethod(GetElementType);
  LightningBindMethod(GetElementCount);
  LightningBindGetterProperty(VertexCount);
}
...
LightningDefineType(IndexBuffer, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetterProperty(Count);
...
LightningDefineType(Mesh, builder, type)
{
  PlasmaBindDocumented();

  LightningBindMethod(CreateRuntime);
  LightningBindMethod(RuntimeClone);

  LightningBindFieldGetter(mVertices);
  LightningBindFieldGetter(mIndices);
  LightningBindFieldProperty(mPrimitiveType);

  LightningBindMethod(Upload);
  LightningBindMethod(UploadNoRayCastInfo);
  LightningBindMethod(UploadNoRayCastInfoOrAabb);
}
...
HandleOf<Mesh> Mesh::CreateRuntime()
{
  ...
  PL::gEngine->has(GraphicsEngine)->AddMesh(mesh);
  return mesh;
}
...
HandleOf<Mesh> Mesh::RuntimeClone()
{
  ...
  PL::gEngine->has(GraphicsEngine)->AddMesh(clone);
  return clone;
}
...
void Mesh::UploadInternal(bool updateAabb, bool updateTree)
{
  ...
  PL::gEngine->has(GraphicsEngine)->AddMesh(this);
...
void Mesh::BuildAabbAndTree()
{
  ...
  for (uint i = 0; i < primitiveCount; ++i)
  {
    ...
    // Make sure tree entries do not have thicknesses of plasma
  ...
  // Make sure final bounding box does not have thicknesses of plasma
