...
LightningDefineType(GraphicalEntry, builder, type)
{
  LightningBindGetter(Cog);
  LightningBindMethod(SetGraphicalSortValue);
}
...
LightningDefineType(GraphicalSortEvent, builder, type)
{
  LightningBindGetterProperty(GraphicalEntries);
  LightningBindGetterProperty(RenderGroup);

  PlasmaBindEvent(Events::GraphicalSort, GraphicalSortEvent);
}
