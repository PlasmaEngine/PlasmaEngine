...
LightningDefineType(GraphicalEvent, builder, type)
{
  PlasmaBindDocumented();
  LightningBindFieldProperty(mViewingObject);
}

LightningDefineType(Graphical, builder, type)
{
  PlasmaBindDocumented();
  ...
  LightningBindGetterSetterProperty(ViewCulling);
  LightningBindFieldProperty(mVisibilityEvents);
  LightningBindGetterSetterProperty(OverrideBoundingBox)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindGetterSetterProperty(LocalAabbCenter)->PlasmaFilterEquality(mOverrideBoundingBox, bool, true);
  LightningBindGetterSetterProperty(LocalAabbHalfExtents)->PlasmaFilterEquality(mOverrideBoundingBox, bool, true);
  LightningBindFieldProperty(mGroupSortValue);
  LightningBindGetterSetterProperty(Material);
  LightningBindGetterSetter(ShaderInputs);

  LightningBindGetter(WorldAabb);

  PlasmaBindEvent(Events::EnterView, GraphicalEvent);
  PlasmaBindEvent(Events::EnterViewAny, GraphicalEvent);
  PlasmaBindEvent(Events::ExitView, GraphicalEvent);
  PlasmaBindEvent(Events::ExitViewAll, GraphicalEvent);

  PlasmaBindTag(Tags::Graphical);
}
...
void Graphical::Initialize(CogInitializer& initializer)
{
  ...
  ConnectThisTo(PL::gEngine->has(GraphicsEngine), Events::ShaderInputsModified, OnShaderInputsModified);
...
{
  // If a LightningComponent becomes a proxy, the GraphicsEngine does not spend time
  ...
  String componentName = LightningVirtualTypeId(component)->Name;
  Array<ShaderMetaProperty>* shaderProperties =
      PL::gEngine->has(GraphicsEngine)->mComponentShaderProperties.FindPointer(componentName);
  ...
  {
    Property* metaProperty = LightningVirtualTypeId(component)->GetProperty(shaderProperty.mMetaPropertyName);
    ...
    if (fragmentName.Empty())
    {
      ...
      {
        BoundType* metaType = LightningVirtualTypeId(materialBlock);
