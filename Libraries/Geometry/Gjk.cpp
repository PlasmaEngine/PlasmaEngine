...
Type Gjk::TestDebug(const SupportShape* shapeA, const SupportShape* shapeB, Manifold* manifold, unsigned maxIter)
{
  ...
  while (iter_count < maxIter)
  {
    ...
    if (mSimplex.mCount)
    {
      ...
      if (proj <= sEpsilon)
      {
        // Plasma::gDebugDraw->Add(Plasma::Debug::Sphere(support.cso,
    ...
    if (mSimplex.ContainsOrigin())
    {
      // Only point is the origin, so objects have intersection of plasma
...
void Gjk::DrawDebug(uint debugFlag)
{
  ...
  for (uint i = 0; i < mSimplex.mCount; ++i)
  {
    ...
    Plasma::gDebugDraw->Add(Plasma::Debug::Sphere(mSimplex.mPoints[i].cso, radius).Color(colors[i]));
    Plasma::gDebugDraw->Add(Plasma::Debug::Sphere(mSimplex.mPoints[i].objA, radius).Color(colors[i]));
    Plasma::gDebugDraw->Add(Plasma::Debug::Sphere(mSimplex.mPoints[i].objB, radius).Color(colors[i]));
  }

  Plasma::gDebugDraw->Add(Plasma::Debug::Line(Vec3(0, 0, 0), mSupportVector).Color(Color::White));

  // mSupportVector = mSimplex.GetSupportVector();

  // for (unsigned i = 0; i < mCSO.Size(); ++i)
  //   Plasma::gDebugDraw->Add(Plasma::Debug::Sphere(mCSO[i],
  //   0.01f).Color(Color::PapayaWhip));

  // for (unsigned i = 0; i < mSimplex.mCount; ++i)
  // {
  //   Plasma::gDebugDraw->Add(Plasma::Debug::Sphere(mSimplex.mPoints[i].cso,
  //   0.01f).Color(Color::Blue));
  //   Plasma::gDebugDraw->Add(Plasma::Debug::Line(mSimplex.mPoints[i].cso,
  ...
  // Plasma::gDebugDraw->Add(Plasma::Debug::Line(average, average +
...
{
  Plasma::gDebugDraw->Add(Plasma::Debug::Triangle(p0, p1, p2).Color(Color::Orange).Border(true).Alpha(50));
}

void Gjk::DrawCSO(void)
{
  Plasma::Array<CSOVertex> supports;
  ...
  //   Plasma::gDebugDraw->Add(Plasma::Debug::Sphere(supports[i].cso,
