...
public:
  ...
  // Length of time the object will be visible, default is plasma or one frame.
...
{
public:
  LightningDeclareType(Arc, TypeCopyMode::ValueType);
...
{
public:
  LightningDeclareType(Box, TypeCopyMode::ValueType);
...
{
public:
  LightningDeclareType(Capsule, TypeCopyMode::ValueType);
  ...
  Capsule(const Plasma::Capsule& capsule) : mStart(capsule.PointA), mEnd(capsule.PointB), mRadius(capsule.Radius)
  {
  }
...
{
public:
  LightningDeclareType(Circle, TypeCopyMode::ValueType);
...
{
public:
  LightningDeclareType(Cone, TypeCopyMode::ValueType);
...
{
public:
  LightningDeclareType(Cylinder, TypeCopyMode::ValueType);
  ...
  Cylinder(const Plasma::Cylinder& cylinder) : mStart(cylinder.PointA), mEnd(cylinder.PointB), mRadius(cylinder.Radius)
  {
  }
...
{
public:
  LightningDeclareType(Frustum, TypeCopyMode::ValueType);
  ...
  Frustum(const Plasma::Frustum& frustum)
  {
...
{
public:
  LightningDeclareType(Line, TypeCopyMode::ValueType);
  ...
  Line(const Plasma::Ray& ray, float t = 1.0f) : mStart(ray.Start), mEnd(ray.Start + ray.Direction * t), mHeadSize(1.0f)
  {
  }

  Line(const Plasma::Segment& segment) : mStart(segment.Start), mEnd(segment.End), mHeadSize(0.0f)
  {
  }
...
{
public:
  LightningDeclareType(LineCross, TypeCopyMode::ValueType);
...
{
public:
  LightningDeclareType(Obb, TypeCopyMode::ValueType);
...
{
public:
  LightningDeclareType(Sphere, TypeCopyMode::ValueType);
  ...
  Sphere(const Plasma::Sphere& sphere) : mPosition(sphere.mCenter), mRadius(sphere.mRadius)
  {
  }
...
public:
  ...
  // type.
  LightningDeclareType(Text, TypeCopyMode::ReferenceType);
...
{
public:
  LightningDeclareType(Triangle, TypeCopyMode::ValueType);
  ...
  Triangle(const Plasma::Triangle& tri) : mPoint0(tri.p0), mPoint1(tri.p1), mPoint2(tri.p2)
  {
  }
...
#define PlasmaDebugPrimitive(X) ConstMax < sizeof(X),
#include "DebugPrimitives.inl"
#undef PlasmaDebugPrimitive
    0
#define PlasmaDebugPrimitive(X) > ::Result
#include "DebugPrimitives.inl"
#undef PlasmaDebugPrimitive
    ;
...
#define PlasmaDebugPrimitive(X) DebugObjectAddMethods(X);
#include "DebugPrimitives.inl"
#undef PlasmaDebugPrimitive
