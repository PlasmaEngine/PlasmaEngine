...
LightningDefineType(SelectionChangedEvent, builder, type)
{
}

LightningDefineType(MetaSelection, builder, type)
{
  LightningBindMethod(Count);
  LightningBindMethod(Empty);
  LightningFullBindMethod(builder, type, &MetaSelection::Clear, (void (MetaSelection::*)()), "Clear", LightningNoNames);
  LightningBindMethod(SelectOnly);
  LightningFullBindMethod(builder, type, &MetaSelection::Add, (void (MetaSelection::*)(HandleParam)), "Add", LightningNoNames);
  LightningBindMethod(Remove);
  LightningBindMethod(Replace);
  LightningBindMethod(Contains);
  LightningBindMethod(FinalSelectionChanged);
  LightningBindGetterSetterProperty(Primary);

  PlasmaBindEvent(Events::SelectionChanged, Event);
  PlasmaBindEvent(Events::SelectionFinal, Event);

  LightningBindCustomGetter(All);
}
