...
{
public:
  LightningDeclareType(Event, TypeCopyMode::ReferenceType);
...
{
public:
  LightningDeclareType(ObjectEvent, TypeCopyMode::ReferenceType);
...
Connect(targetType* dispatcherObject, StringParam eventId, classType* receiver, void (classType::*function)(eventType*))
{
  ...
  // it
  BoundType* type = LightningTypeId(eventType);
  connection->EventType = type;
  ErrorIf(type->IsInitialized() == false,
          "The event type was never initialized using LightningDeclareType / "
          "LightningDefineType / LightningInitializeType");
...
#define ConnectThisTo(target, eventname, handle)                                                                       \
  do                                                                                                                   \
  {                                                                                                                    \
    ::Plasma::Connect(target, eventname, this, &LightningSelf::handle);                                                      \
  } while (false)
...
{
public:
  LightningDeclareType(EventObject, TypeCopyMode::ReferenceType);
