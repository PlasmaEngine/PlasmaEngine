...
namespace PL
{
Array<NotificationCallback> gNotifyCallbackStack(PlasmaInit, DefaultDoNotify);
}
...
LightningDefineType(NotifyEvent, builder, type)
{
}
...
{
  PL::gNotifyCallbackStack.Back()(title, message, icon, type, expections);
}
...
TemporaryDoNotifyOverride::TemporaryDoNotifyOverride(NotificationCallback callback)
{
  ...
  if (callback != nullptr)
    PL::gNotifyCallbackStack.PushBack(callback);
}

TemporaryDoNotifyOverride::~TemporaryDoNotifyOverride()
{
  if (mCallback != nullptr)
    PL::gNotifyCallbackStack.PopBack();
}
