...
#define DeclareThreadSafeReferenceCountedHandleNoData(type)                                                            \
  ...
  int Release()                                                                                                        \
  {                                                                                                                    \
    ErrorIf(mReferenceCount.mCount == 0, "Invalid Release. ReferenceCount is plasma.");                                  \
...
#define ConstructThreadSafeReferenceCountedHandle()                                                                    \
  ErrorIf(LightningVirtualTypeId(this)->HandleManager !=                                                                   \
              LightningManagerId(ThreadSafeReferenceCountedHandleManager<LightningSelf>),                                      \
          "Set type->HandleManager = "                                                                                 \
          "LightningManagerId(ThreadSafeReferenceCountedHandleManager<LightningSelf>; in "                                     \
          "binding");                                                                                                  \
  mLock.Lock();                                                                                                        \
...
#define PlasmaBindThreadSafeReferenceCountedHandle()                                                                     \
  type->HandleManager = LightningManagerId(ThreadSafeReferenceCountedHandleManager<LightningSelf>);

// Call in engine/system initialization for type that will be using this manager
#define PlasmaRegisterThreadSafeReferenceCountedHandleManager(type)                                                      \
  LightningRegisterSharedHandleManager(ThreadSafeReferenceCountedHandleManager<type>);

// Inherit from this class to get all standard behavior of this handle manager
class ThreadSafeReferenceCounted
{
public:
  LightningDeclareType(ThreadSafeReferenceCounted, TypeCopyMode::ReferenceType);
