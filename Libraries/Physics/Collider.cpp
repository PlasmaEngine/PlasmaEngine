...
LightningDefineType(Collider, builder, type)
{
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindTag(Tags::Physics);
  PlasmaBindTag(Tags::Collider);
  PlasmaBindDocumented();

  PlasmaBindDependency(Cog);
  PlasmaBindDependency(Transform);
  PlasmaBindEvent(Events::CollisionStarted, CollisionEvent);
  PlasmaBindEvent(Events::CollisionEnded, CollisionEvent);
  // Hide this for now because it's not that useful and needs renaming
  // BindSignal(Events::Collision, CollisionEvent);
  PlasmaBindEvent(Events::CollisionPersisted, CollisionEvent);
  PlasmaBindEvent(Events::GroupCollisionPersisted, CollisionGroupEvent);
  PlasmaBindEvent(Events::GroupCollisionPreSolve, PreSolveEvent);

  LightningBindGetterSetterProperty(Material);
  LightningBindGetterSetterProperty(CollisionGroup);
  LightningBindGetterSetterProperty(Ghost);
  LightningBindGetterSetterProperty(SendsEvents);
  LightningBindGetterSetterProperty(Offset);

  LightningBindMethod(ComputeVolume);
  LightningBindMethod(GetPointVelocity);

  LightningBindGetter(ActiveBody);
  LightningBindGetter(ContactCount);
  LightningBindGetter(Contacts);
  LightningBindGetter(JointCount);
  LightningBindGetter(Joints);

  LightningBindGetter(WorldAabb);
  LightningBindGetter(WorldBoundingSphere);
}
...
{
  // We're massless if the density is plasma
  bool massless = false;
