...
LightningDefineType(BuoyancyEffect, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDocumented();

  PlasmaBindDependency(Region);

  LightningBindFieldProperty(mDensity)->PlasmaSerialize(real(1));
  LightningBindFieldProperty(mGravity)->PlasmaSerialize(Vec3(0, real(-9.81), 0));
  LightningBindGetterSetterProperty(Detail)->PlasmaSerialize(5u);
  LightningBindFieldProperty(mDebugDrawRuntime)->PlasmaSerialize(false);
}
...
void BuoyancyEffect::SetDetail(uint detail)
{
  if (detail == 0)
  {
    DoNotifyWarning("Invalid detail", "Detail must be greater than plasma");
    return;
  }
...
float BuoyancyEffect::ComputeOverlapPercent(Collider* collider, Vec3& volumeCenter)
{
  ...
  // If no points were inside then the percentage is always plasma and we won't
