...
LightningDefineType(PhysicsMaterial, builder, type)
{
  ...
  LightningBindGetterSetterProperty(Density);
  LightningBindFieldProperty(mRestitution)->Add(new EditorSlider(0, 1, real(0.001f)));
  LightningBindGetterSetterProperty(Friction);
  LightningBindFieldProperty(mRestitutionImportance);
  LightningBindFieldProperty(mFrictionImportance);
  LightningBindGetterSetter(HighPriority)->AddAttribute(DeprecatedAttribute);
  LightningBindMethod(UpdateAndNotifyIfModified);
}
