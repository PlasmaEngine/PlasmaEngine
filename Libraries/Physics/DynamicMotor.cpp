...
LightningDefineType(DynamicMotor, builder, type)
{
  PlasmaBindComponent();
  ...
  LightningBindMethod(MoveInDirection);
  LightningBindMethod(SetReferenceFrameToWorld);
  LightningBindMethod(SetReferenceFrameToObject);
  LightningBindGetterSetter(Active);
  LightningBindGetterSetter(MaxMoveImpulse);
}
