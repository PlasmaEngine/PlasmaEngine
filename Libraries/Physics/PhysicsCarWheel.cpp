...
LightningDefineType(PhysicsCarWheel, builder, type)
{
  PlasmaBindComponent();
  ...
  LightningBindGetterSetterProperty(PhysicsCarPath);
  LightningBindGetterSetterProperty(SpringMinLength);
  LightningBindGetterSetterProperty(SpringStartLength);
  LightningBindGetterSetterProperty(SpringMaxLength);
  LightningBindFieldProperty(mSpringRestLength);
  LightningBindFieldProperty(mDriveFactor);
  LightningBindGetterSetterProperty(SteerFactor);
  LightningBindGetterSetterProperty(Radius);

  LightningBindFieldProperty(mFrequencyHz);
  LightningBindFieldProperty(mDampingCompressionRatio);
  LightningBindFieldProperty(mDampingRelaxationRatio);
  LightningBindGetterSetterProperty(MaxSpringForce);
  LightningBindGetterSetterProperty(MaxBrakeStrength);
  LightningBindGetterSetterProperty(IsDriveWheel);
  LightningBindGetterSetterProperty(Is2DWheel);
  LightningBindFieldProperty(mForwardStaticFriction);
  LightningBindFieldProperty(mForwardDynamicFriction);
  LightningBindFieldProperty(mSideStaticFriction);
  LightningBindFieldProperty(mSideDynamicFriction);
  LightningBindFieldProperty(mGripScalar);
  LightningBindFieldProperty(mMaxSpringCompressionDistance);
  LightningBindFieldProperty(mMaxSpringRelaxationDistance);

  LightningBindGetterSetterProperty(WheelLocalStartPosition);
  LightningBindGetterSetterProperty(PreRotation);

  LightningBindGetterSetterProperty(WorldWheelBasis)->Add(new EditorRotationBasis("PhysicsCarWheelBasisGizmo"));

  LightningBindGetter(IsInContact);
  LightningBindGetter(IsSliding);
  LightningBindGetter(Rotation);
  LightningBindGetter(RotationalVelocity);
  LightningBindGetter(Grip);
  LightningBindGetter(NormalImpulse);
  LightningBindGetter(ForwardImpulse);
  LightningBindGetter(SideImpulse);
  LightningBindGetter(SpringLength);
  LightningBindGetter(ContactedObject);
  LightningBindGetter(ContactPoint);
  LightningBindGetter(ContactNormal);
  LightningBindGetter(WorldAxleAxis);
  LightningBindGetter(WorldForwardAxis);
  LightningBindGetter(WorldSpringAxis);
  LightningBindGetter(WorldLinearVelocity);
  LightningBindGetter(WorldAngularVelocity);
...
void PhysicsCarWheel::CalculateSpringForce()
{
  ...
  // just set the relative velocity to 0 to avoid a plasma division
...
real PhysicsCarWheel::InternalFrictionCalculation(Vec3Param dir, ConstraintInfo& info)
{
  ...
  // Prevent plasma divisions (most likely from 2d mode)
