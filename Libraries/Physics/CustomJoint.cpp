...
LightningDefineType(CustomJointEvent, builder, type)
{
  PlasmaBindDocumented();
  LightningBindFieldProperty(mOwner);
  LightningBindFieldProperty(mDt);
}

LightningDefineType(CustomConstraintInfo, builder, type)
{
  PlasmaBindDocumented();
  LightningBindFieldProperty(mLinear0);
  LightningBindFieldProperty(mAngular0);
  LightningBindFieldProperty(mLinear1);
  LightningBindFieldProperty(mAngular1);

  LightningBindGetterSetterProperty(EffectiveMass);
  LightningBindFieldProperty(mGamma);
  LightningBindFieldProperty(mBias);
  LightningBindFieldProperty(mMinImpulse);
  LightningBindFieldProperty(mMaxImpulse);
  LightningBindFieldProperty(mImpulse);
  LightningBindFieldProperty(mError);
  LightningBindFieldProperty(mBaumgarte);
  LightningBindFieldProperty(mActive);
  LightningBindFieldProperty(mSolvePosition);

  LightningBindMethod(SetJacobian);
  LightningBindMethod(SetErrorAndBias);
  LightningBindMethod(ComputeMotor);
  LightningBindMethod(ComputeSpring);
  LightningBindMethod(DetachFromOwner);
  LightningBindMethod(IsOwned);
...
LightningDefineType(CustomJoint, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindInterface(Joint);
  PlasmaBindDocumented();
  PlasmaBindEvent(Events::ComputeCustomJointInfo, CustomJointEvent);

  LightningBindMethod(CreateConstraint);
  LightningBindMethod(AddConstraint);
  LightningBindMethod(RemoveConstraint);
  LightningBindMethod(ClearConstraints);

  LightningBindGetter(ConstraintCount);
  LightningBindMethod(GetConstraint);
}
