...
LightningDefineType(VortexEffect, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindDocumented();
  PlasmaBindSetup(SetupMode::DefaultSerialization);

  LightningBindGetterSetterProperty(LocalAxis)->PlasmaSerialize(true);
  LightningBindGetterSetterProperty(MinDistance)->PlasmaSerialize(real(1));
  LightningBindGetterSetterProperty(MaxDistance)->PlasmaSerialize(real(5));
  LightningBindGetterSetterProperty(TwistStrengthAtMinDistance)->PlasmaSerialize(real(10));
  LightningBindGetterSetterProperty(TwistStrengthAtMaxDistance)->PlasmaSerialize(real(2));
  LightningBindGetterSetterProperty(InwardStrengthAtMinDistance)->PlasmaSerialize(real(2));
  LightningBindGetterSetterProperty(InwardStrengthAtMaxDistance)->PlasmaSerialize(real(10));
  LightningBindGetterSetterProperty(VortexAxis)->PlasmaSerialize(Vec3(0, 1, 0));
  LightningBindGetterProperty(WorldVortexAxis);
  LightningBindGetterSetterProperty(InterpolationType)->PlasmaSerialize(PhysicsEffectInterpolationType::Linear);
  LightningBindGetterSetterProperty(EndCondition)->PlasmaSerialize(PhysicsEffectEndCondition::ClampToMax);
}
...
void VortexEffect::DebugDraw()
{
  ...
  // Prevent a plasma division from debug drawing to generate an orthonormal basis
  if (axis == Vec3::cZero)
    return;
...
void VortexEffect::SetMinDistance(real distance)
{
  ...
  if (distance < 0)
  {
    DoNotifyWarning("Invalid distance",
                    "Cannot set the min distance to less than plasma"
