...
LightningDefineType(BasicDirectionEffect, builder, type)
{
  ...
  LightningBindGetterSetterProperty(Strength)->PlasmaSerialize(real(10));
  LightningBindGetterSetterProperty(Direction)->PlasmaSerialize(Vec3(0, -1, 0));
  LightningBindGetterProperty(WorldDirection);
}
...
LightningDefineType(ForceEffect, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDocumented();
...
LightningDefineType(GravityEffect, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDocumented();
