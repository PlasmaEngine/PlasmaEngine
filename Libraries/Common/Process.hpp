...
namespace Plasma
{
class PlasmaShared ProcessStartInfo
{
public:
  ProcessStartInfo();
...
protected:
  PlasmaDeclarePrivateData(Process, 48);
};
...
PlasmaShared void GetProcesses(Array<ProcessInfo>& results);
/// Given a process id (from the ProcessInfo struct) kill the process.
PlasmaShared void KillProcess(OsInt processId, int exitCode = 1);
...
/// for flags.
PlasmaShared void RegisterApplicationRestartCommand(StringParam commandLineArgs, uint flags = 0);
