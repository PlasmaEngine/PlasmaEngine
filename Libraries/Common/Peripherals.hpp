...
PlasmaShared bool AreGamepadsEnabled();
...
PlasmaShared bool GetGamepadState(size_t gamepadIndex, GamepadState* stateOut);
// The left and right motor speeds are set on a [0.0 - 1.0] range with 1.0 being
// max rumble.
PlasmaShared bool SetGamepadVibration(size_t gamepadIndex, float leftSpeed, float rightSpeed);
