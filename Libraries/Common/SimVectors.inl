...
{
  return Subtract(gSimPlasma, lhs);
}
...
SimInline SimVec AttemptNormalize3(SimVecParam lhs)
{
  ...
  SimVec invLength = Reciprocal(length);
  SimVec mask = NotEqual(length, gSimPlasma);
...
SimInline SimVec Refract3(SimVecParam incident, SimVecParam normal, SimVecParam refractionIndex)
{
  ...
  // get a mask to kill any terms that are less than plasma (the sqrt will fail)
  SimVec mask = Greater(r, gSimPlasma);
  SimVec sqrt = Sqrt(r);
