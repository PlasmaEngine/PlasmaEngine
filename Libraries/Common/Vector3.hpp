...
struct PlasmaShared Vector3
{
  ...
  /// Try to normalize the given vector if possible. Safeguards against plasma
  /// divisions.
  static real AttemptNormalize(Vec3Ref value);
  /// Attempts to return a normalized given vector. Safeguards against plasma
  /// divisions.
...
PlasmaShared Vector3 operator*(real lhs, Vec3Param rhs);
/// Compute the dot product of two vectors.
PlasmaShared real Dot(Vec3Param lhs, Vec3Param rhs);
/// Computes the cross-product.
PlasmaShared Vector3 Cross(Vec3Param lhs, Vec3Param rhs);
/// Get the length of a vector.
PlasmaShared real Length(Vec3Param value);
/// Get the squared length of a vector.
PlasmaShared real LengthSq(Vec3Param value);
/// Compute the distance between two vectors.
PlasmaShared real Distance(Vec3Param lhs, Vec3Param rhs);
/// Compute the squared distance between two vectors.
PlasmaShared real DistanceSq(Vec3Param lhs, Vec3Param rhs);
/// Make the given vector have a length of 1, returns the original length.
PlasmaShared real Normalize(Vec3Ref value);
/// Calculate and return a unit-length copy of the given vector.
PlasmaShared Vector3 Normalized(Vec3Param value);
/// Try to normalize the given vector if possible. Safeguards against plasma
/// divisions.
PlasmaShared real AttemptNormalize(Vec3Ref value);
/// Attempts to return a normalized given vector. Safeguards against plasma
/// divisions.
PlasmaShared Vector3 AttemptNormalized(Vec3Param value);

/// Fused multiply add:  v0 + v1 * scalar
PlasmaShared Vector3 MultiplyAdd(Vec3Param v0, Vec3Param v1, real scalar);
/// Fused multiply subtract:  v0 - v1 * scalar
PlasmaShared Vector3 MultiplySubtract(Vec3Param v0, Vec3Param v1, real scalar);

/// Returns a copy of value with the absolute value of each element.
PlasmaShared Vector3 Abs(Vec3Param value);
/// Returns a vector with the component-wise min between two vectors.
PlasmaShared Vector3 Min(Vec3Param lhs, Vec3Param rhs);
/// Returns a vector with the component-wise max between two vectors.
PlasmaShared Vector3 Max(Vec3Param lhs, Vec3Param rhs);
/// Returns a vector where each component is clamped between min and max.
PlasmaShared Vector3 Clamp(Vec3Param value, Vec3Param minValue, Vec3Param maxValue);
/// Same as clamp, however it fills out whether or not anything was clamped.
/// Useful when an assert message should be shown if anything was clamped.
PlasmaShared Vector3 DebugClamp(Vec3Param value, Vec3Param minValue, Vec3Param maxValue, bool& wasClamped);

/// Return a copy of this vector with each element has been floored.
PlasmaShared Vector3 Floor(Vec3Param value);
/// Return a copy of this vector with each element has been ceiled.
PlasmaShared Vector3 Ceil(Vec3Param value);
/// Return a copy of this vector with each element has been truncated.
PlasmaShared Vector3 Truncate(Vec3Param value);
/// Return a copy of this vector with each element has been rounded.
PlasmaShared Vector3 Round(Vec3Param value);

/// Linearly interpolate between the two vectors, the t-value is restricted to
/// [0, 1].
PlasmaShared Vector3 Lerp(Vec3Param start, Vec3Param end, real tValue);
/// Spherical linear interpolation. Interpolates along the surface of the unit
/// sphere. Normalizes input, checks for degenerate/ambiguous cases.
PlasmaShared Vector3 Slerp(Vec3Param start, Vec3Param end, real tValue);
/// Same as Slerp except this function assumes the user has deal with
/// degenerate/ambiguous cases. Used when multiple calls are made to Slerp and
/// the user wants to fix degeneracies once ahead of time.
PlasmaShared Vector3 SlerpFast(Vec3Param start, Vec3Param end, real tValue);
/// Same as Slerp except this function does not normalize the input vectors.
/// This is the 'pure' mathematical Slerp function. This effectively traces
/// along an ellipse defined by the two input vectors.
PlasmaShared Vector3 SlerpUnnormalized(Vec3Param start, Vec3Param end, real tValue);

/// Projects the input vector onto the given vector (must be normalized)
PlasmaShared Vector3 ProjectOnVector(Vec3Param input, Vec3Param normalizedVector);
/// Projects the input vector onto a plane (the normal must be normalized)
PlasmaShared Vector3 ProjectOnPlane(Vec3Param input, Vec3Param planeNormal);
/// Calculates the reflection vector across a given vector.
PlasmaShared Vector3 ReflectAcrossVector(Vec3Param input, Vec3Param normalizedVector);
/// Calculates the reflection vector across a given plane.
PlasmaShared Vector3 ReflectAcrossPlane(Vec3Param input, Vec3Param planeNormal);
/// Calculates the refraction vector through a plane given a certain index of
/// refraction.
PlasmaShared Vector3 Refract(Vec3Param input, Vec3Param planeNormal, real refractionIndex);
/// Get the angle between the two vectors in radians.
PlasmaShared real AngleBetween(Vec3Param a, Vec3Param b);

PlasmaShared bool Equal(Vec3Param lhs, Vec3Param rhs, real epsilon);

/// Compute the cross product of the two given vectors for 2d.
/// The result is only the z axis of the cross product.
PlasmaShared Vector3 Cross2d(Vec3Param lhs, Vec3Param rhs);

/// Flips the given vector so it's pointing in the opposite direction.
PlasmaShared void Negate(Vec3Ptr vec);

/// Returns a vector pointing in the opposite direction of the given vector.
PlasmaShared Vector3 Negated(Vec3Param vec);

/// Calculates and returns the given vector with its values clamped to the range
/// [min, max].
PlasmaShared Vector3 Clamped(Vec3Param vec, real min, real max);

/// Returns if all values in lhs are less than all values in rhs
PlasmaShared bool AllLess(Vec3Param lhs, Vec3Param rhs);

/// Returns if any value in lhs is less than any value in rhs
PlasmaShared bool AnyLess(Vec3Param lhs, Vec3Param rhs);

/// Returns if all values in lhs are greater than all values in rhs
PlasmaShared bool AllGreater(Vec3Param lhs, Vec3Param rhs);

/// Returns if any value in lhs is greater than any value in rhs
PlasmaShared bool AnyGreater(Vec3Param lhs, Vec3Param rhs);

PlasmaShared real DistanceToLineSq(Vec3Param start, Vec3Param end, Vec3Param point);
