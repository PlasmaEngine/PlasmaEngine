...
// We really don't care about C++ initializing POD types to plasma
#  pragma warning(disable : 4345)
...
// Disable the exception handling warning. We don't use exceptions in Lightning
...
// LightningDeclareType is used on classes that sometimes derive from a base class
// with LightningGetDerivedType() creating a discrepancy we can't avoid
...
// We declare typedefs that may be useful for people using Lightning, but we don't
// use them ourselves
#  pragma GCC diagnostic ignored "-Wunused-local-typedefs"
#endif

// PlasmaForceInline forces a function to be inlined for optimization purposes
// PlasmaNoInline forces a function to not be inlined (only used to debug, and
...
#define PlasmaOffsetOfHelper(structure, op, member)                                                                      \
...
#  endif
#  define PlasmaNoInline __declspec(noinline)
#else
#  define PlasmaThreadLocal __thread
#  define PlasmaImport __attribute__((visibility("default")))
...
#define PlasmaNoImportExport

#if defined(PlasmaImportDll)
#  define PlasmaShared PlasmaImport
#  define PlasmaSharedTemplate
#else
#  define PlasmaShared PlasmaExport
#  define PlasmaSharedTemplate
#endif
