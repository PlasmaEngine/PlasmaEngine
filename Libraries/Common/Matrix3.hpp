...
PlasmaShared Matrix3 operator*(real lhs, Mat3Param rhs);

/// Multiply the two matrices together. Matrix multiplication order is
/// right-to-left.
PlasmaShared Matrix3 Multiply(Mat3Param lhs, Mat3Param rhs);
/// Multiply the given vector by a matrix.
PlasmaShared Vector3 Multiply(Mat3Param lhs, Vec3Param rhs);
/// Multiply the given vector by a matrix. The vector is promoted to the point
/// Vec3(x, y, 1). No homogeneous division is applied.
PlasmaShared Vector2 MultiplyPoint(Mat3Param lhs, Vec2Param rhs);
/// Multiply the given vector by a matrix. The vector is promoted to the vector
/// Vec3(x, y, 0).
PlasmaShared Vector2 MultiplyNormal(Mat3Param lhs, Vec2Param rhs);

/// This builds a matrix that should be used on 2D points/vectors
PlasmaShared Matrix3 BuildTransform(Vec2Param translate, real radians, Vec2Param scale);

/// This builds a matrix that should be used on 3D points/vectors
PlasmaShared Matrix3 BuildTransform(QuatParam rotate, Vec3Param scale);

PlasmaShared Vector3 Transform(Mat3Param mat, Vec3Param vector);
PlasmaShared void Transform(Mat3Param matrix, Vec3Ptr vector);

/// Applies transformation with the translation (p.x, p.y, 1)
PlasmaShared Vector2 TransformPoint(Mat3Param matrix, Vec2Param vector);

/// Applies transformation without the translation (n.x, n.y, 0)
PlasmaShared Vector2 TransformNormal(Mat3Param matrix, Vec2Param normal);

/// Transforms the given vector by the matrix as if the matrix was transposed.
PlasmaShared Vector3 TransposedTransform(Mat3Param mat, Vec3Param vector);

/// Transforms the given vector by the matrix as if the matrix was transposed.
PlasmaShared void TransposedTransform(Mat3Param matrix, Vec3Ptr vector);

PlasmaShared real Trace(Mat3Param matrix);

PlasmaShared real Cofactor(Mat3Param matrix, uint row, uint column);

/// Takes a symmetric matrix and diagonalizes it.
PlasmaShared void Diagonalize(Mat3Ptr matrix);
PlasmaShared Matrix3 Diagonalized(Mat3Param matrix);

PlasmaShared void Invert(Mat3Ptr matrix);
PlasmaShared Matrix3 Inverted(Mat3Param matrix);
