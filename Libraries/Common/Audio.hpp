...
public:
  ...
  StreamStatus::Enum InitializeAPI(Plasma::String* resultMessage);
  // Initializes the specified audio stream
  StreamStatus::Enum InitializeStream(StreamTypes::Enum whichStream, Plasma::String* resultMessage);
  // Starts the specified audio stream, which will use the provided callback
  // function to either request or provide audio data.
  StreamStatus::Enum
  StartStream(StreamTypes::Enum whichStream, Plasma::String* resultMessage, IOCallbackType* callback, void* callbackData);
  // Stops the specified audio stream
  StreamStatus::Enum StopStream(StreamTypes::Enum whichStream, Plasma::String* resultMessage);
