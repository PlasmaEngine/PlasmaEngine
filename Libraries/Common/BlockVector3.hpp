...
struct PlasmaShared BlockVector3
{
  uint GetSize() const;
  ...
  Plasma::Array<Vector3> mBlocks;
};

struct PlasmaShared BlockMatrix3
{
  uint GetSize() const;
  void SetSize(uint size);
  ...
  typedef Plasma::Array<Matrix3> Cells;
  typedef Plasma::Array<Cells> Rows;
  Rows mBlocks;
};

struct PlasmaShared BlockCgPolicy
{
