...
public:
  ...
  {
    Plasma::Swap(mData, other.mData);
    Plasma::Swap(mSize, other.mSize);
    Plasma::Swap(mCapacity, other.mCapacity);
  }
  ...
  void PushBack(MoveReference<value_type> item)
  {
    ...
    ConstructWith(mData + mSize, PlasmaMove(*toBeAdded));
    ++mSize;
  }
  ...
  {
    Insert(mData + mSize, PlasmaMove(inputRange));
  }
  ...
  void Insert(pointer where, MoveReference<inputRangeType> inputRange)
  {
    ...
    for (; !inputRange->Empty(); inputRange->PopFront())
    {
      ConstructWith(buffer, PlasmaMove(inputRange->Front()));
      ++buffer;
    }
  ...
  void InsertAt(size_type index, MoveReference<inputRangeType> inputRange)
  {
    // for insertion the index can be mSize
    ErrorIf(index > mSize, "Access array out of bounds.");
    Insert(mData + index, PlasmaMove(inputRange));
  }
  ...
  void Insert(pointer where, MoveReference<value_type> value)
  {
    ...
    Insert(where, PlasmaMove(singleValue));
  }
  ...
  void InsertAt(size_type index, MoveReference<value_type> value)
  {
    ...
    ErrorIf(index > mSize, "Access array out of bounds.");
    if (index == mSize)
      PushBack(PlasmaMove(value));
    else
      Insert(mData + index, PlasmaMove(value));
  }
...
protected:
  ...
  void ExpandToNewSize(size_type newSize)
  {
    ...
    {
      // If capacity is not plasma expand by %50 of capacity
      // if it is plasma set it to 2
...
class PlasmaSharedTemplate PodArray : public Array<ValueType, Allocator, PodOverride>
{
public:
