    ...
    mLauncher(launcher)
{
  ...
  textArea->SetSizing(SizeAxis::X, SizePolicy::Fixed, Pixels(429));
  {
    ...
    buildRow->SetLayout(CreateRowLayout());
    {
      ...
      // If we are to run with the debugger (plasma is currently in visual studio
      // with the debugger attached) then add an extra button to communicate
      // with the open plasma
...
void ActiveProjectMenu::SelectProject(CachedProject* cachedProject)
{
  ...
  mActiveSizeTask = PL::gBackgroundTasks->CreateTask(job);
  ...
  PlasmaBuild* build = mLauncher->mVersionSelector->FindExactVersion(mCachedProject);
...
void ActiveProjectMenu::OnProjectNameTextSubmit(Event* e)
{
  ...
  String confirmMsg = String::Format("Rename '%s/%s.plasmaproj' to '%s/%s.plasmaproj'?",
...
void ActiveProjectMenu::OnBackupProject(Event* e)
{
  ...
  PlasmaPrint("Backing up project '%s' to location '%s'.", projectDirectory.c_str(), backupFilePath.c_str());
...
void ActiveProjectMenu::OnBuildStateChanged(Event* e)
{
  ...
  PlasmaBuild* activeBuild = mBuildSelector->GetBuild();
  if (activeBuild != nullptr)
  {
    ...
    PlasmaPrint("Changing project '%s' from Build %s to %s\n",
...
void ActiveProjectMenu::OnConfirmBuildVersionChange(ModalConfirmEvent* e)
{
  ...
  if (e->mConfirmed)
  {
    ...
    if (activeBuild != nullptr)
    {
      PlasmaBuild* newBuild = (PlasmaBuild*)e->mUserData;
...
void ActiveProjectMenu::OnLaunchProject(Event* e)
{
  ...
  // running in plasma) so reload it but make sure to preserve the current version
  // id we have loaded in the cache.
  mLauncher->mProjectCache->ReloadProjectFile(mCachedProject, true);
  PlasmaBuild* version = versionSelector->FindExactVersion(mCachedProject);
  if (version != nullptr)
  {
...
void ActiveProjectMenu::OnInstallPressed(Event* e)
{
  ...
  PlasmaBuild* build = mBuildSelector->GetBuild();
