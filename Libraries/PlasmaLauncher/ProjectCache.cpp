...
LightningDefineType(LauncherProjectInfo, builder, type)
{
  PlasmaBindComponent();
...
bool CachedProject::Load(StringParam projectFilePath)
{
  ...
  mProjectCog = PL::gFactory->CreateFromStream(PL::gEngine->GetEngineSpace(), mLoader, 0, nullptr);
...
void CachedProject::UpdateTexture()
{
  ...
  BackgroundTask* task = PL::gBackgroundTasks->Execute(job, "Update texture");
...
{
  // Rename the plasmaproj file in the old location (avoid calling delete where
  // possible)
  String renamedPlasmaProjPath = FilePath::CombineWithExtension(mProjectFolder, newProjectName, ".plasmaproj");
  PlasmaPrint("Moving and renaming project '%s' to '%s'.\n", mProjectPath.c_str(), renamedPlasmaProjPath.c_str());

  bool success = MoveFile(renamedPlasmaProjPath, mProjectPath);
  ...
  // Update the project path and the project name in the plasmaproj
  SetProjectPropertyValue("ProjectName", newProjectName);

  // Update the path of the plasma proj file
  mProjectPath = FilePath::CombineWithExtension(newFolder, newProjectName, ".plasmaproj");

  // Save out the new plasmaproj file that should have up-to-date properties
  Save(false);
...
void CachedProject::AddOrReplaceDataNodeComponent(Component* component)
{
  ...
  bool foundDuplicate;
  String componentName = component->LightningGetDerivedType()->Name;
...
{
  mRecentProjects = PL::gEngine->GetConfigCog()->has(RecentProjects);
}
...
CachedProject* ProjectCache::LoadProjectFile(StringParam projectFilePath)
{
  ...
  // Set our map of plasmaproj files paths to project
                                                       ...
                                                       const HashSet<String>& projectTags)
{
  ...
  // Get the old plasma proj file (by the template name)
  String oldProjectName = FilePath::GetFileNameWithoutExtension(templatePath);
  String oldProjectFilePath = FindPlasmaProj(projectDir, oldProjectName);
  // Build the new project's full path
  String newProjectFilePath = FilePath::CombineWithExtension(projectDir, projectName, ".plasmaproj");
  ...
  // Create the cached project from this template plasma proj
...
String ProjectCache::FindPlasmaProj(StringParam searchpath, StringParam projectName)
{
  // Get the old plasma proj file (by the template name)
  String expectedProjectFilePath = FilePath::CombineWithExtension(searchpath, projectName, ".plasmaproj");

  // If this file exists then we can just return its path
  if (FileExists(expectedProjectFilePath))
    return expectedProjectFilePath;

  // Otherwise, search the path for any plasma proj file and return the first we
  // find
