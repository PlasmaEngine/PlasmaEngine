...
BuildItem::BuildItem(Composite* parent, PlasmaBuild* version, BuildsMenu* buildsMenu) :
    Composite(parent),
    mVersion(version),
...
{
  // The user wants to retry (they are manually closing their copies of plasma)
  ...
  // The user wants us to close all copies of plasma that are open with that build
...
{
  PlasmaPrint("Failed to download build %s\n", mVersion->GetDebugIdString().c_str());
...
void ReleaseNotes::DisplayReleaseNotes(PlasmaBuild* build)
{
  if (mSelectedBuild)
...
void BuildsMenu::CreateBuildItems()
{
  ...
  PlasmaBuildTagPolicy policy(launcherConfig);
  ...
  forRange (PlasmaBuild* version, results.All())
  {
...
{
  PlasmaPrint("Loading builds list\n");
  CreateBuildItems();
}
...
{
  PlasmaPrint("Builds menu displayed\n");
...
void BuildsMenu::UpdateInstallButton()
{
  ...
  PlasmaBuild* version = selectedBuild->mVersion;
