...
LightningDefineType(Orientation, builder, type)
{
  PlasmaBindComponent();
  ...
  LightningBindMethod(DebugDrawBases);

  LightningBindGetterSetterProperty(DefaultOrientationBases);
  LightningBindGetterSetterProperty(LocalOrientationBasis)->Add(new EditorRotationBasis("OrientationBasisGizmo"));

  LightningBindGetter(OrientationRight);
  LightningBindGetter(OrientationUp);
  LightningBindGetter(OrientationForward);

  LightningBindGetter(LocalRight);
  // Show the local up and forward as read-only properties
  LightningBindGetterProperty(LocalUp);
  LightningBindGetterProperty(LocalForward);

  LightningBindGetter(WorldRight);
  LightningBindGetter(WorldUp);
  LightningBindGetter(WorldForward);

  LightningBindGetter(LocalToWorldRotation);
  LightningBindGetter(LocalToOrientationRotation);
  LightningBindGetter(OrientationToLocalRotation);
  LightningBindGetter(OrientationToWorldRotation);
  LightningBindGetter(WorldToLocalRotation);
  LightningBindGetter(WorldToOrientationRotation);

  LightningBindMethod(SetLocalLookAtRotation);
  LightningBindMethod(SetWorldLookAtRotation);

  LightningBindGetterSetterProperty(GlobalUp);
  LightningBindMethod(LookAtPoint);
  LightningBindMethod(LookAtDirection);

  LightningBindMethod(GetLookAtPointRotation);
  LightningBindMethod(GetLookAtDirectionRotation);

  LightningBindMethod(LookAtPointWithUp);
  LightningBindMethod(LookAtDirectionWithUp);

  LightningBindMethod(GetLookAtPointWithUpRotation);
  LightningBindMethod(GetLookAtDirectionWithUpRotation);

  LightningBindGetter(AbsoluteAngle);
  LightningBindMethod(ComputeSignedAngle);
}
...
void Orientation::SetGlobalUp(Vec3Param globalUp)
{
  ...
  {
    DoNotifyWarning("Invalid Global Up", "The global up cannot be the plasma vector");
    return;
  }
  mGlobalUp = globalUp;
}
