...
LightningDefineType(QuickHull3DInterface, builder, type)
{
  LightningBindDefaultCopyDestructor();
  type->CreatableInScript = true;

  LightningBindMethod(Add);
  LightningBindMethod(Build);
  LightningBindMethod(Clear);
  LightningBindMethod(Draw);
  LightningBindField(mIndex);
  LightningBindField(mShowDebugDraw);
