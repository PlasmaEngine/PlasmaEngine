...
LightningDefineType(PathFinder, builder, type)
{
  PlasmaBindDocumented();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindEvent(Events::PathFinderFinished, PathFinderEvent<Vec3>);

  LightningBindMethod(FindPath);
  LightningBindMethod(FindPathThreaded);

  LightningBindField(mMaxIterations);
}
...
HandleOf<ArrayClass<Vec3>> PathFinder::FindPath(Vec3Param worldStart, Vec3Param worldGoal)
{
  ...
  HandleOf<ArrayClass<Vec3>> array = LightningAllocate(ArrayClass<Vec3>);
...
LightningDefineType(PathFinderRequest, builder, type)
{
  PlasmaBindDocumented();
  LightningBindMethod(Cancel);
  LightningBindFieldGetter(mPathFinderComponent);
  LightningBindFieldGetter(mStatus);
}
...
LightningDefineType(PathFinderBaseEvent, builder, type)
{
  PlasmaBindDocumented();
  LightningBindFieldGetter(mRequest);
  LightningBindFieldGetter(mDuration);
}
