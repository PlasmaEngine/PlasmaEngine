...
LightningDefineType(ContentSystem, builder, type)
{
...
LightningDefineType(ContentSystemEvent, builder, type)
{
}
...
ContentItem* ContentSystem::AddContentItemToLibrary(Status& status, AddContentItemInfo& info)
{
  ...
  if (FileExists(metaFile))
  {
    ...
    entry = PL::gContentSystem->Creators.FindValue(node.TypeName, ContentTypeEntry());
...
{
  return FilePath::Combine(PL::gContentSystem->HistoryPath, library->Name);
}
