...
void AnimationProcessor::ExtractAndProcessAnimationData(const aiScene* scene)
{
  ...
  for (size_t animIndex = 0; animIndex < numAnimations; ++animIndex)
  {
    ...
    AnimationData plasmaAnimationData;
    plasmaAnimationData.AnimationName = CleanAssetName(sceneAnimation->mName.C_Str());

    float ticksPerSecond = (float)sceneAnimation->mTicksPerSecond != 0 ? (float)sceneAnimation->mTicksPerSecond : 1.f;
    plasmaAnimationData.AnimationDuration = (float)sceneAnimation->mDuration / ticksPerSecond;
    plasmaAnimationData.FramesPerSecond = ticksPerSecond;

    // Collect each of the animations channel data (Plasma Object Tracks)
    ...
    for (size_t channelIndex = 0; channelIndex < numChannels; ++channelIndex)
    {
      ...
      plasmaTrackData.FullPath = node.mNodePath;
      ...
      {
        PositionKey positionKey = AssimpToPlasmaPositionKey(sceneChannelNode->mPositionKeys[i]);
        ...
        plasmaTrackData.PositionKeys.PushBack(positionKey);
      }

      size_t numRotationKeys = sceneChannelNode->mNumRotationKeys;
      for (size_t i = 0; i < numRotationKeys; i++)
      {
        RotationKey rotationKey = AssimpToPlasmaRotationKey(sceneChannelNode->mRotationKeys[i]);
        ...
        plasmaTrackData.RotationKeys.PushBack(rotationKey);
      }

      size_t numScalingKeys = sceneChannelNode->mNumScalingKeys;
      for (size_t i = 0; i < numScalingKeys; i++)
      {
        ScalingKey scalingKey = AssimpToPlasmaScalingKey(sceneChannelNode->mScalingKeys[i]);
        ...
        plasmaTrackData.ScalingKeys.PushBack(scalingKey);
      }

      plasmaAnimationData.ObjectTracks.PushBack(plasmaTrackData);
    }
    mAnimationDataArray.PushBack(plasmaAnimationData);
  }
}
