...
Mat4 PivotProcessor::GetLocalTransfromFromAnimation(aiNodeAnim* animationNode)
{
  ...
  if (animationNode->mNumPositionKeys)
    translation = AssimpToPlasmaPositionKey(animationNode->mPositionKeys[0]).Position;

  if (animationNode->mNumRotationKeys)
    rotation = AssimpToPlasmaRotationKey(animationNode->mRotationKeys[0]).Rotation;

  if (animationNode->mNumScalingKeys)
    scale = AssimpToPlasmaScalingKey(animationNode->mScalingKeys[0]).Scale;
