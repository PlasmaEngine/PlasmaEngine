...
// Lightning Component
LightningDefineType(LightningComponent, builder, type)
{
  PlasmaBindComponent();
  ...
  // Temporary solution so that LightningComponent cannot be added on Cog in the
  // property grid
...
{
  BoundType* cogPathType = LightningTypeId(CogPath);
...
void LightningComponent::OnAllObjectsCreated(CogInitializer& initializer)
{
...
{
  BoundType* componentType = LightningTypeId(Component);
  BoundType* virtualBoundType = LightningVirtualTypeId(component);
...
void LightningComponent::ScriptInitialize(CogInitializer& initializer)
{
  BoundType* thisType = LightningVirtualTypeId(this);
  ...
  forRange (Property* prop, thisType->GetProperties())
  {
    ...
    bool isResource = propertyType->IsA(LightningTypeId(Resource));
    ...
    if (boundPropertyType)
    {
      ...
      if (isGetSetProperty && isHandleWithDefaultConstructor)
      {
        ...
        {
          Handle newObject = LightningAllocateUntyped(boundPropertyType);
  ...
  if (!editorMode || thisType->HasAttributeInherited(ObjectAttributes::cRunInEditor))
  {
    ...
    BoundType* cogInit = LightningTypeId(CogInitializer);
...
void LightningComponent::OnDestroy(uint flags)
{
  BoundType* thisType = LightningVirtualTypeId(this);
...
void LightningComponent::Serialize(Serializer& stream)
{
  Component::Serialize(stream);
  MetaSerializeProperties(this, stream);
}

void LightningComponent::DebugDraw()
{
  BoundType* thisType = LightningVirtualTypeId(this);

  Lightning::Core& core = Lightning::Core::GetInstance();
  static String FunctionName("DebugDraw");
  Lightning::Function* function =
      thisType->FindFunction(FunctionName, Array<Lightning::Type*>(), core.VoidType, Lightning::FindMemberOptions::None);

  // Do not want to re-invoke Component's DebugDraw, will not find
  // LightningComponent's DebugDraw because it is not bound Still want find to look
  // for base class methods so that it invokes correctly with inheritance within
  // script.
  if (function != nullptr && function->Owner != LightningTypeId(Component))
  {
    Lightning::ExceptionReport report;
    Lightning::Call call(function);
    call.SetHandle(Lightning::Call::This, this);
    call.Invoke(report);
  }
}

ObjPtr LightningComponent::GetEventThisObject()
{
  return this;
}

void LightningComponent::Delete()
{
...
// LightningEvent
LightningDefineType(LightningEvent, builder, type)
{
  type->Sealed = false;

  // If LightningEvent's created in Lightning were using the same handle manager as C++
  // Events, they would leak. So, we want just LightningEvents to be reference
  // counted. We're using HeapManager over ReferenceCountedHandleManager because
  // manually deleting it could be useful.
  type->HandleManager = LightningManagerId(HeapManager);
  type->CreatableInScript = true;
  LightningBindConstructor();
  // Do not bind copy constructor. The only time it would be need is if this
  // went from C++ to Lightning (because of HeapManager), and this should never be
  // constructed in C++.
  LightningBindDestructor();
}

void LightningEvent::Serialize(Serializer& stream)
{
  Event::Serialize(stream);
  MetaSerializeProperties(this, stream);
}

void LightningEvent::Delete()
{
...
// LightningObject
LightningDefineType(LightningObject, builder, type)
{
  type->HandleManager = LightningManagerId(HeapManager);
  type->Sealed = false;
...
void LightningObject::Serialize(Serializer& stream)
{
  EventObject::Serialize(stream);
  MetaSerializeProperties(this, stream);
}

void LightningObject::Delete()
{
  Handle thisHandle(this);
  thisHandle.Delete();
}

void LightningObject::DispatchEvent(StringParam eventId, Event* event)
{
