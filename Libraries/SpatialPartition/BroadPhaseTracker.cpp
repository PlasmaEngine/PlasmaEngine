...
LightningDefineType(BroadPhaseTracker, builder, type)
{
}
...
{
  stream.StartPolymorphic(LightningVirtualTypeId(this));
  ...
  for (uint i = 0; i < broadPhases.Size(); ++i)
  {
    ...
    BoundType* type = LightningVirtualTypeId(broadPhase);
...
void BroadPhaseTracker::LoadFromStream(Serializer& stream)
{
  ...
  {
    BroadPhaseCreator* broadPhaseCreator = PL::gBroadPhaseLibrary->GetCreatorBy(broadPhaseNode);
  ...
  for (uint i = 0; i < broadPhases.Size(); ++i)
  {
    ...
    else
    {
      ...
      if (!results.Match(currentResults))
      {
        ...
        cstr lastName = LightningVirtualTypeId(lastBroadPhase)->Name.c_str();
        cstr currName = LightningVirtualTypeId(broadPhase)->Name.c_str();
...
void BroadPhaseTracker::ReportMissedCollision(uint bpType, NodePointerPair pair, char bitField)
{
  ...
  for (uint i = 0; !r.Empty(); ++i)
  {
    ...
    if (!(bitField & (1 << i)))
    {
      ...
      // Throw an error
      cstr name = LightningVirtualTypeId(handle->mBroadPhase)->Name.c_str();
