...
DataDrivenLexerShared::DataDrivenLexerShared()
{
  ...
  GrammarRule<Character>& PlasmaOrMore = this->mTokenGrammar["PlasmaOrMore"];
  ...
  this->mPlasmaOrMore = &PlasmaOrMore;
  ...
  TokenStart |= OpenParenthesis | CloseParenthesis | Comma | MemberAccess | Semicolon | Colon | RewriteAssignment |
                OrAssignment | PlasmaOrMore | OneOrMore | Optional | Or | Capture | CaptureRule;
  Whitespace |= +T(" \t\r\n\v\f");
  ...
  GrammarExpressionUnary |=
      P("UnaryOperator", P(PlasmaOrMore) | P(OneOrMore) | P(Optional)) << GrammarExpressionUnary | GrammarExpressionValue;
