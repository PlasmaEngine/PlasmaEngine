...
bool IsLightningQuoteCharacter(Plasma::Rune r)
{
  return r == '"' || r == '`' || r == '\'';
}
...
StringRange StripStringQuotes(StringRange input)
{
  ...
  ErrorIf(input.SizeInBytes() < 2,
          ...
          "than 2");
  ErrorIf(IsLightningQuoteCharacter(input.Front()) == false, "The starting character was not a Lightning quotation");
  ErrorIf(IsLightningQuoteCharacter(input.Back()) == false, "The ending character was not a Lightning quotation");
...
String ToLowerCamelCase(StringRange input)
{
  ...
  Plasma::Rune firstCharacter = input.Front();
  firstCharacter = Plasma::UTF8::ToLower(firstCharacter);
  input.PopFront();
...
String ToUpperCamelCase(StringRange input)
{
  ...
  Plasma::Rune firstCharacter = input.Front();
  firstCharacter = Plasma::UTF8::ToUpper(firstCharacter);
  input.PopFront();
  return BuildString(String(firstCharacter), input);
}
} // namespace Lightning
