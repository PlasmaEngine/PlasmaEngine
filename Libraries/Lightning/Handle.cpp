...
void Handle::Initialize(const byte* data, BoundType* type, HandleManager* manager, ExecutableState* state)
{
  ...
  // Plasma out the data (this is a guarantee we make before we ask the manager to
  // initialize the data)
...
bool Handle::operator==(const Handle& rhs) const
{
  ...
  PlasmaTodo("There are two dereferences for comparing user handles, we should "
...
byte* Handle::Dereference() const
{
  ...
  PlasmaTodo("If a handle returns null but is not the 'null handle manager' then "
...
void Handle::ReleaseReference()
{
  ...
  if (this->Manager && this->IsReferenceCounted())
  {
    ...
    // deleted For example, new an object in Lightning, then create a temporary
