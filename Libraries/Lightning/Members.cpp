...
LightningDefineType(Member, builder, type)
{
  ...
  LightningFullBindGetterSetter(
      builder, type, &Member::GetOwningLibrary, LightningNoOverload, LightningNoSetter, LightningNoOverload, "Library");
  LightningFullBindGetterSetter(
      builder, type, &Member::GetTypeOrNull, LightningNoOverload, LightningNoSetter, LightningNoOverload, "Type");
}
...
{
  type->HandleManager = LightningManagerId(PointerManager);
  // LightningFullBindField(builder, type, &Field::Offset, "Offset",
  // PropertyBinding::Get);
}
...
AttributeParameter::AttributeParameter(StringParam name, Lightning::Type* value) : Constant(value), Name(name)
{
}
