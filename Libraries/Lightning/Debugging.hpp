...
#pragma once
#ifndef LIGHTNING_DEBUGGING_HPP
#  define LIGHTNING_DEBUGGING_HPP
...
LightningDeclareEvent(DebuggerPauseUpdate, DebuggerEvent);

// Sent when we first pause execution (such as entering a breakpoint or clicking
// pause)
LightningDeclareEvent(DebuggerPause, DebuggerEvent);

// Sent when we resume execution after being paused
// Note that stepping a single line will resume and then pause again
LightningDeclareEvent(DebuggerResume, DebuggerEvent);

// Sent when we a breakpoint is added (either locally or by remote client)
LightningDeclareEvent(DebuggerBreakpointedAdded, DebuggerEvent);

// Sent when we a breakpoint is added (either locally or by remote client)
LightningDeclareEvent(DebuggerBreakpointedRemoved, DebuggerEvent);

// Sent when we skip a breakpoint or a step due to an externally defined reason
LightningDeclareEvent(DebuggerBreakNotAllowed, DebuggerEvent);
} // namespace Events

// When the debugger pauses or resumes, we send this event out
class PlasmaShared DebuggerEvent : public EventData
{
public:
  LightningDeclareType(DebuggerEvent, TypeCopyMode::ReferenceType);
...
class PlasmaShared DebuggerTextEvent : public DebuggerEvent
{
public:
  LightningDeclareType(DebuggerTextEvent, TypeCopyMode::ReferenceType);
...
class PlasmaShared Debugger : public EventHandler
{
public:
  ...
  // Pauses the call stack for whatever is currently executing in Lightning.
  void Pause();
  ...
  // function. If there is no Lightning function above this point, this will resume
...
PlasmaShared bool DebugErrorHandler(ErrorSignaler::ErrorData& errorData);
} // namespace Lightning
