...
{
  LightningErrorIfNotStarted(Delegate);
}
...
bool Delegate::operator==(Plasma::NullPointerType) const
{
  return this->IsNull();
}
...
Any Delegate::Invoke(const Array<Any>& arguments)
{
  ...
  // PlasmaOffsetOf with 0) to guarantee it will work
  ArrayClass<Any>* lightningArray = reinterpret_cast<ArrayClass<Any>*>(const_cast<Array<Any>*>(&arguments));
  return this->BoundFunction->Invoke(this->ThisHandle, lightningArray);
}
} // namespace Lightning
