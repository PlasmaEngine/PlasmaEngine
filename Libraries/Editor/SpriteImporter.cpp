...
LightningDefineType(SpriteSheetImporter, builder, type)
{
  LightningBindFieldProperty(Name);
  LightningBindGetterSetterProperty(FrameWidth);
  LightningBindGetterSetterProperty(FrameHeight);
  LightningBindGetterSetterProperty(FramesPerRow);
  LightningBindGetterSetterProperty(NumberOfRows);

  LightningBindGetterSetterProperty(OffsetX);
  LightningBindGetterSetterProperty(OffsetY);

  LightningBindGetterSetterProperty(SpacingX);
  LightningBindGetterSetterProperty(SpacingY);

  LightningBindGetterSetterProperty(FrameRate);
  LightningBindFieldProperty(PixelsPerUnit);
  LightningBindGetterSetterProperty(Smoothing);
  LightningBindFieldProperty(CreatePalette);
  LightningBindFieldProperty(mOrigin);

  LightningBindGetterSetterProperty(PreviewAnimate);
  LightningBindGetterSetterProperty(PreviewFrame);
  LightningBindGetterSetterProperty(ImportFrames);
  LightningBindGetterSetterProperty(UseAlphaColorKey);
  LightningBindGetterSetterProperty(AlphaColor);
  LightningBindGetterProperty(FrameCount);
  LightningBindField(SourceSizeX);
  LightningBindField(SourceSizeY);
}
...
void SpriteSheetImporter::LoadImages(Array<String>& files)
{
  ...
  for (uint i = 0; i < files.Size(); ++i)
  {
    ...
    Status status;
    Plasma::LoadImage(status, files[i], &frameImage);
    if (!status)
    {
...
void SpriteSheetImporter::LoadSprite(SpriteSource* spriteSource)
{
  ...
  Status status;
  Plasma::LoadImage(status, spriteSource->mContentItem->GetFullPath(), &mSourcePixels);
  if (!status)
  {
    DoNotifyStatus(status);
    Close();
...
{
  Status status;
  Plasma::LoadImage(status, filename, &mSourcePixels);
  if (!status)
  {
    DoNotifyStatus(status);
    Close();
...
SpriteSheetImporter::AddSpriteResource(StringParam name, Image& output, IntRect frameSize, uint numberOfFrames)
{
  ...
  Status status;
  Plasma::SaveImage(status, fileName, &output, ImageSaveFormat::Png);
  ...
  ContentItem* newContentItem = PL::gContentSystem->AddContentItemToLibrary(status, addContent);
  if (status.Failed())
  {
