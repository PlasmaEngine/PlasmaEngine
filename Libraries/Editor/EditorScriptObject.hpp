...
{
public:
  typedef EditorScriptObjects<DataType> LightningSelf;
    ...
    mAutoRegister("autoRegister")
{
  ...
  ConnectThisTo(LightningScriptManager::GetInstance(), Events::ResourceRemoved, OnScriptRemoved);
}
...
void EditorScriptObjects<DataType>::OnScriptsCompiled(Event*)
{
  ...
  MetaComposition* composition = LightningTypeId(Cog)->HasInherited<MetaComposition>();
...
bool EditorScriptObjects<DataType>::IsAutoRegister(BoundType* componentType)
{
  ...
  if (componentType->IsA(LightningTypeId(LightningComponent)) == false)
    return false;
