...
{
  ErrorIf(track->mPropertyType != LightningTypeId(float), "Invalid track type.");
...
void AnimationGraphEditor::FocusOnSelectedCurves(IntVec2Param axes)
{
  ...
  if (!validAabb)
  {
    ...
    {
      // Expand the aabb
      if (track->mPropertyType == LightningTypeId(float))
...
ByteColor GetNextCurveColor(uint index)
{
  ...
  // to the Plasma Engine orange, so these initial values start us there
  float hue = 26;
  float saturation = 51;
  float value = 97;
...
void AnimationGraphEditor::RebuildCurves()
{
  ...
  forRange (TrackNode* track, mEditorData->mVisiblePropertyTracks.All())
  {
    ...
    if (keyType != LightningTypeId(float))
      continue;
...
void AnimationGraphEditor::OnMouseMove(MouseEvent* e)
{
  ...
  mScrubber->mGhostPlayHead = ToGraphPosition(ToLocal(PL::gMouse->GetClientPosition())).x;
}
