...
LightningDefineType(SpriteFrame, builder, type)
{
  ...
  LightningBindConstructor(SpriteFrame&);
...
SpriteFrame* GetSpriteFrame(Widget* sidget)
{
  ...
  while (widget && !LightningVirtualTypeId(widget)->IsA(LightningTypeId(SpriteFrame)))
    widget = widget->GetParent();
  return (SpriteFrame*)widget;
}

LightningDefineType(SpritePreview, builder, type)
{
}
...
LightningDefineType(SpriteSourceEditor, builder, type)
{
  LightningBindGetterSetterProperty(SpriteName);

  LightningBindGetterSetterProperty(Origin);
  LightningBindFieldProperty(mOriginX);
  LightningBindFieldProperty(mOriginY);

  LightningBindFieldProperty(mLooping);

  LightningBindGetterSetterProperty(Sampling);
  LightningBindGetterSetterProperty(FrameRate);
  LightningBindFieldProperty(mPixelsPerUnit);

  LightningBindFieldProperty(mSpriteFill);
  ...
  LightningBindGetterSetterProperty(CurrentFrame);
  LightningBindGetterSetterProperty(PreviewAnimation);
}
...
void SpriteSourceEditor::UpdatePreview()
{
  ...
  for (uint i = 0; i < mSpriteData.Size(); ++i)
  {
    ...
    PL::gEngine->has(GraphicsEngine)->AddTexture(mSpriteData[i]->mFrameTexture);
...
void SpriteSourceEditor::SaveToSpriteSource()
{
  ...
  PL::gResources->ReloadEntry(spriteSource, entry);
...
void EditSprite(SpriteSource* spriteSource)
{
  ...
  Vec3 offsetCenter = GetCenterPosition(PL::gEditor, window);
