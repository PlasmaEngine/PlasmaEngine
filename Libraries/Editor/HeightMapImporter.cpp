...
LightningDefineType(HeightMapImporter, builder, type)
{
  LightningBindFieldProperty(mName);
  LightningBindFieldProperty(mBaseHeight);
  LightningBindFieldProperty(mMinHeightRange);
  LightningBindFieldProperty(mMaxHeightRange);
  LightningBindFieldProperty(mPatchSize);
  // These variables must refresh the displayed texture when updated
  LightningBindGetterSetterProperty(ImportMode);
  LightningBindGetterSetterProperty(PatchColumns);
  LightningBindGetterSetterProperty(PatchRows);
}
...
{
  Status status;
  Plasma::LoadImage(status, filename, &mHeightMap);
  if (!status)
  {
