...
LightningDefineType(EditorViewport, builder, type)
{
}
...
{
  mTools = PL::gEditor->Tools;
  mOwnerShip = ownership;

  ConnectThisTo(this, Events::ObjectPoll, OnObjectPoll);

  ConnectThisTo(PL::gResources, Events::ResourceRemoved, OnResourcesRemoved);
  ...
  ConnectThisTo(PL::gEditor, Events::SaveCheck, OnSaveCheck);
...
{
  size_t index = PL::gEditor->mEditInGameViewports.FindIndex(this);
  if (index != PL::gEditor->mEditInGameViewports.InvalidIndex)
    PL::gEditor->mEditInGameViewports.EraseAt(index);
  CleanUp();
  Composite::OnDestroy();
}
...
void EditorViewport::SetAsActive()
{
  ...
  if (Space* space = mEditSpace)
  {
    ...
    if (editLevel && mEditArchetype.IsNull())
      PL::gEditor->mEditLevel = editLevel;

    PL::gEditor->SetEditSpace(space);
  }
}
...
void EditorViewport::SetUpEditorCamera()
{
  ...
  if (editorCamera == NULL)
  {
    ...
    editorCamera = PL::gFactory->Create(space, CoreArchetypes::EditorCamera, 0, nullptr);
...
{
  EditorMode::Enum mode = PL::gEditor->GetEditMode();
...
void EditorViewport::OnSpaceLevelLoaded(Event* event)
{
  ...
  {
    levelSettings = PL::gFactory->CreateCheckedType(
        LightningTypeId(Cog), editSpace, CoreArchetypes::LevelSettings, 0, editSpace->GetGameSession());
  ...
  EditorSettings* editorSettings = PL::gEngine->GetConfigCog()->has(EditorSettings);
...
void EditorViewport::OnKeyDown(KeyboardEvent* e)
{
  ...
  else
  {
    // if(e->Key == Keys::F12)
    //{
    //  String fileName = BuildString("PlasmaScreenShot-", GetTimeAndDateStamp(),
    //  ".png"); String filePath =
    //  FilePath::Combine(GetUserDocumentsDirectory(), fileName);
    //  this->ScreenCapture(filePath);

    //  //Print to console only
    //  PlasmaPrint("Screen Shot Taken %s\n", filePath.c_str());
    //}
    ...
    if (e->Key == Keys::Space)
    {
      PL::gEditor->OpenSearchWindow(this);
    }
  }
