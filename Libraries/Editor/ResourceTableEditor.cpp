...
void ResourceWeightedTableView::CreateItem(StringParam name, StringParam value, float probability)
{
  ...
  Plasma::Connect(entry.mProbability, Events::TextBoxChanged, entry.mBar, &WeightedTableBar::OnProbabilityChanged);
  Plasma::Connect(entry.mNameTag, Events::TextBoxChanged, entry.mBar, &WeightedTableBar::OnNameChanged);
  Plasma::Connect(entry.mValueTag, Events::TextBoxChanged, entry.mBar, &WeightedTableBar::OnValueChanged);
}
...
void ResourceTableEditor::CreateToolbar()
{
  ...
  auto managerRange = PL::gResources->Managers.All();
...
void ResourceTableEditor::RemapResources()
{
  ...
  // reason bail
  auto range = PL::gResources->Managers.Find(resourceType);
  if (range.Empty())
    return;
