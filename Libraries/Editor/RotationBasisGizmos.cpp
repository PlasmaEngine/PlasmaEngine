...
LightningDefineType(RotationBasisGizmoInitializationEvent, builder, type)
{
  LightningBindFieldProperty(mIntData);
}
...
LightningDefineType(RotationBasisGizmoAabbQueryEvent, builder, type)
{
}
...
LightningDefineType(RotationBasisGizmoMetaTransform, builder, type)
{
}
...
MetaTransformInstance RotationBasisGizmoMetaTransform::GetInstance(HandleParam object)
{
  ...
  BoundType* transformType = LightningTypeId(Transform);
...
LightningDefineType(RotationBasisGizmo, builder, type)
{
  PlasmaBindComponent();
  ...
  PlasmaBindEvent(Events::RotationBasisGizmoBegin, Event);
  PlasmaBindEvent(Events::RotationBasisGizmoModified, Event);
  PlasmaBindEvent(Events::RotationBasisGizmoEnd, Event);
  PlasmaBindEvent(Events::RotationBasisAabbQuery, RotationBasisGizmoAabbQueryEvent);

  LightningBindMethod(ActivateAsGizmo);
  // Does not need to be serialized. Purely for editing purposes.
  LightningBindGetterSetterProperty(WorldRotation);

  LightningBindField(mXAxisName)->PlasmaSerialize(String("X-Axis"));
  LightningBindField(mYAxisName)->PlasmaSerialize(String("Y-Axis"));
  LightningBindField(mZAxisName)->PlasmaSerialize(String("Z-Axis"));
}
...
void RotationBasisGizmo::ActivateAsGizmo()
{
  ...
  ConnectThisTo(PL::gEditor->GetSelection(), Events::SelectionChanged, OnSelectionChanged);
}
...
LightningDefineType(OrientationBasisGizmo, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindDependency(RotationBasisGizmo);
  PlasmaBindSetup(SetupMode::DefaultSerialization);
}
...
LightningDefineType(PhysicsCarWheelBasisGizmo, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindDependency(RotationBasisGizmo);
  PlasmaBindSetup(SetupMode::DefaultSerialization);
...
LightningDefineType(RevoluteBasisGizmo, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindDependency(RotationBasisGizmo);
  PlasmaBindSetup(SetupMode::DefaultSerialization);
