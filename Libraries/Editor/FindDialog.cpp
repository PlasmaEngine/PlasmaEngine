...
bool FindTextDialog::GetEntireProjectRegions()
{
  ...
  for (; !range.Empty(); range.PopFront())
  {
    ...
    DocumentResource* current = (DocumentResource*)PL::gResources->GetResource(id);
...
{
  // Start the line count at plasma
  size_t lineCount = 0;
  int debugCount = 0;
...
void Plasma::FindTextDialog::CheckForInvalidInput(ObjectEvent* event)
{
...
ConsoleUi* GetConsoleTextBox()
{
  ...
  Widget* consoleWidget = PL::gEditor->ShowConsole();

  // If we didn't find the console widget...
  if (consoleWidget == NULL)
  {
    // Use the ui-automator to find the widget
    consoleWidget = FindWidgetByName("Console", UiTraversal::DepthFirst, 0, PL::gEditor->GetRootWidget());
  }
...
DocumentEditor* FindTextDialog::GetEditorForRegion(SearchRegion* region)
{
  ...
  if (editor == NULL && region->Resource)
  {
    ...
    editor = PL::gEditor->OpenDocumentResource(region->Resource);
  }

  if (editor == NULL && !region->FileName.Empty())
  {
    editor = PL::gEditor->OpenTextFile(region->FileName);
  }
