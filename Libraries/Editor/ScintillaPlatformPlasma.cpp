...
SurfaceImpl::SurfaceImpl(Plasma::Widget* widget) : mWidget(widget)
{
}
...
{
  MakeViewNode(Plasma::PrimitiveType::Lines, White);
...
{
  MakeViewNode(Plasma::PrimitiveType::Triangles, White);
  ...
  if (npts == 3)
  {
    for (size_t p = 0; p < 3; ++p)
    {
      ...
      Plasma::StreamedVertex vertex(point, Vec2::cZero, color, Vec2::cZero);
...
{
  MakeViewNode(Plasma::PrimitiveType::Triangles, Circle);
    ...
    PRectangle rc, Font& font_, XYPOSITION ybase, const char* s, int len, ColourDesired fore, ColourDesired back)
{
  ...
  MakeViewNode(Plasma::PrimitiveType::Triangles, font->mTexture);
                                        ...
                                        int flags)
{
  ...
  if(mViewNode == nullptr || mDrawType != Lines)
  {
    ...
    Plasma::Texture* texture = Plasma::TextureManager::FindOrNull("White");
    mViewNode = &mWidget->AddRenderNodes(*mViewBlock, *mFrameBlock, mClipRect,
  texture); mViewNode->mStreamedVertexType = Plasma::PrimitiveType::Lines;
  }
  */
  ...
  Plasma::StreamedVertex v1(Math::TransformPoint(mViewNode->mLocalToView, Vec3(leftX, pos1.y, 0)),
                          Vec2(uv0.x, uv1.y),
                          color,
                          Vec2(uv0.x, uv1.y));
  Plasma::StreamedVertex v2(Math::TransformPoint(mViewNode->mLocalToView, Vec3(pos1.x, pos1.y, 0)), uv1, color, uv1);
  Plasma::StreamedVertex v3(Math::TransformPoint(mViewNode->mLocalToView, Vec3(rightX, pos0.y, 0)),
                          Vec2(uv1.x, uv0.y),
                          color,
                          Vec2(uv1.x, uv0.y));
...
void SurfaceImpl::MakeViewNode(Plasma::PrimitiveType::Enum primitive, StringParam textureName)
{
  Plasma::Texture* texture = Plasma::TextureManager::FindOrNull(textureName);
  return MakeViewNode(primitive, texture);
}

void SurfaceImpl::MakeViewNode(Plasma::PrimitiveType::Enum primitive, Plasma::Texture* texture)
{
...
public:
  ...
  Plasma::EventReceiver* GetReceiver()
  {
    return &mReceiver;
  }
...
{
  return Plasma::Os::GetDoubleClickTimeMs();
}
