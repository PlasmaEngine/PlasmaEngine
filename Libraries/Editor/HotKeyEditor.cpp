...
{
public:
  typedef CogCommandScriptEditor LightningSelf;
  Element* mIcon;
  ...
  void OnLeftClick(MouseEvent* event)
  {
    ...
    Resource* resource = PL::gResources->GetResource(resourceId);
...
{
public:
  typedef BindingEditor LightningSelf;
...
{
public:
  typedef TreeViewSearchHotKeys LightningSelf;
...
public:
  ...
  bool IsSelected(DataIndex index) override
  {
    ...
    Cog* object = ((CogCommand*)row->mPlasmaCommand)->mCog;
  ...
  void Select(DataIndex index, bool sendsEvents) override
  {
    ...
    Cog* object = ((CogCommand*)row->mPlasmaCommand)->mCog;
    if (object)
      mSelection->SelectOnly(object->GetComponentByName(row->mPlasmaCommand->Name));
  }
...
{
  lhs.mIsACogCommand = (rhs->LightningGetDerivedType() == LightningTypeId(CogCommand));
  lhs.mDevOnly = rhs->DevOnly;

  lhs.mPlasmaCommand = rhs;
  lhs.mName = rhs->Name;
...
void HotKeyCommands::GetData(DataEntry* dataEntry, Any& variant, StringParam column)
{
  ...
  if (column == cScriptColumn)
  {
    ...
    if (row->mIsACogCommand)
      variant = (CogCommand*)row->mPlasmaCommand;
  }
...
LightningDefineType(HotKeyEditor, builder, type)
{
}
...
void HotKeyEditor::OnCogCommandSort(MouseEvent* event)
{
  if (mCogCommandSortToggle)
  {
    ...
    if (ToolTip* toolTip = mSortToolTip)
    {
      ...
      toolTip->AddText("  Plasma Commands", cPlasmaCommandColor);
    }
  }
  else
  {
...
void HotKeyEditor::OnMouseEnterIconHeader(MouseEvent* event)
{
  ...
  if (mCogCommandSortToggle)
    ...
    CreateCommandHeaderToolTip(source, "Native Commands", cPlasmaCommandColor);
}
...
void HotKeyEditor::OnTreeViewHeaderAdded(TreeViewHeaderAddedEvent* event)
{
  ...
  Plasma::Connect(header, Events::LeftMouseUp, this, handler[index]);
  Plasma::Connect(header, Events::MouseEnterHierarchy, this, handler[index + 4]);
...
LightningDefineType(HotKeyBinding, builder, type)
{
  LightningBindFieldProperty(mModifier1);
  LightningBindFieldProperty(mModifier2);
  LightningBindFieldProperty(mMainKey);
}
