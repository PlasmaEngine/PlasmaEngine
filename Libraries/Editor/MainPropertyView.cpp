...
LightningDefineType(MainPropertyView, builder, type)
{
}
...
void MainPropertyView::OnReloadContent(Event* e)
{
  ...
  PL::gEditor->mMainPropertyView->GetHistory()->Reselect();
}
...
void MainPropertyView::EditResources(MetaSelection* selection)
{
  ...
  forRange (Resource* resource, selection->AllOfType<Resource>())
  {
    ...
    if (count < cMaxItems)
    {
      ...
      if (preview && LightningVirtualTypeId(resource)->IsA(LightningTypeId(Archetype)))
      {
