...
LightningDefineType(BugReporter, builder, type)
{
}

LightningDefineType(BugReporterResponse, builder, type)
{
}
...
BugReporter::BugReporter(Composite* parent) : Composite(parent)
{
  ...
  new Label(this, cText, "PlasmaHub Username:");
...
void BugReporter::Reset()
{
  ...
  if (PL::gEditor != nullptr && PL::gEditor->mConfig != nullptr)
  {
    if (EditorConfig* editorConfig = PL::gEditor->mConfig->has(EditorConfig))
...
void BugReportJob::OnWebResponseComplete(WebResponseEvent* event)
{
  ...
  if (response.StartsWith("Success:"))
  {
    ...
    {
      String message = String::Format("PlasmaHub returned success, but did not include a TaskID. Please "
                                      "visit %s to find your task, or contact a PlasmaHub administrator.",
