...
LightningDefineType(PropertyWidgetObject, builder, type)
{
}
  ...
  if (objectType)
  {
    ...
    if (MetaResource* metaResource = objectType->HasInherited<MetaResource>())
    {
      if (mNode->mProperty == nullptr && !mNode->IsPropertyGroup())
      {
        ...
        Resource* resource = PL::gResources->GetResource(metaResource->mResourceId);
...
void PropertyWidgetObject::LayoutChildren(bool animate)
{
  ...
  BoundType* widgetObjectType = LightningTypeId(PropertyWidgetObject);
  ...
  forRange (PropertyWidget& child, ChildWidgets.All())
  {
    bool childIsObject = (LightningVirtualTypeId(&child) == widgetObjectType);
    ...
    if (LightningVirtualTypeId(&child) == LightningTypeId(AddObjectWidget))
...
void PropertyWidgetObject::AnimateCloseNode()
{
  ...
  sequence->Add(new CallAction<LightningSelf, &LightningSelf::CloseNode>(this));
  sequence->Add(new ActionDelayOnce());
  sequence->Add(new CallAction<LightningSelf, &LightningSelf::AnimationFinished>(this));
}
...
MetaPropertyEditor* GetPropertyEditor(Property* property)
{
  ...
  {
    BoundType* extensionType = LightningVirtualTypeId(extension);
...
void PropertyWidgetObject::AnimateRemoveSelf()
{
  ...
  sequence->Add(new CallAction<LightningSelf, &LightningSelf::RemoveSelf>(this));
...
void PropertyWidgetObject::RemoveRedHighlight()
{
  ...
  seq->Add(AnimatePropertyGetSet(Element, Color, Ease::Quad::InOut, mTitleBackground, ComponentUi::OpenTime, color));
  seq->Add(new CallAction<LightningSelf, &LightningSelf::AnimationFinished>(this));
