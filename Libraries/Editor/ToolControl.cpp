...
LightningDefineType(ToolUiEvent, builder, type)
{
  LightningBindFieldProperty(mNeedsPropertyGrid);
  LightningBindGetterProperty(Parent);
  LightningBindGetterProperty(SelectTool);
...
public:
                                      ...
                                      Property* objectProperty = nullptr) override
  {
    ...
    if (objectType->IsA(LightningTypeId(Cog)))
    {
...
Space* ToolData::GetSpace()
{
  if (mSpace.IsNull())
  {
    GameSession* gameSession = PL::gEditor->GetEditGameSession();
    ...
    {
      mSpace = PL::gFactory->CreateSpace(CoreArchetypes::DefaultSpace, CreationFlags::Default, nullptr);
    }
  }
...
LightningDefineType(ToolControl, builder, type)
{
  PlasmaBindEvent(Events::GetToolInfo, ToolUiEvent);
}
...
ToolControl::ToolControl(Composite* parent) : Composite(parent), mTools(this), mCustomUi(nullptr)
{
  ...
  MetaDatabase::GetInstance()->mEventMap[Events::ToolActivate] = LightningTypeId(Event);
  MetaDatabase::GetInstance()->mEventMap[Events::ToolDeactivate] = LightningTypeId(Event);
  MetaDatabase::GetInstance()->mEventMap[Events::ToolDraw] = LightningTypeId(Event);
  ...
  ConnectThisTo(lightningManager, Events::ScriptsCompiledPostPatch, OnScriptsCompiled);
}
...
void ToolControl::OnInfoMouseEnter(MouseEvent*)
{
  ...
  {
    BoundType* type = LightningVirtualTypeId(component);
    const ShortcutSet* shortcuts = PL::gShortcutsDoc->FindSet(type->Name);
...
{
  // EditorViewport* lastViewport = PL::gEditor->mActiveViewport;
