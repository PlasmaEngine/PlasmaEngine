...
LightningDefineType(AddObjectWidget, builder, type)
{
}
    ...
    PropertyWidget(init)
{
  ...
  mBorder->SetColor(Vec4(0.278, 0.705, 0.858, 1.0f));
}
...
void AddObjectWidget::OnPostResourceAdded(PostAddResourceEvent* event)
{
  ...
  // a LightningComponent - then add that new component to those compositions.
  if (resourceType == LightningTypeId(LightningScript))
  {
...
{
  AddResourceWindow* addDialog = OpenAddWindow(LightningTypeId(LightningScript), nullptr, event->mSearchText);
...
void AddObjectWidget::UpdateTransform()
{
  ...
  mBorder->SetColor(Vec4(0.278, 0.705, 0.858, 0.75f));
