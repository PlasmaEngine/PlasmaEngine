...
LightningDefineType(AnimationSettings, builder, type)
{
  type->HandleManager = LightningManagerId(PointerManager);
  LightningBindGetterSetterProperty(EditFps)->Add(new EditorIndexedStringArray(GetEditFpsPresets));
  LightningBindFieldProperty(mTimeDisplay);
  LightningBindFieldProperty(mSnappingX);
  LightningBindFieldProperty(mSnappingY);
  LightningBindFieldProperty(mAutoKey);
  LightningBindFieldProperty(mAutoFocus);
  LightningBindFieldProperty(mOnionSkinning);
  LightningBindFieldProperty(mPlaybackSpeed);
  LightningBindFieldProperty(mPreviewMode);
