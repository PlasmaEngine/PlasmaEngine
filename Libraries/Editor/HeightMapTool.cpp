...
LightningDefineType(HeightMapSubTool, builder, type)
{
...
/// Meta
LightningDefineType(HeightManipulationTool, builder, type)
{
  LightningBindFieldProperty(mRadius);
  LightningBindFieldProperty(mFeatherRadius);
...
LightningDefineType(RaiseLowerTool, builder, type)
{
  LightningBindFieldProperty(mStrength)->Add(new EditorSlider(0, 1, 0.001f));
  LightningBindFieldProperty(mRelative);
...
LightningDefineType(SmoothSharpenTool, builder, type)
{
  LightningBindFieldProperty(mStrength)->Add(new EditorSlider(0, 1, 0.001f));
  LightningBindField(mUniformSamples);
  LightningBindField(mRandomSamples);
  LightningBindField(mRandomSampleDistance);
  LightningBindField(mAutoDetermineSamples);
...
LightningDefineType(FlattenTool, builder, type)
{
  LightningBindFieldProperty(mHeight);
  LightningBindFieldProperty(mSlopeNormal);
  LightningBindFieldProperty(mSampleOnMouseDown);
  LightningBindFieldProperty(mSampleNormal);
...
LightningDefineType(CreateDestroyTool, builder, type)
{
  LightningBindFieldProperty(mBaseHeight);
  LightningBindFieldProperty(mUsePerlinNoise);
  LightningBindFieldProperty(mPerlinFrequency);
  LightningBindFieldProperty(mPerlinAmplitude);
...
LightningDefineType(WeightPainterTool, builder, type)
{
  LightningBindFieldProperty(mTextureChannel);
...
LightningDefineType(HeightMapTool, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultConstructor);
  type->AddAttribute(ObjectAttributes::cTool);

  LightningBindFieldProperty(mShowPatchIndex)->PlasmaAdvancedGroup();
  LightningBindFieldProperty(mShowCellIndex)->PlasmaAdvancedGroup();
  LightningBindFieldProperty(mCellIndexType)->PlasmaAdvancedGroup();

  LightningBindMethodProperty(ImportHeightMap);

  LightningBindGetterSetterProperty(CurrentTool)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindFieldProperty(mSubTool);
}
...
{
  ConnectThisTo(PL::gEditor->GetSelection(), Events::SelectionFinal, OnSelectionFinal);
...
void HeightMapTool::OnToolDeactivate(Event*)
{
  ...
  PL::gEditor->GetSelection()->GetDispatcher()->Disconnect(this);
}
...
void HeightMapTool::OnSelectionFinal(SelectionChangedEvent* event)
{
  ...
  if (cog && cog->has(HeightMap))
  {
    ...
    // event first
    PL::gEditor->ShowWindow("Tools");
  }
  else
  {
    // Return to select tool if no object has a HeightMap
    PL::gEditor->Tools->SelectToolIndex(0);
  }
}
...
void HeightMapTool::ImportHeightMap()
{
  Plasma::ImportHeightMap(PL::gEditor);
}
...
HeightMap* HeightMapTool::GetHeightMap()
{
  ...
  HeightMap* heightMap = static_cast<HeightMap*>(
      Tool::GetOrCreateEditComponent(LightningTypeId(HeightMap), cHeightMapName, cHeightMapArchetype, mLastEdited, false));
