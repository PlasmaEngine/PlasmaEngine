...
LightningDefineType(SelectToolFrustumEvent, builder, type)
{
  LightningBindFieldProperty(Handled);
  LightningBindFieldProperty(HandledEventScript);

  LightningBindGetterProperty(Space);
  LightningBindGetterProperty(Frustum);
}
...
public:
      ...
      MouseManipulation(mouse, owner)
  {
    ...
    forRange (Cog* cog, PL::gEditor->GetSelection()->AllOfType<Cog>())
...
{
  return PL::gEditor->Tools->mSelectTool->RayCastSelect(viewport, mousePosition);
}

LightningDefineType(SelectTool, builder, type)
{
  PlasmaBindDocumented();
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);

  LightningBindFieldProperty(mArchetypeSelect);
  LightningBindFieldProperty(mRootSelect);
  LightningBindFieldProperty(mSmartGroupSelect);
  type->AddAttribute(ObjectAttributes::cTool);
  LightningBindMethod(RayCast);
  LightningBindMethod(SmartSelect);

  PlasmaBindEvent(Events::SelectToolPreSelect, ViewportMouseEvent);
...
void SelectTool::OnToolDraw(Event* e)
{
  ...
  forRange (Cog* cog, selection->AllOfType<Cog>())
  {
    ...
    if (cog->GetSpace() != PL::gEditor->GetEditSpace())
      continue;
...
void SelectTool::OnMouseUpdate(ViewportMouseEvent* e)
{
  ...
  if (selectedCog) // && !PL::gEditor->mActiveSelection->IsPresent(selectedCog))
...
LightningDefineType(CreationTool, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);

  LightningBindFieldProperty(mPlacementMode);
  LightningBindGetterSetterProperty(ObjectToSpawn);
  LightningBindFieldProperty(mOffset);
  LightningBindFieldProperty(mSnapping);
  LightningBindFieldProperty(mSnapDistance);
  LightningBindFieldProperty(mDepth);
  LightningBindFieldProperty(mDepthPlane);
...
Cog* CreationTool::CreateAt(Viewport* viewport, Archetype* archetype, Vec3Param position)
{
  ...
  if (cogType->IsA(LightningTypeId(GameSession)) || cogType->IsA(LightningTypeId(Space)))
  {
  ...
  Space* space = viewport->GetTargetSpace();
  Cog* object = CreateFromArchetype(PL::gEditor->mQueue, space, archetype, position);
  MetaSelection* selection = PL::gEditor->GetSelection();
...
LightningDefineType(ObjectConnectingTool, builder, type)
{
...
LightningDefineType(ParentingTool, builder, type)
{
  PlasmaBindComponent();
  type->AddAttribute(ObjectAttributes::cTool);
  PlasmaBindSetup(SetupMode::DefaultSerialization);

  LightningBindFieldProperty(mMaintainPosition);
}
