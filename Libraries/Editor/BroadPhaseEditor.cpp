...
BroadPhaseEditor::BroadPhaseEditor(Editor* parent) : Composite(parent)
{
  ...
  for (uint i = 0; i < BPStats::Size; ++i)
  {
    ...
    mCheckBoxes[i]->SetText(Plasma::BuildString("Track ", BPStats::Names[i]));
  }
...
void BroadPhaseEditor::AddBroadPhasesToTracker(BroadPhase::Type type, BroadPhaseTracker* tracker)
{
  for (uint i = 0; i < mActiveBroadPhases[type].Size(); ++i)
  {
    ...
    IBroadPhase* broadPhase = PL::gBroadPhaseLibrary->CreateBroadPhase(name);
...
Composite* BroadPhaseEditor::BuildPropertyGrid(StringParam label, BroadPhase::Type type, Composite* parent)
{
  ...
  composite->SetLayout(CreateStackLayout());
  {
    ...
    addComposite->SetLayout(CreateRowLayout());
    {
      ...
      // Enumerate the names
      PL::gBroadPhaseLibrary->EnumerateNamesOfType(type, mBroadPhaseNames[type].Strings);
