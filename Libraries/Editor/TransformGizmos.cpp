...
LightningDefineType(ObjectTransformGizmoEvent, builder, type)
{
  LightningBindGetterSetter(FinalLocalTranslation);
  LightningBindGetterSetter(FinalLocalScale);
  LightningBindGetterSetter(FinalLocalRotation);
}
...
{
  mSource = sourceGizmo;
  mGizmoType = sourceGizmo->LightningGetDerivedType();
  ...
  if (mGizmoType == LightningTypeId(ObjectScaleGizmo))
    mCanAlterScale = true;
  else if (mGizmoType == LightningTypeId(ObjectRotateGizmo))
...
LightningDefineType(ObjectTransformGizmo, builder, type)
{
  PlasmaBindComponent();
  ...
  PlasmaBindEvent(Events::GizmoObjectsDuplicated, Event);
  PlasmaBindEvent(Events::ObjectTransformGizmoStart, GizmoUpdateEvent);
  PlasmaBindEvent(Events::ObjectTranslateGizmoModified, ObjectTransformGizmoEvent);
  PlasmaBindEvent(Events::ObjectScaleGizmoModified, ObjectTransformGizmoEvent);
  PlasmaBindEvent(Events::ObjectRotateGizmoModified, ObjectTransformGizmoEvent);
  PlasmaBindEvent(Events::ObjectTransformGizmoEnd, ObjectTransformGizmoEvent);

  PlasmaBindDependency(Transform);
  PlasmaBindDependency(Gizmo);

  LightningBindGetterSetterProperty(Basis);
  LightningBindGetterSetterProperty(Pivot);

  LightningBindMethod(AddObject);
  LightningBindMethod(RemoveObject);
  LightningBindMethod(ClearObjects);
  LightningBindMethod(SetOperationQueue);
  LightningBindMethod(ToggleCoordinateMode);
  LightningBindGetterProperty(ObjectCount);
  LightningBindMethod(GetObjectAtIndex);
}
  ...
  forRange (ObjectTransformState& objectState, mObjectStates.All())
  {
    ...
    if ((objectState.StartSize - objectState.EndSize).LengthSq() != 0.0f)
    {
      if (Cog* areaCog = target.Get<Cog*>())
      {
        ...
        {
          Property* sizeProperty = LightningVirtualTypeId(area)->GetProperty("Size");
          ...
          if (mSizeBoxCollider)
          {
            ...
            {
              BoundType* colliderType = LightningVirtualTypeId(collider);
...
LightningDefineType(ObjectTranslateGizmo, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindInterface(ObjectTransformGizmo);
  PlasmaBindDependency(TranslateGizmo);

  LightningBindFieldProperty(mDuplicateOnCtrlDrag);
}
...
LightningDefineType(ObjectScaleGizmo, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindInterface(ObjectTransformGizmo);
  PlasmaBindDependency(ScaleGizmo);

  LightningBindFieldProperty(mAffectTranslation);
}
...
LightningDefineType(ObjectRotateGizmo, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindInterface(ObjectTransformGizmo);
  PlasmaBindDependency(RotateGizmo);
