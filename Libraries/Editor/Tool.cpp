...
LightningDefineType(Tool, builder, type)
{
  LightningBindMethod(BeginDrag);
}

LightningDefineType(ViewportTextWidget, builder, type)
{
}
    ...
    BoundType* componentType, StringParam defaultName, StringParam defaultArchetype, CogId& lastEdited, bool canCreate)
{
  ...
  auto space = PL::gEditor->GetEditSpace();
...
{
  EditorViewport* viewport = PL::gEditor->mEditorViewport;
  if (viewport == nullptr)
    return nullptr;
