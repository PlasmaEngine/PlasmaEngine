...
void GroupSelected(Editor* editor, Space* space)
{
  ...
  Plasma::Sort(orderedSelection.All(), CogHierarchyIndexCompareFn);
...
{
  MetaSelection* active = editor->GetSelection();
  AddToSelection(space, active, LightningTypeId(Transform), SelectComponentMode::WithComponent);
}
...
void MoveToLookPoint(Editor* editor, Space* space)
{
  ...
  {
    BoundType* transformMeta = LightningTypeId(Transform);
    OperationQueue* opQueue = editor->GetOperationQueue();
    opQueue->BeginBatch();
    ...
    MetaSelection* activeSelection = PL::gEditor->GetSelection();
...
{
  CameraFocusSpace(PL::gEditor->GetEditSpace());
}
...
bool ExecuteShortCuts(Space* space, Viewport* viewport, KeyboardEvent* event)
{
  ...
  {
    DeleteSelectedObjects(PL::gEditor, space);
    event->Handled = true;
    return true;
  }
  ...
  if (event->Key == Keys::F && !event->CtrlPressed)
  {
    ...
    ObjectView* objectView = PL::gEditor->mObjectView;
    if (objectView->IsMouseOver())
    {
      Cog* primary = PL::gEditor->GetSelection()->GetPrimaryAs<Cog>();
      if (primary)
...
void GoToDefinition(Editor* editor)
{
  ...
  {
    LightningDocumentResource* docResource = Type::DynamicCast<LightningDocumentResource*>(docEditor->GetResource());
    if (docResource != nullptr && LightningVirtualTypeId(docResource) == LightningVirtualTypeId(resource))
    {
