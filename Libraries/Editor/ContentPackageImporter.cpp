...
#define GetSorter(type, comparer) type<comparer>(comparer())
    ...
    {
      if (column == CommonColumns::Name)
        Plasma::Sort(mListing->SortedEntries.All(), GetSorter(ContentEntryNameSorter, greater<String>));
      else if (column == CommonColumns::Type)
        Plasma::Sort(mListing->SortedEntries.All(), GetSorter(ContentEntryExtensionSorter, greater<String>));
      else if (column == "Import")
        Plasma::Sort(mListing->SortedEntries.All(), GetSorter(ContentEntryActiveSorter, greater<bool>));
    }
    else
    {
      if (column == CommonColumns::Name)
        Plasma::Sort(mListing->SortedEntries.All(), GetSorter(ContentEntryNameSorter, less<String>));
      else if (column == CommonColumns::Type)
        Plasma::Sort(mListing->SortedEntries.All(), GetSorter(ContentEntryExtensionSorter, less<String>));
      else if (column == "Import")
        Plasma::Sort(mListing->SortedEntries.All(), GetSorter(ContentEntryActiveSorter, less<bool>));
    }
  }
};
...
void ContentImporter::OnImport(Event* event)
{
  ...
  ImportContentPackageListing(mListing, PL::gEditor->mProjectLibrary, mFileName);

  PL::gEditor->mLibrary->SetSearchTags(mListing.Tags);
...
void ImportContentPackage()
{
  ...
  config->AddFilter("Import Pack File", "*.plasmapack");
