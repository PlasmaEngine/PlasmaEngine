...
LightningDefineType(WebServerRequestEvent, builder, type)
{
  LightningBindFieldGetter(mWebServer);
  LightningBindFieldGetter(mMethod);
  LightningBindFieldGetter(mMethodString);
  LightningBindFieldGetter(mOriginalUri);
  LightningBindFieldGetter(mDecodedUri);
  LightningBindFieldGetter(mPostData);
  LightningBindFieldGetter(mData);
  LightningBindMethod(HasHeader);
  LightningBindMethod(GetHeaderValue);
  LightningBindMethod(GetHeaderNames);
  LightningBindOverloadedMethod(Respond, LightningInstanceOverload(void, WebResponseCode::Enum, StringParam, StringParam));
  LightningBindOverloadedMethod(Respond, LightningInstanceOverload(void, StringParam, StringParam, StringParam));
  LightningBindOverloadedMethod(Respond, LightningInstanceOverload(void, StringParam));
}
  ...
  while (webServer->mRunning)
  {
    ...
    if (stage == ReadStage::Content)
    {
      ...
      if (contentLength == 0)
      {
        ...
        PL::gDispatch->Dispatch(webServer, Events::WebServerRequestRaw, toSend);
        toSend = nullptr;
        break;
      }
      else
...
LightningDefineType(WebServer, builder, type)
{
  PlasmaBindEvent(Events::WebServerRequest, WebServerRequestEvent);
  PlasmaBindEvent(Events::WebServerUnhandledRequest, WebServerRequestEvent);
  LightningBindMethod(Create);
  LightningBindMethod(Host);
  LightningBindMethod(Close);
  LightningBindMethod(GetWebResponseCodeString);
  LightningBindMethod(UrlParamEncode);
  LightningBindMethod(UrlParamDecode);
  LightningBindMethod(MapExtensionToMimeType);
  LightningBindMethod(GetMimeTypeFromExtension);
  LightningBindMethod(ClearMimeTypes);
  LightningBindFieldProperty(mPath);
}
...
{
  return Plasma::UrlParamEncode(string);
}

String WebServer::UrlParamDecode(StringParam string)
{
  return Plasma::UrlParamDecode(string);
}
...
OsInt WebServer::AcceptThread(void* userData)
{
  ...
  while (self->mRunning)
  {
    ...
    if (status.Succeeded() && acceptedSocket.IsOpen())
    {
      ...
      connection->mSocket = PlasmaMove(acceptedSocket);
