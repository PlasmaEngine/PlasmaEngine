...
bool IsValidNetPropertyType(Type* propertyType)
{
  ...
  // NetEvents, for now)
  return BitStreamCanSerializeType(propertyType) || propertyType == LightningTypeId(Cog) ||
         propertyType == LightningTypeId(CogPath);
}
bool IsValidNetPeerIdPropertyType(Type* propertyType)
{
  return propertyType == LightningTypeId(int);
}
...
Variant GetNetChannelAuthorityProperty(const Variant& propertyData)
{
  ...
  // Success
  // (We wrap the enum in an Any to take advantage of Lightning meta later during
...
Variant GetComponentCogProperty(const Variant& propertyData)
{
  ...
  BoundType* componentBoundType = LightningVirtualTypeId(component);
