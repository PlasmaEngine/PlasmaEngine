...
LightningDefineType(NetObject, builder, type)
{
  PlasmaBindComponent();

  // Bind tags
  PlasmaBindTag(Tags::Networking);
  ...
  // Bind peer interface
  LightningBindGetterProperty(Role)->Add(new EditInGameFilter);
  LightningBindCustomGetter(IsClient);
  LightningBindCustomGetter(IsServer);
  LightningBindCustomGetter(IsOffline);
  LightningBindCustomGetter(IsClientOrOffline);
  LightningBindCustomGetter(IsServerOrOffline);
  LightningBindCustomGetter(IsClientOrServer);

  // Bind object interface
  LightningBindGetterSetterProperty(DetectOutgoingChanges);
  LightningBindGetterSetterProperty(AcceptIncomingChanges);
  LightningBindGetterSetterProperty(AllowNapping);
  LightningBindGetterSetterProperty(AccurateTimestampOnOnline);
  LightningBindGetterSetterProperty(AccurateTimestampOnChange);
  LightningBindGetterSetterProperty(AccurateTimestampOnOffline);
  LightningBindGetterProperty(OnlineTimestamp)->Add(new EditInGameFilter);
  LightningBindGetterProperty(LastChangeTimestamp)->Add(new EditInGameFilter);
  LightningBindGetterProperty(OfflineTimestamp)->Add(new EditInGameFilter);
  LightningBindCustomGetterProperty(IsOnline)->Add(new EditInGameFilter);
  LightningBindGetterProperty(NetObjectId)->Add(new EditInGameFilter);
  LightningBindCustomGetterProperty(IsNapping)->Add(new EditInGameFilter);
  LightningBindGetterProperty(OnlineTimePassed)->Add(new EditInGameFilter);
  LightningBindGetterProperty(LastChangeTimePassed)->Add(new EditInGameFilter);
  LightningBindGetterProperty(OfflineTimePassed)->Add(new EditInGameFilter);
  LightningBindMethodProperty(SelectRemote)->Add(new EditInGameFilter);
  LightningBindMethodProperty(ReplicateNow)->Add(new EditInGameFilter);
  LightningBindMethodProperty(WakeUp)->Add(new EditInGameFilter);
  LightningBindMethodProperty(TakeNap)->Add(new EditInGameFilter);
  LightningBindMethodProperty(Forget)->Add(new EditInGameFilter);

  // Bind channel interface
  LightningBindMethod(HasNetChannel);
  LightningBindMethod(GetNetChannel);

  // Bind ownership interface
  LightningBindCustomGetter(IsOwnedByAUser);
  LightningBindCustomGetter(IsNotOwnedByAUser);
  LightningBindMethod(IsOwnedByUser);
  LightningBindMethod(IsOwnedByPeer);
  LightningBindCustomGetterProperty(IsMine)->Add(new EditInGameFilter);
  LightningBindCustomGetter(IsNotMine);
  LightningBindCustomGetter(IsClientAndMine);
  LightningBindCustomGetter(IsClientButNotMine);
  LightningBindCustomGetter(IsServerAndMine);
  LightningBindCustomGetter(IsServerButNotMine);
  LightningBindCustomGetter(IsOfflineAndMine);
  LightningBindCustomGetter(IsOfflineButNotMine);
  LightningBindGetterSetter(NetUserOwner);
  LightningBindGetterProperty(NetUserOwnerPath)->Add(new EditInGameFilter);
  LightningBindGetterProperty(NetUserOwnerUserId)->Add(new EditInGameFilter);
  LightningBindGetterProperty(NetUserOwnerPeerId)->Add(new EditInGameFilter);
  LightningBindMethod(SetNetUserOwnerUp);
  LightningBindMethod(SetNetUserOwnerDown);

  // Bind network dispatch interface
  LightningBindMethod(DispatchLocal);
  LightningBindMethod(DispatchRemote);
  LightningBindMethod(DispatchBroadcast);
  LightningBindMethod(DispatchLocalAndRemote);
  LightningBindMethod(DispatchLocalAndBroadcast);

  // Bind channel configuration interface
  LightningBindGetterSetterProperty(AutomaticChannel);

  // Bind property info interface
  LightningBindFieldProperty(mNetPropertyInfos);
}
...
void NetObject::Serialize(Serializer& stream)
{
  ...
  BoundType* derivedType = LightningVirtualTypeId(this);
  ...
  bool accurateTimestampsByDefault = (derivedType != LightningTypeId(NetObject));
...
bool NetObject::SelectRemote()
{
  ...
  forRange (GameSession* gameSession, PL::gEngine->GetGameSessions())
  {
  ...
  MetaSelection* activeSelection = PL::gRuntimeEditor->GetActiveSelection();
...
NetPropertyInfo* NetObject::AddNetPropertyInfo(BoundType* componentType, StringParam propertyName)
{
  ...
  if (componentType == LightningTypeId(Transform) || componentType == LightningTypeId(RigidBody))
  {
...
LightningDefineType(EditInGameFilter, builder, type)
{
}
