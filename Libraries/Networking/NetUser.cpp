...
LightningDefineType(NetUser, builder, type)
{
  PlasmaBindComponent();
  ...
  LightningBindFieldGetterProperty(mNetUserId)->Add(new EditInGameFilter);
  LightningBindFieldGetterProperty(mNetPeerId)->Add(new EditInGameFilter);

  // Bind ownership interface
  LightningBindCustomGetterProperty(AddedByMyPeer)->Add(new EditInGameFilter);
  LightningBindMethod(AddedByPeer);
  LightningBindMethod(FindOwnedNetObjectByNameInSpace);
  LightningBindMethod(FindOwnedNetObjectByName);
  LightningBindGetter(OwnedNetObjects);
  LightningBindGetterProperty(OwnedNetObjectCount)->Add(new EditInGameFilter);
  LightningBindMethodProperty(ReleaseOwnedNetObjects)->Add(new EditInGameFilter);
}
