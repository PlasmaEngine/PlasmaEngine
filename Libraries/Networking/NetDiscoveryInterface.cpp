...
RespondingHostData::RespondingHostData(MoveReference<RespondingHostData> rhs) :
    mRoundTripTime(rhs->mRoundTripTime),
    mBasicHostInfo(PlasmaMove(rhs->mBasicHostInfo)),
    mExtraHostInfo(PlasmaMove(rhs->mExtraHostInfo)),
    mRefreshResult(rhs->mRefreshResult)
{
}
...
void NetDiscoveryInterface::DispatchHost(IpAddress const& hostIp, OpenHostRequest& hostRequest)
{
  ...
  if (hostData->mRefreshResult != NetRefreshResult::NoResponse)
  {
    ...
    {
      netHost->mBasicHostInfo = PlasmaMove(hostData->mBasicHostInfo);
    }
    if (!hostData->mExtraHostInfo.IsEmpty())
    {
      netHost->mExtraHostInfo = PlasmaMove(hostData->mExtraHostInfo);
    }
  }
                                            ...
                                            PendingHostPing& pendingHostPing)
{
  ...
  pongBundle = PlasmaMove(netHostPongData.mEventBundleData);
  ...
  else if (mDiscoveryMode == NetDiscoveryMode::Refresh)
  {
    ...
    if (pendingHostPing.mHostPingType == HostPingType::MasterServerRefreshHost)
    {
      ...
      // pong.
      pongBundle = PlasmaMove(refreshData.mBasicHostInfo);
  ...
  respondingHostData->mBasicHostInfo = PlasmaMove(pongBundle.GetBitStream());
...
void MultiHostRequest::FlushHostRequest(NetPeer& netPeer, NetDiscoveryInterface& netDiscoveryInstance)
{
  ...
  typedef Plasma::Pair<Plasma::IpAddress, Plasma::RespondingHostData> HostDataPair;
  ...
  Plasma::String eventId = mAllowDiscovery ? Events::NetHostListDiscovered : Events::NetHostListRefreshed;
