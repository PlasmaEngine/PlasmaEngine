...
LightningDefineType(EventBundleMetaComposition, builder, type)
{
}
...
EventBundleMetaComposition::EventBundleMetaComposition() : MetaComposition(LightningTypeId(Event))
{
...
void EventBundleMetaComposition::RemoveComponent(HandleParam instance, HandleParam subObject, bool ignoreDependencies)
{
  ...
  // Get event instance
  Event* event = subObject.Get<Event*>();
  Assert(LightningVirtualTypeId(event) == typeToRemove);
...
LightningDefineType(EventBundle, builder, type)
{
  // Bind documentation
  ...
  LightningBindConstructor(Event*);
  LightningBindConstructor(GameSession*);
  LightningBindConstructor(GameSession*, Event*);
  LightningBindConstructor(const EventBundle&);

  // Bind interface
  LightningBindCustomGetterProperty(IsEmpty);
  LightningBindMethod(AddEvent);
  LightningBindGetterSetter(GameSession);
  // LightningBindMethod(GetEventByTypeName);
  // LightningBindMethod(GetEventByType);
  // LightningBindMethod(GetEventByIndex);
  // LightningBindMethod(GetEventIndexByType);
  LightningBindMethod(GetEvents);
  // LightningBindMethod(GetEventCount);
  LightningBindMethod(RemoveEvent);
  LightningBindMethodAs(RemoveEventByTypeName, "RemoveEvent");
  // LightningBindMethod(RemoveEventByType);
  // LightningBindMethod(RemoveEventByIndex);
...
Event* EventBundle::GetEventByTypeName(StringParam eventTypeName)
{
  ...
  for (uint i = 0; i < mEvents.Size(); ++i)
    if (LightningVirtualTypeId(static_cast<Event*>(mEvents[i]))->Name == eventTypeName) // Found?
    {
      // Return event by index
...
Event* EventBundle::GetEventByType(BoundType* eventType)
{
  ...
  for (uint i = 0; i < mEvents.Size(); ++i)
    if (LightningVirtualTypeId(static_cast<Event*>(mEvents[i])) == eventType) // Found?
    {
      // Return event by index
      return GetEventByIndex(i);
    }
...
uint EventBundle::GetEventIndexByType(BoundType* eventType)
{
  ...
  // Find event by type ID
  for (uint i = 0; i < mEvents.Size(); ++i)
    if (LightningVirtualTypeId(static_cast<Event*>(mEvents[i])) == eventType)
