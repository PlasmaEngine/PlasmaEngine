...
public:
  LightningDeclareType(BitStreamExtended, TypeCopyMode::ReferenceType);
...
inline Bits SerializeKnownExtendedVariant(SerializeDirection::Enum direction, BitStream& bitStream, Variant& value)
{
  ...
  else if (value.Is<Any>())
  {
    ...
    else if (anyValue.StoredType->IsA(LightningTypeId(Resource)))
    {
    ...
    else if (anyValue.StoredType == LightningTypeId(BitStreamExtended))
    {
...
/// Returns true if BitStream can serialize the specified lightning type, else
/// false.
inline bool BitStreamCanSerializeType(Type* lightningType)
{
  // Get basic native type (or null)
  NativeType* basicNativeType = LightningTypeToBasicNativeType(lightningType);

  //    Is a basic any type?
  // OR Is an enum?
  // OR Is a resource?
  // OR Is a bitstream extended?
  return (basicNativeType != nullptr) || (lightningType->IsEnum()) || (lightningType->IsA(LightningTypeId(Resource))) ||
         (lightningType == LightningTypeId(BitStreamExtended));
}
