...
LightningDefineType(CommandUpdateEvent, builder, type)
{
}
...
LightningDefineType(CommandEvent, builder, type)
{
...
LightningDefineType(CommandExecuter, builder, type)
{
}
...
CommandExecuter* BuildMetaCommandExecuter(StringParam executionFunction)
{
  ...
  // Find object
  Object* systemObject = PL::gSystemObjects->FindObject(object);
  ReturnIf(systemObject == nullptr, nullptr, "Failed to find system object '%s'", object.c_str());

  // Find Function
  BoundType* metaType = LightningVirtualTypeId(systemObject);
...
LightningDefineType(Command, builder, type)
{
}
...
{
  if (PL::gEngine->IsReadOnly() && !ReadOnly)
  {
...
// Manager
LightningDefineType(CommandManager, builder, type)
{
  PlasmaBindEvent(Events::CommandExecute, CommandEvent);
}
...
void CommandManager::RunParsedCommandsDelayed()
{
  ...
  else
  {
    ...
    ActionSequence* seq = new ActionSequence(&PL::gWidgetManager->RootWidgets.Front());
...
void CommandManager::RunParsedCommandsImmediately()
{
  ...
  for (StringMap::Range range = arguments.All(); !range.Empty(); range.PopFront())
  {
    ...
    if (command)
    {
      PlasmaPrint("Executing command: %s\n", commandName.c_str());
      command->ExecuteCommand();
    }
  }
}
...
bool CommandManager::TestCommandCopyPasteShortcuts(ClipboardEvent* event)
{
  ...
  command->ExecuteCommand();
  GetContext()->Remove(LightningVirtualTypeId(event));
...
LightningDefineType(CommandCaptureContextEvent, builder, type)
{
}
