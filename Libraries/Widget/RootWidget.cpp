...
LightningDefineType(RootWidget, builder, type)
{
  LightningBindGetterProperty(OsWindow);

  PlasmaBindEvent(Events::Closing, HandleableEvent);
}
...
RootWidget::RootWidget(OsWindow* osWindow) : Composite(NULL)
{
  ...
  ConnectThisTo(PL::gWidgetManager, Events::WidgetUpdate, OnManagerUpdate);
  ...
  ConnectThisTo(PL::gEngine, Events::DebuggerPause, OnDebuggerPause);
  ConnectThisTo(PL::gEngine, Events::DebuggerResume, OnDebuggerResume);

  ConnectThisTo(PL::gEngine->has(TimeSystem), "UiUpdate", OnUiUpdate);
  ConnectThisTo(PL::gEngine->has(GraphicsEngine), "UiRenderUpdate", OnUiRenderUpdate);
}
...
RootWidget::~RootWidget()
{
  ...
  if (PL::gMouse->mActiveWindow == mOsWindow)
    PL::gMouse->mActiveWindow = nullptr;
  delete mOsWindow;
}
...
void RootWidget::OnUiRenderUpdate(Event* event)
{
  ...
  BuildOrthographicTransformPlasma(viewBlock.mViewToPerspective, mSize.y, mSize.x / mSize.y, -1.0f, 1.0f);

  Mat4 apiPerspective;
  PL::gRenderer->BuildOrthographicTransform(apiPerspective, mSize.y, mSize.x / mSize.y, -1.0f, 1.0f);
  ...
  HandleOf<RenderTarget> renderTarget =
      PL::gEngine->has(GraphicsEngine)->GetRenderTarget((uint)mSize.x, (uint)mSize.y, TextureFormat::RGBA8);
...
void PrintTreeR(Widget* widget)
{
  if (widget != NULL)
  {
    ...
    PlasmaPrint("%s/", widget->GetDebugName().c_str());
  }
}
...
{
  //  DrawContext* context = PL::gGraphics->mDrawContext;
...
{
  // Not on this window
  if (PL::gMouse->mActiveWindow != this->mOsWindow)
    return;
  ...
  if (hoverObject)
  {
    ...
    mouseEvent.EventMouse = PL::gMouse;
    mouseEvent.Source = hoverObject;
    mouseEvent.Position = PL::gMouse->mClientPosition;

    for (uint i = 0; i < MouseButtons::Size; ++i)
      mouseEvent.mButtonDown[i] = PL::gMouse->mButtonDown[i];
...
void RootWidget::BuildMouseEvent(MouseEvent& event, OsMouseEvent* mouseEvent)
{
  ...
  for (uint i = 0; i < MouseButtons::Size; ++i)
    event.mButtonDown[i] = PL::gMouse->mButtonDown[i];
}
...
void RootWidget::OnOsMouseMoved(OsMouseEvent* osMouseEvent)
{
  ...
  {
    mouseMovement = ToVec2(osMouseEvent->ClientPosition) - PL::gMouse->mClientPosition;
  }

  PL::gMouse->mClientPosition = ToVec2(osMouseEvent->ClientPosition);
  PL::gMouse->mCursorMovement = mouseMovement;

  if (Interaction::DebugMouseEvents)
    PlasmaPrint("Mouse Moved by %f, %f \n", mouseMovement.x, mouseMovement.y);
  ...
  if (mAnyMouseDown && downObject && captured == nullptr)
  {
    ...
    {
      if (PL::gMouse->mButtonDown[MouseButtons::Left])
        downObject->DispatchBubble(Events::LeftMouseDrag, &mouseEvent);
      else if (PL::gMouse->mButtonDown[MouseButtons::Right])
...
void RootWidget::UpdateMouseButtons(OsMouseEvent* mouseEvent)
{
  ...
  for (uint i = 0; i < MouseButtons::Size; ++i)
  {
    PL::gMouse->mButtonDown[i] = mouseEvent->ButtonDown[i];
    mAnyMouseDown = mAnyMouseDown || PL::gMouse->mButtonDown[i];
  }
}
