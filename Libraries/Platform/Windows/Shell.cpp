...
cwstr cWindowClassName = L"PlasmaWindow";
...
{
  // Create Windows OPENFILENAME structure and plasma it
  ...
  forRange (FileDialogFilter& fileFilter, config.mSearchFilters.All())
  {
    ...
    PlasmaStrCpyW(fileFilterPosition, description.Size(), description.c_str());
    fileFilterPosition += description.Size();
    PlasmaStrCpyW(fileFilterPosition, filter.Size(), filter.c_str());
    fileFilterPosition += filter.Size();
  }

  // File filters are double null terminated, so add one more at the end
  WString nullChar = Widen("\0");
  PlasmaStrCpyW(fileFilterPosition, 1, nullChar.c_str());

  // Set the default file name
  WString defaultFileName = Widen(config.DefaultFileName);
  PlasmaStrCpyW(fileName, cFileBufferSize, defaultFileName.c_str());
  ...
  {
    PlasmaStrCpyW(fileName, cFileBufferSize, L"Folder");
  }
  ...
  if (success)
  {
    // If nFileExtension is plasma, there is multiple files
    // from a multiselect
...
#define ProcessInput(VKValue, PlasmaValue)                                                                               \
  case VKValue:                                                                                                        \
    return PlasmaValue;
#include "Keys.inl"
#undef ProcessInput
...
void ScanDevice(Array<PlatformInputDevice>& devices, HANDLE deviceHandle, RID_DEVICE_INFO& ridDeviceInfo)
{
  ...
  if (ridDeviceInfo.dwType == RIM_TYPEHID && ridDeviceInfo.hid.usUsagePage == UsbUsagePage::GenericDesktop)
  {
    ...
    {
      PlasmaPrint("Device Name: %s ", deviceName.c_str());
      PlasmaPrint("Vendor Id: %d ", ridDeviceInfo.hid.dwVendorId);
      PlasmaPrint("Product Id: %d ", ridDeviceInfo.hid.dwProductId);
      PlasmaPrint("Version Number: %d ", ridDeviceInfo.hid.dwVersionNumber);
      PlasmaPrint("Usage: %X ", ridDeviceInfo.hid.usUsage);
      PlasmaPrint("Usage Page: %X ", ridDeviceInfo.hid.usUsagePage);
      PlasmaPrint("\n");
    }
  }
}
      ...
      forRange (PlatformInputDevice& inputDevice, window->mShell->mInputDevices)
      {
        ...
        if (GetRawInputDeviceInfo(deviceHandle, RIDI_PREPARSEDDATA, NULL, &bufferSize) == 0)
        {
          ...
          for (uint i = 0; i < Caps.NumberInputValueCaps; i++)
          {
            ...
            axes.PushBack(value);
            // PlasmaPrint("Axis with usage %x is %d\n",
...
LRESULT CALLBACK ShellWindowWndProc(ShellWindow* window, HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_SYSKEYDOWN:
  {
    ...
    if (shell->IsKeyDown(Keys::Control) && key == Keys::V && shell->mOnPaste)
    {
      ...
      if (data.mHasImage || data.mHasText)
...
Shell::Shell() : mCursor(Cursor::Arrow), mMainWindow(nullptr), mUserData(nullptr), mOnCopy(nullptr), mOnPaste(nullptr)
{
  ...
  PlasmaConstructPrivateData(ShellPrivateData);
  ...
  winClass.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(PlasmaDefaultIcon));
  winClass.hIconSm = LoadIcon(hInstance, MAKEINTRESOURCE(PlasmaDefaultIcon));
...
Shell::~Shell()
{
  ...
  PlasmaDestructPrivateData(ShellPrivateData);
}
...
{
  POINT ptPlasma = {0};
  HMONITOR hmonPrimary = MonitorFromPoint(ptPlasma, MONITOR_DEFAULTTOPRIMARY);
...
{
  PlasmaGetPrivateData(ShellPrivateData);
...
{
  PlasmaConstructPrivateData(ShellWindowPrivateData);
  memset(self, 0, sizeof(*self));
...
{
  Destroy();
  PlasmaDestructPrivateData(ShellWindowPrivateData);
}
...
{
  PlasmaGetPrivateData(ShellWindowPrivateData);
  switch (windowState)
  {
