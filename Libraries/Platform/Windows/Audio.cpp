...
{
  PlasmaPrint(BuildString(message, "\n").c_str());
  if (savedMessage)
...
Plasma::String GetMessageForHresult(Plasma::StringParam message, HRESULT hr)
{
  ...
  if (FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
                    ...
                    nullptr) != 0)
  {
    Plasma::String string(Plasma::String::Format("%s 0x%08x %s", message.c_str(), hr, errorMessage));
    LocalFree(errorMessage);
    return string;
  }

  return Plasma::String::Format("%s 0x%08x", message.c_str(), hr);
}
...
public:
  ...
  void ReleaseData();
  StreamStatus::Enum Initialize(IMMDeviceEnumerator* enumerator, bool render, Plasma::String* message);
...
StreamStatus::Enum WasapiDevice::Initialize(IMMDeviceEnumerator* enumerator, bool render, Plasma::String* message)
{
  if (render)
      ...
      (Format->wFormatTag != WAVE_FORMAT_EXTENSIBLE && Format->wFormatTag != WAVE_FORMAT_IEEE_FLOAT))
  {
    ...
    LogAudioIoError(
        GetMessageForHresult(Plasma::String::Format("Incompatible audio format 0x%04x.", Format->wFormatTag), result),
        message);
    HandleInitializationError();
    return StreamStatus::DeviceProblem;
  }
  ...
  result = AudioClient->Initialize(AUDCLNT_SHAREMODE_SHARED,          // Shared mode, not exclusive
                                   ...
                                   0,                                 // Pass in plasma to get smallest buffer size
...
StreamStatus::Enum WasapiDevice::StartStream(WASAPICallbackType* callback, void* data)
{
  ...
  if (!StreamOpen && Enumerator)
  {
    ...
    {
      // For output, set entire buffer to plasma to avoid audio glitches
      if (RenderClient)
      {
    ...
    else
    {
      ...
      PlasmaPrint("Started audio %s stream with fallback for no device\n", StreamTypeName.c_str());
    }
  ...
  {
    PlasmaPrint("Unable to start audio %s stream: ", StreamTypeName.c_str());

    if (!Enumerator)
    {
      PlasmaPrint("stream is uninitialized.\n");
      return StreamStatus::Uninitialized;
    }
    else
    {
      PlasmaPrint("stream is already open.\n");
      return StreamStatus::Started;
    }
  }
}
...
{
  PlasmaPrint("Resetting audio due to default device change\n");
...
void WasapiDevice::HandleInitializationError()
{
  ...
  PlasmaPrint("Audio %s initialization unsuccessful. Using fallback.\n", StreamTypeName.c_str());
}
...
{
  PlasmaPrint("Initializing WASAPI\n");
...
void AudioInputOutput::ShutDownAPI()
{
  if (((AudioIOWindowsData*)PlatformData)->Enumerator)
  {
    ...
    PlasmaPrint("WASAPI audio IO was shut down\n");
  }
  CoUninitialize();
}
