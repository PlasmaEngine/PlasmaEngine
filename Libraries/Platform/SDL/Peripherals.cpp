...
void SetButtonState(GamepadState* stateOut,
                    ...
                    GamepadButtonFlag plasmaButtonFlag,
                    SDL_GameControllerButton sdlButtonFlag)
{
  int bitShift = (int)Math::Log2((real)plasmaButtonFlag);
  stateOut->mButtons =
      ((stateOut->mButtons & plasmaButtonFlag) | (SDL_GameControllerGetButton(gamepad, sdlButtonFlag) << bitShift));
}
