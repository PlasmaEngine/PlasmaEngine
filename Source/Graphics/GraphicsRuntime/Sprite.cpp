...
LightningDefineType(BaseSprite, builder, type)
{
  ...
  LightningBindFieldProperty(mGeometryMode);
}
...
void BaseSprite::ComponentAdded(BoundType* typeId, Component* component)
{
  ...
  if (typeId == LightningTypeId(Area))
...
LightningDefineType(Sprite, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindDocumented();
  PlasmaBindInterface(BaseSprite);
  PlasmaBindSetup(SetupMode::DefaultSerialization);

  LightningBindGetterSetterProperty(SpriteSource);
  LightningBindFieldProperty(mFlipX);
  LightningBindFieldProperty(mFlipY);
  LightningBindFieldProperty(mAnimationActive);
  LightningBindFieldProperty(mAnimationSpeed);
  LightningBindGetterSetterProperty(StartFrame);
...
LightningDefineType(SpriteText, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindDocumented();
  PlasmaBindInterface(BaseSprite);
  ...
  LightningBindGetterSetterProperty(Text)->AddAttribute(PropertyAttributes::cLocalModificationOverride);
  LightningBindGetterSetterProperty(Font);
  LightningBindGetterSetterProperty(FontSize);
  LightningBindGetterSetterProperty(PixelsPerUnit);
  LightningBindGetterSetterProperty(TextAlign);
...
LightningDefineType(MultiSpriteEntry, builder, type)
{
  LightningBindCopyConstructor();
  LightningBindDestructor();

  LightningBindGetterProperty(Index);
  LightningBindGetterProperty(SpriteSource);
}
...
LightningDefineType(MultiSprite, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
