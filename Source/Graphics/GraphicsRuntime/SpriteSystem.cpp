...
namespace Plasma
{
    LightningDefineType(SpriteParticleSystem, builder, type)
    {
        PlasmaBindComponent();
        PlasmaBindDocumented();
        PlasmaBindInterface(ParticleSystem);
        PlasmaBindSetup(SetupMode::DefaultSerialization);

        LightningBindFieldProperty(mVertexColor);
        LightningBindFieldProperty(mGeometryMode)->AddAttribute(PropertyAttributes::cInvalidatesObject);
        LightningBindFieldProperty(mSpriteSource);
        LightningBindFieldProperty(mParticleAnimation);
        LightningBindFieldProperty(mParticleSort);
        LightningBindFieldProperty(mBeamBaseScale)
      ->PlasmaFilterEquality(mGeometryMode, SpriteParticleGeometryMode::Enum, SpriteParticleGeometryMode::Beam);
        LightningBindFieldProperty(mBeamVelocityScale)
      ->PlasmaFilterEquality(mGeometryMode, SpriteParticleGeometryMode::Enum, SpriteParticleGeometryMode::Beam);
    }
    ...
    void SpriteParticleSystem::ExtractViewData(ViewNode& viewNode, ViewBlock& viewBlock, FrameBlock& frameBlock)
    {
        ...
        while (particle != nullptr)
        {
            ...
            case SpriteParticleGeometryMode::Billboarded:
                {
                    ...
                    center = TransformPoint(viewNode.mLocalToView, particle->Position);
            ...
            case SpriteParticleGeometryMode::Beam:
                {
                    Vec3 velocityDir = TransformNormal(viewNode.mLocalToView, particle->Velocity);
            ...
            case SpriteParticleGeometryMode::Outward:
                {
                    ...
                    Vec3 xAxis, yAxis;
                    GenerateOrthonormalBasis(zAxis, &xAxis, &yAxis);

                    xAxis = TransformNormal(viewNode.mLocalToView, xAxis);
                    xAxis.AttemptNormalize();
                    yAxis = TransformNormal(viewNode.mLocalToView, yAxis);
                    yAxis.AttemptNormalize();
                    zAxis = TransformNormal(viewNode.mLocalToView, zAxis);
                    zAxis.AttemptNormalize();
            ...
            case SpriteParticleGeometryMode::Flat:
                {
                    Vec3 facing = TransformNormal(viewNode.mLocalToView, Vec3::cZAxis);
                    facing.AttemptNormalize();
                    Vec3 xAxis = TransformNormal(viewNode.mLocalToView, Vec3::cXAxis);
                    xAxis.AttemptNormalize();
                    Vec3 yAxis = TransformNormal(viewNode.mLocalToView, Vec3::cYAxis);
                    yAxis.AttemptNormalize();
            ...
            if (mSpriteSource->FrameCount > 1)
            {
                ...
                if (mParticleAnimation == SpriteParticleAnimationMode::Single)
                    frame = static_cast<uint>(particle->Time / particle->Lifetime * static_cast<float>(mSpriteSource->
                        FrameCount));
                else
                    frame = static_cast<uint>(particle->Time / mSpriteSource->FrameDelay) % mSpriteSource->FrameCount;
    ...
    u32 GetParticleSortValue(SpriteParticleSortMode::Enum sortMode, Vec3 pos, Vec3 camPos, Vec3 camDir)
    {
        ...
        case SpriteParticleSortMode::BackToFrontView:
            *floatValue = -Dot(pos - camPos, camDir);
            break;
