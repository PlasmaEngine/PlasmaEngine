...
LightningDefineType(GraphicsStatics, builder, type)
{
  LightningBindGetter(DriverSupport);
}

GraphicsDriverSupport* GraphicsStatics::GetDriverSupport()
{
  return &PL::gRenderer->mDriverSupport;
}
...
void GraphicsEngine::Initialize(SystemInitializer& initializer)
{
  ...
  mShaderGenerator = CreateLightningShaderGenerator();
  ...
  ConnectThisTo(PL::gEngine, Events::LoadingProgress, UpdateProgress);
  ConnectThisTo(PL::gEngine, Events::LoadingFinish, EndProgress);
  ConnectThisTo(PL::gEngine, Events::ProjectLoaded, OnProjectLoaded);
  ConnectThisTo(PL::gEngine, Events::NoProjectLoaded, OnNoProjectLoaded);

  ConnectThisTo(PL::gResources, Events::ResourcesLoaded, OnResourcesAdded);
  ConnectThisTo(PL::gResources, Events::ResourcesUnloaded, OnResourcesRemoved);
  ...
  ConnectThisTo(LightningFragmentManager::GetInstance(), Events::ResourceAdded, OnLightningFragmentAdded);
  ConnectThisTo(LightningFragmentManager::GetInstance(), Events::ResourceModified, OnLightningFragmentModified);
  ConnectThisTo(LightningFragmentManager::GetInstance(), Events::ResourceRemoved, OnLightningFragmentRemoved);
  ...
  ConnectThisTo(LightningManager::GetInstance(), Events::CompileLightningFragments, OnCompileLightningFragments);
  ConnectThisTo(LightningManager::GetInstance(), Events::ScriptsCompiledPrePatch, OnScriptsCompiledPrePatch);
  ConnectThisTo(LightningManager::GetInstance(), Events::ScriptsCompiledCommit, OnScriptsCompiledCommit);
  ConnectThisTo(LightningManager::GetInstance(), Events::ScriptsCompiledPostPatch, OnScriptsCompiledPostPatch);
  ConnectThisTo(LightningManager::GetInstance(), Events::ScriptCompilationFailed, OnScriptCompilationFailed);
...
#if defined(PlasmaLazyShaderCompositing)
...
void GraphicsEngine::Update(bool debugger)
{
  ...
  LightningManager::GetInstance()->mDebugger.DoNotAllowBreakReason =
  ...
  LightningManager::GetInstance()->mDebugger.DoNotAllowBreakReason.Clear();
}
...
{
  PL::gRenderer->mThreadLock.Lock();
  PL::gRenderer->mBackBufferSafe = false;
  PL::gRenderer->mThreadLock.Unlock();
}

void GraphicsEngine::OnOsWindowRestored(Event* event)
{
  PL::gRenderer->mThreadLock.Lock();
  PL::gRenderer->mBackBufferSafe = true;
  PL::gRenderer->mThreadLock.Unlock();
}
...
void GraphicsEngine::CheckTextureYInvert(Texture* texture)
{
  ...
  // work correctly Uv coordinate correction from Plasma to Api is done by the
  // shader translation of texture samples
  if (!PL::gRenderer->YInvertImageData(texture->mType))
    return;

  // All incoming image data from Plasma should be a color format and/or block
  // compressed
...
void GraphicsEngine::AddMaterial(Material* material)
{
  if (!material->mRenderData)
    material->mRenderData = PL::gRenderer->CreateMaterialRenderData();
...
{
  if (!mesh->mRenderData)
    mesh->mRenderData = PL::gRenderer->CreateMeshRenderData();
...
void GraphicsEngine::AddTexture(Texture* texture, bool subImage, uint xOffset, uint yOffset)
{
  ...
  if (!texture->mRenderData)
    texture->mRenderData = PL::gRenderer->CreateTextureRenderData();
...
void GraphicsEngine::OnLightningFragmentAdded(ResourceEvent* event)
{
  // OnResourcesAdded will invoke a compile after this
  mModifiedFragmentFiles.PushBack(event->EventResource->Name);
}

void GraphicsEngine::OnLightningFragmentModified(ResourceEvent* event)
{
  // Happens on save, wait for successful compilation to process
  mModifiedFragmentFiles.PushBack(event->EventResource->Name);
}

void GraphicsEngine::OnLightningFragmentRemoved(ResourceEvent* event)
{
...
void GraphicsEngine::ProcessModifiedScripts(LibraryRef library)
{
  forRange (BoundType* type, library->BoundTypes.Values())
  {
    ...
    if (type->IsA(LightningTypeId(Component)))
    {
...
LightningFragmentType::Enum GraphicsEngine::GetFragmentType(MaterialBlock* materialBlock)
{
  return mShaderGenerator->mFragmentTypes.FindValue(LightningVirtualTypeId(materialBlock)->Name,
                                                    LightningFragmentType::Fragment);
}
...
void GraphicsEngine::ForceCompileAllShaders()
{
  ...
  PL::gEngine->DispatchEvent(Events::BlockingTaskStart, &event);
...
void GraphicsEngine::ModifiedFragment(LightningFragmentType::Enum type, StringParam name)
{
  switch (type)
  {
  case Plasma::LightningFragmentType::CoreVertex:
    mModifiedCoreVertex.PushBack(name);
    break;
  case Plasma::LightningFragmentType::RenderPass:
    mModifiedRenderPass.PushBack(name);
    break;
  case Plasma::LightningFragmentType::PostProcess:
...
void GraphicsEngine::RemovedFragment(LightningFragmentType::Enum type, StringParam name)
{
  switch (type)
  {
  case Plasma::LightningFragmentType::CoreVertex:
...
void GraphicsEngine::OnCompileLightningFragments(LightningCompileFragmentEvent* event)
{
...
void GraphicsEngine::OnScriptsCompiledPrePatch(LightningCompileEvent* event)
{
  forRange (ResourceLibrary* modifiedLibrary, event->mModifiedLibraries.All())
  {
    ...
    if (currentLibrary != nullptr)
    {
      forRange (LightningShaderIRType* shaderType, currentLibrary->mTypes.Values())
      {
        ...
        if (pendingLibrary->mTypes.ContainsKey(shaderTypeMeta->mLightningName))
          continue;

        LightningFragmentType::Enum fragmentType =
            currentFragmentTypes.FindValue(shaderTypeMeta->mLightningName, LightningFragmentType::Fragment);
        RemovedFragment(fragmentType, shaderTypeMeta->mLightningName);
      }
    }
    ...
    if (mModifiedFragmentFiles.Empty() == false)
    {
      forRange (LightningShaderIRType* shaderType, pendingLibrary->mTypes.Values())
      {
        ShaderIRTypeMeta* shaderTypeMeta = shaderType->mMeta;
        if (shaderTypeMeta == nullptr)
          continue;
        ...
        if (mModifiedFragmentFiles.Contains(resourceName))
        {
          ...
          LightningFragmentType::Enum currentFragmentType =
              currentFragmentTypes.FindValue(shaderTypeMeta->mLightningName, LightningFragmentType::Fragment);
          LightningFragmentType::Enum pendingFragmentType =
              pendingFragmentTypes.FindValue(shaderTypeMeta->mLightningName, LightningFragmentType::Fragment);

          if (pendingFragmentType != currentFragmentType)
            RemovedFragment(currentFragmentType, shaderTypeMeta->mLightningName);

          ModifiedFragment(pendingFragmentType, shaderTypeMeta->mLightningName);
          ...
          // be checked
          if (pendingFragmentType == LightningFragmentType::Fragment)
          {
            forRange (UniqueComposite& composite, mUniqueComposites.Values())
            {
              if (composite.mFragmentNameMap.Contains(shaderTypeMeta->mLightningName))
                mModifiedComposites.Insert(composite.mName);
            }
          }

          // Find all types dependent on this one and also list them as modified
          HashSet<LightningShaderIRType*> dependents;
          pendingLibrary->GetAllDependents(shaderType, dependents);
          forRange (LightningShaderIRType* dependent, dependents.All())
          {
            ShaderIRTypeMeta* dependentTypeMeta = dependent->mMeta;
            if (dependentTypeMeta == nullptr)
              continue;

            LightningFragmentType::Enum dependentType =
                pendingFragmentTypes.FindValue(dependentTypeMeta->mLightningName, LightningFragmentType::Fragment);
            ...
            if (dependentType == LightningFragmentType::Fragment)
            {
              ...
              forRange (UniqueComposite& composite, mUniqueComposites.Values())
              {
                if (composite.mFragmentNameMap.Contains(dependentTypeMeta->mLightningName))
                  mModifiedComposites.Insert(composite.mName);
              }
            }
            else
            {
              ModifiedFragment(dependentType, dependentTypeMeta->mLightningName);
            }
          }
        }
      }
...
void GraphicsEngine::OnScriptsCompiledCommit(LightningCompileEvent* event)
{
...
void GraphicsEngine::OnScriptsCompiledPostPatch(LightningCompileEvent* event)
{
