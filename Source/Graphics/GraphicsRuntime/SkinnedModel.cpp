...
namespace Plasma
{
    LightningDefineType(SkinnedModel, builder, type)
    {
        PlasmaBindComponent();
        ...
        LightningBindGetterSetterProperty(SkeletonPath);
    }
    ...
    void SkinnedModel::ExtractFrameData(FrameNode& frameNode, FrameBlock& frameBlock)
    {
        ...
        frameNode.mLocalToWorldNormal = frameNode.mLocalToWorldNormal * ToMatrix3(mMesh->mBindOffsetInv);
    ...
    {
        if (static_cast<Mesh*>(event->EventResource) == mMesh)
            SetMesh(mMesh);
    }
