...
LightningDefineType(Camera, builder, type)
{
  ...
  LightningBindGetterSetterProperty(Size)->PlasmaFilterEquality(
...
Mat4 Camera::GetPerspectiveTransform()
{
  ...
  else
  {
    ...
    PL::gRenderer->BuildOrthographicTransform(mViewToApiPerspective, mSize, mAspectRatio, mNearPlane, mFarPlane);
  }
