...
LightningDefineType(SphericalParticleEmitter, builder, type)
{
  PlasmaBindComponent();
...
LightningDefineType(BoxParticleEmitter, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindInterface(ParticleEmitterShared);
  PlasmaBindSetup(SetupMode::DefaultSerialization);
...
LightningDefineType(MeshParticleEmitter, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  ...
  LightningBindGetterSetterProperty(MeshEmitMode);
  LightningBindFieldProperty(mNormalExtrude);
}
