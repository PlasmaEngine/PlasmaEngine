...
LightningDefineExternalBaseType(SamplerSettings, TypeCopyMode::ReferenceType, builder, type)
{
  ...
  LightningBindFieldProperty(mCompareMode);
  LightningBindFieldProperty(mCompareFunc);

  LightningBindDestructor();
  LightningBindConstructor();
}

LightningDefineType(RenderTarget, builder, type)
{
  PlasmaBindDocumented();

  LightningBindFieldGetter(mTexture);
  LightningBindMethod(Release);
}
