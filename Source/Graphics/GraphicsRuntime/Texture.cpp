...
LightningDefineType(Texture, builder, type)
{
  ...
  LightningBindGetterSetterProperty(AddressingY);
  LightningBindGetterSetterProperty(Filtering);
  LightningBindGetterSetterProperty(Anisotropy);
  LightningBindGetterSetterProperty(MipMapping);
  LightningBindGetterSetterProperty(CompareMode);
  LightningBindGetterSetterProperty(CompareFunc);

  LightningBindOverloadedMethod(Upload, LightningInstanceOverload(void, TextureData&));
...
void Texture::Upload(TextureData& textureData)
{
  ...
  PL::gEngine->has(GraphicsEngine)->AddTexture(this);
}
