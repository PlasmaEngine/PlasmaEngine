...
LightningDefineType(GraphicsSpace, builder, type)
{
  PlasmaBindComponent();
  ...
  LightningBindFieldProperty(mRandomSeed)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindFieldProperty(mSeed)->PlasmaFilterEquality(mRandomSeed, bool, false);
}
...
void GraphicsSpace::Initialize(CogInitializer& initializer)
{
  ...
  mGraphicsEngine = PL::gEngine->has(GraphicsEngine);
...
void GraphicsSpace::OnFrameUpdate(float frameDt)
{
  ...
  PlasmaForRangeVar (Graphical& graphical, graphicalRange, mGraphicalsToAdd.All())
  {
    GraphicalList::Unlink(&graphical);
