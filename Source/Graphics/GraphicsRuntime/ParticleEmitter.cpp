...
LightningDefineType(ParticleEmitter, builder, type)
{
...
// Hide Base Filter
LightningDefineType(HideBaseFilter, builder, type)
{
}
...
LightningDefineType(ParticleEmitterShared, builder, type)
{
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDocumented();

  PlasmaBindInterface(ParticleEmitter);
  PlasmaBindDependency(Transform);
  PlasmaBindDependency(ParticleSystem);
  PlasmaBindEvent(Events::ParticlesExhausted, ObjectEvent);
  PlasmaBindEvent(Events::AllParticlesDead, ObjectEvent);

  LightningBindFieldProperty(mActive);

  LightningBindFieldProperty(mEmitDelay);
  LightningBindFieldProperty(mEmitCount);

  LightningBindGetterSetterProperty(EmitRate);
  LightningBindFieldProperty(mEmitVariance);
  LightningBindFieldProperty(mEmitRateSoftStartTime);

  LightningBindFieldProperty(mSize);
  LightningBindFieldProperty(mSizeVariance);

  LightningBindFieldProperty(mLifetime);
  LightningBindFieldProperty(mLifetimeVariance);

  LightningBindFieldProperty(mSpin);
  LightningBindFieldProperty(mSpinVariance);

  LightningBindFieldProperty(mRandomSpin);

  LightningBindFieldProperty(mFill)->Add(new EditorSlider(0, 1, 0.01f))->HideOnDerivedType(MeshParticleEmitter);
  LightningBindFieldProperty(mEmitterVelocityPercent);

  LightningBindFieldProperty(mStartVelocity);
  LightningBindFieldProperty(mRandomVelocity);
  LightningBindFieldProperty(mTangentVelocity);

  LightningBindFieldProperty(mEmitterSize)->HideOnDerivedType(MeshParticleEmitter);

  LightningBindFieldProperty(mFastMovingEmitter);

  LightningBindMethodProperty(ResetCount);
}
