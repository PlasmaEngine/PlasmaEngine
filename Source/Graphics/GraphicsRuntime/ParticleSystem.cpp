...
LightningDefineType(ParticleEvent, builder, type)
{
  LightningBindGetterProperty(NewParticles);
  LightningBindGetterProperty(NewParticleCount);
}
...
LightningDefineType(ParticleSystem, builder, type)
{
  PlasmaBindDocumented();
  PlasmaBindInterface(Graphical);
  PlasmaBindSetup(SetupMode::DefaultSerialization);

  LightningBindGetterSetterProperty(BoundingBoxSize);
  LightningBindGetterSetterProperty(ChildSystem);
  LightningBindFieldProperty(mSystemSpace);
  LightningBindGetterSetterProperty(WarmUpTime);
  LightningBindGetterSetterProperty(PreviewInEditor);

  LightningBindMethod(AllParticles);
  LightningBindMethod(Clear);

  PlasmaBindTag(Tags::Particle);
  PlasmaBindEvent(Events::ParticlesSpawned, ParticleEvent);
}
...
void ParticleSystem::Initialize(CogInitializer& initializer)
{
  ...
  {
    PL::gRuntimeEditor->Visualize(this, "SpriteParticleSystem");
    ConnectThisTo(PL::gRuntimeEditor->GetActiveSelection(), Events::SelectionFinal, OnSelectionFinal);
  }
...
void ParticleSystem::ScriptInitialize(CogInitializer& initializer)
{
  ...
  float timeStep = PL::gEngine->has(TimeSystem)->GetTargetDt();
...
{
  return PL::gRuntimeEditor->GetActiveSelection()->Contains(GetOwner());
}
