...
LightningDefineType(SelectionIcon, builder, type)
{
  ...
  // SelectionIcon data
  LightningBindGetterSetterProperty(SpriteSource)->Add(new MetaEditorResource(true, false, "SelectionIcon"));
  LightningBindFieldProperty(mViewScale);
  LightningBindGetterSetterProperty(OverrideSelections);
}
