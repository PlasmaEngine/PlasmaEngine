...
NewProjectMenu::NewProjectMenu(Composite* parent, LauncherWindow* launcher) : Composite(parent), mLauncher(launcher)
{
  ...
  topRow->SetLayout(CreateRowLayout());
  {
    ...
    PlasmaBuild* mostRecent = nullptr;
  ...
  // settings menu)
  ConnectThisTo(PL::gEngine->GetConfigCog(), Events::LauncherConfigChanged, OnLauncherConfigChanged);
}

BuildId NewProjectMenu::GetBuildId() const
{
  PlasmaBuild* currentBuild = mBuildSelector->GetBuild();
...
void NewProjectMenu::ValidateProjectCreation()
{
  ...
  String filePath = FilePath::CombineWithExtension(projectFolder, projectName, ".plasmaproj");

  bool projectExists = FileExists(filePath);

  PlasmaBuild* selectedBuild = mBuildSelector->GetBuild();
  ...
  else
  {
    ...
    // libraries
    ContentLibrary* library = PL::gContentSystem->Libraries.FindValue(projectName, NULL);
    if (library)
    {
...
void NewProjectMenu::OnMenuDisplayed(Event* e)
{
  ...
  PlasmaBuild* latestInstalledBuild = nullptr;

  VersionSelector* versionSelector = mLauncher->mVersionSelector;
  forRange (PlasmaBuild* build, versionSelector->mVersions.All())
  {
...
void NewProjectMenu::OnCreateProject(Event* e)
{
  ...
  PlasmaPrint("Creating template:\n");
  PlasmaPrint("  Project: %s\n", selectedProject->GetIdString().c_str());
  PlasmaPrint("  Folder: %s\n", projectFolder.c_str());
  PlasmaPrint("  ProjectName: %s\n", projectName.c_str());
  PlasmaPrint("  Build: %s\n", projectBuildId.GetFullId().c_str());
  PlasmaPrint("  Project Tags: %s\n", mTags->GetText().c_str());
...
void NewProjectMenu::RunNewlyCreatedProject(CachedProject* project)
{
  ...
  PlasmaBuild* version = mLauncher->mVersionSelector->FindExactVersion(project);
