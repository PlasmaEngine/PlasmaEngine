...
void RecentProjectItem::OnRemoveModalResult(ModalConfirmEvent* e)
{
  if (e->mConfirmed)
  {
    ...
    PlasmaPrint("Removing project '%s' from recent projects\n", projectPath.c_str());
    ...
    mLauncher(launcher)
{
  ...
  ConnectThisTo(PL::gEngine->GetConfigCog(), Events::RecentProjectsUpdated, OnRecentProjectsUpdated);
...
void RecentProjectsMenu::UpdateTransform()
{
  ...
  // in) and the xCount we get will be 0. This will cause a plasma division (from
