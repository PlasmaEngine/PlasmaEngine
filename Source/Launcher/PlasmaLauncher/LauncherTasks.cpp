...
void GetVersionListingTaskJob::PopulatePackageList()
{
  ...
  Space* space = PL::gEngine->GetEngineSpace();
  ...
  forRange (JsonValue* jsonRelease, jsonReleases->ArrayElements)
  {
    ...
    forRange (JsonValue* jsonAsset, jsonAssets->ArrayElements)
    {
     ...
     //   continue;
      ...
      PlasmaBuildContent* plasmaBuildContent = HasOrAdd<PlasmaBuildContent>(cog);
      plasmaBuildContent->mPackageName = name;

      plasmaBuildContent->mBuildId = id;
      plasmaBuildContent->mPackageExtension = id.mPackageExtension;

      plasmaBuildContent->mDownloadUrl = jsonAsset->MemberAsString(cJsonDownloadUrl);

      // Parse tags or anything else that we just populated (normally happens
      // during Initialize).
      plasmaBuildContent->Parse();

      String releaseNotes = jsonRelease->MemberAsString(cJsonBody, String(), JsonErrorMode::DefaultValue);
      if (!releaseNotes.Empty())
      {
        PlasmaBuildReleaseNotes* plasmaBuildReleaseNotes = HasOrAdd<PlasmaBuildReleaseNotes>(cog);
        plasmaBuildReleaseNotes->mNotes = jsonRelease->MemberAsString(cJsonBody);
      }
    }
  }
}
...
void GetTemplateListingTaskJob::PopulateTemplateList()
{
  ...
  // _UserId is optional.
  static const Regex cNameRegex("(\\w+)(_[\\w\\d\\.\\,\\-]+)?_([\\w\\d\\.\\,\\-]+)\\.plasmatemplate");
  ...
  forRange (JsonValue* jsonRelease, jsonReleases->ArrayElements)
  {
    ...
    forRange (JsonValue* jsonAsset, jsonAssets->ArrayElements)
    {
      ...
      // release that's not a plasmatemplate). Specificially, if this is a png for
      ...
      PlasmaTemplate* plasmaTemplate = HasOrAdd<PlasmaTemplate>(cog);

      plasmaTemplate->mSKU = matches[1];
      plasmaTemplate->mVersionId = matches[3];

      plasmaTemplate->mDate = jsonAsset->MemberAsString(cJsonUpdatedAt);

      // Remove the time portion (otherwise, the date is in the exact format we
      // expect: YYYY-MM-DD).
      StringIterator begin = plasmaTemplate->mDate.Begin();
      StringIterator end = plasmaTemplate->mDate.FindFirstOf(Rune('T')).Begin();
      if (!end.Empty())
        plasmaTemplate->mDate = StringRange(begin, end);

      plasmaTemplate->mDownloadUrl = jsonAsset->MemberAsString(cJsonDownloadUrl);

      // The icon url is always the same as the template url, but with a png
      // extension instead.
      plasmaTemplate->mIconUrl = plasmaTemplate->mDownloadUrl.Replace(".plasmatemplate", ".png");

      plasmaTemplate->mDisplayName = jsonRelease->MemberAsString(cJsonName);
      plasmaTemplate->mDescription = jsonRelease->MemberAsString(cJsonBody);

      // Parse tags/build-ids and anything else that we just populated (normally
      // happens during Initialize).
      plasmaTemplate->Parse();
    }
  }
}
...
void DownloadTemplateTaskJob::OnReponse(WebResponseEvent* event)
{
  ...
  PlasmaTemplate* plasmaTemplate = mTemplate->GetPlasmaTemplate(false);
...
String DownloadLauncherPatchInstallerJob::FindPatchId(Archive& archive)
{
  ...
  for (Archive::range range = archive.GetEntries(); !range.Empty(); range.PopFront())
  {
    ...
    if (entry.Name != "PlasmaLauncherVersionId.txt")
      continue;
...
void DownloadLauncherMajorInstallerJob::OnReponse(WebResponseEvent* event)
{
  ...
  mInstallerPath = FilePath::Combine(temporaryDirectory, "PlasmaLauncherInstaller.exe");
