...
void MainButton::SetVersionAndProject(PlasmaBuild* version, Project* project)
{
...
{
  PlasmaPrint("Displaying Launcher Window\n");
  ...
  LauncherConfig* launcherConfig = PL::gEngine->GetConfigCog()->has(LauncherConfig);
  ...
  mBottomBar->SetLayout(CreateStackLayout(LayoutDirection::LeftToRight, Vec2::cZero, Thickness(Pixels(5, 5, 5, 0))));
  mBottomBar->SetName("BottomBar");
  {
    Text* titleText = new Text(mBottomBar, "BuildVersionText");
    titleText->SetColor(Vec4(0.2,0.2,0.2,1.0));
    titleText->mVerticalAlignment = VerticalAlignment::Bottom;
    titleText->SetText(("Version:" + ToString(GetConfigVersion())));
  ...
  mFileBugTextButton->SetText("Report a bug");
...
{
  PlasmaPrint("Checking for updates\n");
  ...
  */
  ...
  sequence->Add(new CallAction<LightningSelf, &LightningSelf::CheckForUpdates>(this));
}
...
void LauncherWindow::AutoCheckForLauncherUpdates()
{
  ...
  ActionSequence* sequence = new ActionSequence(this);
  sequence->Add(new ActionDelay(secondsForRecheck));
  sequence->Add(new CallAction<LightningSelf, &LightningSelf::AutoCheckForLauncherUpdates>(this));
}
...
void LauncherWindow::OnInstallMajorVersion(ModalConfirmEvent* e)
{
  ...
  if (!e->mConfirmed)
  {
    PlasmaPrint("User cancelled major version install.\n");
    return;
  }
...
void LauncherWindow::OnMajorLauncherUpdateDownloaded(BackgroundTaskEvent* e)
{
  ...
  {
    PlasmaPrint("Downloading major update failed.\n");
    return;
  }
  ...
  if (!job->mIsNewInstallerAvailable)
  {
    ...
    PlasmaPrint("No major update available.\n");
    return;
  }
...
void LauncherWindow::OnInstallPatchVersion(ModalConfirmEvent* e)
{
  ...
  if (!e->mConfirmed)
  {
    PlasmaPrint("User cancelled patch update.\n");
    return;
  }
...
void LauncherWindow::OnPatchLauncherUpdateDownloaded(BackgroundTaskEvent* e)
{
  ...
  if (!e->mTask->IsCompleted())
  {
    PlasmaPrint("Patch update failed.\n");
    return;
  }
  ...
  {
    // This case only happens if the download fails.
    PlasmaPrint("No patch downloaded.\n");
    return;
  }

  PlasmaPrint("Patch installed. Restarting launcher.\n");
...
void LauncherWindow::ForceUpdateBuilds()
{
  ...
  {
    PlasmaBuild* build = mVersionSelector->mVersions[i];
  ...
  String extraMsg = "Please close all open builds of plasma and try again";
...
void LauncherWindow::OnPackageListing(BackgroundTaskEvent* taskEvent)
{
  ...
  {
    PlasmaBuild* latest = mVersionSelector->GetLatestBuild();
...
void LauncherWindow::OnLauncherListing(BackgroundTaskEvent* taskEvent)
{
  ...
  String minorIdFile = FilePath::Combine(GetApplicationDirectory(), "PlasmaLauncherVersionId.txt");
  int minorId = GetVersionId(minorIdFile);
  int majorId = (int)GetMajorVersion();

  PlasmaBuildContent* newestInstaller = nullptr;
  PlasmaBuildContent* newestPatch = nullptr;

  static const String cPatchExtension("plasmabuild");

  // Get the newest launcher build from the listing.
  forRange (Cog* cog, job->mPackages)
  {
    PlasmaBuildContent* content = cog->has(PlasmaBuildContent);
    if (!content)
      continue;
  ...
  {
    PlasmaPrint("New major update found. Asking user to upgrade.\n");
  ...
  {
    PlasmaPrint("New launcher patch version available. Asking user to upgrade.\n");
...
void LauncherWindow::OnNewBuildAvailable(Event* e)
{
  ...
  // available?
  PlasmaPrint("New Build Available\n");
}
...
void LauncherWindow::OnOsMouseDrop(OsMouseDropEvent* e)
{
  for (size_t i = 0; i < e->Files.Size(); ++i)
  {
    ...
    PlasmaPrint("File '%s' was dropped on the launcher\n", filePath.c_str());

    String ext = FilePath::GetExtension(filePath).ToLower();
    // If the file is a plasmaproj then display it
    if (ext == "plasmaproj")
    {
    ...
    // If it is a .plasmabuild then install the build file
    else if (ext == PlasmaBuild::mExtension)
    {
      LauncherCommunicationEvent toSend;
      toSend.mProjectFile = filePath;
      DispatchEvent(Events::LauncherOpenBuild, &toSend);
    }
    // If this is a .plasmatemplate then install the template package
...
void LauncherWindow::OnInstallProjectPack(ModalConfirmEvent* e)
{
  ...
  // Find the plasmaproj file
  Array<String> files;
  ExtensionFilterFile filter("plasmaproj");
  FindFilesRecursively(exportDirectory, files, &filter);

  // If there's no plasmaproj then return (prob should front-load this during
  // drag-drop)
  if (files.Empty())
    return;

  String plasmaProjPath = files[0];

  // Load the project and display it
  CachedProject* project = mProjectCache->LoadProjectFile(plasmaProjPath);
  if (project == nullptr)
  {
    String msg = "Invalid Project File";
    ModalButtonsAction* modal = new ModalButtonsAction(this, msg.ToUpper(), "CLOSE");
    modal->mStringUserData = plasmaProjPath;
    mActiveModal = modal;
    return;
  }
...
void LauncherWindow::OnBrowsePressed(Event* e)
{
  ...
  config->AddFilter("Project File", "*.plasmaproj");
...
void LauncherWindow::StartListening()
{
  ...
  // Listen for interprocess communication from plasma or another launcher
...
{
  PlasmaPrint("New Project invoked by another process\n");
...
{
  PlasmaPrint("Open Project invoked by another process\n");
  // This needs to be before selection so focus doesn't change
  OsWindowTakeFocus();
  // Update our tags (if any were passed to us)
  OnLauncherUpdateTags(e);
...
{
  PlasmaPrint("Run Project invoked by another process\n");
  // This needs to be before selection so focus doesn't change
  OsWindowTakeFocus();
  // Update our tags (if any were passed to us)
  OnLauncherUpdateTags(e);
  ...
  PlasmaBuild* standalone = mVersionSelector->FindExactVersion(project);
...
{
  PlasmaPrint("Recent Projects invoked by another process\n");
  // This needs to be before selection so focus doesn't change
  OsWindowTakeFocus();
  // Update our tags (if any were passed to us)
  OnLauncherUpdateTags(e);
...
{
  PlasmaPrint("Run Command invoked by another process\n");
  // This needs to be before selection so focus doesn't change
  OsWindowTakeFocus();
