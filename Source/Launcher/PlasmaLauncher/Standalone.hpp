...
/// Represents a standalone "installer" of plasma that is either installed locally
/// or on the server.
class PlasmaBuild : public EventObject
{
public:
  PlasmaBuild();
  ...
  PlasmaBuildContent* GetBuildContent(bool createIfNull);
  PlasmaBuildDeprecated* GetDeprecatedInfo(bool createIfNull);
  ...
  // No copying allowed
private:
  PlasmaBuild(const PlasmaBuild& rhs){};
  void operator=(const PlasmaBuild& rhs){};
...
{
public:
  typedef TemplateProject LightningSelf;

  TemplateProject();

  /// Get the serialized data representing this template
  PlasmaTemplate* GetPlasmaTemplate(bool createIfNull);
  /// If we don't already have a meta cog, then create one from the empty
  /// archetype and make sure it has all of the necessary components. Returns
  /// true if the meta was null.
  bool CreateMetaIfNull();
