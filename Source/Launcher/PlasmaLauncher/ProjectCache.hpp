...
{
public:
  LightningDeclareType(LauncherProjectInfo, TypeCopyMode::ReferenceType);
...
{
public:
  typedef CachedProject LightningSelf;
  ...
  // The location of the project ".plasmaproj" file
  String GetProjectPath();
  void SetProjectPath(StringParam projectFilePath);

  // The directory of the project's ".plasmaproj"
  String GetProjectFolder();
  // The name of the project inside the ".plasmaproj"
  String GetProjectName();
  /// Rename this project to the given name. Also renames the folder containing
  /// the plasmaproj to the new name (assumes the user has checked for existing
...
private:
  /// Given a path and an expected project name, find the plasma project (not
  /// currently recursive)
  String FindPlasmaProj(StringParam searchpath, StringParam projectName);
