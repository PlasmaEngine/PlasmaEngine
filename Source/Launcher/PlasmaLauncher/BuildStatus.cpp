...
LightningDefineType(LauncherBuildEvent, builder, type)
{
}

BuildStatusView::BuildStatusView(Composite* parent, PlasmaBuild* version) : Composite(parent)
{
  mBuild = nullptr;
...
void BuildStatusView::SetVersion(PlasmaBuild* version)
{
...
BuildList::BuildList(Composite* parent, VersionSelector* versionSelector, PlasmaBuild* selected, bool installedOnly) :
    Composite(parent),
    mSelected(selected),
    mVersionSelector(versionSelector)
{
  ...
  PlasmaBuild* latestBuild = versionSelector->GetLatestBuild();
  ...
  PlasmaBuildTagPolicy policy(mVersionSelector->mConfig);
  forRange (PlasmaBuild* version, versionSelector->mVersions.All())
  {
...
BuildSelector::BuildSelector(Composite* parent, VersionSelector* versionSelector, PlasmaBuild* version) :
    Composite(parent),
    mVersionSelector(versionSelector)
{
  mInstalledOnly = true;
...
void BuildSelector::SetBuild(PlasmaBuild* version)
{
...
PlasmaBuild* BuildSelector::GetBuild()
{
