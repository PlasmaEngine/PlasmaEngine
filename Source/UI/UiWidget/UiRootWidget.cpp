...
LightningDefineType(UiRootWidget, builder, type)
{
  ...
  // Events
  PlasmaBindEvent(Events::UiFocusGainedPreview, UiFocusEvent);
  PlasmaBindEvent(Events::UiFocusGained, UiFocusEvent);
  PlasmaBindEvent(Events::UiFocusLost, UiFocusEvent);
  PlasmaBindEvent(Events::UiFocusLostHierarchy, UiFocusEvent);
  PlasmaBindEvent(Events::UiFocusGainedHierarchy, UiFocusEvent);
  PlasmaBindEvent(Events::UiFocusReset, UiFocusEvent);

  // Keyboard events
  PlasmaBindEvent(Events::HoverKeyPreview, KeyboardEvent);
  PlasmaBindEvent(Events::HoverKeyDown, KeyboardEvent);
  PlasmaBindEvent(Events::HoverKeyUp, KeyboardEvent);
  PlasmaBindEvent(Events::HoverKeyRepeated, KeyboardEvent);

  // LightningBindFieldProperty(mSnapSize);

  LightningBindFieldProperty(mMouseHoverTime);
  LightningBindFieldProperty(mMouseHoldTime);
  LightningBindFieldProperty(mDepthSeparation);

  LightningBindGetterSetterProperty(DebugSelected);

  LightningBindFieldProperty(mDebugMouseInteraction);

  LightningBindGetterSetter(FocusWidget);
  LightningBindGetter(MouseOverWidget);
  LightningBindGetter(MouseDownWidget);

  // Methods
  LightningBindMethod(Update);
  LightningBindMethod(Render);
  // LightningBindMethod(DispatchAt);
}
