...
LightningDefineType(UiLayout, builder, type)
{
  PlasmaBindDocumented();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDependency(UiWidget);

  LightningBindGetterSetterProperty(PaddingLeft);
  LightningBindGetterSetterProperty(PaddingTop);
  LightningBindGetterSetterProperty(PaddingRight);
  LightningBindGetterSetterProperty(PaddingBottom);

  // Only show the debug button if we're in Visual Studio
  if (Os::IsDebuggerAttached())
    LightningBindMethodProperty(Debug);
}
