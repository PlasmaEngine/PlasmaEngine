...
LightningDefineType(MainWindowTransformEvent, builder, type)
{
  LightningBindFieldGetter(mTargetWindow);
  LightningBindFieldGetter(mOldScreenPosition);
  LightningBindFieldGetter(mNewScreenPosition);
  LightningBindFieldGetter(mOldScreenSize);
  LightningBindFieldGetter(mNewScreenSize);
}
...
LightningDefineType(MainWindow, builder, type)
{
  PlasmaBindEvent(Events::MainWindowTransformUpdated, MainWindowTransformEvent);
}
...
void MainWindow::UpdateTransform()
{
  ...
  if (distanceSq > 1.0f)
  {
    ...
    PL::gEngine->DispatchEvent(Events::MainWindowTransformUpdated, &eventToSend);
