...
public:
  ...
  void Execute(Command* command, CommandManager* manager) override
  {
    ...
    if (param0 == NULL)
      return CommandFailed(command, LightningTypeId(pt0));

    if (param1 == NULL)
      return CommandFailed(command, LightningTypeId(pt1));
...
{
public:
  LightningDeclareType(MetaScriptTagAttribute, TypeCopyMode::ReferenceType);
...
{
public:
  LightningDeclareType(MetaScriptShortcutAttribute, TypeCopyMode::ReferenceType);
