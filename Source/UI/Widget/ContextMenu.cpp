...
LightningDefineType(ContextMenuEvent, builder, type)
{
}
...
LightningDefineType(ContextMenuEntry, builder, type)
{
  PlasmaBindDocumented();
  LightningBindConstructor();

  LightningBindOverloadedMethodAs(AddEntry, LightningInstanceOverload(ContextMenuEntry*, StringParam, bool), "AddEntry");
  LightningBindMethod(AddDivider);
  LightningBindMethod(AddCommandByName);
  LightningBindOverloadedMethodAs(RemoveEntry, LightningInstanceOverload(void, StringParam), "RemoveEntry");

  LightningBindMethod(GetEntry);
  LightningBindMethodAs(GetEntries, "Entries");

  LightningBindFieldProperty(mName);
  LightningBindFieldProperty(mIcon);

  PlasmaBindEvent(Events::ContextMenuCreated, ContextMenuEvent);
}
...
LightningDefineType(ContextMenuEntryDivider, builder, type)
{
  LightningBindConstructor();
}
...
LightningDefineType(ContextMenuEntryCommand, builder, type)
{
  LightningFullBindConstructor(builder, type, ContextMenuEntryCommand, "commandName", StringParam);
  LightningBindFieldProperty(mCommandName);
}
...
LightningDefineType(ContextMenuEntryMenu, builder, type)
{
  LightningFullBindConstructor(builder, type, ContextMenuEntryMenu, "menuName", StringParam);
  LightningBindFieldProperty(mMenuName);
}
...
{
  if (PL::gEngine->IsReadOnly() && !mReadOnly)
  {
...
Plasma::ContextMenuEntry* ContextMenu::GetRootEntry()
{
  return mRootEntry;
}
...
void ContextMenu::AddPlasmaContextMenu(StringParam menuName)
{
...
LightningDefineType(MenuBarItem, builder, type)
{
}
...
void MenuBarItem::OpenContextMenu()
{
  ...
  contextMenu->AddPlasmaContextMenu(mName);
...
LightningDefineType(MenuBar, builder, type)
{
}
