...
u64 WidgetHandleManager::HandleToId(const Handle& handle)
{
  ...
  ReturnIf(!Type::BoundIsA(handle.StoredType, LightningTypeId(Widget)), 0, "A handle to a non widget was passed in.");
...
void Widget::DebugValidate() const
{
  ...
  if (!mDestroyed)
  {
    Widget* widget = PL::gWidgetManager->Widgets.FindValue(this->mId, nullptr);
