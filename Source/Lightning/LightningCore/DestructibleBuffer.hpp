...
#pragma once
#ifndef LIGHTNING_DESTRUCTiBLE_BUFFER_HPP
#  define LIGHTNING_DESTRUCTiBLE_BUFFER_HPP

namespace Lightning
{
// Aligns a number to a machine byte boundary (either the same or a larger
// value)
PlasmaShared size_t AlignToBusWidth(size_t value);
...
template <typename T>
PlasmaSharedTemplate void DestructorWrapper(void* object)
{
...
template <typename T>
PlasmaSharedTemplate DestructFn GetDestructor()
{
...
// signature
template <typename T>
PlasmaSharedTemplate void CopyConstructorWrapper(const void* from, void* to)
{
...
template <typename T>
PlasmaSharedTemplate CopyConstructFn GetCopyConstructor()
{
  // If the type is considered to be pod, ignore the destructor
  if (StandardTraits<T>::is_pod_::value)
  {
...
// destructed
class PlasmaShared DestructibleBuffer
{
public:
