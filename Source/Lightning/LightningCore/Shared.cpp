...
// Copy
#define LightningCopyOperators(WithType)                                                                                   \
  {                                                                                                                    \
...
#define LightningEqualityOperators(WithType, ResultType)                                                                   \
  {                                                                                                                    \
    BoundType* type = core.WithType##Type;                                                                             \
    this->AddBinary(                                                                                                   \
...
#define LightningComparisonOperators(WithType, ResultType)                                                                 \
  {                                                                                                                    \
    BoundType* type = core.WithType##Type;                                                                             \
    this->AddBinary(                                                                                                   \
...
#define LightningNumericOperators(WithType, ComparisonType)                                                                \
  LightningCopyOperators(WithType) LightningEqualityOperators(WithType, ComparisonType)                                        \
  {                                                                                                                    \
    BoundType* type = core.WithType##Type;                                                                             \
...
#define LightningScalarOperators(WithType)                                                                                 \
  LightningNumericOperators(WithType, Boolean) LightningComparisonOperators(WithType, Boolean)

// Vector operations, generic numeric operators, copy, equality
#define LightningVectorOperators(VectorType, ScalarType, ComparisonType)                                                   \
  LightningNumericOperators(VectorType, Boolean) LightningComparisonOperators(VectorType, ComparisonType)                      \
  {                                                                                                                    \
...
// comparison
#define LightningIntegralOperators(WithType)                                                                               \
  {                                                                                                                    \
    BoundType* type = core.WithType##Type;                                                                             \
  ...
  LightningIntegralOperators(Byte);
  LightningScalarOperators(Byte);
  LightningIntegralOperators(Integer);
  LightningScalarOperators(Integer);
  LightningVectorOperators(Integer2, Integer, Boolean2);
  LightningVectorOperators(Integer3, Integer, Boolean3);
  LightningVectorOperators(Integer4, Integer, Boolean4);
  LightningIntegralOperators(Integer2);
  LightningIntegralOperators(Integer3);
  LightningIntegralOperators(Integer4);
  LightningScalarOperators(Real);
  LightningVectorOperators(Real2, Real, Boolean2);
  LightningVectorOperators(Real3, Real, Boolean3);
  LightningVectorOperators(Real4, Real, Boolean4);
  LightningScalarOperators(DoubleReal);
  LightningIntegralOperators(DoubleInteger);
  LightningScalarOperators(DoubleInteger);

  LightningEqualityOperators(Boolean, Boolean);
  // Handle, Delegate, and Value equality operators are handled specially above

  LightningCopyOperators(Boolean);
...
BinaryOperator Shared::GetBinaryOperator(Type* lhs, Type* rhs, Grammar::Enum oper, bool allowRecursiveLookup)
{
  ...
  Type* integerType = LightningTypeId(Integer);
...
CastOperator Shared::GetCastOperator(Type* from, Type* to)
{
  ...
  // handle...
  if (from == LightningTypeId(Handle) && Type::IsHandleType(to))
    return this->FromAnyHandleCast;

  // If we're casting from an Integer to an enum...
  if (Type::IsSame(from, LightningTypeId(Integer)) && Type::IsEnumOrFlagsType(to))
    return this->IntegerEnumCast;

  // If we're casting from an enum to an Integer... (can be implicit)
  if (Type::IsEnumOrFlagsType(from) && Type::IsSame(to, LightningTypeId(Integer)))
  ...
  if (Type::IsSame(LightningTypeId(NullPointerType), from) && Type::IsDelegateType(to))
    return this->NullToDelegate;

  // If we're converting from null to any other handle type...
  if (Type::IsSame(LightningTypeId(NullPointerType), from) && Type::IsHandleType(to))
    return this->RawImplicitCast;
