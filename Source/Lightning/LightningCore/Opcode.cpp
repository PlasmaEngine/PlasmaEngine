...
#define LightningOperand(array, type, member, primitive, isLocal)                                                          \
  array.PushBack(DebugOperand(PlasmaOffsetOf(type, member), primitive, isLocal, #member));
...
void GenerateDebugInstructionInfo(Array<DebugInstruction>& debugOut)
{
  ...
  // ToHandle
  {
    ...
    LightningOperand(info.ReadOperands, ToHandleOpcode, ToHandle, DebugPrimitive::Memory, false);
    LightningOperand(info.WriteOperands, ToHandleOpcode, SaveLocal, DebugPrimitive::Memory, true);
  }

  // CreateStaticDelegate
  {
    DebugInstruction& info = debugOut[Instruction::CreateStaticDelegate];
    info.FunctionPointers.PushBack(PlasmaOffsetOf(CreateStaticDelegateOpcode, BoundFunction));
    LightningOperand(info.WriteOperands, CreateStaticDelegateOpcode, SaveLocal, DebugPrimitive::Delegate, true);
  }

  // CreateInstanceDelegate
  {
    DebugInstruction& info = debugOut[Instruction::CreateInstanceDelegate];
    info.FunctionPointers.PushBack(PlasmaOffsetOf(CreateInstanceDelegateOpcode, BoundFunction));
    LightningOperand(info.ReadOperands, CreateInstanceDelegateOpcode, ThisHandle, DebugPrimitive::Handle, false);
    LightningOperand(info.WriteOperands, CreateInstanceDelegateOpcode, SaveLocal, DebugPrimitive::Delegate, true);
  }
  ...
  // IfFalseRelativeGoTo / IfTrueRelativeGoTo
  {
    ...
    for (size_t i = 0; i < LightningCArrayCount(instructions); ++i)
    {
      Instruction::Enum instruction = instructions[i];
      DebugInstruction& info = debugOut[instruction];
      LightningOperand(info.ReadOperands, IfOpcode, Condition, DebugPrimitive::Boolean, false);
      info.OpcodeOffsets.PushBack(PlasmaOffsetOf(IfOpcode, JumpOffset));
    }
  }
  ...
  // RelativeGoTo
  {
    ...
    info.OpcodeOffsets.PushBack(PlasmaOffsetOf(RelativeJumpOpcode, JumpOffset));
  }

  // PrepForFunctionCall
  {
    DebugInstruction& info = debugOut[Instruction::PrepForFunctionCall];
    LightningOperand(info.ReadOperands, PrepForFunctionCallOpcode, Delegate, DebugPrimitive::Delegate, false);
    info.OpcodeOffsets.PushBack(PlasmaOffsetOf(PrepForFunctionCallOpcode, JumpOffsetIfStatic));
  }

  // NewObject
  {
    DebugInstruction& info = debugOut[Instruction::NewObject];
    info.TypePointers.PushBack(PlasmaOffsetOf(CreateTypeOpcode, CreatedType));
    LightningOperand(info.WriteOperands, CreateTypeOpcode, SaveHandleLocal, DebugPrimitive::Handle, true);
  }

  // LocalObject
  {
    DebugInstruction& info = debugOut[Instruction::LocalObject];
    info.TypePointers.PushBack(PlasmaOffsetOf(CreateLocalTypeOpcode, CreatedType));
    LightningOperand(info.WriteOperands, CreateLocalTypeOpcode, SaveHandleLocal, DebugPrimitive::Handle, true);
    LightningOperand(info.WriteOperands, CreateLocalTypeOpcode, StackLocal, DebugPrimitive::Memory, true);
  }

  // DeleteObject
  {
    DebugInstruction& info = debugOut[Instruction::DeleteObject];
    LightningOperand(info.WriteOperands, DeleteObjectOpcode, Object, DebugPrimitive::Handle, false);
  }
  ...
  // [UnaryRValueOpcode]
  {
    ...
    for (size_t i = 0; i < LightningCArrayCount(instructions); ++i)
    {
      ...
      DebugInstruction& info = debugOut[instruction];
      LightningOperand(info.ReadOperands, UnaryRValueOpcode, SingleOperand, primitive, false);
      LightningOperand(info.WriteOperands, UnaryRValueOpcode, Output, primitive, true);
    }
  }
  ...
  // [UnaryLValueOpcode]
  {
    ...
    for (size_t i = 0; i < LightningCArrayCount(instructions); ++i)
    {
      ...
      DebugPrimitive::Enum primitive = types[i];
      DebugInstruction& info = debugOut[instruction];
      LightningOperand(info.WriteOperands, UnaryLValueOpcode, SingleOperand, primitive, false);
    }
  }
  ...
  // [BinaryRValueOpcode] (Result + Operands all the same type)
  {
    ...
    for (size_t i = 0; i < LightningCArrayCount(instructions); ++i)
    {
      ...
      DebugPrimitive::Enum primitive = types[i];
      DebugInstruction& info = debugOut[instruction];
      LightningOperand(info.ReadOperands, BinaryRValueOpcode, Left, primitive, false);
      LightningOperand(info.ReadOperands, BinaryRValueOpcode, Right, primitive, false);
      LightningOperand(info.WriteOperands, BinaryRValueOpcode, Output, primitive, true);
    }
  }
  ...
  // [BinaryRValueOpcode] (Operands all the same type, Result is a Boolean)
  {
    ...
    for (size_t i = 0; i < LightningCArrayCount(instructions); ++i)
    {
      ...
      LightningOperand(info.ReadOperands, BinaryRValueOpcode, Left, primitive, false);
      LightningOperand(info.ReadOperands, BinaryRValueOpcode, Right, primitive, false);
      LightningOperand(info.WriteOperands, BinaryRValueOpcode, Output, DebugPrimitive::Boolean, true);
    }
  }
  ...
  // [BinaryRValueOpcode] (Scalar + Vector operations)
  {
    ...
    for (size_t i = 0; i < LightningCArrayCount(instructions); ++i)
    {
      ...
      DebugInstruction& info = debugOut[instruction];
      LightningOperand(info.ReadOperands, BinaryRValueOpcode, Left, primitive, false);
      LightningOperand(info.ReadOperands, BinaryRValueOpcode, Right, DebugPrimitive::Real, false);
      LightningOperand(info.WriteOperands, BinaryRValueOpcode, Output, primitive, true);
    }
  }
  ...
  // [BinaryLValueOpcode] (Operands all the same type)
  {
    ...
    for (size_t i = 0; i < LightningCArrayCount(instructions); ++i)
    {
      ...
      DebugPrimitive::Enum primitive = types[i];
      DebugInstruction& info = debugOut[instruction];
      LightningOperand(info.ReadOperands, BinaryLValueOpcode, Right, primitive, false);
      LightningOperand(info.WriteOperands, BinaryLValueOpcode, Output, primitive, false);
    }
  }
  ...
  // [BinaryLValueOpcode] (Scalar + Vector assignment operations)
  {
    ...
    for (size_t i = 0; i < LightningCArrayCount(instructions); ++i)
    {
      ...
      DebugPrimitive::Enum primitive = types[i];
      DebugInstruction& info = debugOut[instruction];
      LightningOperand(info.ReadOperands, BinaryLValueOpcode, Right, DebugPrimitive::Real, false);
      LightningOperand(info.WriteOperands, BinaryLValueOpcode, Output, primitive, false);
    }
  }
  ...
  // [ConversionOpcode]
  {
    ...
    for (size_t i = 0; i < LightningCArrayCount(instructions); ++i)
    {
      ...
      DebugInstruction& info = debugOut[instruction];
      LightningOperand(info.ReadOperands, ConversionOpcode, ToConvert, fromPrimitive, false);
      LightningOperand(info.WriteOperands, ConversionOpcode, Output, toPrimitive, true);
    }
  }
  ...
  // [CopyOpcode]
  {
    ...
    for (size_t i = 0; i < LightningCArrayCount(instructions); ++i)
    {
      ...
      DebugInstruction& info = debugOut[instruction];
      info.IsCopy = true;
      LightningOperand(info.ReadOperands, CopyOpcode, Source, primitive, false);
      LightningOperand(info.WriteOperands, CopyOpcode, Destination, primitive, false);
      info.Sizes.PushBack(PlasmaOffsetOf(CopyOpcode, Size));
      info.Options.PushBack(PlasmaOffsetOf(CopyOpcode, Mode));
    }
  }
}
} // namespace Lightning
