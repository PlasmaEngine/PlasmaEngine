...
#pragma once
#ifndef LIGHTNING_COMPOSITION_HPP
#  define LIGHTNING_COMPOSITION_HPP
...
// Allows the user to attach any type to a Lightning primitive
// The primitive must be default constructable
class PlasmaShared Composition : public ILightningObject
{
public:
  Composition();
  ~Composition();
  ...
  {
    Type* type = LightningVirtualTypeId(component);
  ...
  T* Has()
  {
    ...
    Handle* handle = this->Components.FindPointer(LightningTypeId(T));
