...
LightningDefineType(ColorClass, builder, type)
{
  LightningFullBindMethod(builder,
                      ...
                      LightningStaticOverload(Real4, Integer, Integer, Integer),
                      "FromBytes",
                      "r, g, b")
      ->Description = LightningDocumentString("Converts RGB Bytes [0-255] into an RGBA Real4 (alpha defaults to 1.0). "
                                          "Integer is used in place of Byte for convenience.");
  LightningFullBindMethod(builder,
                      type,
                      &ColorClass::FromBytes,
                      LightningStaticOverload(Real4, Integer, Integer, Integer, Integer),
                      "FromBytes",
                      "r, g, b, a")
      ->Description = LightningDocumentString("Converts RGBA Bytes [0-255] into an RGBA Real4. "
                                          "Integer is used in place of Byte for convenience.");
  LightningFullBindMethod(
      builder, type, &ColorClass::FromBytes, LightningStaticOverload(Real4, Integer4Param), "FromBytes", "rgba")
      ->Description = LightningDocumentString("Converts RGBA Bytes [0-255] into an RGBA Real4. "
                                          "Integer is used in place of Byte for convenience.");

  LightningFullBindMethod(
      builder, type, &ColorClass::FromInteger, LightningStaticOverload(Real4, Integer), "FromInteger", "rgba")
      ->Description = LightningDocumentString("Converts an RGBA packed Integer into an RGBA Real4. Endianness is "
                                          "handled so that 0x00FF00FF always means full green and full alpha.");
  LightningFullBindMethod(
      builder, type, &ColorClass::FromInteger, LightningStaticOverload(Real4, Integer, Integer), "FromInteger", "rgb, a")
      ->Description = LightningDocumentString("Converts an RGB packed Integer and an alpha Byte [0-255] into an RGBA "
                                          "Real4. Endianness is handled so that 0x00FF00FF always means full green "
                                          "and full alpha.");
  LightningFullBindMethod(
      builder, type, &ColorClass::FromInteger, LightningStaticOverload(Real4, Integer, Real), "FromInteger", "rgb, a")
      ->Description = LightningDocumentString("Converts an RGB packed Integer and an alpha Real "
                                          "into an RGBA Real4. Endianness is handled so that "
                                          "0x00FF00FF always means full green and full alpha.");

  LightningFullBindMethod(
      builder, type, &ColorClass::FromHsva, LightningStaticOverload(Real4, Real, Real, Real), "FromHsva", "h, s, v")
      ->Description = LightningDocumentString("Converts HSV Reals into an RGBA Real4 (alpha defaults to 1.0). The hue "
                                          "parameter will wrap if is beyond [0-1]. The saturation and value "
                                          "parameters may go beyond [0-1] to represent HDR values.");
  LightningFullBindMethod(builder,
                      type,
                      &ColorClass::FromHsva,
                      LightningStaticOverload(Real4, Real, Real, Real, Real),
                      "FromHsva",
                      "h, s, v, a")
      ->Description = LightningDocumentString("Converts HSVA Reals into an RGBA Real4. The hue parameter will wrap if "
  ...
  LightningFullBindMethod(builder, type, &ColorClass::FromHsva, LightningStaticOverload(Real4, Real4Param), "FromHsva", "hsva")
      ->Description = LightningDocumentString("Converts an HSVA Real4 into an RGBA Real4. The hue parameter will wrap "
  ...
  LightningFullBindMethod(builder, type, &ColorClass::FromHexString, LightningNoOverload, "FromHexString", "value")
      ->Description = LightningDocumentString("Converts a hex String into an RGBA Real4. Must be a 3, 4, 6, or 8 digit "
  ...
  LightningFullBindMethod(builder, type, &ColorClass::ToBytes, LightningStaticOverload(Integer4, Real4Param), "ToBytes", "rgba")
      ->Description = LightningDocumentString("Converts an RGBA Real4 into an RGBA Integer4 [0-255]. Integer is used "
                                          "in place of Byte for convenience.");
  LightningFullBindMethod(
      builder, type, &ColorClass::ToBytes, LightningStaticOverload(Integer4, Real, Real, Real), "ToBytes", "r, g, b")
      ->Description = LightningDocumentString("Converts RGB Reals into an RGBA Integer4 [0-255] (alpha defaults to "
                                          "255). Integer is used in place of Byte for convenience.");
  LightningFullBindMethod(builder,
                      type,
                      &ColorClass::ToBytes,
                      LightningStaticOverload(Integer4, Real, Real, Real, Real),
                      "ToBytes",
                      "r, g, b, a")
      ->Description = LightningDocumentString("Converts RGBA Reals into an RGBA Integer4 [0-255]. "
                                          "Integer is used in place of Byte for convenience.");

  LightningFullBindMethod(
      builder, type, &ColorClass::ToInteger, LightningStaticOverload(Integer, Real4Param), "ToInteger", "rgba")
      ->Description = LightningDocumentString("Converts an RGBA Real4 into an RGBA packed Integer. Endianness is "
                                          "handled so that 0x00FF00FF always means full green and full alpha.");
  LightningFullBindMethod(
      builder, type, &ColorClass::ToInteger, LightningStaticOverload(Integer, Real, Real, Real), "ToInteger", "r, g, b")
      ->Description = LightningDocumentString("Converts RGB Reals into an RGBA packed Integer (alpha defaults to 255). "
                                          "Endianness is handled so that 0x00FF00FF always means full green and "
                                          "full alpha.");
  LightningFullBindMethod(builder,
                      type,
                      &ColorClass::ToInteger,
                      LightningStaticOverload(Integer, Real, Real, Real, Real),
                      "ToInteger",
                      "r, g, b, a")
      ->Description = LightningDocumentString("Converts RGBA Reals into an RGBA packed Integer. Endianness is handled "
  ...
  LightningFullBindMethod(
      builder, type, &ColorClass::ToHsva, LightningStaticOverload(Real4, Real, Real, Real), "ToHsva", "r, g, b")
      ->Description = LightningDocumentString("Converts RGB Reals into an HSVA Real4 (alpha defaults to 1.0).");
  LightningFullBindMethod(
      builder, type, &ColorClass::ToHsva, LightningStaticOverload(Real4, Real, Real, Real, Real), "ToHsva", "r, g, b, a")
      ->Description = LightningDocumentString("Converts RGBA Reals into an HSVA Real4.");

  LightningFullBindMethod(
      builder, type, &ColorClass::ToHexString, LightningStaticOverload(String, Real4Param), "ToHexString", "rgba")
      ->Description = LightningDocumentString("Converts an RGBA Real4 into the 8 digit hex format RRGGBBAA.");
  LightningFullBindMethod(
      builder, type, &ColorClass::ToHexString, LightningStaticOverload(String, Real, Real, Real), "ToHexString", "r, g, b")
      ->Description = LightningDocumentString("Converts RGB Reals into the 8 digit hex format RRGGBBAA (alpha defaults "
                                          "to 1.0 so the end will always be FF).");
  LightningFullBindMethod(builder,
                      type,
                      &ColorClass::ToHexString,
                      LightningStaticOverload(String, Real, Real, Real, Real),
                      "ToHexString",
                      "r, g, b, a")
