...
namespace Lightning
{
LightningDefineType(Library, builder, type)
{
}
...
{
  ErrorIf(Type::IsEnumOrFlagsType(owner) == false, "Use LightningFullBindEnum before calling LightningFullBindEnumValue");
  ErrorIf(owner->Size != sizeof(Integer), "Must be the same size as an Integer");
  GetterSetter* getset = this->AddBoundGetterSetter(
      owner, name, owner, nullptr, &LS::VirtualMachine::EnumerationProperty, LS::MemberOptions::Static);
                                                         ...
                                                         const LibraryArray& fromLibraries)
{
  ...
  for (size_t i = 0; i < fromLibraries.Size(); ++i)
  {
    ...
    if (info != nullptr)
    {
      ...
      {
        LightningForEach (Library* dependentLibrary, fromLibraries)
        {
...
LibraryBuilder::AddSendsEvent(BoundType* forType, StringParam name, BoundType* sentType, StringParam description)
{
  ...
  GuidType guid = LightningTypeId(EventsClass)->Hash();
  ...
  if (existingProperty == nullptr)
  {
    ...
    // be accessed by name
    Property* property = this->AddExtensionGetterSetter(LightningTypeId(EventsClass),
                                                        name,
                                                        LightningTypeId(String),
                                                        nullptr,
  ...
  else
  {
    ...
    // event type...
    PlasmaTodo("Don't allow two sends events with the same name, unless maybe "
...
String LibraryBuilder::FixIdentifier(StringParam ident, TokenCheck::Flags flags, char invalidCharacter)
{
  ...
  {
    Plasma::Rune first = identifier.Front();
    Plasma::Rune last = identifier.Back();
    ...
    if (readFirstLetter == false)
    {
      ...
      if (CharacterUtilities::IsAlpha(r))
      {
        ...
        if (isUpper && expectLowerIdentifier)
        {
          ...
          builder.Append(Plasma::UTF8::ToLower(r.mValue));
        ...
        else if (isLower && expectUpperIdentifier)
        {
          ...
          {
            builder.Append(Plasma::UTF8::ToUpper(r.mValue));
            readFirstLetter = true;
          }
        }
        else
...
void LibraryBuilder::ComputeDelegateAndFunctionSizesOnce()
{
  ...
  while (delegateTypes.Empty() == false)
  {
    ...
    // Set the return stack offset, which is always plasma
    ...
    // (void has a size of plasma, so this always works!)
...
void Library::GenerateDefinitionStubCode()
{
  ...
  LightningForEach (BoundType* type, this->BoundTypes.Values())
  {
