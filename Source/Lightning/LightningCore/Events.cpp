...
#if !defined(LightningUnsafeEvents)
#  define LightningUnsafeEvents false
#endif
...
LightningDefineType(EventHandler, builder, type)
{
  LightningFullBindDestructor(builder, type, EventHandler);
  LightningFullBindConstructor(builder, type, EventHandler, LightningNoNames);
  type->GetEventHandlerFunction = EventHandlerGetEventHandlerFunction;
}

LightningDefineType(EventsClass, builder, type)
{
  LightningFullBindMethod(builder, type, &EventsClass::Send, LightningNoOverload, "Send", "sender, eventName, event");
  LightningFullBindMethod(builder, type, &EventsClass::Connect, LightningNoOverload, "Connect", "sender, eventName, callback");
}

LightningDefineType(EventData, builder, type)
{
  LightningFullBindDestructor(builder, type, EventData);
  LightningFullBindConstructor(builder, type, EventData, LightningNoNames);
}
...
int EventDelegateList::Send(EventData* event)
{
  ...
  BoundType* sentEventType = LightningVirtualTypeId(event);
  ...
  for (size_t j = 0; j < totalOutgoing; ++j)
  {
    ...
    if (LightningUnsafeEvents || Type::BoundIsA(sentEventType, delegate->Type))
    {
...
{
  this->Type = LightningTypeId(EventData);
}
...
LightningEventDelegate::LightningEventDelegate(const Delegate& delegate, ExecutableState* state) :
...
int LightningEventDelegate::Invoke(EventData* event)
{
  ...
  // Otherwise, we successfully invoked a Lightning callback!
  return 1;
}

void* LightningEventDelegate::GetThisPointerOrUniqueId()
{
...
void EventsClass::Connect(const Handle& sender, StringParam eventName, const Delegate& callback)
{
  ...
  if (Type::IsSame(signature->Return, LightningTypeId(void)) == false)
  {
  ...
  if (Type::GenericIsA(parameters[0].ParameterType, LightningTypeId(EventData)) == false)
  {
  ...
  // Create a the event connection that will invoke the Lightning delegate
  LightningEventDelegate* eventDelegate = new LightningEventDelegate(callback, state);
