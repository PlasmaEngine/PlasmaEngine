...
#pragma once
#ifndef LIGHTNING_CONSOLE_HPP
#  define LIGHTNING_CONSOLE_HPP
...
LightningDeclareEvent(ConsoleWrite, ConsoleEvent);

// Sent when anyone attempts to read from the console
LightningDeclareEvent(ConsoleRead, ConsoleEvent);
} // namespace Events
...
class PlasmaShared ConsoleEvent : public EventData
{
public:
  LightningDeclareType(ConsoleEvent, TypeCopyMode::ReferenceType);
...
PlasmaShared void DefaultWriteText(ConsoleEvent* event);

// The default read text callback that reads text from stdin
PlasmaShared void DefaultReadText(ConsoleEvent* event);

// The type that we use to bind a console to the language
class PlasmaShared Console
{
public:
  LightningDeclareType(Console, TypeCopyMode::ReferenceType);

  // Write to the console (not bound to Lightning)
