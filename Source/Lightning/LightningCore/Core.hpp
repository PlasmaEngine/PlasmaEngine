...
#pragma once
#ifndef LIGHTNING_CORE_HPP
#  define LIGHTNING_CORE_HPP
...
// The base type for all enumerations within Lightning
class PlasmaShared Enum
{
public:
  LightningDeclareType(Enum, TypeCopyMode::ValueType);
  Integer Value;
};

// The core of Lightning Contains all types that the compiler uses internally
// The core also acts as a global database of all immutable and sharable types
class PlasmaShared Core : public StaticLibrary
{
public:
  // Friends
...
private:
  ...
  // Not copyable
  LightningNoCopy(Core);
};

// A single entry in a stack trace
class PlasmaShared StackEntry
{
public:
  // Constructor
  StackEntry();
...
// Contains all the locations that we're at in an execution of Lightning
class PlasmaShared StackTrace
{
public:
  ...
  // We have to use a special allocator here that supports being memset to plasma
  // (because exceptions are just memset to 0, no constructor called)
  Array<StackEntry, Plasma::MemsetPlasmaDefaultAllocator> Stack;
};

// The base class of any exception that gets thrown
class PlasmaShared Exception
{
public:
  LightningDeclareType(Exception, TypeCopyMode::ReferenceType);
...
class PlasmaShared PropertyDelegateTemplate
{
public:
...
PlasmaShared String BooleanToString(Boolean value);
PlasmaShared String Boolean2ToString(Boolean2Param value);
PlasmaShared String Boolean3ToString(Boolean3Param value);
PlasmaShared String Boolean4ToString(Boolean4Param value);
PlasmaShared String ByteToString(Byte value);
PlasmaShared String IntegerToString(Integer value);
PlasmaShared String Integer2ToString(Integer2Param value);
PlasmaShared String Integer3ToString(Integer3Param value);
PlasmaShared String Integer4ToString(Integer4Param value);
PlasmaShared String RealToString(Real value);
PlasmaShared String Real2ToString(Real2Param value);
PlasmaShared String Real3ToString(Real3Param value);
PlasmaShared String Real4ToString(Real4Param value);
PlasmaShared String QuaternionToString(QuaternionParam value);
PlasmaShared String DoubleIntegerToString(DoubleInteger value);
PlasmaShared String DoubleRealToString(DoubleReal value);
} // namespace Lightning
