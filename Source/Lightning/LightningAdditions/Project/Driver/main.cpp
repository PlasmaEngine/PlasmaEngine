...
#define LightningPrintAndFlush(...) printf(__VA_ARGS__); fflush(stdout)
#define LightningPauseInDebugger() Plasma::Os::DebugBreak()
...
{
  LightningPrintAndFlush("#BEGIN: %s\n", testAndFunctionName.c_str());
  
  // Find the scripting function by name
  BoundType* type = state->Dependencies.FindType(className);
  Function* function = type->FindFunction(testAndFunctionName, Array<Type*>(), LightningTypeId(ReturnType), FindMemberOptions::Static);
  ...
  if (report.HasThrownExceptions())
  {
    ...
    LightningPrintAndFlush("#FAILED:\n");
    LightningPrintAndFlush("  Lightning: Exception was thrown\n");
    LightningPrintAndFlush("    C++: '%s'\n", cppVal.c_str());
    LightningPauseInDebugger();
  }
  else
  {
    // Call the scripting function and get it's return value
    ...
    {
      LightningPrintAndFlush("#SUCCESS\n");
    }
    else
    {
      ...
      LightningPrintAndFlush("#FAILED:\n");
      LightningPrintAndFlush("  Lightning: '%s'\n", scriptVal.c_str());
      LightningPrintAndFlush("    C++: '%s'\n", cppVal.c_str());
      LightningPauseInDebugger();
    }
  }

  LightningPrintAndFlush("#END\n\n");
}
...
void LightningCallMe5Times(Call& call, ExceptionReport& report)
{
...
void RunSanityTests()
{
  // Create a scripting engine
  {
    ...
    // Callbacks
    {
      ...
      DelegateType* parameterType = builder.GetDelegateType(ParameterArray(), LightningTypeId(void));
      builder.AddBoundFunction(type, "CallMe5Times", LightningCallMe5Times, OneParameter(parameterType), LightningTypeId(void), FunctionOptions::Static);
...
void UnitTestErrorCallback(ErrorEvent* e)
{
  ...
  ActualErrorMessage = e->GetFormattedMessage(MessageFormat::Lightning);
}
...
void UnitTestErrorFile(Module& dependencies, ErrorCode::Enum error)
{
  ...
  else
  {
    ...
    if (status.Failed())
    {
      LightningPrintAndFlush("#FAILED: Unable to open %s\n", fileName.c_str());
      LightningPauseInDebugger();
    }
    else
    {
      ...
      {
        LightningPrintAndFlush("#SKIPPED\n");
      }
      else
      {
        ...
        if (lib != nullptr)
        {
          LightningPrintAndFlush("#FAILED: The test compiled when it should not have\n");
          LightningPauseInDebugger();
        }
        else
        {
          ...
          {
            LightningPrintAndFlush("#FAILED: Produced a different error then we were expecting\n");
            LightningPrintAndFlush("  Error: %s\n", ActualErrorMessage.c_str());
            LightningPauseInDebugger();
          }
        }
      }
...
void EvaluateDiffs(void (*diffFunction)(Array<Diff>& diffs), const char* name)
{
  ...
  bool success = true;
  LightningForEach(Diff& diff, diffs)
  {
  ...
  {
    LightningPrintAndFlush("#FAILED: One of the tests did not diff\n");
    LightningPauseInDebugger();
  }
...
void RunUnitTests()
{
  ...
  if(lib == nullptr)
  {
    LightningPrintAndFlush("#FAILED: Unit test files did not compile\n");
    LightningPauseInDebugger();
    return;
  }
