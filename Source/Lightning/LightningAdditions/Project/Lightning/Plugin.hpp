...
namespace Lightning
{
  ...
  // The signature of the CreateLightningPlugin function that we look for in the shared library
  ...
  public:
    ...
    // All plugins loaded should have the '.lightningPlugin' extension (a shared object or dynamic linked library)
    // It may fail to load if the path specified isn't accessible, isn't a valid shared library,
    // or doesn't export the CreateLightningPlugin function
  ...
  protected:
    ...
    // This will only run upon a full compilation of Lightning (not when AutoComplete or Definition info is queried)
  ...
  // conveniently make calls to types bound to Lightning (or even Lightning scripts themselves)
  ...
  #define LightningDefinePluginInterface(PluginClass)                                                   \
    PlasmaExportC long GetLightningPluginVersion() { return 0; }                                          \
    PlasmaExportC Lightning::Plugin* CreateLightningPlugin() { return new PluginClass(); }
  ...
  #define LightningDeclareStaticLibraryAndPlugin(LibraryName, PluginName, ...)                          \
    ...
    class PluginName : public Lightning::Plugin                                                            \
    {                                                                                               \
    public:                                                                                         \
      ~PluginName() override;                                                                       \
      void PreBuild(Lightning::BuildEvent* event) override;                                                \
      Lightning::LibraryRef GetLibrary() override;                                                         \
  ...
  #define LightningDefineStaticLibraryAndPlugin(LibraryName, PluginName, ...)                           \
    ...
    void PluginName::PreBuild(Lightning::BuildEvent* event)                                                \
    {                                                                                               \
      __VA_ARGS__;                                                                                  \
      if (LibraryName::Instance == nullptr)                                                         \
        LibraryName::InitializeInstance();                                                          \
      Lightning::LibraryRef library = LibraryName::GetInstance().GetLibrary();                             \
      library->UserData = this->UserData;                                                           \
    }                                                                                               \
    Lightning::LibraryRef PluginName::GetLibrary()                                                         \
    {                                                                                               \
