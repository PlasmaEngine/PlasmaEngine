...
namespace Lightning
{
  ...
  template <typename T>
  PlasmaForceInline T& GetField(PerFrameData* stackFrame, PerFrameData* reportFrame, OperandIndex handleOperand, size_t memberOperand)
  {
  ...
  //***************************************************************************
  PlasmaForceInline void CopyHandlerEx(PerFrameData* ourFrame, PerFrameData* topFrame, const byte*& sourceOut, byte*& destinationOut, const CopyOpcode& op)
  {
  ...
  PlasmaForceInline void CopyHandler
  (
  ...
  #define LightningVirtualInstruction(Name) void VirtualMachine::Instruction##Name (ExecutableState* state, Call& call, ExceptionReport& report, size_t& programCounter, PerFrameData* ourFrame, const Opcode& opcode)
  ...
  // Copy
  #define LightningCopyCases(WithType)                                                                                                                \
    LightningCaseSimpleCopy(WithType)
  ...
  #define LightningNumericCases(WithType, ComparisonType)                                                                                             \
    LightningCopyCases(WithType)                                                                                                                      \
    LightningEqualityCases(WithType, ComparisonType)                                                                                                  \
    ...
    LightningCaseBinaryRValue(WithType, WithType, Divide,             if (GenericIsZero(right))                                                       \
                                                                  {                                                                               \
                                                                    state->ThrowException(report, "Attempted to divide by zero");                 \
                                                                    longjmp(ourFrame->ExceptionJump, ExceptionJumpResult);                        \
                                                                  }                                                                               \
                                                                  output = left / right);                                                         \
    LightningCaseBinaryRValue(WithType, WithType, Modulo,             if (GenericIsZero(right))                                                       \
                                                                  {                                                                               \
    ...
    LightningCaseBinaryLValue(WithType,           AssignmentDivide,   if (GenericIsZero(right))                                                       \
                                                                  {                                                                               \
                                                                    state->ThrowException(report, "Attempted to divide by zero");                 \
                                                                    longjmp(ourFrame->ExceptionJump, ExceptionJumpResult);                        \
                                                                  }                                                                               \
                                                                  output /= right);                                                               \
    LightningCaseBinaryLValue(WithType,           AssignmentModulo,   if (GenericIsZero(right))                                                       \
                                                                  {                                                                               \
                                                                    state->ThrowException(report, "Attempted to modulo by zero");                 \
                                                                    longjmp(ourFrame->ExceptionJump, ExceptionJumpResult);                        \
                                                                  }                                                                               \
  ...
  #define LightningScalarCases(WithType)                                                                                                              \
    LightningNumericCases(WithType, Boolean)                                                                                                          \
    LightningComparisonCases(WithType, Boolean)
  ...
  #define LightningVectorCases(VectorType, ScalarType, ComparisonType)                                                                                \
    LightningNumericCases(VectorType, Boolean)                                                                                                        \
    LightningComparisonCases(VectorType, ComparisonType)                                                                                              \
    LightningCaseBinaryRValue2(VectorType, ScalarType, VectorType,  ScalarMultiply,           output = left * right);                                 \
    LightningCaseBinaryRValue2(VectorType, ScalarType, VectorType,  ScalarDivide,             if (GenericIsZero(right))                               \
                                                                                          {                                                       \
                                                                                          ...
                                                                                          output = left / right);                                 \
    LightningCaseBinaryRValue2(VectorType, ScalarType, VectorType,  ScalarModulo,             if (GenericIsZero(right))                               \
                                                                                          {                                                       \
                                                                                            state->ThrowException(report,                         \
    ...
    LightningCaseBinaryLValue2(VectorType, ScalarType,              AssignmentScalarDivide,   if (GenericIsZero(right))                               \
                                                                                          {                                                       \
                                                                                            state->ThrowException(report,                         \
                                                                                              "Attempted to divide by zero");                     \
                                                                                            longjmp(ourFrame->ExceptionJump, ExceptionJumpResult);\
                                                                                          ...
                                                                                          output /= right);                                       \
    LightningCaseBinaryLValue2(VectorType, ScalarType,              AssignmentScalarModulo,   if (GenericIsZero(right))                               \
                                                                                          {                                                       \
                                                                                            state->ThrowException(report,                         \
                                                                                              "Attempted to modulo by zero");                     \
                                                                                            longjmp(ourFrame->ExceptionJump, ExceptionJumpResult);\
  ...
  LightningVirtualInstruction(PropertyDelegate)
  {
    ...
    propertyDelegate->ReferencedProperty = Handle((const byte*)op.ReferencedProperty, LightningTypeId(Property), nullptr, state);
  ...
  LightningIntegralCases(Byte)
  LightningScalarCases(Byte)
  LightningIntegralCases(Integer)
  LightningScalarCases(Integer)
  LightningVectorCases(Integer2, Integer, Boolean2)
  LightningVectorCases(Integer3, Integer, Boolean3)
  LightningVectorCases(Integer4, Integer, Boolean4)
  LightningIntegralCases(Integer2)
  LightningIntegralCases(Integer3)
  LightningIntegralCases(Integer4)
  LightningScalarCases(Real)
  LightningVectorCases(Real2, Real, Boolean2)
  LightningVectorCases(Real3, Real, Boolean3)
  LightningVectorCases(Real4, Real, Boolean4)
  LightningScalarCases(DoubleReal)
  LightningIntegralCases(DoubleInteger)
  LightningScalarCases(DoubleInteger)

  LightningEqualityCases(Boolean, Boolean)
  LightningEqualityCases(Handle, Boolean)
  LightningEqualityCases(Delegate, Boolean)
  LightningEqualityCases(Any, Boolean)
  ...
  LightningCaseConversion(Real4,    Integer4,  output = Integer4((Integer)value.x, (Integer)value.y, (Integer)value.z, (Integer)value.w));
  LightningCaseConversion(Real4,    Boolean4,  output = Boolean4(value.x != 0.0f, value.y != 0.0f, value.z != 0.0f, value.w != 0.0f));
  LightningCaseConversion(Boolean4, Integer4,  output = Integer4((Integer)value.x, (Integer)value.y, (Integer)value.z, (Integer)value.w));
