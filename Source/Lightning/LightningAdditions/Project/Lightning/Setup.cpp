...
{
  //***************************************************************************
  LightningSetup* LightningSetup::Instance = nullptr;

  //***************************************************************************
  LightningSetup::LightningSetup(SetupFlags::Type flags)
  {
    Instance = this;
    ...
    // The user can disable runtime documentation processing by passing in a flag to LightningStartup
    // However, if the user defines 'LightningDisableDocumentation', this will completely disable
    ...
    bool useLightningErrorHandler = !(flags & SetupFlags::CustomAssertHandlerOrNoAsserts);
    if (useLightningErrorHandler)
    {
      ErrorSignaler::SetErrorHandler(Plasma::Os::ErrorProcessHandler);
    }

    // Register the command handle managers we use
    LightningRegisterUniqueHandleManager(HeapManager);
    LightningRegisterUniqueHandleManager(StackManager);
    LightningRegisterSharedHandleManager(PointerManager);
    LightningRegisterSharedHandleManager(StringManager);
  ...
  void ReconstructSingleton()
  {
    ...
    PlasmaTodo("Make singletons into pointers so we don't have to do this silly stuff");
  ...
  //***************************************************************************
  void LightningParseMainArguments(int argc, char* argv[], MainArguments& argumentsOut)
  {
  ...
  //***************************************************************************
  int LightningMain(int argc, char* argv[])
  {
    int result = 0;
    ...
    LightningParseMainArguments(argc, argv, arguments);

    // For our own internal use, we may want to attach a debugger to Lightning
    if (arguments.HasCommand("-WaitForDebugger"))
      LightningWaitForDebugger(true);
    ...
    Module module;
    {
      String pluginDirectory = Plasma::GetApplicationDirectory();
      Status status;
      Plugin::LoadFromDirectory(status, module, module, Plasma::GetApplicationDirectory());
      if (status.Failed())
        printf("* Unable to load plugin from directory '%s': '%s'\n", pluginDirectory.c_str(), status.Message.c_str());
    }
    LightningForEach(String& pluginDirectory, arguments.GetCommandValues("-PluginDirectory"))
    {
      Status status;
    ...
    LightningForEach(String& pluginFile, arguments.GetCommandValues("-Plugin"))
    {
      Status status;
    ...
    LightningForEach(String& fileName, arguments.InputValues)
    {
    ...
    if (compileAndReport || compileOnly || run)
    {
      ...
      if (compileAndReport)
      {
        ...
        builder.Begin(JsonType::Object);
        {
          ...
          builder.Value(errorEvent.GetFormattedMessage(MessageFormat::Lightning));
        }
        builder.End();
      ...
      else
      {
        if (library == nullptr)
        {
          ...
          String errorMessage = errorEvent.GetFormattedMessage(MessageFormat::Lightning);
          printf("* %s\n", errorMessage.c_str());
          result = -1;
        }
        else if (run)
        {
          LightningForEach(LibraryRef library, module)
          {
            if (library->Plugin)
          ...
          {
            Function* mainFunction = programType->FindFunction("Main", Array<Type*>(), LightningTypeId(int), FindMemberOptions::None);
    ...
    else
    {
      ...
      if (autoCompleteCursor != nullptr && autoCompleteOrigin != nullptr)
      {
        ...
        Plasma::ToValue(*autoCompleteCursor, cursorPosition);
