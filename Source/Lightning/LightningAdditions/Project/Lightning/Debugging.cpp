...
namespace Lightning
{
  ...
  namespace Events
  {
    LightningDefineEvent(DebuggerPauseUpdate);
    LightningDefineEvent(DebuggerPause);
    LightningDefineEvent(DebuggerResume);
  }

  //***************************************************************************
  LightningDefineType(DebuggerEvent, builder, type)
  {
  }
  ...
  void Debugger::OnQueryExpression(const DebuggerMessage& message, void* userData)
  {
    ...
    builder.Begin(JsonType::Object);
    {
      ...
      {
        Plasma::StringTokenRange splitter(expression, '.');
...
END:
  ...
  void Debugger::OnConsoleWrite(ConsoleEvent* event)
  {
    ...
    // Moreover, if the call was made from C++ which was being called from Lightning, we'll only show the Lightning location
    if (event->State != nullptr)
    {
      ...
      StackEntry* lightningStackEntry = trace.GetMostRecentNonNativeStackEntry();

      // If there was indeed a some Lightning function calling this (or called C++ that called this)
      if (lightningStackEntry != nullptr)
        location = lightningStackEntry->Location;
    }
