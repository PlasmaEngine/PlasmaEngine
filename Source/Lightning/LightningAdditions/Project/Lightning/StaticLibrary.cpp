...
namespace Lightning
{
  ...
  {
    LightningErrorIfNotStarted(StaticLibrary);
  ...
  void StaticLibrary::BuildLibrary()
  {
    ...
    // This is generally used to provide clear error messages in LightningTypeId
    this->Build = BuildState::Building;

    // Build all dependent libraries
    LightningForEach(StaticLibrary* dependency, this->Dependencies)
    {
    ...
    // Make sure the user properly initialized all types that we did LightningTypeId for
