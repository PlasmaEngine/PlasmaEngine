#pragma once
#ifndef LIGHTNING_EVENTS_HPP
#define LIGHTNING_EVENTS_HPP

namespace Lightning
{
  // Declares an event to be sent by an EventHandler
  // The typical pattern in C++ is to declare these within the Events namespace
  #define LightningDeclareEvent(EventName, EventType) PlasmaShared extern const String EventName;

  // Defines the event so only cpp uint allocates the string 
  #define LightningDefineEvent(EventName) PlasmaShared const String EventName = #EventName;

  // All events that are sent must be derived from this type
  class PlasmaShared EventData : public ILightningObject
  {
  public:
    LightningDeclareType(EventData, TypeCopyMode::ReferenceType);
  ...
  // This class is specialized for C++ static and member and Lightning delegates (can be made for other languages too)
  class PlasmaShared EventDelegate
  {
  public:
  ...
  class PlasmaShared EventDelegateList
  {
  public:
  ...
  class PlasmaShared EventHandler : public ILightningObject
  {
  public:
    LightningDeclareType(EventHandler, TypeCopyMode::ReferenceType);
  ...
  PlasmaShared int EventDisconnect(EventHandler* sender, EventHandler* receiver, StringParam eventName, void* thisPointerOrUniqueId);
  ...
  PlasmaShared int EventSend(EventHandler* sender, StringParam eventName, EventData* event);

  // When we want to connect up member functions, we use this template
  template <typename ClassType, typename EventType>
  class PlasmaSharedTemplate MemberFunctionEventDelegate : public EventDelegate
  {
  public:
    LightningDefineEventDelegateHelpers(MemberFunctionEventDelegate);
    ...
    {
      this->Type = LightningTypeId(EventType);
    }
  ...
  template <typename ClassType, typename EventType>
  PlasmaSharedTemplate void EventConnect(EventHandler* sender, StringParam eventName, void (ClassType::*function)(EventType*), ClassType* receiver)
  {
  ...
  template <typename ClassType, typename EventType>
  PlasmaSharedTemplate void EventConnect(EventHandler* sender, StringParam eventName, void (ClassType::*function)(EventType*), ClassType* selfPointer, EventHandler* receiver)
  {
    // Create a member function delegate
    typedef MemberFunctionEventDelegate<ClassType, EventType> EventDelegateType;
  ...
  class PlasmaSharedTemplate StaticFunctionEventDelegate : public EventDelegate
  {
  public:
    LightningDefineEventDelegateHelpers(StaticFunctionEventDelegate);
  ...
  template <typename EventType>
  PlasmaSharedTemplate void EventConnect(EventHandler* sender, StringParam eventName, void (*function)(EventType*), EventHandler* receiver = nullptr)
  {
  ...
  template <typename EventType>
  class PlasmaSharedTemplate StaticFunctionUserDataEventDelegate : public EventDelegate
  {
  public:
    LightningDefineEventDelegateHelpers(StaticFunctionUserDataEventDelegate);
  ...
  template <typename EventType>
  PlasmaSharedTemplate void EventConnect(EventHandler* sender, StringParam eventName, void (*function)(EventType*, void*), void* userData = nullptr, EventHandler* receiver = nullptr)
  {
    // We use the global event handler here (if there is no receiver...)
    if (receiver == nullptr)
      receiver = &EventHandler::Global;
  ...
  class PlasmaShared ForwardingEventDelegate : public EventDelegate
  {
  public:
    LightningDefineEventDelegateHelpers(ForwardingEventDelegate);
  ...
  // An event delegate that can call Lightning code
  class PlasmaShared LightningEventDelegate : public EventDelegate
  {
  public:
    LightningDefineEventDelegateHelpers(LightningEventDelegate);

    // Constructs the event from the Lightning delegate
    // NOTE: Must be validated by 'ValidateEventConnection prior to constructing!
    LightningEventDelegate(const Delegate& delegate, ExecutableState* state);
  ...
  PlasmaShared void EventForward(EventHandler* sender, StringParam eventName, EventHandler* receiver);

  // A class that represents the 'Events' class within Lightning code (for binding purposes)
  class PlasmaShared EventsClass
  {
  public:
    LightningDeclareType(EventsClass, TypeCopyMode::ReferenceType);
