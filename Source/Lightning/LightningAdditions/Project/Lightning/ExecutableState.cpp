...
namespace Lightning
{
  ...
  LightningDefineType(ExecutableState, builder, type)
  {
    ...
    LightningFullBindMethod(builder, type, &ExecutableState::ExecuteStatement, LightningNoOverload, "ExecuteStatement", LightningNoNames);
  }
  ...
  //***************************************************************************
  PlasmaForceInline PerFrameData* ExecutableState::PopFrame()
  {
  ...
  //***************************************************************************
  PlasmaForceInline void ExecutableState::SendOpcodeEvent(StringParam eventId, PerFrameData* frame)
  {
  ...
  Handle ExecutableState::AllocateCopyConstructedHeapObject(BoundType* type, ExceptionReport& report, HeapFlags::Enum flags, const Handle& fromObject)
  {
    ...
    (
      ...
      "If you are using automatic binding call LightningBindConstructor(const %s&);",
      type->ToString().c_str(),
      type->ToString().c_str()
    );
    return Handle();
  ...
  Handle ExecutableState::AllocateHeapObject(BoundType* type, ExceptionReport& report, HeapFlags::Enum flags)
  {
    ...
    if (this->DoNotAllowAllocation != 0)
    {
      ...
      PlasmaTodo("We should make this throw an exception, but then it cannot be allocated currently... InternalException?");
      return Handle();
    }
  ...
  bool ExecutableState::ThrowExceptionOnTimeout(ExceptionReport& report)
  {
    ...
    // This MUST be called before the early out so that we don't accumulate up time not spent in Lightning
  ...
  {
    PlasmaTodo("We need to verify that this handle is indeed a handle to an Exception type");
  ...
  Call::~Call()
  {
    ...
    PlasmaTodo("Make sure we handle exceptions here (could have thrown before returning)");
