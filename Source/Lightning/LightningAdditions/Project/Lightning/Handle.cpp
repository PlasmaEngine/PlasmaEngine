...
namespace Lightning
{
  ...
  Handle::Handle()
  {
    LightningErrorIfNotStarted(Handle);
  ...
  bool Handle::operator==(const Handle& rhs) const
  {
    ...
    PlasmaTodo("There are two dereferences for comparing user handles, we should refactor this code path to make it one");
  ...
  //***************************************************************************
  bool Handle::operator==(Plasma::NullPointerType) const
  {
    return this->IsNull();
  }
  ...
  ::byte* Handle::Dereference() const
  {
    ...
    PlasmaTodo("If a handle returns null but is not the 'null handle manager' then it should be an optimization to Clear it");
  ...
  void Handle::ReleaseReference()
  {
    ...
    if (this->Manager && this->IsReferenceCounted())
    {
      ...
      // For example, new an object in Lightning, then create a temporary handle, and delete the original
