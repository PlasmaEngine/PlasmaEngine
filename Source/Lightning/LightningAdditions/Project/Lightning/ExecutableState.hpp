#pragma once
#ifndef LIGHTNING_EXECUTABLE_STATE_HPP
#define LIGHTNING_EXECUTABLE_STATE_HPP
...
namespace Lightning
{
  namespace Events
  {
    ...
    LightningDeclareEvent(FatalError, FatalErrorEvent);

    // Sent on ExecutableState when an exception occurs and isn't handled (can be user thrown, etc)
    LightningDeclareEvent(UnhandledException, ExceptionEvent);

    // Sent on ExecutableState when an exception occurs and is caught by the user
    LightningDeclareEvent(HandledException, ExceptionEvent);

    // Debugger and profiler events
    LightningDeclareEvent(OpcodePreStep, OpcodeEvent);
    LightningDeclareEvent(OpcodePostStep, OpcodeEvent);
    LightningDeclareEvent(EnterFunction, OpcodeEvent);
    LightningDeclareEvent(ExitFunction, OpcodeEvent);

    // Whenever an scripted object is leaked, it is reported when the executable state is torn down
    LightningDeclareEvent(MemoryLeak, MemoryLeakEvent);
  }
  ...
  class PlasmaShared MemoryLeakEvent : public EventData
  {
  public:
    LightningDeclareType(MemoryLeakEvent, TypeCopyMode::ReferenceType);
  ...
  class PlasmaShared OpcodeEvent : public EventData
  {
  public:
    LightningDeclareType(OpcodeEvent, TypeCopyMode::ReferenceType);
  ...
  class PlasmaShared ExceptionEvent : public EventData
  {
  public:
    LightningDeclareType(ExceptionEvent, TypeCopyMode::ReferenceType);
    ExceptionEvent();
    ExecutableState* State;
    Exception* ThrownException;
  };
  
  // The types of fatal errors that can occur inside of Lightning (non recoverable errors)
  ...
  class PlasmaShared FatalErrorEvent : public EventData
  {
  public:
    LightningDeclareType(FatalErrorEvent, TypeCopyMode::ReferenceType);
  ...
  class PlasmaShared PerScopeData
  {
  public:
  ...
  class PlasmaShared PerFrameData
  {
  public:
    // Constructor
  ...
  class PlasmaShared OperandLocation
  {
  public:
    // Constructor
    OperandLocation();
  ...
  class PlasmaShared ExceptionReport
  {
  public:
    // Friends
  ...
  PlasmaShared void DefaultExceptionCallback(ExceptionEvent* e);

  // Stores the generated functions, and anything else needed for a VM to execute
  class PlasmaShared ExecutableState : public EventHandler
  {
  public:
    LightningDeclareType(ExecutableState, TypeCopyMode::ReferenceType);
    ...
    // that is the last running state (set before each call to Lightning, and reset to the previous after the call)
    ...
    HandleOf<T> AllocateDefaultConstructed(BoundType* typeToAllocate = nullptr, HeapFlags::Enum flags = HeapFlags::ReferenceCounted)
    {
      ...
      if (typeToAllocate == nullptr)
        typeToAllocate = LightningTypeId(T);
    ...
    T* GetHandleManager()
    {
      return (T*)HandleManagers::GetInstance().GetManager(LightningManagerId(T), this);
    }
  ...
  private:
    ...
    PlasmaForceInline PerFrameData* PushFrame(Function* function);

    // A slightly more optimal version of pushing a stack frame (used internally in execution)
    PlasmaForceInline PerFrameData* PushFrame(byte* frame, Function* function);

    // Pops a stack frame and return a pointer to where the return value should be
    PlasmaForceInline PerFrameData* PopFrame();
    ...
    PlasmaForceInline void SendOpcodeEvent(StringParam eventId, PerFrameData* frame);
  ...
  public:
    ...
    // Not copyable
    LightningNoCopy(ExecutableState);
  };
  ...
  PlasmaSharedTemplate void InternalWriteValue(const T& value, byte* stackFrame)
  {
    ...
    LightningStaticType(T)::Write(TypeBinding::ReferenceCast<FromType, ToType>::Cast((FromType)value), stackFrame);
  }
  ...
  PlasmaSharedTemplate void InternalWriteRef(const T& value, byte* stackFrame, ExecutableState* state)
  {
    ...
    BoundType* type = LightningVirtualTypeId(pointerToValue);
    ...
    if (LightningStaticType(T)::DirectRead)
    {
    ...
    else
    {
      ...
      typedef typename LightningStaticType(T)::RepresentedType RepresentedType;
      ((RepresentedType*)data)->~RepresentedType();
    }
  }

  template <typename T>
  PlasmaSharedTemplate class CallHelper
  {
  public:
  ...
  #define LightningCallHelperSpecialization(T, SetT)                              \
    class PlasmaShared CallHelper<T>                                            \
    {                                                                         \
    public:                                                                   \
      static T Get(Call& call, size_t index);                                 \
  ...
  #define LightningCallHelperTemplateSpecialization(T, SetT)                      \
    PlasmaSharedTemplate class CallHelper<T>                                    \
    {                                                                         \
    public:                                                                   \
      static T Get(Call& call, size_t index);                                 \
      static void Set(Call& call, size_t index, SetT value);                  \
  ...
  // Facilitates invoking Lightning functions including parameter passing and grabbing return values
  // Also is passed into each call when implementing a custom function that is bound to Lightning
  class PlasmaShared Call
  {
  public:
    friend class ExecutableState;
    friend class VirtualMachine;
    ...
    void SetValue(size_t index, const T& value)
    {
      ...
      byte* stack = this->GetChecked(index, sizeof(typename LightningStaticType(T)::RepresentedType), valueType, CheckPrimitive::Value, Direction::Set);
    ...
    {
      // Get the stack location and perform checks
      byte* stack = this->GetChecked(index, sizeof(typename LightningStaticType(T)::RepresentedType), LightningTypeId(T), CheckPrimitive::Value, Direction::Get);
    ...
    {
      // Get the stack location and perform checks
      return this->GetChecked(index, sizeof(typename LightningStaticType(T)::RepresentedType), LightningTypeId(T), CheckPrimitive::Value, Direction::Get);
    }
  ...
  // A helper for allocating a type within Lightning using the current executable state
  #define LightningAllocate(T, ...) (PL::ExecutableState::GetCallingState()->AllocateDefaultConstructed<T>(__VA_ARGS__))
  #define LightningAllocateUntyped(...) (PL::ExecutableState::GetCallingState()->AllocateDefaultConstructedHeapObject(__VA_ARGS__))

  template <> LightningCallHelperSpecialization(      Any*,       Any* const&);
  template <> LightningCallHelperSpecialization(      Any , const Any&);
  template <> LightningCallHelperSpecialization(      Any&,       Any&);
  template <> LightningCallHelperSpecialization(const Any*, const Any* const&);
  template <> LightningCallHelperSpecialization(const Any&, const Any&);
  
  template <> LightningCallHelperSpecialization(      Handle*,       Handle* const&);
  template <> LightningCallHelperSpecialization(      Handle , const Handle&);
  template <> LightningCallHelperSpecialization(      Handle&,       Handle&);
  template <> LightningCallHelperSpecialization(const Handle*, const Handle* const&);
  template <> LightningCallHelperSpecialization(const Handle&, const Handle&);

  template <> LightningCallHelperSpecialization(      Delegate*,       Delegate* const&);
  template <> LightningCallHelperSpecialization(      Delegate , const Delegate&);
  template <> LightningCallHelperSpecialization(      Delegate&,       Delegate&);
  template <> LightningCallHelperSpecialization(const Delegate*, const Delegate* const&);
  template <> LightningCallHelperSpecialization(const Delegate&, const Delegate&);

  template <typename T> LightningCallHelperTemplateSpecialization(      HandleOf<T>*,       HandleOf<T>* const&);
  template <typename T> LightningCallHelperTemplateSpecialization(      HandleOf<T> , const HandleOf<T>&);
  template <typename T> LightningCallHelperTemplateSpecialization(      HandleOf<T>&,       HandleOf<T>&);
  template <typename T> LightningCallHelperTemplateSpecialization(const HandleOf<T>*, const HandleOf<T>* const&);
  template <typename T> LightningCallHelperTemplateSpecialization(const HandleOf<T>&, const HandleOf<T>&);
