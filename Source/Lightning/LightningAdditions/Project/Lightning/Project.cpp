...
namespace Lightning
{
  ...
  namespace Events
  {
    LightningDefineEvent(PreParser);
    LightningDefineEvent(TypeParsed);
    LightningDefineEvent(PostSyntaxer);
  }

  //***************************************************************************
  LightningDefineType(ParseEvent, builder, type)
  {
  }
  ...
  {
    LightningErrorIfNotStarted(Project);
  }
  ...
  String Project::ReadTextFile(Status& status, StringParam fileName)
  {
    ...
    File file;
    file.Open(fileName, Plasma::FileMode::Read, Plasma::FileAccessPattern::Random, Plasma::FileShare::Read, &status);
    ...
    for (size_t i = 0; i < comments.Size(); ++i)
    {
      ...
      for (size_t line = comment.Location.StartLine; line <= origin.MaxLine && line <= endLine; ++line)
      {
        ...
        if (node != nullptr && node->Location.PrimaryLine == line)
        {
          ...
          NodeChildren children;
          node->PopulateChildren(children);
          LightningForEach(SyntaxNode** childPtr, children)
          {
  ...
  CompletionOverload& Project::AddAutoCompleteOverload(AutoCompleteInfo& info, DelegateType* delegateType)
  {
    ...
    if (Lightning::Type::IsSame(LightningTypeId(void), delegateType->Return) == false)
    {
  ...
  void Project::GetDefinitionInfo(Module& dependencies, size_t cursorPosition, StringParam cursorOrigin, CodeDefinition& resultOut)
  {
    ...
    if (resultOut.ToolTip.Empty())
    {
      ...
      {
        String name = LightningVirtualTypeId(resultOut.DefinedObject)->ToString();
  ...
  void Project::GetDefinitionInfoInternal(Module& dependencies, size_t cursorPosition, StringParam cursorOrigin, CodeDefinition& resultOut)
  {
    ...
    Plasma::SetAndRecallOnDestruction<bool> changeTolerantMode(&this->TolerantMode, true);
    ...
    for (int i = (int)nodes.Size() - 1; i >= 0; --i)
    {
      ...
      if (node != nullptr)
      {
        ...
        BoundType* nodeType = LightningVirtualTypeId(node);
  ...
  void Project::GetAutoCompleteInfo(Module& dependencies, size_t cursorPosition, StringParam cursorOrigin, AutoCompleteInfo& resultOut)
  {
    ...
    Lightning::BoundType* boundType = Lightning::Type::DynamicCast<Lightning::BoundType*>(resultOut.NearestType);
    ...
    if (boundType == nullptr)
    {
      ...
      Lightning::IndirectionType* indirectType = Lightning::Type::DynamicCast<Lightning::IndirectionType*>(resultOut.NearestType);
  ...
  void Project::GetAutoCompleteInfoInternal(Module& dependencies, size_t cursorPosition, StringParam cursorOrigin, AutoCompleteInfo& resultOut)
  {
    ...
    for (int i = (int)nodes.Size() - 1; i >= 0; --i)
    {
      ...
      // (note that LocalVariableReferenceNode inherits from ValueNode and is NOT a literal, hence the LightningGetDerivedType == check).
      resultOut.IsLiteral = (expression->LightningGetDerivedType() == LightningTypeId(ValueNode));
