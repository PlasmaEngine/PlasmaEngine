#pragma once
#ifndef LIGHTNING_SHARED_HPP
#define LIGHTNING_SHARED_HPP
...
namespace Lightning
{
  ...
  class PlasmaShared BinaryOperator
  {
  public:
    // Constructor
    BinaryOperator();
  ...
  // This struct is given back to the user when asking for tokens
  class PlasmaShared UnaryOperator
  {
  public:
    // Constructor
    UnaryOperator();
  ...
  class PlasmaShared CastOperator
  {
  public:
    // Constructor
    CastOperator();
  ...
  class PlasmaShared UntypedOperator
  {
  public:
    // Constructor
    UntypedOperator();
  ...
  private:
    ...
    PlasmaTodo("This should actually use some kind of policy because we're not hashing types correctly (works because we only care about BoundType* right now, Real/Integer, etc )");
    HashMap<Type*, Array<CastOperator> > PrimitiveCastOperatorsFrom;

    // We use this as a key into a hash map
    class PlasmaShared OperatorWithArity
    {
    public:
