#pragma once
#ifndef LIGHTNING_PROJECT_HPP
#define LIGHTNING_PROJECT_HPP
...
namespace Lightning
{
  namespace Events
  {
    ...
    // This is an ideal place to add new types to this builder that our parsed Lightning code may depend upon
    LightningDeclareEvent(PreParser, ParseEvent);
    ...
    LightningDeclareEvent(TypeParsed, ParseEvent);

    // Sent when the engine finishes syntax checking on the abstract syntax tree (but before code generation)
    // Note: Any types added in this phase will NOT be able to be used by the Lightning scripts
    // Note: None of the code from the library should be executed at this time
    LightningDeclareEvent(PostSyntaxer, ParseEvent);
  }

  // An even that it sent out from parsing (such as when a type is parsed)
  class PlasmaShared ParseEvent : public EventData
  {
  public:
    LightningDeclareType(ParseEvent, TypeCopyMode::ReferenceType);
  ...
  class PlasmaShared CompletionEntry
  {
  public:
    CompletionEntry();
  ...
  class PlasmaShared CompletionParameter
  {
  public:
    // Constructor
    CompletionParameter();
  ...
  // Note that we don't sort overloads because their order is actually important to Lightning
  class PlasmaShared CompletionOverload
  {
  public:
  ...
  class PlasmaShared AutoCompleteInfo
  {
  public:
  ...
  class PlasmaShared CodeDefinition
  {
  public:
    CodeDefinition();
  ...
  class PlasmaShared Project : public CompilationErrors
  {
  public:
