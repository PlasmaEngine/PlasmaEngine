...
namespace Lightning
{
  ...
  String StubCode::Finalize()
  {
    ...
    LightningForEach(CodeLocation* location, this->NativeLocations)
  ...
  {
    LightningCodeBuilder& codeBuilder = this->Builder;
    CodeFormat& format = codeBuilder.Format;
    
    this->StartNativeLocation(type->Location);
    this->GenerateHeader(type);

    PlasmaTodo("We may want to handle attributes for GetEventHandlerFunction being non-null (may be an interface) / CreatableInScript / Native");
    ...
    if (type->BaseType != nullptr && !type->IsEnumOrFlags())
    {
      ...
      LightningForEach(BoundType* interfaceType, type->InterfaceTypes)
      {
    ...
    LightningForEach(SendsEvent* sendsEvent, sendsEventsSorted)
    {
    ...
    LightningForEach(Property* property, allPropertiesSorted)
    {
  ...
  void StubCode::Generate(FunctionArray& functions)
  {
    ...
    LightningForEach(Function* function, functionsSorted)
    {
  ...
  void StubCode::Generate(Function* function)
  {
    ...
    LightningForRange(DelegateParameter& parameter, parameterRange, function->FunctionType->Parameters)
    {
    ...
    if(function->FunctionType->Return != LightningTypeId(void))
    {
      codeBuilder.WriteKeywordOrSymbolSpaceStyle(Grammar::TypeSpecifier, format.SpaceStyleTypeColon, format.SpaceStyleGlobalDefaultColon);
  ...
  void StubCode::GenerateHeader(ReflectionObject* object, Array<Attribute>& attributes)
  {
    ...
    String wrappedDescription = Plasma::WordWrap(object->Description, format.CommentWordWrapLength);
    LightningForEach(StringRange line, wrappedDescription.Split("\n"))
    {
      codeBuilder.WriteSingleLineComment(line);
      codeBuilder.WriteLineIndented();
    }

    PlasmaTodo("We may want to handle the 'IsHidden' attribute specially here because it may not exist inside 'Attributes'");

    // Write out all attributes
    LightningForEach(Attribute& attribute, attributes)
    {
