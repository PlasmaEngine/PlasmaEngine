#pragma once
#ifndef LIGHTNING_LIBRARY_HPP
#define LIGHTNING_LIBRARY_HPP
...
namespace Lightning
{
  ...
  class PlasmaShared InstantiateTemplateDelegate
  {
  public:
    // Constructor
  ...
  class PlasmaShared TemplateParameter
  {
  public:
  ...
  class PlasmaShared InstantiateTemplateInfo
  {
  public:
  ...
  class PlasmaShared Library
  {
  public:
    LightningDeclareType(Library, TypeCopyMode::ReferenceType);
  ...
  class PlasmaShared DelegateTypePolicy
  {
  public:
    // Hashing operator
  ...
  class PlasmaShared InstantiatedTemplate
  {
  public:
    // Constructor
  ...
  #define LightningTypes(...) Array<Type*>(PlasmaInit, __VA_ARGS__)

  // Create an array of constants
  #define LightningConstants(...) Array<Constant>(PlasmaInit, __VA_ARGS__)

  // Create an array of libraries
  #define LightningLibraries(...) Array<LibraryRef>(PlasmaInit, __VA_ARGS__)

  // Helper functions for generating parameter arrays (used in library building)
  PlasmaShared ParameterArray OneParameter(Type* type);
  PlasmaShared ParameterArray OneParameter(Type* type, StringParam name);
  PlasmaShared ParameterArray TwoParameters(Type* type);
  PlasmaShared ParameterArray TwoParameters(Type* type1, Type* type2);
  PlasmaShared ParameterArray TwoParameters(Type* type, StringParam name1, StringParam name2);
  PlasmaShared ParameterArray TwoParameters(Type* type1, StringParam name1, Type* type2, StringParam name2);
  PlasmaShared ParameterArray ThreeParameters(Type* type);
  PlasmaShared ParameterArray ThreeParameters(Type* type1, Type* type2, Type* type3);
  PlasmaShared ParameterArray ThreeParameters(Type* type, StringParam name1, StringParam name2, StringParam name3);
  PlasmaShared ParameterArray ThreeParameters(Type* type1, StringParam name1, Type* type2, StringParam name2, Type* type3, StringParam name3);
  PlasmaShared ParameterArray FourParameters(Type* type);
  PlasmaShared ParameterArray FourParameters(Type* type, StringParam name1, StringParam name2, StringParam name3, StringParam name4);
  PlasmaShared ParameterArray FiveParameters(Type* type);
  PlasmaShared ParameterArray FiveParameters(Type* type, StringParam name1, StringParam name2, StringParam name3, StringParam name4, StringParam name5);

  class PlasmaShared LibraryBuilder
  {
  public:
    // Friends
    friend class Core;
  ...
  private:
    ...
    // Not copyable
    LightningNoCopy(LibraryBuilder);
  };

  // A module Contains many libraries
  class PlasmaShared Module : public LibraryArray
  {
  public:
  ...
  PlasmaSharedTemplate void ForEachExtensionGetterSetter(bool isStatic, const LibraryArray& libraries, BoundType* type, Functor functor)
  {
    ...
    for (size_t i = 0; i < libraries.Size(); ++i)
    {
      ...
      while (baseIterator != nullptr)
      {
        ...
        {
          LightningForEach(GetterSetter* property, properties->Values())
          {
  ...
  template <typename Functor>
  PlasmaSharedTemplate void ForEachExtensionFunction(bool isStatic, const LibraryArray& libraries, BoundType* type, Functor functor)
  {
    // Loop through all the libraries
    for (size_t i = 0; i < libraries.Size(); ++i)
    {
      ...
      while (baseIterator != nullptr)
      {
        ...
        if (functionMultiMap != nullptr)
        {
          ...
          LightningForEach(FunctionArray& overloads, functionMultiMap->Values())
          {
            LightningForEach(Function* function, overloads.All())
            {
  ...
  template <typename Functor>
  PlasmaSharedTemplate void ForEachGetterSetter(bool isStatic, const LibraryArray& libraries, BoundType* type, Functor functor)
  {
  ...
  template <typename Functor>
  PlasmaSharedTemplate void ForEachFunction(bool isStatic, const LibraryArray& libraries, BoundType* type, Functor functor)
  {
    // Walk up the base class heirarchy
    BoundType* baseType = type;
    while (baseType != nullptr)
...
namespace Plasma
{
  template class Array<Lightning::LibraryRef>;
