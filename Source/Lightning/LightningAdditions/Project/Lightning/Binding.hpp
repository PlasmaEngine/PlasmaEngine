...
namespace Lightning
{
  ...
  public:
    ...
    // The macro LightningVirtualTypeId will create this template using the above SFINAE template (checking for LightningGetDerivedType)
    ...
    // If the type T has LightningGetDerivedType when invoking the macro LightningVirtualTypeId, this one will get chosen, and will actually invoke LightningGetDerivedType
    template <typename T>
    ...
    #define LightningVirtualTypeId(Pointer) \
      Lightning::TypeBinding::DiscoverDerivedType<LightningStrip(decltype(Pointer)), Lightning::TypeBinding::CanGetDerivedType<LightningStrip(decltype(Pointer))>::value>::Get(Pointer)
    ...
    #define LightningStrip(T) typename Lightning::TypeBinding::StripQualifiers<T>::Type
    ...
    // To just get the type as a pointer rather than the expression, use LightningStrip(T)*
    #define LightningToPointer(Expression) (Lightning::TypeBinding::InternalToPointer<LightningStrip(decltype(Expression))>(Expression))
    ...
    public:
      ...
      // This type gets shadowed by the other partial specializations (and will be the actual type when accessed from the LightningStaticType macro)
      ...
      static void AssertBadType(const char* prependedMessage)
      {
        Error
        (
          "%sA type '%s' was gotten via LightningTypeId but was never "
  ...
  // If we're in debug mode, add extra checks...
    ...
    #define LightningDeclareChecks(SelfType, BaseType)                                                                                                      \
      ...
      {                                                                                                                                                 \
        ...
        Lightning::CheckTypesAreRelated<SelfType, BaseType>::Test();                                                                                           \
      }                                                                                                                                                 \
  ...
  #define LightningDeclareCustomType(SelfType, CustomBoundType, Linkage)                                                                                  \
    ...
    public:                                                                                                                                           \
      ...
      static Lightning::BoundType* (GetType)()                                                                                    \
      {                                                                                                                                               \
  ...
  // Note that the Type may be null when allocated directly from C++  (not via Lightning API)
...
#define LightningDeclareInheritableType(SelfType, TypeCopyMode)                 \
  ...
  Lightning::AutoGrabAllocatingType LightningDerivedType;                              \
  Lightning::BoundType* LightningDerivedTypeOverride() const                           \
  {                                                                         \
  ...
  void SetupType(LibraryBuilder& builder, BoundType* type, SetupFunction setupType, P_ENABLE_IF(HasLightningSetupType<LightningSelf>::value))
  {
    builder.AddNativeBoundType(type, LightningTypeId(typename LightningSelf::LightningBase), LightningSelf::LightningCopyMode);
  ...
  template <typename LightningSelf, typename SetupFunction>
  void SetupType(LibraryBuilder& builder, BoundType* type, SetupFunction setupType, P_DISABLE_IF(HasLightningSetupType<LightningSelf>::value))
  {
    ErrorIf(setupType == nullptr,
            "No setup function provided for externally BoundType %s. "
            "Be sure you are calling the correct initialize function: LightningInitializeExternalType,"
  ...
  BoundType* InitializeType(const char* initializingTypeName, SetupFunction setupType = nullptr)
  {
    ...
    if (library.CanBuildTypes() == false)
    {
      ...
      (
        "LightningInitializeType can only be called on "
...
#define LightningInitializeTypeAs(Type, Name)                                                                                                              \
    Lightning::InitializeType<Type, LightningLibrary, void(*)(Type*, Lightning::LibraryBuilder&, Lightning::BoundType*)>(Name)
#define LightningInitializeType(Type)                                                                                                                     \
    LightningInitializeTypeAs(Type, #Type)

  // A helper for initializing types that belong to a library
#define LightningInitializeExternalTypeAs(Type, Name)                                                                                                     \
    Lightning::InitializeType<Type, LightningLibrary, void(*)(Type*, Lightning::LibraryBuilder&, Lightning::BoundType*)>(Name, LightningSetupType)
#define LightningInitializeExternalType(Type)                                                                                                             \
    LightningInitializeExternalTypeAs(Type, #Type)
...
#define LightningDeclareDerivedTypeExplicit(SelfType, BaseType, CopyMode)                                                                                 \
    ...
    /*virtual*/ Lightning::BoundType* LightningGetDerivedType() const                                                                                            \
    {                                                                                                                                                 \
      return Lightning::GetDerivedTypeOverride<LightningSelf>(this);                                                                                             \
    }                                                                                                                                                 \
    /* This lets binding know how the class should be treated when just referred to by its name */                                                    \
    static const Lightning::TypeCopyMode::Enum LightningCopyMode = CopyMode;                                                                                     \
    /* This function is to be implemented by the user so that */                                                                                      \
    /* they may choose what they want to bind to reflection */                                                                                        \
    static void LightningSetupType(Lightning::LibraryBuilder& builder, Lightning::BoundType* type)
...
#define LightningDeclareBaseTypeExplicit(SelfType, CopyMode)                                                                                              \
  LightningDeclareDerivedTypeExplicit(SelfType, Lightning::NoType, CopyMode)

  // Completely prevents binding from occurring with this type,
  // including calls like LightningTypeId() and any sort of bind macro
#define LightningDoNotAllowBinding() \
  static void LightningDoNotBind();
...
#define LightningDeclareType(ClassType, CopyMode)                                                                                                         \
    ...
    template <typename LightningT>                                                                                                                        \
    static Lightning::NoType SfinaeBase(...);                                                                                                                \
    typedef typename PL::remove_pointer<decltype(SfinaeBase<ClassType>(0))>::type LightningTempBase;                                                      \
    LightningDeclareDerivedTypeExplicit(ClassType, LightningTempBase, CopyMode)

#define LightningDefineType(SelfType, builder, type)                                                                                                      \
    void SelfType::LightningSetupType(Lightning::LibraryBuilder& builder, Lightning::BoundType* type)
...
#define LightningDeclareExternalType(SelfType)                                                                                                            \
    void LightningSetupType(SelfType*, Lightning::LibraryBuilder& builder, Lightning::BoundType* type);
...
#define LightningDefineExternalDerivedType(SelfType, SelfBaseType, TypeCopyMode, builder, type)                                                           \
    ...
    /* We want LightningSelf and LightningBase to act as if they are typedefs (because many binding macros depend on them) */                                 \
    template <typename LightningSelf, typename LightningBase>                                                                                                 \
    void LightningSetupExternalType(SelfType*, Lightning::LibraryBuilder& builder, Lightning::BoundType* type);                                                         \
    void LightningSetupType(SelfType*, Lightning::LibraryBuilder& builder, Lightning::BoundType* type)                                                                  \
    {                                                                                                                                                 \
...
#define LightningDefineExternalBaseType(SelfType, CopyMode, builder, type)                                                                              \
    LightningDefineExternalDerivedType(SelfType, Lightning::NoType, CopyMode, builder, type)
...
#define LightningDefineEnum(enumType)                                                         \
  LightningDefineExternalBaseType(enumType::Enum, Lightning::TypeCopyMode::ValueType, builder, type) \
  {                                                                                       \
    LightningFullBindEnum(builder, type, Lightning::SpecialType::Enumeration);                       \
