...
namespace Lightning
{
  ...
  BoundType*& TypeBinding::StaticTypeId<T>::GetType()
  {
    ...
    // This ensures that LightningTypeId will always result in a BoundType, even if it is not initialized
    // Note: This does however result BoundTypes being unintentionally created just
    // by using LightningTypeId on a type that is not bound. Usage of all types is scanned when the library is finalized
