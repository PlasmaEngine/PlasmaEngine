...
namespace Lightning
{
  ...
  public:
    ...
    // Note: No Lightning classes should be created before this occurs
    ...
    // Note: No Lightning classes should be created after this occurs
    ~LightningSetup();

    // The setup is a singleton, which means two may not exist at the same time
    // However, you can create LightningSetup and destroy it, then create another LightningSetup
  ...
  // A simple macro that we specle everywhere to ensure that the user initializes Lightning
  ...
  // Processes command line arguments for running Lightning standalone (invokes Startup/Shutdown)
