...
{
  //***************************************************************************
  LightningDefineType(ReflectionObject, builder, type)
  {
  ...
  //***************************************************************************
  LightningDefineType(Member, builder, type)
  {
    type->CopyMode = TypeCopyMode::ReferenceType;
    type->HandleManager = LightningManagerId(PointerManager);
    LightningFullBindField(builder, type, &Member::Name, "Name", PropertyBinding::Get);
    LightningFullBindField(builder, type, &Member::Owner, "Owner", PropertyBinding::Get);
    LightningFullBindField(builder, type, &Member::IsStatic, "IsStatic", PropertyBinding::Get);
    LightningFullBindGetterSetter(builder, type, &Member::GetOwningLibrary, LightningNoOverload, LightningNoSetter, LightningNoOverload, "Library");
    LightningFullBindGetterSetter(builder, type, &Member::GetTypeOrNull, LightningNoOverload, LightningNoSetter, LightningNoOverload, "Type");
  }

  //***************************************************************************
  LightningDefineType(Property, builder, type)
  {
    type->HandleManager = LightningManagerId(PointerManager);
    LightningFullBindField(builder, type, &Property::Get, "Getter", PropertyBinding::Get);
    LightningFullBindField(builder, type, &Property::Set, "Setter", PropertyBinding::Get);
    LightningFullBindMethod(builder, type, &Property::GetValue, LightningNoOverload, "GetValue", "instance");
    LightningFullBindMethod(builder, type, &Property::SetValue, LightningNoOverload, "SetValue", "instance, value");
  }

  //***************************************************************************
  LightningDefineType(GetterSetter, builder, type)
  {
    type->HandleManager = LightningManagerId(PointerManager);
  }

  //***************************************************************************
  LightningDefineType(Field, builder, type)
  {
    type->HandleManager = LightningManagerId(PointerManager);
    //LightningFullBindField(builder, type, &Field::Offset, "Offset", PropertyBinding::Get);
  }

  //***************************************************************************
  LightningDefineType(Variable, builder, type)
  {
    type->HandleManager = LightningManagerId(PointerManager);
  }
  ...
  //***************************************************************************
  Constant::Constant(Lightning::Type* value)
  {
    this->Clear();
  ...
  String Constant::ToString() const
  {
    ...
    Error("Invalid case detected in Lightning constant");
    return String();
  }
  ...
  //***************************************************************************
  AttributeParameter::AttributeParameter(StringParam name, Lightning::Type* value) :
    Constant(value),
    Name(name)
  {
  }
  ...
  bool Member::ValidateInstanceHandle(const Any& instance, Handle& thisHandle)
  {
    ...
    if (type != nullptr)
    {
      if (type->IsA(this->Owner))
      {
        ...
        {
          PlasmaTodo("This is unsafe, should this be allocated and boxed?");
          HandleManager* manager = HandleManagers::GetInstance().GetManager(LightningManagerId(PointerManager));
  ...
  void Property::SetValue(const Any& instance, const Any& value)
  {
    ...
    // Now call the function by copying all the arguments to the Lightning stack
