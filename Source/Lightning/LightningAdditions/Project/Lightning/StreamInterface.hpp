#pragma once
#ifndef LIGHTNING_STREAM_HPP
#define LIGHTNING_STREAM_HPP
...
namespace Lightning
{
  ...
  namespace StreamCapabilities
  {
    enum Enum
    {
      ...
      /// All enums bound to Lightning must be of Integer size, so force it on all platforms
      ForceIntegerSize = 0x7FFFFFFF
    };
  }

  /// When we seek in a stream, we can specify to seek from the start, end, etc
  // Note: This must match up to the Plasma::SeekOrigin
  ...
  class PlasmaShared IEncoding
  {
  public:
    LightningDeclareType(IEncoding, TypeCopyMode::ReferenceType);
    ...
    /// (the Lightning string is already in UTF8 form, so it would be written directly)
  ...
  /// Ascii encoding will strip wide characters and turn them into spaces
  /// All characters that fit within a single byte will be directly written to the stream
  class PlasmaShared AsciiEncoding : public IEncoding
  {
  public:
    LightningDeclareType(AsciiEncoding, TypeCopyMode::ReferenceType);
  ...
  /// Ascii encoding will strip wide characters and turn them into spaces
  /// All characters that fit within a single byte will be directly written to the stream
  class PlasmaShared Utf8Encoding : public IEncoding
  {
  public:
    LightningDeclareType(Utf8Encoding, TypeCopyMode::ReferenceType);
    Integer Write(Rune rune, IStreamClass& stream) override;
    Rune Read(IStreamClass& stream) override;
  };

  /// A generic interface for reading and writing data to a stream (file, network, etc)
  class PlasmaShared IStreamClass
  {
  public:
    LightningDeclareType(IStreamClass, TypeCopyMode::ReferenceType);
    ...
    // Check the stream capabilities for Read before calling this function
    // Note: The memory is allocated within Lightning's Heap Manager and must be freed after using ObjectToHandle
