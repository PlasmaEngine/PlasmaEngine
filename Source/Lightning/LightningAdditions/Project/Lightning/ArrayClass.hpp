...
namespace Lightning
{
  ...
  #define LightningDeclareDefineArray(ElementType, Linkage)           \
    LightningDeclareCustomType(ArrayClass<ElementType>, Lightning::Core::GetInstance().GetBuilder()->InstantiateTemplate("Array", LightningConstants(LightningTypeId(ElementType)), LibraryArray(PlasmaInit, Core::GetInstance().GetBuilder()->BuiltLibrary)).Type, Linkage);
