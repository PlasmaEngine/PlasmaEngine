...
namespace Lightning
{
  ...
  BoundType* Syntaxer::RetrieveBoundType(BoundSyntaxType* type, const CodeLocation& location, BoundType* classWeAreInsideOf)
  {
    ...
    // that the user was trying to implicitly access a member without 'this.', which is not legal in Lightning (but exists in C++)
  ...
  {
    PlasmaTodo("Finish up initializer lists (at least checking if they pass type validation)");
