...
void UniformBufferDescription::AddField(Lightning::BoundType* type, StringParam fieldName)
{
  ShaderIRFieldMeta* fieldMeta = new ShaderIRFieldMeta();
  fieldMeta->mLightningName = fieldName;
  fieldMeta->mLightningType = type;
  mFields.PushBack(fieldMeta);
}
...
void BuiltInBlockDescription::BuiltInFieldMeta::CopyFrom(const BuiltInFieldMeta& source)
{
  ...
  mMeta->mLightningName = source.mMeta->mLightningName;
  mMeta->mLightningType = source.mMeta->mLightningType;
  mMeta->mAttributes = source.mMeta->mAttributes;
  mId = source.mId;
}

void BuiltInBlockDescription::AddField(Lightning::BoundType* type,
                                       ...
                                       StringParam attribute)
{
  ...
  builtInMeta.mMeta->mLightningName = fieldName;
  builtInMeta.mMeta->mLightningType = type;
...
{
  // Override the lightning name of anything that matches the given id
  for (size_t i = 0; i < mFields.Size(); ++i)
  {
    ...
    fieldMeta.mMeta->mLightningName = name;
  }
}
...
void VertexDefinitionDescription::AddField(Lightning::BoundType* type, StringParam fieldName)
{
  ShaderIRFieldMeta* fieldMeta = new ShaderIRFieldMeta();
  fieldMeta->mLightningName = fieldName;
  fieldMeta->mLightningType = type;
...
LightningShaderErrorSettings::LightningShaderErrorSettings()
{
  mFrontEndErrorOnNoMainFunction = false;
}

String LightningShaderSpirVSettings::mLanguageSpecConstantName = "LanguageId";
String LightningShaderSpirVSettings::mLanguageVersionSpecConstantName = "LanguageVersion";

LightningShaderSpirVSettings::LightningShaderSpirVSettings()
{
  mFinalized = false;
...
bool LightningShaderSpirVSettings::IsValidUniform(FragmentType::Enum fragmentType,
...
void LightningShaderSpirVSettings::SetHardwareBuiltInName(spv::BuiltIn builtInId, StringParam name)
{
  ReturnIf(mFinalized, , "Cannot set built-in names once finalized");
...
bool LightningShaderSpirVSettings::IsValidHardwareBuiltIn(FragmentType::Enum fragmentType,
                                                      StringParam fieldType,
                                                      StringParam fieldName,
                                                      bool isInput)
{
...
void LightningShaderSpirVSettings::SetMaxSimultaneousRenderTargets(size_t maxNumber)
{
  ReturnIf(mFinalized, , "Cannot set max render targets once finalized");
  mRenderTargetNames.Resize(maxNumber);
}

void LightningShaderSpirVSettings::SetRenderTargetName(StringParam varName, size_t targetIndex)
{
...
void LightningShaderSpirVSettings::SetRenderTargetType(Lightning::BoundType* targetType)
{
  mRenderTargetType = targetType;
}

String LightningShaderSpirVSettings::GetLanguageSpecializationName()
{
  return mLanguageSpecConstantName;
}

String LightningShaderSpirVSettings::GetLanguageVersionSpecializationName()
{
  return mLanguageVersionSpecConstantName;
}

void* LightningShaderSpirVSettings::GetLanguageSpecializationKey()
{
  return &mLanguageSpecConstantName;
}

void* LightningShaderSpirVSettings::GetLanguageVersionSpecializationKey()
{
  return &mLanguageVersionSpecConstantName;
}

void LightningShaderSpirVSettings::Finalize()
{
  if (mFinalized)
  {
...
bool LightningShaderSpirVSettings::IsFinalized() const
{
  return mFinalized;
}

void LightningShaderSpirVSettings::Validate()
{
...
void LightningShaderSpirVSettings::ValidateUniformsDescriptions()
{
...
void LightningShaderSpirVSettings::ValidateBuiltInNames()
{
...
void LightningShaderSpirVSettings::ValidateBuiltInNames(BuiltInBlockDescription& blockDescription,
                                                    HashMap<String, spv::BuiltIn>& keyMappings)
{
...
void LightningShaderSpirVSettings::ValidateAppBuiltInsAgainstHardwareBuiltIns()
{
  for (size_t i = 0; i < mUniformBufferDescriptions.Size(); ++i)
  {
    UniformBufferDescription& description = mUniformBufferDescriptions[i];
    ...
    for (size_t j = 0; j < description.mFields.Size(); ++j)
    {
      ...
      ErrorIf(isHardwardBuiltIn,
              ...
              fieldMeta->mLightningName.c_str(),
              fieldMeta->mLightningType->ToString().c_str());
    }
  }
}

void LightningShaderSpirVSettings::InitializeBuiltIns()
{
  ...
  Lightning::BoundType* int3Type = LightningTypeId(Lightning::Integer3);
  Lightning::BoundType* boolType = LightningTypeId(Lightning::Boolean);
...
bool LightningShaderSpirVSettings::IsValidHardwareBuiltInAnyStage(ShaderFieldKey& fieldKey, bool isInput)
{
  for (size_t i = 0; i < FragmentType::Size; ++i)
  {
...
bool LightningShaderSpirVSettings::ValidateAgainstHardwareBuiltIns(ShaderFieldKey& fieldKey)
{
  for (size_t i = 0; i < FragmentType::Size; ++i)
  {
    BuiltInStageDescription& stageBuiltIns = mBuiltIns[i];
