...
namespace Plasma
{
	...
	LightningShaderGlslBackend::LightningShaderGlslBackend()
	{
		mTargetVersion = 150;
		mTargetGlslEs = false;
	}

	String LightningShaderGlslBackend::GetExtension()
	{
		return "glsl";
	}

	bool LightningShaderGlslBackend::RunTranslationPass(ShaderTranslationPassResult& inputData,
	...
	String LightningShaderGlslBackend::GetErrorLog()
	{
		return mErrorLog;
	}

	void LightningShaderGlslBackend::FixInterfaceBlockNames(GlslBackendInternalData& internalData)
	{
	...
	void LightningShaderGlslBackend::FixUniformBufferNameoverlaps(GlslBackendInternalData& internalData)
	{
		spirv_cross::CompilerGLSL* compiler = internalData.mCompiler;
		spirv_cross::ShaderResources* resources = internalData.mResources;
		...
		for (size_t i = 0; i < resources->uniform_buffers.size(); ++i)
		{
			...
			{
				String materialBufferName =
					BuildString(bufferName.c_str(), "_", FragmentType::Names[shaderStage]);
				compiler->set_name(uniformBuffer.id, materialBufferName.c_str());
			}
		}
	}

	void LightningShaderGlslBackend::FindUsedSampledImageBindings(GlslBackendInternalData& internalData)
	{
		spirv_cross::CompilerGLSL* compiler = internalData.mCompiler;
		spirv_cross::ShaderResources* resources = internalData.mResources;
	...
	void LightningShaderGlslBackend::HandleCompiledSampledImages(GlslBackendInternalData& internalData)
	{
		spirv_cross::CompilerGLSL* compiler = internalData.mCompiler;
		spirv_cross::ShaderResources* resources = internalData.mResources;
		...
		for (auto& remap : compiler->get_combined_image_samplers())
		{
			...
			String plasmaFullName = fullName.c_str();
			stageReflection.mImageRemappings[imageName.c_str()].mSampledImageRemappings.PushBack(plasmaFullName);
			stageReflection.mSamplerRemappings[samplerName.c_str()].mSampledImageRemappings.PushBack(plasmaFullName);

			// Mark this as a generated sampled image so we can make
			// sure to add this to the final reflection data.
			internalData.mGeneratedSampledImage.Insert(plasmaFullName);
	...
	// table to map this to plasma's names.
	void LightningShaderGlslBackend::PopulateTypeName(GlslBackendInternalData& internalData,
	                                                  ...
	                                                  ShaderResourceReflectionData& reflectionData)
	{
		...
		// the sub-parameters to get the final lightning name.
		if (spirVType.basetype == spirv_cross::SPIRType::Image)
		{
			if (spirVType.image.depth)
			{
				if (spirVType.image.dim == spv::Dim2D)
					reflectionData.mTypeName = LightningTypeId(Lightning::DepthImage2d)->ToString();
			}
			else
			{
				if (spirVType.image.dim == spv::Dim2D)
		...
		else if (spirVType.basetype == spirv_cross::SPIRType::SampledImage)
		{
			...
			{
				if (spirVType.image.dim == spv::Dim2D)
					reflectionData.mTypeName = LightningTypeId(Lightning::SampledDepthImage2d)->ToString();
			}
			else
			{
				if (spirVType.image.dim == spv::Dim2D)
					reflectionData.mTypeName = LightningTypeId(Lightning::SampledImage2d)->ToString();
				else if (spirVType.image.dim == spv::Dim3D)
					reflectionData.mTypeName = LightningTypeId(Lightning::SampledImage3d)->ToString();
				else if (spirVType.image.dim == spv::DimCube)
					reflectionData.mTypeName = LightningTypeId(Lightning::SampledImageCube)->ToString();
			}
		}
		else if (spirVType.basetype == spirv_cross::SPIRType::Sampler)
		{
			reflectionData.mTypeName = LightningTypeId(Lightning::Sampler)->ToString();
		}
	                                                        ...
	                                                        bool isInterfaceType)
	{
		...
		reflectionData.mSizeInBytes = static_cast<u32>(compiler->get_declared_struct_member_size(
			parentType, memberIndex));
	...
	void LightningShaderGlslBackend::PopulateTypeInfo(GlslBackendInternalData& internalData,
	                                                  spirv_cross::SPIRType& spirvType,
	                                                  ShaderResourceReflectionData& reflectionData,
	                                                  bool isInterfaceType)
	{
	...
	void LightningShaderGlslBackend::ExtractResourceReflection(GlslBackendInternalData& internalData,
	...
	void LightningShaderGlslBackend::ExtractResourcesReflection(GlslBackendInternalData& internalData,
	...
	void LightningShaderGlslBackend::ExtractResourceReflectionData(GlslBackendInternalData& internalData)
	{
		ShaderStageInterfaceReflection& stageReflection = internalData.mOutputResult->mReflectionData;
		spirv_cross::ShaderResources* resources = internalData.mResources;
		...
		ExtractResourcesReflection(internalData, resources->storage_buffers, stageReflection.mStructedStorageBuffers,
		                           false);
