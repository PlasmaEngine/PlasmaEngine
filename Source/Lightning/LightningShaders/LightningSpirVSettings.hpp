...
class LightningShaderIRCompositor;
...
/// Name settings used in the LightningSpirV translator. This allows
...
private:
  friend LightningShaderSpirVSettings;
  friend BuiltInStageDescription;
  friend EntryPointGeneration;
  friend LightningShaderIRCompositor;
  ...
  void AddField(Lightning::BoundType* type, StringParam fieldName, spv::BuiltIn builtInId, StringParam attribute);
  /// Overrides the lightning name that is used to map to a spirv built-in variable.
...
class LightningShaderErrorSettings
{
public:
  LightningShaderErrorSettings();
...
/// A collection of common settings for Lightning shader translation (current SpirV
/// specific).
class LightningShaderSpirVSettings
{
public:
  LightningShaderSpirVSettings();
  LightningShaderSpirVSettings(const SpirVNameSettings& nameSettings);
  ...
  /// Mappings of lightning names and spirv built-in types for each shader stage.
...
private:
  ...
  LightningRefLink(LightningShaderSettings);
  bool mFinalized;
