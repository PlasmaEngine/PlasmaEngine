...
#  include <unistd.h>
#  include <dirent.h>
#  define PlasmaAllPermissions (S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IXOTH)
#endif
...
{
  int failCode = mkdir(dest.c_str(), PlasmaAllPermissions);
...
String GetUserLocalDirectory()
{
  ...
  PlasmaStrCpy(local, File::MaxPath, GetUserDocumentsDirectory().c_str());
  PlasmaStrCat(local, File::MaxPath, "/.cache");
  return local;
}
