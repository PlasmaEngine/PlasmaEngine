...
#if defined(PlasmaTargetOsWindows) && !defined(PLASMA_COMPILLER_MSVC)
...
public:
  ...
  void Initialize(Status& status)
  {
    ...
    else
    {
      ...
      //
      PlasmaPrint("Initializing Socket Library...\n");
  ...
  void Uninitialize(Status& status)
  {
    ...
    else
    {
      ...
      //
      PlasmaPrint("Uninitializing Socket Library...\n");
...
{
  PlasmaMemClearPrivateData(SOCKET_ADDRESS_STORAGE);
...
void Destroy(Socket& socket)
{
  ...
  if (socket.IsOpen())
  {
    ...
    if (socket.HasConnectedRemoteAddress())
    {
      ...
      if (status.Failed()) // Unable?
        PlasmaPrint("Error shutting down socket connection (%d : %s)\n", status.Context, status.Message.c_str());
    }
    ...
    socket.Close(status);
    if (status.Failed()) // Unable?
      PlasmaPrint("Error closing socket (%d : %s)\n", status.Context, status.Message.c_str());
  }
}
...
{
  Assert(valueLength && *valueLength, "valueLength must contain the non-plasma size of the value parameter");
...
{
  Assert(valueLength, "valueLength must contain the non-plasma size of the value parameter");
