...
void InitializeGamepad()
{
  ...
  if (XInputModule)
  {
    PlasmaPrint("Using XINPUT for gamepads.\n");
  ...
  {
    PlasmaPrint("Failed to load XINPUT. Gamepads will not be available.\n");
  }
}
...
bool SetGamepadVibration(size_t gamepadIndex, float leftSpeed, float rightSpeed)
{
  ...
  XINPUT_VIBRATION plasmavibration;
  plasmavibration.wLeftMotorSpeed = (WORD)(leftSpeed * MaxRumble);
  plasmavibration.wRightMotorSpeed = (WORD)(rightSpeed * MaxRumble);
  return (*XInputSetState_P)(gamepadIndex, &plasmavibration) == ERROR_SUCCESS;
}
