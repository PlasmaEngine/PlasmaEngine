...
BOOL CALLBACK TestPlasmaMiniDumpCallback(PVOID pParam,
...
void CrashHandler::WriteMiniDump(CrashHandlerParameters& params, void* crashData, CrashInfo& info)
{
  ...
  if (pException != NULL && pException->ExceptionRecord != NULL)
  {
    ...
    PlasmaStrCatW(dumpFileName, MAX_PATH, Widen(info.mDumpName).c_str());
    ...
    mci.CallbackRoutine = (MINIDUMP_CALLBACK_ROUTINE)TestPlasmaMiniDumpCallback;
...
{
  PlasmaPrint("Set to auto restart on crash with '%s'\n", CrashHandler::mRestartCommandLine.c_str());
