...
// Notes:
//  - We do not handle mOnFrozenUpdate (Plasma may freeze on window drags)
...
Keys::Enum SDLScancodeToKey(SDL_Scancode code)
{
  switch (code)
  {
#define ProcessInput(Scancode, Keycode, PlasmaValue)                                                                     \
  case Scancode:                                                                                                       \
    return PlasmaValue;
#include "Keys.inl"
#undef ProcessInput
...
#if !defined(PlasmaPlatformNoShellGetScrollLineCount)
...
#if !defined(PlasmaPlatformNoShellGetColorAtMouse)
...
#if !defined(PlasmaPlatformNoShellSetMonitorCursorClip)
void Shell::SetMonitorCursorClip(const IntRect& monitorRectangle)
{
}
#endif

#if !defined(PlasmaPlatformNoShellClearMonitorCursorClip)
...
#if !defined(PlasmaPlatformNoShellGetPrimaryMonitorImage)
...
#if !defined(PlasmaPlatformNoShellOpenFile)
...
void Shell::Update()
{
  ...
  while (SDL_PollEvent(&e))
  {
    ...
    case SDL_TEXTINPUT:
    {
      ...
      // In Plasma we ignore these because we don't want the events here.
...
#if !defined(PlasmaPlatformNoClipboardEvents)
