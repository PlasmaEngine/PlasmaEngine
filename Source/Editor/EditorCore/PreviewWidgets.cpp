    ...
    PreviewWidget(initializer)
{
  ...
  else
  {
    ...
    // Create the space
    GameSession* editorGameSession = PL::gEditor->GetEditGameSession();
  ...
  uint creationFlags = CreationFlags::Editing | CreationFlags::Preview;
  Cog* camera =
      PL::gFactory->Create(space, CoreArchetypes::PreviewCamera, creationFlags, PL::gEditor->GetEditGameSession());
...
ResourceTablePreview::ResourceTablePreview(PreviewWidgetInitializer& initializer) : PreviewWidget(initializer)
{
  ...
  if (table->mResourceType == LightningTypeId(ResourceTable)->Name)
    return;
    ...
    IconPreview(initializer, "Level")
{
  // Space archetype
  Space* space =
      PL::gFactory->CreateSpace(archetype->ResourceIdName, CreationFlags::Editing, PL::gEditor->GetEditGameSession());
  mObject = space;
}
    ...
    IconPreview(initializer, "Level")
{
  ...
  if (PL::gEditor->GetEditGameSession()->GetArchetype() == archetype)
  {
    mObject = PL::gEditor->GetEditGameSession();
    UsingEditorGameSession = true;
  }
  else
  {
    mObject = (GameSession*)PL::gFactory->CreateCheckedType(
        LightningTypeId(GameSession), nullptr, archetype->Name, CreationFlags::Editing | CreationFlags::Preview, nullptr);
...
PreviewWidget* CreateArchetypePreviewWidget(PreviewWidgetInitializer& initializer)
{
  ...
  if (archetype->mStoredType == LightningTypeId(Cog))
  {
    // Basic archetype
    preview = new ArchetypePreview(initializer);
  }
  else if (archetype->mStoredType == LightningTypeId(Space))
  {
    preview = new SpaceArchetypePreview(initializer, archetype);
  }
  else if (archetype->mStoredType == LightningTypeId(GameSession))
  {
...
void RegisterEditorTileViewWidgets()
{
  ...
  previewFactory->Creators["LightningScript"] =
      PreviewWidgetCreator(PreviewImportance::Simple, &CreatePreviewWidgetT<ScriptPreview>);
  previewFactory->Creators["LightningFragment"] =
      PreviewWidgetCreator(PreviewImportance::Simple, &CreatePreviewWidgetT<ScriptPreview>);
  previewFactory->Creators["TextBlock"] =
      PreviewWidgetCreator(PreviewImportance::Simple, &CreatePreviewWidgetT<ScriptPreview>);
  previewFactory->Creators["LightningPluginSource"] =
      PreviewWidgetCreator(PreviewImportance::Simple, &CreatePreviewWidgetT<ScriptPreview>);
