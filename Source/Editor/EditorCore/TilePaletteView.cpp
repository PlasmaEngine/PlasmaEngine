...
// Sprite
LightningDefineType(TilePaletteSprite, builder, type)
{
}
...
LightningDefineType(TilePaletteView, builder, type)
{
  ...
  LightningBindGetterSetterProperty(TilePalette)->Add(new MetaEditorResource(true, false));

  LightningBindGetterSetterProperty(Archetype);

  LightningBindGetterSetterProperty(Sprite)->Add(new MetaEditorResource(true, true));

  LightningBindGetterSetterProperty(Collision)->Add(new MetaEditorResource(true, true, "TileMesh"));

  LightningBindGetterSetterProperty(Mergeable);
}
...
{
  PL::gResources->GetDispatcher()->Disconnect(this);
}
