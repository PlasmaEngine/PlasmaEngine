...
LightningDefineType(HeightMapDebugDrawer, builder, type)
{
  ...
  LightningBindFieldProperty(mDrawTriangles)->PlasmaSerialize(true);
  LightningBindFieldProperty(mDrawOffset);
}
...
LightningDefineType(HeightMapAabbChecker, builder, type)
{
  type->AddAttribute(ObjectAttributes::cHidden);
  PlasmaBindComponent();
  ...
  LightningBindFieldProperty(mDrawHeightMap);
  LightningBindFieldProperty(mSkipNonCollidingCells);
  LightningBindFieldProperty(mHeightMapPath);
}
