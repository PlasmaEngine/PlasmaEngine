...
bool DropOnObject(MetaDropEvent* event, Cog* droppedOn)
{
  ...
  // Search for property
  {
    ...
    BoundType* cogType = LightningVirtualTypeId(droppedOn);
...
bool DropOnObjectViewport(MetaDropEvent* event, Viewport* viewport, Space* space)
{
  ...
  if (DocumentResource* script = instance.Get<DocumentResource*>())
  {
    if (event->Testing)
      ...
      PL::gEditor->OpenDocumentResource(script);
    return true;
  }
  ...
  if (Archetype* archetype = instance.Get<Archetype*>())
  {
    ...
    {
      PL::gEditor->Tools->mCreationTool->CreateWithViewport(viewport, dropPosition, archetype);
    }
    return true;
  }
  ...
  if (SpriteSource* spriteSource = instance.Get<SpriteSource*>())
  {
    ...
    {
      Cog* object =
          PL::gEditor->Tools->mCreationTool->CreateWithViewport(viewport, dropPosition, CoreArchetypes::Sprite);
      if (object)
      {
  ...
  if (Mesh* mesh = instance.Get<Mesh*>())
  {
    ...
    {
      Cog* object =
          PL::gEditor->Tools->mCreationTool->CreateWithViewport(viewport, dropPosition, CoreArchetypes::Transform);
      if (object)
      {
...
bool EditorDrop(MetaDropEvent* event, Viewport* viewport, Space* space, Cog* droppedOn)
{
  ...
  // other Resources
  if (droppedType->IsA(LightningTypeId(Mesh)) || droppedType->IsA(LightningTypeId(SpriteSource)))
  {
