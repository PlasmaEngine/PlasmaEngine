...
/// Callback for adding custom icons to the property gridF
...
Animation* FindFirstAnimationOnCog(Cog* cog)
{
  ...
  forRange (Component* component, cog->GetComponents())
  {
    ...
    forRange (Property* metaProperty, componentType->GetProperties())
    {
      ...
      if (metaProperty->PropertyType == LightningTypeId(Animation))
      {
...
LightningDefineType(AnimationEditor, builder, type)
{
}
...
AnimationEditor::AnimationEditor(Composite* parent) : Composite(parent), mGraphData(Vec2(60, 20)), mOnionSkinning(this)
{
  ...
  ConnectThisTo(PL::gEditor, Events::Save, OnSave);
...
void AnimationEditor::SetAnimation(Animation* animation)
{
  if (Cog* animGraphObject = mAnimGraphObject)
  {
    ...
    if (simpleAnim == nullptr)
    {
      ...
      if (QueueAddComponent(opQueue, animGraphObject, LightningTypeId(SimpleAnimation)))
...
void AnimationEditor::ObjectSelected(Cog* cog)
{
  ...
  if (!refreshing)
  {
    ...
    Widget* widget = PL::gEditor->mManager->FindWidgetWith(cog);
...
void AnimationEditor::OnPropertyContextMenu(ContextMenuEvent* e)
{
  ...
  if (mErrorState == ErrorState::None)
  {
    ...
    ConnectThisTo(keyFrameCreate, Plasma::Events::MenuItemSelected, OnCreateKeyFrame);
  }
}
...
void AnimationEditor::OnMouseDownErrorText(MouseEvent* event)
{
  ...
  bool hasEditorConfig = PL::gEngine->GetConfigCog()->has(DeveloperConfig) != nullptr;
  ...
  if (mErrorState == ErrorState::NoAnimGraph)
  {
    ...
    OperationQueue* opQueue = PL::gEditor->GetOperationQueue();
    QueueAddComponent(opQueue, cog, LightningTypeId(AnimationGraph));
  ...
  else if (mErrorState == ErrorState::AnimationFromGeometry && hasEditorConfig)
  {
    ...
    {
      OperationQueue* opQueue = PL::gEditor->GetOperationQueue();
      if (QueueAddComponent(opQueue, cog, LightningTypeId(SimpleAnimation)))
...
{
  AddResourceWindow* addWidget = OpenAddWindow(LightningTypeId(Animation));
  ...
  ConnectThisTo(PL::gResources, Events::ResourceAdded, OnAnimationAdded);
}

void AnimationEditor::OnAnimationAdded(ResourceEvent* e)
{
  PL::gResources->GetReceiver()->Disconnect(this);

  Cog* cog = mAnimGraphObject;
  if (cog && (LightningVirtualTypeId(e->EventResource) == LightningTypeId(Animation)))
  {
    // Set the new animation on the AutoPlayAnimation and refresh
    SimpleAnimation* autoPlay = cog->has(SimpleAnimation);
    if (autoPlay == nullptr)
...
void AnimationEditor::CreateKeyFrameAtPlayHead(Property* property, HandleParam componentInstance, Any* newValue)
{
  ...
  BoundType* componentTypeId = LightningVirtualTypeId(component);
