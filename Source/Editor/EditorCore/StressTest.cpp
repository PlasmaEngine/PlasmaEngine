...
String ToString(Cog* sourceCog)
{
  ...
  while (range.Empty() == false)
  {
    ...
    BoundType* componentType = LightningVirtualTypeId(range.Front());
...
// StressTest
LightningDefineType(StressTest, builder, type)
{
  type->HandleManager = LightningManagerId(PointerManager);
  LightningBindFieldProperty(Seed);
  LightningBindFieldProperty(Frames);
  LightningBindFieldProperty(LogFile);

  LightningBindFieldProperty(CreateObjects);
  LightningBindFieldProperty(DestroyObjects);
  LightningBindFieldProperty(SetProperties);
  LightningBindFieldProperty(AddComponents);
  LightningBindFieldProperty(RemoveComponents);
  LightningBindFieldProperty(ChangeSelection);
  LightningBindFieldProperty(RayCasts);
  LightningBindFieldProperty(ParentObjects);
  LightningBindFieldProperty(UnparentObjects);
  LightningBindFieldProperty(MouseEvents);
  LightningBindFieldProperty(KeyEvents);
  LightningBindFieldProperty(ChangeTools);
  LightningBindFieldProperty(StartGameInstances);
  LightningBindFieldProperty(StopGameInstances);
  LightningBindFieldProperty(SwitchViewports);
  LightningBindFieldProperty(Undo);
  LightningBindFieldProperty(Redo);
  // LightningBindFieldProperty(CreateObjectLinks)->SetFlag(PropertyFlags::Serialized);
  // LightningBindFieldProperty(ModifyArchetypes)->SetFlag(PropertyFlags::Serialized);
  // LightningBindFieldProperty(CreateResources)->SetFlag(PropertyFlags::Serialized);
  // LightningBindFieldProperty(LoadResources)->SetFlag(PropertyFlags::Serialized);
  // LightningBindFieldProperty(ResourceProperties)->SetFlag(PropertyFlags::Serialized);
  // LightningBindFieldProperty(ConfigProperties)->SetFlag(PropertyFlags::Serialized);
  // LightningBindFieldProperty(ProjectProperties)->SetFlag(PropertyFlags::Serialized);
  // LightningBindFieldProperty(RunCommands)->SetFlag(PropertyFlags::Serialized);
  // LightningBindFieldProperty(ShapeCasts)->SetFlag(PropertyFlags::Serialized);
  // LightningBindFieldProperty(CollisionPairs)->SetFlag(PropertyFlags::Serialized);
  // LightningBindFieldProperty(LightingModes)->SetFlag(PropertyFlags::Serialized);
}
...
// StressTestDialog
LightningDefineType(StressTestDialog, builder, type)
{
}
    ...
    mRandom(0)
{
  ...
  mExtensionFunctions[LightningTypeId(Resource)] = &StressTestDialog::GetVariantForResourceExtension;
  mExtensionFunctions[LightningTypeId(Enum)] = &StressTestDialog::GetVariantForEnumExtension;
  mExtensionFunctions[LightningTypeId(EditorRange)] = &StressTestDialog::GetVariantForRangeExtension;
  mExtensionFunctions[LightningTypeId(EditorIndexedStringArray)] = &StressTestDialog::GetVariantForStringArray;
...
void StressTestDialog::OnStartTest(ObjectEvent* event)
{
  ...
  String newProjLocation = FilePath::Combine(dest, BuildString(projName, ".plasmaproj"));

  PL::gEditor->mConfig->has(MainConfig)->mSave = false;
...
{
  if (boundType == LightningTypeId(String))
  ...
  // else if(boundType == LightningTypeId(uint))
  //  return mRandom.RandomUint();
  else if (boundType == LightningTypeId(bool))
    return mRandom.RandomBool();
  else if (boundType == LightningTypeId(float))
    return mRandom.RandomFloatRange(-50.0f, 50.0f);
  else if (boundType == LightningTypeId(Vec2))
    return mRandom.RandomPosition2d();
  else if (boundType == LightningTypeId(Vec3))
    return mRandom.RandomPosition3d();
  else if (boundType == LightningTypeId(Vec4))
    return mRandom.RandomPosition4d();
  else if (boundType == LightningTypeId(Quat))
    return mRandom.RandomRotation();

  if (boundType == LightningTypeId(CogPath))
    return CogPath(mRandom.RandomString());

  if (boundType == LightningTypeId(Cog))
  {
...
{
  Space* targetSpace = PL::gEditor->GetEditSpace();
  ...
  if (mTestInfo.SetProperties)
  {
    ...
    {
      BoundType* boundType = LightningVirtualTypeId(component);
      ...
      if (property != nullptr)
      {
        ...
        if (!property->IsReadOnly())
        {
          ...
          if (EditorPropertyExtension* extension = property->HasInherited<EditorPropertyExtension>())
            function = mExtensionFunctions.FindValue(LightningVirtualTypeId(extension), nullptr);

          // Check if this is an enum
          if (function == nullptr && property->PropertyType->IsA(LightningTypeId(Enum)))
            function = &GetVariantForEnumExtension;
          // Check if this is a resource
          if (function == nullptr && property->PropertyType->IsA(LightningTypeId(Resource)))
          ...
          if (function == nullptr)
          {
            ...
            BoundType* propertyEditorType = LightningVirtualTypeId(propertyEditor);
          ...
          Log("Set Property '%s' on '%s' (old value %s, new value %s) on Cog "
              "%s",
              property->Name.c_str(),
              LightningVirtualTypeId(component)->Name.c_str(),
  ...
  if (mTestInfo.AddComponents)
  {
    if (Cog* object = GetRandomObject(targetSpace))
    {
      auto meta = LightningVirtualTypeId(object);
  ...
  if (mTestInfo.RemoveComponents)
  {
    if (Component* component = GetRandomComponent(targetSpace))
    {
      auto componentMeta = LightningVirtualTypeId(component);
      auto owner = component->GetOwner();

      Log("Removing component %s from %s", componentMeta->Name.c_str(), ToString(owner).c_str());

      auto meta = LightningVirtualTypeId(owner);
  ...
  {
    PL::gEditor->Tools->SelectToolIndex(mRandom.RandomIntMax(PL::gEditor->Tools->mTools.mToolArray.Size() + 5));
  }
  ...
  if (mTestInfo.StartGameInstances)
  {
    ...
    {
      if (mRandom.RandomBool())
        PL::gEditor->PlayGame(PlayGameOptions::SingleInstance);
      else
        PL::gEditor->PlayGame(PlayGameOptions::MultipleInstances);
    }
  }
  ...
  if (mTestInfo.SwitchViewports)
  {
    if (mRandom.Chance(0.01f))
    {
      ...
      for (; !children.Empty(); children.PopFront())
      {
        ...
        if (LightningVirtualTypeId(&child)->IsA(LightningTypeId(Window)) || child.GetName() == "Window")
        {
...
Property* StressTestDialog::GetRandomSetProperty(BoundType* boundType)
{
  ...
  auto range = boundType->GetProperties(Lightning::Members::All);
...
Any StressTestDialog::GetVariantForResourceExtension(StressRandom& random, Property* prop, Component* component)
{
  ...
  ResourceManager* manager = PL::gResources->Managers.FindValue(resourceType->Name, NULL);
...
Any StressTestDialog::GetVariantForEnumExtension(StressRandom& random, Property* prop, Component* component)
{
  ...
  Lightning::PropertyArray& properties = propertyType->AllProperties;
  if (properties.Empty())
    return Any(0);
...
Any StressTestDialog::GetVariantForRangeExtension(StressRandom& random, Property* prop, Component* component)
{
  ...
  if (propertyType == LightningTypeId(float))
  {
    float value = 0;
    if (random.Chance(0.9f))
  ...
  // else if(propertyType == LightningTypeId(uint))
  //{
  //  if(random.Chance(0.9f))
  ...
  //}
  else if (propertyType == LightningTypeId(int))
  {
    int value = 0;
    if (random.Chance(0.9f))
