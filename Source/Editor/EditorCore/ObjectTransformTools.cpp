...
LightningDefineType(ToolGizmoEvent, builder, type)
{
}

LightningDefineType(GizmoCreator, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);

  LightningBindGetterSetterProperty(GizmoArchetype);
}
...
LightningDefineType(ObjectTransformTool, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);

  PlasmaBindDependency(GizmoCreator);

  LightningBindGetterSetterProperty(Grab);
...
void ObjectTransformTool::OnToolGizmoCreated(ToolGizmoEvent* event)
{
  ...
  transformGizmo->SetOperationQueue(PL::gEditor->GetOperationQueue());
...
LightningDefineType(ObjectTranslateTool, builder, type)
{
  PlasmaBindComponent();
  type->AddAttribute(ObjectAttributes::cTool);
  ...
  LightningBindGetterSetterProperty(SnapMode);
}
...
LightningDefineType(ObjectScaleTool, builder, type)
{
  PlasmaBindComponent();
  type->AddAttribute(ObjectAttributes::cTool);
  PlasmaBindDocumented();
  ...
  LightningBindGetterSetterProperty(AffectTranslation);
  LightningBindGetterSetterProperty(AffectScale);
}
...
LightningDefineType(ObjectRotateTool, builder, type)
{
  PlasmaBindComponent();
  type->AddAttribute(ObjectAttributes::cTool);
  PlasmaBindDocumented();
  ...
  LightningBindGetterSetterProperty(Snapping);
  LightningBindGetterSetterProperty(SnapAngle);
  LightningBindGetterSetterProperty(AffectTranslation);
  LightningBindGetterSetterProperty(AffectRotation);
}
