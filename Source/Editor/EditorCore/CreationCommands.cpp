...
#include "UI/Widget/Command.hpp"
#include "Editor.hpp"
#include "EditorCameraController.hpp"
#include "Core/Engine/CogOperations.hpp"
#include "UI/Widget/CommandBinding.hpp"
#include "Core/Engine/Configuration.hpp"
...
void EditorCreateObjectCommand::Execute(Command* command, CommandManager* manager)
{
  ...
  if (space == nullptr)
    return CommandFailed(command, LightningTypeId(Space));
...
void BindCreationCommands(Cog* configCog, CommandManager* commands)
{
  ...
  bool devConfig = PL::gEngine->GetConfigCog()->has(Plasma::DeveloperConfig) != nullptr;
  if (devConfig)
  {
