...
LightningDefineType(CurveEvent, builder, type)
{
}
...
void CurveEditorToolbar::range::FindNextControlPoint()
{
  ...
  while (!mRange.Empty())
  {
    ...
    if (LightningVirtualTypeId(draggable)->IsA(LightningTypeId(ControlPoint)))
      return;
...
LightningDefineType(Draggable, builder, type)
{
}
...
LightningDefineType(ControlPoint, builder, type)
{
}
...
LightningDefineType(Tangent, builder, type)
{
}
...
void Tangent::OnRightMouseUp(MouseEvent* e)
{
  ...
  Plasma::Connect(itemSplit, Events::MenuItemSelected, mControlPoint, &ControlPoint::OnSplitTangents);
