...
void ResourceHotspot::OnClick(Matches& matches)
{
  ...
  if (matches.Size() == 2)
  {
    ...
    if (resource && LightningVirtualTypeId(resource) != LightningTypeId(Level))
    {
      // Edit the resource
      ...
      PL::gEditor->ShowWindow("Properties");
    }
    else if (LightningVirtualTypeId(resource) == LightningTypeId(Level))
    {
      return;
    }
    else
...
void ObjectHotspot::OnClick(Matches& matches)
{
  if (matches.Size() > 0)
  {
    ...
    Cog* cog = PL::gTracker->RawFind(cogIdValue);
    if (cog)
    {
      // Focus on the object
...
void FileHotspot::OnClick(Matches& matches)
{
  ...
  if (matches.Size() == 3)
  {
    ...
    // Subtract 1 since line numbers are actually plasma based, and the printed
    // form is 1 based
    --line;
