...
LightningDefineType(UiLegacyToolTip, builder, type)
{
  ...
  LightningBindMethod(SetPlacement);
  LightningBindMethod(SetPriority);

  LightningBindSetter(Padding);

  LightningBindMethod(SetColorScheme);
  LightningBindSetter(BackgroundColor);
  LightningBindSetter(BorderColor);

  LightningBindMethod(ClearText);
  LightningBindMethod(AddText);
}

UiLegacyToolTip::UiLegacyToolTip()
{
  ToolTip* toolTip = mToolTip = new ToolTip(PL::gEditor->GetRootWidget());
