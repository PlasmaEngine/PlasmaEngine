...
LightningDefineType(PostAddResourceEvent, builder, type)
{
}
...
AddResourceWindow* OpenAddWindow(BoundType* resourceType, Window** window, StringParam resourceName)
{
  ...
  // Select lightning script by default
  else
    addDialog->SelectResourceType(LightningTypeId(LightningScript));
...
// Add Resource Window
LightningDefineType(AddResourceWindow, builder, type)
{
}
...
void CloseAddWindow(Composite* windowElement)
{
  ...
  PL::gEditor->GetCenterWindow()->TryTakeFocus();
}
...
void AddResourceWindow::AddTags(TagList& tags)
{
  ...
  forRange (StringParam& tag, tags.All())
  {
    ...
    if (type && type->IsA(LightningTypeId(Resource)))
      continue;
...
LightningDefineType(ResourceTypeSearch, builder, type)
{
}
...
LightningDefineType(ResourceTemplateSearch, builder, type)
{
}
...
void ResourceTemplateSearch::ShowTemplates(BoundType* resourceType)
{
  ...
  mManager = PL::gResources->GetResourceManager(resourceType);
...
LightningDefineType(ResourceTemplateDisplay, builder, type)
{
}
...
void ResourceTemplateDisplay::ShowResourceTemplate(Resource* resource)
{
  ...
  if (resource)
  {
    ...
    if (LightningVirtualTypeId(resource)->IsA(LightningTypeId(ColorGradient)))
...
{
  bool devConfig = PL::gEngine->GetConfigCog()->has(Plasma::DeveloperConfig);
  forRange (ResourceLibrary* resourceLibrary, PL::gResources->LoadedResourceLibraries.Values())
  {
    ContentLibrary* contentLibrary = PL::gContentSystem->Libraries.FindValue(resourceLibrary->Name, nullptr);
...
void ResourceTemplateDisplay::OnCreate(Event*)
{
  ...
  BoundType* resourceType = LightningVirtualTypeId(resourceTemplate);
  ResourceManager* manager = PL::gResources->GetResourceManager(resourceType);
  ...
  if (resourceAdd.WasSuccessful())
  {
    ...
    forRange (Handle instance, mPostAdd.mObjects)
    {
      if (instance.IsNotNull())
      {
        ...
        if (property && LightningVirtualTypeId(resourceAdd.SourceResource)->IsA(property->PropertyType))
          ChangeAndQueueProperty(
              PL::gEditor->GetOperationQueue(), instance, mPostAdd.mProperty, resourceAdd.SourceResource);
      }
    }
    ...
    PL::gResources->DispatchEvent(Events::ResourceTagsModified, &e);
...
// Resource Template Display
LightningDefineType(ImportButton, builder, type)
{
}
