...
BackgroundTaskButton::BackgroundTaskButton(Composite* parent) : IconButton(parent)
{
  ...
  ConnectThisTo(PL::gBackgroundTasks, Events::BackgroundTaskStarted, OnTaskStarted);
  ConnectThisTo(PL::gBackgroundTasks, Events::BackgroundTaskUpdated, OnTaskUpdated);
  ConnectThisTo(PL::gBackgroundTasks, Events::BackgroundTaskCompleted, OnTaskCompleted);
...
void BackgroundTaskButton::Flash(bool toFlash)
{
  ...
  seq->Add(new CallParamAction<LightningSelf, bool, &LightningSelf::Flash>(this, !toFlash));
}
...
void BackgroundTaskButton::UpdateProgressBar()
{
  ...
  mActiveTasks = false;
  forRange (BackgroundTask* task, PL::gBackgroundTasks->mActiveTasks.All())
  {
