...
{
public:
  typedef PropertyArchetype LightningSelf;
  TextBox* mEditText;
  IconButton* mUpload;
  ...
  void RevertCog(Cog* cog, HashSet<MetaSelection*>* modifiedSelections)
  {
    ...
    RevertToArchetype(PL::gEditor->GetOperationQueue(), cog);
  }
  ...
  void OnRevert(ObjectEvent* event)
  {
    ...
    if (MetaSelection* selection = mInstance.Get<MetaSelection*>())
    {
      ...
      OperationQueue* opQueue = PL::gEditor->GetOperationQueue();
  ...
  void Refresh()
  {
    ...
    bool revertTypeDisabled =
        (mInstance.StoredType->IsA(LightningTypeId(GameSession)) || mInstance.StoredType->IsA(LightningTypeId(Space)));
...
{
public:
  typedef CogPickerManipulation LightningSelf;
...
{
public:
  typedef PropertyEditorCogRef LightningSelf;
  TextBox* mName;
...
{
public:
  typedef PropertyEditorCogPath LightningSelf;
  ...
  Cog* FindRelativeToCog()
  {
    ...
    while (node)
    {
      ...
      if (type && type->IsA(LightningTypeId(Cog)))
  ...
  void SetReferencedCog(Cog* to)
  {
    if (to != nullptr)
    {
      ...
      EnsureCogPathIsRelativeTo();
      Property* prop = LightningTypeId(CogPath)->GetProperty("Cog");
  ...
  void OnTextEnter(ObjectEvent* event)
  {
    ...
    EnsureCogPathIsRelativeTo();
    Property* prop = LightningTypeId(CogPath)->GetProperty("Path");
    propertyPath.AddPropertyToPath(prop);
  ...
  void Refresh() override
  {
    ...
    if (ObjectState* objectState = modifications->GetObjectState(mInstance))
    {
      ...
      forRange (Property* subProperty, LightningTypeId(CogPath)->GetProperties())
      {
...
{
public:
  typedef FocusComposite LightningSelf;
...
{
public:
  typedef ResourceEditor LightningSelf;
      ...
      mForceCompact(forceCompact)
  {
    ...
    mResourceManager = PL::gResources->Managers.FindValue(mResourceType->Name, nullptr);
    ...
    if (resourceType->IsA(LightningTypeId(ColorGradient)))
    {
  ...
  void SetResource(Resource* resource)
  {
    ...
    if (resource == nullptr)
    {
      ...
      if (mResourceType->IsA(LightningTypeId(ColorGradient)))
...
{
public:
  typedef PropertyEditorResource LightningSelf;
  Any mVariantValue;
  ...
  void SetResource(Resource* resource, bool preview = false)
  {
    if (resource)
    {
      if (mProperty->PropertyType == LightningTypeId(String))
      {
  ...
  void OnLeftClick(Event* event)
  {
    ...
    if (searchView == NULL)
    {
      ...
      if (mMetaEdit)
        searchProvider->SetCallbackFilter<LightningSelf, &LightningSelf::OnSearchFilter>(this);
  ...
  MetaSelection* GetMetaSelection()
  {
    ...
    while (node)
    {
      ...
      if (object.StoredType && object.StoredType->IsA(LightningTypeId(MetaSelection)))
...
{
public:
  typedef ListItem LightningSelf;
...
{
public:
  typedef AddItemButton LightningSelf;
...
{
public:
  typedef ResourceListItem LightningSelf;
  ...
  {
    Lightning::BoundType* type = LightningTypeId(typename ResourceList::ManagerType::ResourceType);
    Resource* resource = PL::gResources->GetResourceByTypeAndName(type->Name, mResourceIdName);
    if (resource != nullptr)
      PL::gEditor->EditResource(resource);
  }
...
{
public:
  typedef ResourceListEditor<ResourceList> LightningSelf;
...
{
  LightningTypeId(CogPath)->Add(new MetaPropertyEditor(&CreateProperty<PropertyEditorCogPath>));
  ...
  LightningTypeId(Resource)->Add(new MetaPropertyEditor(&CreateProperty<PropertyEditorResource>));
  LightningTypeId(MetaEditorResource)->Add(new MetaPropertyEditor(&CreateProperty<PropertyEditorResource>));
  LightningTypeId(CogArchetypeExtension)->Add(new MetaPropertyEditor(&CreateProperty<PropertyArchetype>));
  LightningTypeId(RenderGroupList)->Add(new MetaPropertyEditor(&CreateProperty<ResourceListEditor<RenderGroupList>>));
  LightningTypeId(ChildRenderGroupList)
      ->Add(new MetaPropertyEditor(&CreateProperty<ResourceListEditor<ChildRenderGroupList>>));
  LightningTypeId(MaterialList)->Add(new MetaPropertyEditor(&CreateProperty<ResourceListEditor<MaterialList>>));
  LightningTypeId(CompositionLabelExtension)->Add(new MetaPropertyEditor(&CreateProperty<CompositionLabel>));
}
