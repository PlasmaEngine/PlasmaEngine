...
namespace Plasma
{
    ...
    LightningDefineType(EditorCameraController, builder, type)
    {
        PlasmaBindComponent();
        PlasmaBindDependency(Transform);
        PlasmaBindDependency(Camera);
        type->AddAttribute(ObjectAttributes::cCore);
        PlasmaBindEvent(Events::CameraControllerUpdated, Event);

        LightningBindFieldProperty(mMoveSensitivity);
        LightningBindFieldProperty(mInvertGamepadPitch);
        LightningBindFieldProperty(mGamepadMoveSensitivity);
        LightningBindFieldProperty(mGamepadRotateSensitivity);
        LightningBindGetterSetterProperty(LookTarget);
        LightningBindGetterSetterProperty(LookDistance);
        LightningBindGetterSetterProperty(VerticalAngle);
        LightningBindGetterSetterProperty(HorizontalAngle);
        LightningBindGetterSetterProperty(ControlMode);
        LightningBindMethodProperty(Reset);
    }
    ...
    EditorCameraController::EditorCameraController()
    {
        ...
        mInvertGamepadPitch = false;
        mGamepadMoveSensitivity = 0.1f;
        mGamepadRotateSensitivity = 10.0f;
        mLookDistance = 40.0f;
        mEnabled = false;
    ...
    {
        mGamepad = PL::gGamepads->GetGamePad(0);
    ...
    void EditorCameraController::Serialize(Serializer& stream)
    {
        ...
        SerializeNameDefault(mInvertGamepadPitch, false);
        SerializeNameDefault(mGamepadMoveSensitivity, 0.1f);
        SerializeNameDefault(mGamepadRotateSensitivity, 10.0f);
    }
    ...
    { 
        ...
        Vec2 GamepadMovement = Vec2::cZero;
        Vec2 GamepadLook = Vec2::cZero;

        if (mGamepad->mIsActive)
        {
            GamepadMovement = mGamepad->mLeftStick * mGamepadMoveSensitivity;
            GamepadLook = mGamepad->mRightStick * mGamepadRotateSensitivity;
            GamepadLook.y =  mInvertGamepadPitch ? GamepadLook.y : -GamepadLook.y;
        }
        ...
        if (mControlMode != ControlMode::ZPlane)
        {
            ...
            mLookTarget += direction * GamepadMovement.y;
        }
        else
        {
            if (mMovement[ControllerButton::MoveForward])
            ...
            // Gamepad movement Y
            mLookTarget += up * GamepadMovement.y;
        }
        ...
        // Gamepad movement X
        mLookTarget += right * GamepadMovement.x;

        // Gamepad rotation
        if (mControlMode != ControlMode::ZPlane)
        {
            uint oldMouseMode = mDragMode;
            mDragMode = CameraDragMode::Rotation;
            DragMovement(GamepadLook, nullptr);
            mDragMode = oldMouseMode;
        }
