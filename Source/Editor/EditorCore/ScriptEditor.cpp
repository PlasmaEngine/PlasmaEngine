...
LightningDefineType(AutoCompletePopUp, builder, type)
{
}

LightningDefineType(CallTipPopUp, builder, type)
{
}
...
void AutoCompletePopUp::ShowToolTip()
{
  ...
  if (Resource* resource = PL::gResources->GetResource(completion->AssociatedResourceId))
...
LightningDefineType(DocumentEditor, builder, type)
{
}
...
void ScriptEditor::OnKeyDown(KeyboardEvent* event)
{
  ...
  {
    if (this->AutoCompletePlasmaConnect())
      return;
  }
...
{
  LightningBase::OnTextModified(event);
...
void ScriptEditor::OnCharacterAdded(TextEditorEvent* event)
{
  ...
  {
    if (this->CanCompletePlasmaConnect())
    {
...
bool ScriptEditor::GetCompletePlasmaConnectInfo(String& eventNameOut, String& indentOut, int& functionPositionOut)
{
  functionPositionOut = -1;

  ICodeInspector* code = this->GetCodeInspector();
  if (code && code->SupportsPlasmaConnect())
  {
    String line = GetCurrentLineText();
    Regex plasmaConnect("Plasma.Connect\\(.+,\\s*Events\\.([a-zA-Z0-9_]+)\\s*,\\s*$");
    Matches matches;
    plasmaConnect.Search(line, matches);

    if (matches.Size() != 2)
    {
      Regex plasmaConnectString("Plasma.Connect\\(.+,\\s*\"([a-zA-Z0-9_]+)\"\\s*,\\s*$");
      plasmaConnectString.Search(line, matches);
      ...
      if (matches.Size() != 2)
      {
        ...
        if (matches.Size() != 2)
        {
          Regex plasmaConnectString("Plasma.Connect\\(.+,\\s*([^,;]*)\\s*,\\s*$");
          plasmaConnectString.Search(line, matches);
        }
      }
    }
    ...
    if (matches.Size() == 2)
    {
      eventNameOut = Lightning::LibraryBuilder::FixIdentifier(matches[1], TokenCheck::IsUpper);
...
bool ScriptEditor::CanCompletePlasmaConnect()
{
  String eventName;
  String indent;
  int functionPosition;
  return GetCompletePlasmaConnectInfo(eventName, indent, functionPosition);
}

bool ScriptEditor::AutoCompletePlasmaConnect()
{
  String eventName;
  String indent;
  int functionPosition;
  if (GetCompletePlasmaConnectInfo(eventName, indent, functionPosition))
  {
    this->StartUndo();

    // We know this is always valid since the
    // 'GetCompletePlasmaConnectInfo' wouldn't return true if it wasn't
    ICodeInspector* code = this->GetCodeInspector();
    ...
    if (eventTypeName.Empty())
    {
      ...
      if (eventType == nullptr)
        eventType = LightningTypeId(Event);
