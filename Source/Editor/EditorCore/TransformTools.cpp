...
LightningDefineType(ManipulatorToolEvent, builder, type)
{
  LightningBindGetter(OperationQueue);
  LightningBindGetter(Finished);

  LightningBindFieldGetter(mGrabLocation);
  LightningBindFieldGetter(mStartWorldRectangle);
  LightningBindField(mEndWorldRectangle);
}
...
// be the same magnitude. This is the size considered to be equal to plasma.
// Addtionally: add in the standard tool epsilon to capture float-rounding
// errors.
//
// This size epsilon will be used for detecting a plasma size object. If an object
// is plasma size then it's scale change needs to be special-cased. [ie, cannot
// scale 0].
static const float cSizeEpsilon = cMinimumBoundingSize + cToolEpsilon;

LightningDefineType(ManipulatorTool, builder, type)
{
  PlasmaBindComponent();
  ...
  PlasmaBindEvent(Events::ManipulatorToolStart, ManipulatorToolEvent);
  PlasmaBindEvent(Events::ManipulatorToolModified, ManipulatorToolEvent);
  PlasmaBindEvent(Events::ManipulatorToolEnd, ManipulatorToolEvent);

  PlasmaBindDependency(MouseCapture);

  LightningBindFieldProperty(mSizeBoxCollider);
  LightningBindFieldProperty(mDuplicateOnCtrlDrag);
  LightningBindGetterSetterProperty(IncludeMode);
  LightningBindFieldProperty(mGrabMode);
  LightningBindFieldProperty(mSnapping);
  LightningBindGetterSetterProperty(SnapDistance);

  LightningBindFieldProperty(mToolColor)->PlasmaSetPropertyGroup("Colors");
  LightningBindFieldProperty(mHoverColor)->PlasmaSetPropertyGroup("Colors");
}
...
void ManipulatorTool::OnToolActivate(Event*)
{
  ...
  MetaSelection* selection = PL::gEditor->mSelection;
...
void ManipulatorTool::OnMouseDragMove(ViewportMouseEvent* event)
{
  ...
  Property* areaSizeProperty = LightningTypeId(Area)->GetProperty("Size");
  Property* colliderOffsetProperty = LightningTypeId(BoxCollider)->GetProperty("Offset");
  Property* colliderSizeProperty = LightningTypeId(BoxCollider)->GetProperty("Size");
  ...
  forRange (TransformingObject& target, mTransformingObjects.All())
  {
    ...
    if (Cog* cog = handle.Get<Cog*>())
    {
      ...
      if (area != nullptr && affectRootOnly)
      {
        ...
        // Recover from plasma size if the start size was plasma.
        //
        // NOTE:
