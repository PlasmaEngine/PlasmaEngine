...
{
  mOperationQueue = PL::gEditor->GetOperationQueue();
}
...
bool RenderGroupSource::Move(DataEntry* destinationEntry, DataEntry* movingEntry, InsertMode::Type insertMode)
{
  ...
  if (destinationEntry == this)
  {
    ...
    ChangeAndQueueProperty(mOperationQueue, moving, PropertyPath("ParentRenderGroup"), Any(LightningTypeId(RenderGroup)));
...
LightningDefineType(RenderGroupHierarchies, builder, type)
{
}
