...
// --------------------------------------------------------- TileEditor2DSubTool
LightningDefineType(TileEditor2DSubTool, builder, type)
{
}
...
LightningDefineType(TileEditor2DDrawTool, builder, type)
{
}
...
LightningDefineType(TileEditor2DSelectTool, builder, type)
{
}
...
LightningDefineType(TileEditor2D, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindDependency(MouseCapture);
  PlasmaBindSetup(SetupMode::DefaultSerialization);

  LightningBindGetterSetterProperty(ToolType);
  LightningBindMethodProperty(CreateTileMap);
  type->AddAttribute(ObjectAttributes::cTool);

  LightningBindGetterSetterProperty(ShowCollision);
  LightningBindGetterSetterProperty(ShowCoordinates);
  LightningBindFieldProperty(mShowArchetype);
  LightningBindFieldProperty(mShowInvalid);
  LightningBindFieldProperty(mShowGrid);
  LightningBindFieldProperty(mTiledDrawing);
}
...
void TileEditor2D::OnToolDraw(Event*)
{
  ...
  if (mShowGrid)
  {
    ...
    for (uint i = 1; i < gridLines.Size(); i += 2)
      Plasma::gDebugDraw->Add(Plasma::Debug::Line(gridLines[i - 1], gridLines[i]).Color(Color::Gray));
  }
  ...
  if (mShowArchetype)
  {
    forRange (auto pair, tileMap->GetTiles())
    {
      ...
      if (archetype)
        Plasma::gDebugDraw->Add(Plasma::Debug::Text(pos, real(0.15f), archetype->Name).Centered(true).Color(Color::Orange));
    }
  }
...
TileMap* TileEditor2D::GetTileMap()
{
  ...
  TileMap* tileMap = static_cast<TileMap*>(
      Tool::GetOrCreateEditComponent(LightningTypeId(TileMap), cDefaultName, cDefaultArchetype, mLastEdited, false));
