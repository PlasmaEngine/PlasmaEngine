...
{
public:
  typedef FlagEditor LightningSelf;
  Element* mIcon;
  bool mEnabled;
...
public:
  ...
  bool FilterComponentTypes(DataEntry* entry)
  {
    ...
    {
      int priority = PartialMatch(SubFilterString, LightningVirtualTypeId(component)->Name.All(), CaseInsensitiveCompare);
  ...
  bool FilterResourceUsage(DataEntry* entry)
  {
    ...
    forRange (Component* component, cog->GetComponents())
    {
      BoundType* metaType = LightningVirtualTypeId(component);

      forRange (Property* property, metaType->GetProperties())
      {
        if (property->PropertyType->IsA(LightningTypeId(Resource)))
        {
...
{
public:
  typedef TreeViewSearchObjectTree LightningSelf;
...
// Removed Entry
LightningDefineType(RemovedEntry, builder, type)
{
}
...
public:
  ...
  typedef SpaceObjectSource LightningSelf;
  ...
  DataEntry* ToEntry(DataIndex index) override
  {
    ...
    Cog* object = PL::gTracker->GetObjectWithId(index.Id);
  ...
  bool SetData(DataEntry* dataEntry, AnyParam variant, StringParam column) override
  {
    ...
    if (column == CommonColumns::Name)
    {
      Property* property = LightningTypeId(Cog)->GetProperty("Name");
      ChangeAndQueueProperty(PL::gEditor->GetOperationQueue(), cog, property, variant);
    }
  ...
  bool Move(DataEntry* destinationEntry, Array<DataIndex>& indicesToMove, InsertMode::Type insertMode) override
  {
    ...
    Plasma::Sort(sortedCogs.All(), CogHierarchyIndexCompareFn);

    if (insertMode == InsertMode::After)
      Plasma::Reverse(sortedCogs.Begin(), sortedCogs.End());
...
{
  return PL::gTracker->GetObjectWithId(index.Id);
}
...
LightningDefineType(ObjectView, builder, type)
{
}
...
void ObjectView::SetSpace(Space* space)
{
  ...
  mTree->SetSelection(new SpaceSelection(PL::gEditor->GetSelection()));
...
void ObjectView::OnKeyDown(KeyboardEvent* event)
{
  ...
  {
    Cog* object = PL::gEditor->GetSelection()->GetPrimaryAs<Cog>();
...
void ObjectView::OnRightMouseUp(MouseEvent* event)
{
  ...
  ContextMenu* menu = new ContextMenu(this);
  menu->AddPlasmaContextMenu("Create");
