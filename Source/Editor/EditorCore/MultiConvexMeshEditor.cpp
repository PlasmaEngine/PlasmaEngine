...
LightningDefineType(MultiConvexMeshPoint, builder, type)
{
}
...
LightningDefineType(MultiConvexMeshPropertyViewInfo, builder, type)
{
  LightningBindDefaultConstructor();
  PlasmaBindDocumented();
  type->HandleManager = LightningManagerId(PointerManager);

  LightningBindGetterSetterProperty(MeshThickness);
  LightningBindGetterSetterProperty(SpriteSource);
  LightningBindGetterSetterProperty(ClearColor);
  LightningBindGetterSetterProperty(OuterContourColor);
  LightningBindFieldProperty(mDrawMode);
  LightningBindMethodProperty(AutoCompute);
  LightningBindFieldProperty(mAutoComputeMode);
  LightningBindFieldProperty(mSurfaceLevelThreshold)->Add(new EditorSlider(0, 1, 0.05f));
  LightningBindFieldProperty(mAutoComputeMethod);
  LightningBindFieldProperty(mSimplificationThreshold);
}
...
LightningDefineType(MultiConvexMeshEditor, builder, type)
{
}
...
void MultiConvexMeshEditor::SetupPreviewSpace()
{
  ...
  mPreviewSpace =
      PL::gFactory->CreateSpace(CoreArchetypes::DefaultSpace, CreationFlags::Editing, PL::gEditor->GetEditGameSession());
  ...
  if (mCameraController != NULL)
  {
    ...
    Plasma::Connect(mViewport, Events::KeyDown, mCameraController, &EditorCameraController::ProcessKeyboardEvent);
    Plasma::Connect(mViewport, Events::KeyUp, mCameraController, &EditorCameraController::ProcessKeyboardEvent);
  }
  ...
  previewMeshCog->AddComponentByName(LightningTypeId(MultiConvexMeshCollider)->Name);
