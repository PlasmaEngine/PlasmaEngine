...
LightningDefineType(GizmoUpdateEvent, builder, type)
{
  LightningBindFieldProperty(mConstrainedWorldMovement);
  LightningBindFieldProperty(mConstrainedWorldDelta);
  LightningBindFieldProperty(mInitialGrabPoint);

  LightningBindFieldPropertyAs(mConstrainedWorldMovement, "MouseWorldMovement")->AddAttribute(DeprecatedAttribute);
  LightningBindFieldPropertyAs(mConstrainedWorldDelta, "MouseWorldDelta")->AddAttribute(DeprecatedAttribute);
}
...
LightningDefineType(GizmoDrag, builder, type)
{
  PlasmaBindComponent();
  ...
  PlasmaBindEvent(Events::GizmoModified, GizmoUpdateEvent);
  PlasmaBindEvent(Events::GizmoPreDrag, GizmoEvent);

  LightningBindFieldProperty(mDragMode)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindFieldProperty(mLineDirection)->PlasmaFilterEquality(mDragMode, GizmoDragMode::Enum, GizmoDragMode::Line);
  LightningBindFieldProperty(mPlaneNormal)->PlasmaFilterEquality(mDragMode, GizmoDragMode::Enum, GizmoDragMode::Plane);
  LightningBindFieldProperty(mNormalInWorld)->PlasmaFilterEquality(mDragMode, GizmoDragMode::Enum, GizmoDragMode::Plane);
  LightningBindGetter(GrabPoint);
  LightningBindFieldProperty(mGrabMode);
  LightningBindFieldProperty(mAutoDrag)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindFieldProperty(mDragDistance)->PlasmaFilterBool(mAutoDrag);

  LightningBindGetter(DragActive);
}
