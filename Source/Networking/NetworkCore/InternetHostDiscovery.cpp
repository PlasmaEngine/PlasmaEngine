...
void InternetHostDiscovery::ReceiveNetHostRecordList(IpAddress const& theirIpAddress, Plasma::Message const& message)
{
  ...
  forRange (NetHostRecord& record, netHostRecordList.mNetHostRecords.All())
  {
    ...
    respondingHostData->mBasicHostInfo = PlasmaMove(record.mBasicHostInfo.GetBitStream());
