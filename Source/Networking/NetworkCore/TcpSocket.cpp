...
LightningDefineType(ConnectionData, builder, type)
{
  LightningBindGetterProperty(Host);
  LightningBindGetterProperty(Port);
  LightningBindFieldProperty(Index);
  LightningBindFieldProperty(Incoming);
}
...
LightningDefineType(ConnectionEvent, builder, type)
{
  LightningBindFieldProperty(Host);
  LightningBindFieldProperty(Port);
  LightningBindFieldProperty(Index);
  LightningBindFieldProperty(Incoming);
}
...
LightningDefineType(ReceivedDataEvent, builder, type)
{
...
{
  Handle = PlasmaMove(const_cast<SocketData&>(rhs).Handle);
}
...
LightningDefineType(TcpSocket, builder, type)
{
  ...
  LightningBindOverloadedMethod(Listen, LightningInstanceOverload(bool, int));
  LightningBindOverloadedMethod(Listen, LightningInstanceOverload(bool, int, uint));
  LightningBindOverloadedMethod(Listen, LightningInstanceOverload(bool, int, uint, TcpSocketBind::Enum));
  LightningBindMethod(Close);
  LightningBindMethod(CloseConnection);
  LightningBindMethod(SendTo);
  LightningBindMethod(SendToAll);
  LightningBindMethod(SendToAllExcept);
  LightningBindMethod(SendToAllAndSelf);
  LightningBindMethod(IsConnected);

  LightningBindGetterProperty(OutgoingConnectionCount);
  LightningBindGetterProperty(IncomingConnectionCount);
  LightningBindGetterProperty(ConnectionCount);

  PlasmaBindEvent(Events::SocketError, TextErrorEvent);
  PlasmaBindEvent(Events::ConnectionCompleted, ConnectionEvent);
  PlasmaBindEvent(Events::ConnectionFailed, ConnectionEvent);
  PlasmaBindEvent(Events::Disconnected, ConnectionEvent);
  PlasmaBindEvent(Events::ReceivedData, ReceivedDataEvent);
}
...
void TcpSocket::Initialize()
{
  ...
  ConnectThisTo(PL::gEngine, Events::EngineUpdate, Update);

  // Set the socket to an invalid socket
  mServer.Close();

  // Initialize all the statistics to plasma
  mSendCount = 0;
  mSendSize = 0;
  mReceiveCount = 0;
  mReceieveSize = 0;
...
void TcpSocket::InternalConnect(const ConnectionData& info)
{
  ...
  else
  {
    ...
    else
    {
      ...
      newSocketData.Handle = PlasmaMove(newSocket);
...
void TcpSocket::TrackSend(const byte* data, size_t size, Socket& socket)
{
  ...
  // Printing
  // PlasmaPrint("%s Send Count (%d) Size (%d) - Sent data of size %d to socket
...
void TcpSocket::TrackReceive(const byte* data, size_t size, Socket& socket)
{
  ...
  // Printing
  // PlasmaPrint("%s Receive Count (%d) Size (%d) - Received data of size %d from
