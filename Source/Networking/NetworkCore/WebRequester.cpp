...
static const String cCacheDirectory("PlasmaCache");

LightningDefineType(AsyncWebRequest, builder, type)
{
  PlasmaBindDocumented();
  PlasmaBindEvent(Events::WebResponseHeaders, WebResponseEvent);
  PlasmaBindEvent(Events::WebResponsePartialData, WebResponseEvent);
  PlasmaBindEvent(Events::WebResponseComplete, WebResponseEvent);
  LightningBindField(mUrl);
  LightningBindMethod(Run);
  LightningBindMethod(Cancel);
  LightningBindMethod(ClearAll);
  LightningBindMethod(ClearRequestData);
  LightningBindMethod(ClearResponseData);
  LightningBindMethod(AddHeader);
  LightningBindMethod(AddFile);
  LightningBindMethod(AddField);
  LightningBindGetterProperty(IsRunning);

  LightningBindMethod(Create);

  LightningBindFieldGetterProperty(mStoreData);

  LightningBindFieldGetterProperty(mTotalDownloaded);
  LightningBindFieldGetterProperty(mTotalExpected);
  LightningBindFieldGetterProperty(mProgress);
  LightningBindFieldGetterProperty(mProgressType);
  LightningBindFieldGetterProperty(mError);
}
...
void AsyncWebRequest::OnHeadersReceived(const Array<String>& headers, WebResponseCode::Enum code, WebRequest* request)
{
  ...
  if (self->mSendEventsOnRequestThread)
    ...
    PL::gDispatch->Dispatch(self, Events::WebResponseHeadersInternal, toSend);
}
...
void AsyncWebRequest::OnDataReceived(const byte* data, size_t size, u64 totalDownloaded, WebRequest* request)
{
  ...
  // Create a plasma-style string out of the data
  ...
  if (self->mSendEventsOnRequestThread)
    ...
    PL::gDispatch->Dispatch(self, Events::WebResponsePartialDataInternal, toSend);
}
...
void AsyncWebRequest::OnComplete(Status& status, WebRequest* request)
{
  ...
  if (self->mSendEventsOnRequestThread)
    ...
    PL::gDispatch->Dispatch(self, Events::WebResponseCompleteInternal, toSend);
}
...
LightningDefineType(WebResponseEvent, builder, type)
{
  LightningBindField(mAsyncWebRequest);
  LightningBindFieldProperty(mResponseCode);
  LightningBindFieldProperty(mData);
  LightningBindGetterProperty(HeaderCount);
  LightningBindMethod(GetHeader);
  LightningBindFieldGetterProperty(mTotalDownloaded);
  LightningBindFieldGetterProperty(mTotalExpected);
  LightningBindFieldGetterProperty(mProgress);
  LightningBindFieldGetterProperty(mProgressType);
...
JsonValue* WebResponseEvent::ReadJson(Status& status)
{
  ...
  JsonValue* value = Lightning::JsonReader::ReadIntoTreeFromString(errors, mData, cOrigin, nullptr);
  if (!value)
  {
...
LightningDefineType(WebRequester, builder, type)
{
  PlasmaBindComponent();
  ...
  LightningBindMethod(CancelActiveRequests);
  LightningBindMethod(AddHeader);
  LightningBindMethod(AddFile);
  LightningBindMethod(AddField);

  LightningBindMemberProperty(mCancelOnDestruction);
}
