...
LinkInbox::LinkInbox(MoveReference<LinkInbox> rhs)
    ...
    /// Packet Data
    mReceivedPackets(PlasmaMove(rhs->mReceivedPackets)),
    mLastReceiveTime(rhs->mLastReceiveTime),
    mIncomingPacketSequence(PlasmaMove(rhs->mIncomingPacketSequence)),
    ...
    /// Channel Data
    mCustomDefaultChannel(PlasmaMove(rhs->mCustomDefaultChannel)),
    mProtocolDefaultChannel(PlasmaMove(rhs->mProtocolDefaultChannel)),
    mChannels(PlasmaMove(rhs->mChannels)),

    /// Message Data
    mReleasedCustomMessages(PlasmaMove(rhs->mReleasedCustomMessages)),
    mReleasedProtocolMessages(PlasmaMove(rhs->mReleasedProtocolMessages))
{
}
...
void LinkInbox::ReceivePacket(MoveReference<InPacket> packet)
{
  ...
  if (!packet->GetMessages().Empty())
  {
    ...
    mReceivedPackets.PushBack(PlasmaMove(packet));
  }
...
void LinkInbox::Update(ACKArray& remoteACKs, NAKArray& remoteNAKs)
{
    ...
    forRange (Message& message, packet.GetMessages().All())
    {
      ...
      case ProtocolMessageType::ChannelOpened:
      {
        ...
        if (mLink->PluginEventOnIncomingChannelOpen(channelId))
        {
          ...
          ArraySet<InMessageChannel>::pointer_bool_pair result = mChannels.Insert(PlasmaMove(newChannel));
      ...
      default:
      {
        ...
        channel->Push(PlasmaMove(message));
      }
      break;
      }
      ...
      (mLink->GetSendRate() * mLink->GetPacketSequenceHistoryRateFactor()))
  {
    ...
    if (packetSequenceHistoryData.mNext > mLastOutPacketSequenceHistoryNESQ)
    {
      ...
      Status status;
      mLink->SendInternal(status, PlasmaMove(message), false);
...
void LinkInbox::ReleaseCustomMessages(Array<Message>& messages)
{
  ...
  forRange (Message& message, messages.All())
  {
    ...
    mReleasedCustomMessages.PushBack(PlasmaMove(message));
  }
  messages.Clear();
}
void LinkInbox::ReleaseProtocolMessages(Array<Message>& messages)
{
  ...
  forRange (Message& message, messages.All())
  {
    ...
    mReleasedProtocolMessages.PushBack(PlasmaMove(message));
  }
  messages.Clear();
}
