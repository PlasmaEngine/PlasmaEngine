...
struct MoveWithoutDestructionOperator<FragmentedReceipt>
{
  ...
  {
    new (dest) FragmentedReceipt(PlasmaMove(*source));
  }
};
...
class LinkOutbox
{
  ...
  /// Channel Data
  OutMessageChannel mDefaultChannel;         /// Outgoing default (plasma) message channel
  ArraySet<OutMessageChannel> mChannels;     /// Outgoing (non-plasma) message channels
