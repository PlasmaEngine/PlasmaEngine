...
InMessageChannel::InMessageChannel(MoveReference<InMessageChannel> rhs) :
    MessageChannel(*rhs),
    mLastSequenceId(rhs->mLastSequenceId),
    mFragmentedMessages(PlasmaMove(rhs->mFragmentedMessages)),
    mMessages(PlasmaMove(rhs->mMessages)),
    mMessageSequence(PlasmaMove(rhs->mMessageSequence)),
    mClosed(rhs->mClosed),
    mFinalSequenceId(rhs->mFinalSequenceId)
{
}
...
bool InMessageChannel::Push(MoveReference<Message> message)
{
  ...
  if (message->IsFragment())
  {
    ...
    if (iter != mFragmentedMessages.End())
    {
      ...
      if (iter->IsComplete())
      {
        ...
        // Push whole message
        bool result = Push(PlasmaMove(wholeMessage));
        Assert(result);
      }
  ...
  case TransferMode::Ordered:
    ...
    if (message->IsFragment())
    {
      ...
      FragmentedMessage fragmentedMessage(PlasmaMove(message));
      ArraySet<FragmentedMessage>::pointer_bool_pair insertResult =
          mFragmentedMessages.Insert(PlasmaMove(fragmentedMessage));
    ...
    else
    {
      ...
      // Keep message
      mMessages.Insert(PlasmaMove(message));
    }
...
Array<Message> InMessageChannel::Release()
{
  ...
  case TransferMode::Sequenced:
    ...
    result = PlasmaMove(static_cast<Array<Message>&>(mMessages));
    mMessages.Clear();
    break;
  ...
  case TransferMode::Ordered:
    ...
    for (ArraySet<Message>::iterator iter = mMessages.Begin(); iter != mMessages.End();)
      ...
      {
        result.PushBack(PlasmaMove(*iter));
