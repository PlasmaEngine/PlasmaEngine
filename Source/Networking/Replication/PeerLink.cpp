...
void PeerLink::ResetSession()
{
  ...
  mInbox = PlasmaMove(defaultInbox);
  LinkOutbox defaultOutbox(this);
  mOutbox = PlasmaMove(defaultOutbox);
                                ...
                                TimeMs lifetime)
{
  ...
  // Send message
  return SendInternal(status, PlasmaMove(messageCopy), reliable, channelId, receipt, priority, lifetime, false);
}
MessageReceiptId PeerLink::Send(Status& status,
                                ...
                                TimeMs lifetime)
{
  ...
  // Send message
  return SendInternal(status, PlasmaMove(message), reliable, channelId, receipt, priority, lifetime, false);
}
                                        ...
                                        bool isProtocol)
{
  MessageReceiptId receiptId =
      mOutbox.PushMessage(status, PlasmaMove(message), reliable, channelId, receipt, priority, lifetime, isProtocol);
...
void PeerLink::ReceivePacket(MoveReference<InPacket> inPacket)
{
  ...
  mInbox.ReceivePacket(PlasmaMove(inPacket));
}
...
UpdateLinkState:
  ...
  case LinkState::Disconnected:
  {
    ...
    forRange (Message& message, protocolMessages.All())
      if (message.GetType() == ProtocolMessageType::ConnectRequest)
      {
        ...
        else
        {
          ...
          Status status;
          mStateACKId = SendInternal(status, PlasmaMove(connectResponseMessage), true, 0, true);
    ...
    if (mConnectRequested)
    {
      ...
      Status status;
      SendInternal(status, PlasmaMove(connectRequestMessage), true);
  ...
  case LinkState::ReceivedConnectRequest:
  {
    ...
    if (response != UserConnectResponse::Pending)
    {
      ...
      // Send connect response message
      Status status;
      MessageReceiptId receiptId = SendInternal(status, PlasmaMove(connectResponseMessage), true, 0, true);
  ...
  case LinkState::Connected:
  {
    ...
    if (mState == LinkState::Connected)
    {
      ...
      if (shouldDisconnect)
      {
        ...
        Status status;
        mStateACKId = SendInternal(status, PlasmaMove(disconnectNoticeMessage), true, 0, true);
...
bool PeerLink::ProcessReceivedCustomMessage(Message& message, bool isEvent)
{
  ...
  bool belongsToPlugin = AttemptPluginMessageReceive(PlasmaMove(message), continueProcessingCustomMessages);
...
bool PeerLink::AttemptPluginMessageReceipt(MoveReference<OutMessage> message, Receipt::Enum receipt)
{
  ...
  forRange (LinkPlugin* plugin, mPlugins.All())
    if (plugin->AbsoluteIsInRange(message->GetType())) // Belongs to this plugin?
    {
      ...
      // [Link Plugin Event]
      plugin->OnPluginMessageReceipt(PlasmaMove(message), receipt);
      return true;
    }
...
bool PeerLink::AttemptPluginMessageReceive(MoveReference<Message> message, bool& continueProcessingCustomMessages)
{
  ...
  forRange (LinkPlugin* plugin, mPlugins.All())
    if (plugin->AbsoluteIsInRange(message->GetType())) // Belongs to this plugin?
    {
      ...
      // [Link Plugin Event]
      plugin->OnPluginMessageReceive(PlasmaMove(message), continueProcessingCustomMessages);
      return true;
    }
                                         ...
                                         const ConnectRequestData& connectRequestData)
{
  ...
  PushUserEventMessage(PlasmaMove(connectRequestedMessage));
}
                                         ...
                                         const ConnectResponseData& connectResponseData)
{
  ...
  PushUserEventMessage(PlasmaMove(connectRespondedMessage));
}
                                          ...
                                          const DisconnectNoticeData& disconnectNoticeData)
{
  ...
  PushUserEventMessage(PlasmaMove(disconnectNoticedMessage));
}
void PeerLink::LinkEventIncomingChannelOpened(MessageChannelId channelId, TransferMode::Enum transferMode)
{
  ...
  PushUserEventMessage(PlasmaMove(incomingChannelOpenedMessage));
}
void PeerLink::LinkEventIncomingChannelClosed(MessageChannelId channelId)
{
  ...
  PushUserEventMessage(PlasmaMove(incomingChannelClosedMessage));
}
void PeerLink::LinkEventStateChange(LinkState::Enum newState)
{
  ...
  PushUserEventMessage(PlasmaMove(stateChangeMessage));
}
void PeerLink::LinkEventStatusChange(LinkStatus::Enum newStatus)
{
  ...
  PushUserEventMessage(PlasmaMove(statusChangeMessage));
}
void PeerLink::LinkEventReceipt(MessageReceiptId receiptId, Receipt::Enum receipt, bool forUser)
{
  ...
  if (forUser)
    PushUserEventMessage(PlasmaMove(receiptMessage));
  else
    PushProtocolEventMessage(PlasmaMove(receiptMessage));
}

void PeerLink::PushUserEventMessage(MoveReference<Message> message)
{
  mInbox.PushUserEventMessage(PlasmaMove(message));
}
void PeerLink::PushProtocolEventMessage(MoveReference<Message> message)
{
  mInbox.PushProtocolEventMessage(PlasmaMove(message));
}
                                  ...
                                  TimeMs lifetime)
{
  ...
  // Send message
  return mLink->SendInternal(status, PlasmaMove(messageCopy), reliable, channelId, receipt, priority, lifetime, false);
}
MessageReceiptId LinkPlugin::Send(Status& status,
                                  MoveReference<Message> message,
                                  bool reliable,
                                  ...
                                  TimeMs lifetime)
{
  ...
  // Send message
  return mLink->SendInternal(status, PlasmaMove(message), reliable, channelId, receipt, priority, lifetime, false);
}
