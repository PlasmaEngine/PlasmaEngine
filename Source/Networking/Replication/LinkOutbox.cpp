...
FragmentedReceipt::FragmentedReceipt(MoveReference<FragmentedReceipt> rhs) :
    ...
    mPacketRecords(PlasmaMove(rhs->mPacketRecords)),
...
LinkOutbox::LinkOutbox(MoveReference<LinkOutbox> rhs)
    ...
    mChannels(PlasmaMove(rhs->mChannels)),
    mChannelIdStore(PlasmaMove(rhs->mChannelIdStore)),

    /// Message Data
    mNextReceiptID(rhs->mNextReceiptID),
    mOutMessages(PlasmaMove(rhs->mOutMessages)),

    /// Packet Data
    mNextSequenceId(rhs->mNextSequenceId),
    mLastSendTime(rhs->mLastSendTime),
    mSentPackets(PlasmaMove(rhs->mSentPackets)),
    mResendPackets(PlasmaMove(rhs->mResendPackets)),
    mFragmentedReceipts(PlasmaMove(rhs->mFragmentedReceipts))
{
}
...
OutMessageChannel* LinkOutbox::OpenOutgoingChannel(TransferMode::Enum transferMode)
{
  ...
  Status status;
  mLink->SendInternal(status, PlasmaMove(message), true, channelId);
...
void LinkOutbox::RecordFragmentReceipt(const OutPacket& packet, const OutMessage& message, const OutPacket* prevPacket)
{
  ...
  if (fragmentedReceiptIter == mFragmentedReceipts.End()) // Doesn't exist?
  {
    ...
    ArraySet<FragmentedReceipt>::pointer_bool_pair result = mFragmentedReceipts.Insert(PlasmaMove(newFragmentedReceipt));
  ...
  forRange (OutMessage& message, packet.GetMessages().All())
  {
    ...
    case ACKState::ACKd:
      ...
      case TransferMode::Ordered:
        ReceiptMessage(PlasmaMove(message), Receipt::ACK);
        break;

      // MAYBE
      case TransferMode::Sequenced:
        ReceiptMessage(PlasmaMove(message), Receipt::MAYBE);
        break;
      }
      break;
    ...
    case ACKState::NAKd:
      ...
      ReceiptMessage(PlasmaMove(message), Receipt::NAK);
      break;
    } // (Receipt message based on ACK state)
  }   // (For all receipted messages in the given packet)
}
...
void LinkOutbox::NAKSentPacket(ArraySet<OutPacket>::iterator& sentPacketIter)
{
  ...
  if (sentPacketIter->HasMessages())
    mResendPackets.PushBack(PlasmaMove(*sentPacketIter)); // Move to resend packets
...
void LinkOutbox::SendPacket(MoveReference<OutPacket> packet)
{
  ...
  mSentPackets.Insert(PlasmaMove(packet));
}
                                         ...
                                         bool isProtocol)
{
  ...
  // Default (plasma) channel?
  if (channelId == 0)
  {
    // Get default channel
    channel = &mDefaultChannel;
  }
  // Non-default (non-plasma) channel?
  else
  {
  ...
  mOutMessages.Insert(OutMessagePtr(new OutMessage(PlasmaMove(message),
                                                   reliable,
                                                   channelId,
                                                   sequenceId,
                                                   transferMode,
...
bool LinkOutbox::WriteMessageToPacket(OutPacket& packet, Bits& remBits, OutMessage& message, OutPacket* prevPacket)
{
      ...
      !message.IsFragment() && message.HasExpired(now))
  {
    ...
    if (message.IsReceipted())
      ReceiptMessage(PlasmaMove(message), Receipt::EXPIRED);
...
LinkOutbox::WriteMessage(OutPacket& packet, OutMessage& message, Bits& remBits, bool isResendMessage)
{
  ...
  if (isResendMessage && message.IsFragment())
  {
    ...
    if (messageSize <= remBits)
    {
      ...
      packet.mMessages.PushBack(PlasmaMove(message));
  ...
  else
  {
         ...
         (message.IsCustomType() ? !packet.HasCustomMessages() : !packet.HasProtocolMessages())))
    {
      ...
      // Write fragment message
      packet.mMessages.PushBack(PlasmaMove(fragment));
...
void LinkOutbox::Update(const ACKArray& remoteACKs, const NAKArray& remoteNAKs)
{
  ...
  LinkOutbox_Update_DoneWritingResendMessages:
    ...
    if (newPacket.HasMessages() || GetDuration(mLastSendTime, now) > RATE_TO_INTERVAL(mLink->GetHeartbeatPacketRate()))
      SendPacket(PlasmaMove(newPacket)); // Send new Packet
    else
...
void LinkOutbox::RemoveExpiredMessages(OutPacket& packet)
{
  ...
  for (Array<OutMessage>::iterator iter = messages.Begin(); iter != messages.End();)
  {
        ...
        !iter->IsFragment() && iter->HasExpired(now))
    {
      // Receipt expired message
      if (iter->IsReceipted())
        ReceiptMessage(PlasmaMove(*iter), Receipt::EXPIRED);
...
void LinkOutbox::ReceiptMessage(MoveReference<OutMessage> message, Receipt::Enum receipt)
{
  ...
  if (mLink->AttemptPluginMessageReceipt(PlasmaMove(message),
                                         receipt)) // Successful?
    return;
