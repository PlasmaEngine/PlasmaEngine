...
LightningDefineType(SoundEntryDisplay, builder, type)
{
}
...
LightningDefineType(SoundEntry, builder, type)
{
  ...
  LightningBindGetterSetterProperty(Sound);
  LightningBindGetterSetterProperty(Weight);
  LightningBindGetterSetterProperty(StartTime);
  LightningBindGetterSetterProperty(EndTime);
  LightningBindGetterSetterProperty(LoopStartTime);
  LightningBindGetterSetterProperty(LoopEndTime);
  LightningBindGetterSetterProperty(LoopTailLength);
  LightningBindFieldProperty(mCrossFadeLoopTail);
  LightningBindMethodProperty(Preview);
  LightningBindMethodProperty(StopPreview);
}
...
void SoundEntry::Preview()
{
  ...
  if (!status.Failed())
  {
    ...
    instance->Play(false, PL::gSound->mOutputNode, false);
    // Save the SoundInstance
    PL::gSound->mPreviewInstance = instance;
  }
  else
  {
...
LightningDefineType(SoundTagEntryDisplay, builder, type)
{
}
...
LightningDefineType(SoundTagEntry, builder, type)
{
  PlasmaBindDocumented();
  type->HandleManager = LightningManagerId(PointerManager);
  ...
  LightningBindGetterSetterProperty(SoundTag);
}
...
LightningDefineType(SoundCueDisplay, builder, type)
{
}
...
LightningDefineType(SoundCue, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetterProperty(PlayMode);
  LightningBindGetterSetterProperty(SelectMode);
  LightningBindGetterSetterProperty(Volume)->Add(new EditorSlider(0.0f, 2.0f, 0.01f));
  LightningBindGetterSetterProperty(Decibels)->Add(new EditorSlider(-32.0f, 6.0f, 0.1f));
  LightningBindFieldProperty(mUseDecibelVariation)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindGetterSetterProperty(VolumeVariation)
      ->Add(new EditorSlider(0.0f, 1.0f, 0.01f))
      ->PlasmaFilterNotBool(mUseDecibelVariation);
  LightningBindGetterSetterProperty(DecibelVariation)
      ->Add(new EditorSlider(0.0f, 6.0f, 0.1f))
      ->PlasmaFilterBool(mUseDecibelVariation);
  LightningBindGetterSetterProperty(Pitch)->Add(new EditorSlider(-2.0f, 2.0f, 0.1f));
  LightningBindGetterSetterProperty(Semitones)->Add(new EditorSlider(-24.0f, 24.0f, 0.1f));
  LightningBindFieldProperty(mUseSemitoneVariation)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindGetterSetterProperty(PitchVariation)
      ->Add(new EditorSlider(0.0f, 1.0f, 0.1f))
      ->PlasmaFilterNotBool(mUseSemitoneVariation);
  LightningBindGetterSetterProperty(SemitoneVariation)
      ->Add(new EditorSlider(0.0f, 12.0f, 0.1f))
      ->PlasmaFilterBool(mUseSemitoneVariation);
  LightningBindGetterSetterProperty(Attenuator);
  LightningBindFieldProperty(mShowMusicOptions)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindGetterSetterProperty(BeatsPerMinute)->PlasmaFilterBool(mShowMusicOptions);
  LightningBindGetterSetterProperty(TimeSigBeats)->PlasmaFilterBool(mShowMusicOptions);
  LightningBindGetterSetterProperty(TimeSigValue)->PlasmaFilterBool(mShowMusicOptions);

  LightningBindMethodProperty(Preview);
  LightningBindMethodProperty(StopPreview);

  LightningBindFieldProperty(Sounds);
  LightningBindFieldProperty(SoundTags);

  LightningBindMethod(AddSoundTagEntry);
  LightningBindMethod(AddSoundEntry);
  LightningBindMethod(PlayCueOnNode);

  PlasmaBindEvent(Events::SoundCuePostPlay, SoundInstanceEvent);
  PlasmaBindEvent(Events::SoundCuePrePlay, SoundInstanceEvent);
...
void SoundCue::Preview()
{
  ...
  PL::gSound->mPreviewInstance = PlayCue(nullptr, nullptr, false);
  // Check if it successfully played
  SoundInstance* instance = PL::gSound->mPreviewInstance;
  if (instance)
  {
    // Connect it to the audio system's output
    PL::gSound->mOutputNode->AddInputNode(instance);
  }
}
...
HandleOf<SoundInstance> SoundCue::PlayCue(SoundSpace* space, HandleOf<SoundNode> outputNode, bool startPaused)
{
  ...
  if (mUseDecibelVariation && mDecibelVariation > 0)
    volume = DecibelsToVolume(PL::gSound->mRandom.FloatVariance(GetDecibels(), mDecibelVariation));
  else if (!mUseDecibelVariation && mVolumeVariation > 0)
    volume = PL::gSound->mRandom.FloatVariance(mVolume, mVolumeVariation);

  // Get pitch value
  float pitch(mPitch);
  if (mUseSemitoneVariation && mSemitoneVariation > 0)
    pitch = SemitonesToPitch(PL::gSound->mRandom.FloatVariance(GetSemitones(), mSemitoneVariation));
  else if (!mUseSemitoneVariation && mPitchVariation > 0)
    pitch = PL::gSound->mRandom.FloatVariance(mPitch, mPitchVariation);
  ...
  else if (mSelectMode == SoundSelectMode::Random)
  {
    ...
    entry = weightedtable.Sample(PL::gSound->mRandom);
  }
