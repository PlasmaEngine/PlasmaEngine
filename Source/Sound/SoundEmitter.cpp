...
LightningDefineType(SoundEmitterDisplay, builder, type)
{
}
...
LightningDefineType(SoundEmitter, builder, type)
{
  PlasmaBindComponent();
  ...
  LightningBindFieldProperty(mDirectional)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindGetterSetterProperty(EmitAngle)->Add(new EditorSlider(0.0f, 360.0f, 1.0f))->PlasmaFilterBool(mDirectional);
  LightningBindGetterSetterProperty(RearVolume)->Add(new EditorSlider(0.0f, 1.0f, 0.1f))->PlasmaFilterBool(mDirectional);
...
void SoundEmitter::SetPaused(bool pause)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&EmitterNode::SetPausedThreaded, node, mIsPaused), node);
}
...
void SoundEmitter::SetEmitAngle(float angleInDegrees)
{
  ...
  EmitterNode* node = mEmitterObject;
  PL::gSound->Mixer.AddTask(CreateFunctor(&EmitterNode::SetDirectionalAngleThreaded, node, mEmitAngle, mRearVolume),
                           node);
}
...
Lightning::HandleOf<Plasma::SoundNode> SoundEmitter::GetSoundNodeInput()
{
...
Lightning::HandleOf<Plasma::SoundNode> SoundEmitter::GetSoundNodeOutput()
{
...
void SoundEmitter::Update(float dt)
{
  ...
  if (velocity != Vec3(0, 0, 0))
  {
    ...
    PL::gSound->Mixer.AddTask(CreateFunctor(&EmitterNode::SetPositionThreaded, node, newPosition, velocity), node);
  ...
  if (mDirectional)
  {
    ...
    PL::gSound->Mixer.AddTask(CreateFunctor(&EmitterNode::SetForwardDirectionThreaded, node, forward), node);
  }
...
HandleOf<SoundInstance> SoundEmitter::PlayCueInternal(SoundCue* cue, bool startPaused)
{
  ...
  if (attenuator && attenuator->Name != "DefaultNoAttenuation")
  {
    ...
    if (!attenuatorNode)
    {
      attenuatorNode = attenuator->GetAttenuationNode("CueAttenuator", PL::gSound->mCounter++);
      if (attenuatorNode)
      {
...
void SoundEmitter::SetUpAttenuatorNode(SoundAttenuator* newAttenuator)
{
  ...
  else
  {
    ...
    if (mAttenuatorNode && oldAttenuator->Name != "DefaultNoAttenuation")
    {
      ...
      mAttenuatorNode = newAttenuator->GetAttenuationNode("EmitterAttenuator", PL::gSound->mCounter++);
