...
LightningDefineType(ListenerNode, builder, type)
{
}

ListenerNode::ListenerNode(Plasma::StringParam name, unsigned ID, ListenerWorldPositionInfo positionInfo) :
...
{
  PL::gSound->Mixer.SendListenerRemovedEvent(this);
}

void ListenerNode::SetPositionData(ListenerWorldPositionInfo positionInfo)
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&ListenerNode::SetPositionDataThreaded, this, positionInfo), this);
}

void ListenerNode::SetActive(const bool active)
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&ListenerNode::SetActiveThreaded, this, active), this);
}
