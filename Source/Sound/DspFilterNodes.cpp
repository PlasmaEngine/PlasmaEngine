...
LightningDefineType(VolumeNode, builder, type)
{
...
void VolumeNode::InterpolateVolume(float volume, float time)
{
  ...
  else
  {
    ...
    PL::gSound->Mixer.AddTask(CreateFunctor(&VolumeNode::InterpolateVolumeThreaded, this, volume, time), this);
  }
}
...
LightningDefineType(PanningNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetter(SumToMono);
  LightningBindGetterSetter(LeftVolume);
  LightningBindGetterSetter(RightVolume);
  LightningBindMethod(InterpolateLeftVolume);
  LightningBindMethod(InterpolateRightVolume);
  LightningBindMethod(InterpolateVolumes);
}
...
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&PanningNode::SetVolumeThreaded,
                                         this,
                                         true,
...
LightningDefineType(PitchNode, builder, type)
{
  PlasmaBindDocumented();
...
{
  pitchRatio = Math::Clamp(pitchRatio, cMinPitchValue, cMaxPitchValue);
  PL::gSound->Mixer.AddTask(CreateFunctor(&PitchNode::SetPitchThreaded, this, PitchToSemitones(pitchRatio), time), this);
}
...
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&PitchNode::SetPitchThreaded, this, pitchSemitones, time), this);
}
                                 ...
                                 const bool firstRequest)
{
  ...
  // If no valid input, set the last samples to plasma and return
...
LightningDefineType(LowPassNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetter(CutoffFrequency);
}
...
void LowPassNode::SetCutoffFrequency(float frequency)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&LowPassNode::SetCutoffFrequencyThreaded, this, frequency), this);
}
...
LightningDefineType(HighPassNode, builder, type)
{
  PlasmaBindDocumented();
...
{
  mCutoffFrequency.Set(frequency, AudioThreads::MainThread);
  PL::gSound->Mixer.AddTask(CreateFunctor(&HighPassNode::SetCutoffFrequencyThreaded, this, frequency), this);
}
...
LightningDefineType(BandPassNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetter(CentralFrequency);
  LightningBindGetterSetter(QualityFactor);
}
...
void BandPassNode::SetCentralFrequency(float frequency)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&BandPassNode::SetCentralFrequencyThreaded, this, frequency), this);
}
...
void BandPassNode::SetQualityFactor(float Q)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&BandPassNode::SetQualityFactorThreaded, this, Q), this);
}
...
LightningDefineType(EqualizerNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetter(LowPassGain);
  LightningBindGetterSetter(HighPassGain);
  LightningBindGetterSetter(Band1Gain);
  LightningBindGetterSetter(Band2Gain);
  LightningBindGetterSetter(Band3Gain);
  LightningBindMethod(InterpolateAllBands);
}
...
void EqualizerNode::SetLowPassGain(float gain)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&EqualizerNode::SetBandGainThreaded,
                                         this,
    ...
    float lowPass, float band1, float band2, float band3, float highPass, float timeToInterpolate)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&EqualizerNode::InterpolateAllThreaded, this, values, timeToInterpolate),
                           this);
}
...
LightningDefineType(ReverbNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetter(Length);
  LightningBindGetterSetter(WetPercent)->AddAttribute(DeprecatedAttribute);
  LightningBindGetterSetter(WetValue);
  LightningBindMethod(InterpolateWetPercent)->AddAttribute(DeprecatedAttribute);
  LightningBindMethod(InterpolateWetValue);
}
...
void ReverbNode::SetLength(float time)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&ReverbNode::SetLengthMsThreaded, this, time * 1000.0f), this);
}
...
void ReverbNode::SetWetPercent(float percent)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&ReverbNode::SetWetValueThreaded, this, percent), this);
}
...
void ReverbNode::SetWetValue(float value)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&ReverbNode::SetWetValueThreaded, this, value), this);
}
...
void ReverbNode::InterpolateWetPercent(float percent, float time)
{
  percent = Math::Clamp(percent, 0.0f, 100.0f) / 100.0f;
  mWetLevelValue.Set(percent, AudioThreads::MainThread);
  PL::gSound->Mixer.AddTask(CreateFunctor(&ReverbNode::InterpolateWetValueThreaded, this, percent, time), this);
}
...
void ReverbNode::InterpolateWetValue(float value, float time)
{
  value = Math::Clamp(value, 0.0f, 1.0f);
  mWetLevelValue.Set(value, AudioThreads::MainThread);
  PL::gSound->Mixer.AddTask(CreateFunctor(&ReverbNode::InterpolateWetValueThreaded, this, value, time), this);
}
...
LightningDefineType(DelayNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetter(Delay);
  LightningBindGetterSetter(FeedbackPercent)->AddAttribute(DeprecatedAttribute);
  LightningBindGetterSetter(FeedbackValue);
  LightningBindGetterSetter(WetPercent)->AddAttribute(DeprecatedAttribute);
  LightningBindGetterSetter(WetValue);
  LightningBindMethod(InterpolateWetPercent)->AddAttribute(DeprecatedAttribute);
  LightningBindMethod(InterpolateWetValue);
...
void DelayNode::SetDelay(float seconds)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&DelayNode::SetDelayMsThreaded, this, seconds * 1000.0f), this);
}
...
void DelayNode::SetFeedbackPercent(float feedback)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&DelayNode::SetFeedbackThreaded, this, feedback), this);
}
...
void DelayNode::SetWetPercent(float wetLevel)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&DelayNode::SetWetValueThreaded, this, wetLevel), this);
}
                                 ...
                                 const bool firstRequest)
{
  ...
  // If there is no input data make sure the array is plasmaed out
  if (!isThereInput)
...
LightningDefineType(FlangerNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetter(MaxDelayMillisec);
  LightningBindGetterSetter(ModulationFrequency);
  LightningBindGetterSetter(FeedbackPercent)->AddAttribute(DeprecatedAttribute);
  LightningBindGetterSetter(FeedbackValue);
}
...
void FlangerNode::SetModulationFrequency(float frequency)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&FlangerNode::SetModFreqThreaded, this, frequency), this);
}
...
void FlangerNode::SetFeedbackPercent(float percent)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&FlangerNode::SetFeedbackThreaded, this, percent), this);
}
...
void FlangerNode::SetFeedbackValue(float value)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&FlangerNode::SetFeedbackThreaded, this, value), this);
}
...
{
  Delay.SetMaxDelayMSec(maxDelayMs, PL::gSound->Mixer.mSystemChannels.Get(AudioThreads::MixThread));
...
LightningDefineType(ChorusNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetter(MaxDelayMillisec);
  LightningBindGetterSetter(MinDelayMillisec);
  LightningBindGetterSetter(ModulationFrequency);
  LightningBindGetterSetter(FeedbackPercent)->AddAttribute(DeprecatedAttribute);
  LightningBindGetterSetter(FeedbackValue);
  LightningBindGetterSetter(OffsetMillisec);
}
...
void ChorusNode::SetModulationFrequency(float frequency)
{
  ...
  mModFrequency.Set(frequency, AudioThreads::MainThread);
  PL::gSound->Mixer.AddTask(CreateFunctor(&ChorusNode::SetModFreqThreaded, this, frequency), this);
}
...
void ChorusNode::SetFeedbackPercent(float percent)
{
  percent = Math::Clamp(percent, 0.0f, 100.0f) / 100.0f;
  mFeedback.Set(percent, AudioThreads::MainThread);
  PL::gSound->Mixer.AddTask(CreateFunctor(&ChorusNode::SetFeedbackThreaded, this, percent), this);
}
...
void ChorusNode::SetFeedbackValue(float value)
{
  value = Math::Clamp(value, 0.0f, 1.0f);
  mFeedback.Set(value, AudioThreads::MainThread);
  PL::gSound->Mixer.AddTask(CreateFunctor(&ChorusNode::SetFeedbackThreaded, this, value), this);
}
...
LightningDefineType(CompressorNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetter(InputGainDecibels);
  LightningBindGetterSetter(ThresholdDecibels);
  LightningBindGetterSetter(AttackMillisec);
  LightningBindGetterSetter(ReleaseMillisec);
  LightningBindGetterSetter(Ratio);
  LightningBindGetterSetter(OutputGainDecibels);
  LightningBindGetterSetter(KneeWidth);
}
...
void CompressorNode::SetInputGainDecibels(float dB)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&DynamicsProcessor::SetInputGain, &Filter, mInputGainDB), this);
}
...
void CompressorNode::SetThresholdDecibels(float dB)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&DynamicsProcessor::SetThreshold, &Filter, mThresholdDB), this);
}
...
void CompressorNode::SetAttackMillisec(float attack)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&DynamicsProcessor::SetAttackMSec, &Filter, mAttackMSec), this);
}
...
void CompressorNode::SetReleaseMillisec(float release)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&DynamicsProcessor::SetReleaseMSec, &Filter, mReleaseMSec), this);
}
...
void CompressorNode::SetRatio(float ratio)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&DynamicsProcessor::SetRatio, &Filter, mRatio), this);
}
...
void CompressorNode::SetOutputGainDecibels(float dB)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&DynamicsProcessor::SetOutputGain, &Filter, mOutputGainDB), this);
}
...
void CompressorNode::SetKneeWidth(float knee)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&DynamicsProcessor::SetKneeWidth, &Filter, mKneeWidth), this);
}
...
LightningDefineType(ExpanderNode, builder, type)
{
  PlasmaBindDocumented();
...
LightningDefineType(AddNoiseNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetter(AdditiveGain);
  LightningBindGetterSetter(MultiplicativeGain);
  LightningBindGetterSetter(AdditiveCutoff);
  LightningBindGetterSetter(MultiplicativeCutoff);
}
...
void AddNoiseNode::SetAdditiveGain(float decibels)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&AddNoiseNode::mAddGainThreaded, this, ValueFromDecibels(mAdditiveNoiseDB)),
                           this);
}
...
LightningDefineType(ModulationNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetter(UseAmplitudeModulation);
  LightningBindGetterSetter(Frequency);
  LightningBindGetterSetter(WetPercent)->AddAttribute(DeprecatedAttribute);
  LightningBindGetterSetter(WetValue);
}
...
void ModulationNode::SetUseAmplitudeModulation(bool useAmplitude)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&ModulationNode::SetUseAmplitudeThreaded, this, useAmplitude), this);
}
