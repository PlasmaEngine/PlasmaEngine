...
LightningDefineType(GeneratedWaveNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetter(WaveType);
  LightningBindGetterSetter(WaveFrequency);
  LightningBindGetterSetter(Volume);
  LightningBindGetterSetter(Decibels);
  LightningBindGetterSetter(SquareWavePulseValue);
  ...
  LightningBindMethod(InterpolateWaveFrequency);
}
...
void GeneratedWaveNode::SetWaveType(SynthWaveType::Enum newType)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&Oscillator::SetType, &WaveDataThreaded, newType), this);
}
...
void GeneratedWaveNode::SetWaveFrequency(float frequency)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&Oscillator::SetFrequency, &WaveDataThreaded, frequency), this);
}
...
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&GeneratedWaveNode::InterpolateVolumeThreaded,
                                         this,
...
LightningDefineType(AdsrEnvelope, builder, type)
{
  LightningBindDefaultConstructor();
  PlasmaBindDocumented();
  type->CreatableInScript = true;

  LightningBindField(mDelayTime);
  LightningBindField(mAttackTime);
  LightningBindField(mDecayTime);
  LightningBindField(mSustainTime);
  LightningBindField(mSustainLevel);
  LightningBindField(mReleaseTime);
}
...
LightningDefineType(AdditiveSynthNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindMethod(AddHarmonic);
  LightningBindMethod(RemoveAllHarmonics);
  LightningBindMethod(NoteOn);
  LightningBindMethod(NoteOff);
  LightningBindMethod(StopAllNotes);
}
...
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&HarmonicDataListType::Clear, &HarmonicsListThreaded), this);
}
...
void AdditiveSynthNode::NoteOn(float midiNote, float volume)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&AdditiveSynthNode::NoteOnThreaded, this, (int)midiNote, volume), this);
}

void AdditiveSynthNode::NoteOff(float midiNote)
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&AdditiveSynthNode::NoteOffThreaded, this, (int)midiNote), this);
}

void AdditiveSynthNode::StopAllNotes()
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&AdditiveSynthNode::StopAllNotesThreaded, this), this);
}
...
LightningDefineType(MicrophoneInputNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetterSetter(Volume);
  LightningBindGetterSetter(Active);
}
...
void MicrophoneInputNode::SetActive(bool active)
{
  ...
  if (active && !PL::gSound->Mixer.StartInput())
  {
  ...
  // Otherwise we are activating
  {
    ...
    PL::gSound->Mixer.AddTask(CreateFunctor(&InterpolatingObject::SetValues,
...
bool MicrophoneInputNode::GetOutputSamples(BufferType* outputBuffer,
                                           ...
                                           const bool firstRequest)
{
  if (!mActive.Get(AudioThreads::MixThread) || PL::gSound->Mixer.InputBuffer.Empty())
    return false;

  unsigned bufferSize = outputBuffer->Size();

  // Start with the number of samples in the system's input buffer
  unsigned samplesToCopy = PL::gSound->Mixer.InputBuffer.Size();
  ...
  memcpy(outputBuffer->Data(), PL::gSound->Mixer.InputBuffer.Data(), sizeof(float) * samplesToCopy);
...
LightningDefineType(GranularSynthNode, builder, type)
{
  PlasmaBindDocumented();

  LightningBindMethod(SetSound);
  LightningBindMethod(Play);
  LightningBindMethod(Stop);
  LightningBindGetterSetterProperty(GrainVolume);
  LightningBindGetterSetterProperty(GrainVolumeVariance);
  LightningBindGetterSetterProperty(GrainDelay);
  LightningBindGetterSetterProperty(GrainDelayVariance);
  LightningBindGetterSetterProperty(GrainLength);
  LightningBindGetterSetterProperty(GrainLengthVariance);
  LightningBindGetterSetterProperty(GrainResampleRate);
  LightningBindGetterSetterProperty(GrainResampleRateVariance);
  LightningBindGetterSetterProperty(BufferScanRate);
  LightningBindGetterSetterProperty(GrainPanningValue);
  LightningBindGetterSetterProperty(GrainPanningVariance);
  LightningBindGetterSetterProperty(RandomLocationValue);
  LightningBindGetterSetterProperty(WindowType);
  LightningBindGetterSetterProperty(WindowAttack);
  LightningBindGetterSetterProperty(WindowRelease);
}
...
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&GranularSynthNode::SetSoundThreaded, this, sound, startTime, stopTime), this);
}
...
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&GranularSynthNode::SetGrainLengthThreaded, this, lengthMS), this);
}
