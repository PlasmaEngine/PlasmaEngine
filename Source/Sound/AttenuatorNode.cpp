...
LightningDefineType(AttenuatorNode, builder, type)
{
}
...
void AttenuatorNode::SetStartDistance(float distance)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&AttenuatorNode::UpdateDistanceInterpolator, this), this);
}
...
void AttenuatorNode::SetCurveType(const FalloffCurveType::Enum type, Array<Math::Vec3>* customCurveData)
{
  if (customCurveData)
    ...
    // another curve
    PL::gSound->Mixer.AddTask(CreateFunctor(&InterpolatingObject::SetCustomCurve,
                                           &DistanceInterpolator,
                                           new Array<Math::Vec3>(*customCurveData)),
                             this);
  else
    PL::gSound->Mixer.AddTask(CreateFunctor(&InterpolatingObject::SetCurve, &DistanceInterpolator, type), this);
}
...
void AttenuatorNode::SetLowPassDistance(const float distance)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&AttenuatorNode::UpdateLowPassInterpolator, this), this);
}
                                      ...
                                      const bool firstRequest)
{
  ...
  // If we are outside the max distance and the minimum volume is plasma, there is
  // no audio
