...
LightningDefineType(SoundInstanceEvent, builder, type)
{
  PlasmaBindDocumented();

  LightningBindGetter(SoundInstance);
}
...
LightningDefineType(SoundInstance, builder, type)
{
  PlasmaBindDocumented();
  ...
  LightningBindGetterSetter(Looping);
  LightningBindGetterSetter(Time);
  LightningBindGetter(FileLength);
  LightningBindGetterSetter(EndTime);
  LightningBindGetterSetter(LoopStartTime);
  LightningBindGetterSetter(LoopEndTime);
  LightningBindGetterSetter(LoopTailTime);
  LightningBindGetterSetter(CrossFadeLoopTail);
  LightningBindGetterSetter(CustomEventTime);
  LightningBindGetter(SoundName);

  PlasmaBindEvent(Events::SoundLooped, SoundInstanceEvent);
  PlasmaBindEvent(Events::SoundStopped, SoundInstanceEvent);
  PlasmaBindEvent(Events::MusicBeat, SoundInstanceEvent);
  PlasmaBindEvent(Events::MusicBar, SoundInstanceEvent);
  PlasmaBindEvent(Events::MusicEighthNote, SoundInstanceEvent);
  PlasmaBindEvent(Events::MusicQuarterNote, SoundInstanceEvent);
  PlasmaBindEvent(Events::MusicHalfNote, SoundInstanceEvent);
  PlasmaBindEvent(Events::MusicWholeNote, SoundInstanceEvent);
  PlasmaBindEvent(Events::MusicCustomTime, SoundInstanceEvent);
}

SoundInstance::SoundInstance(Status& status, SoundSpace* space, SoundAsset* asset, float volume, float pitch) :
    SimpleCollapseNode("SoundInstance", PL::gSound->mCounter++, false, true),
    mAssetObject(asset),
    mSpace(space),
    mCurrentTime(0.0),
    mLooping(cFalse),
    ...
    mSavedOutputVersionThreaded(PL::gSound->Mixer.mMixVersionThreaded - 1)
{
...
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&SoundInstance::SetVolumeThreaded,
                                         this,
...
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&SoundInstance::SetPitchThreaded,
                                         this,
...
void SoundInstance::InterpolateSemitones(float newSemitones, float interpolationTime)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&SoundInstance::SetPitchThreaded, this, newSemitones, interpolationTime),
                           this);
}
...
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&SoundInstance::SetPausedThreaded, this, pause), this);
}

void SoundInstance::Stop()
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&SoundInstance::StopThreaded, this), this);
}
...
void SoundInstance::SetTime(float seconds)
{
  ...
  if (!mAssetObject->mStreaming)
    PL::gSound->Mixer.AddTask(CreateFunctor(&SoundInstance::SetTimeThreaded, this, seconds), this);
  else
...
void SoundInstance::SetEndTime(float seconds)
{
  ...
  // If the end time is at plasma or past the end of the file, set it to the
  // length of the file
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&SoundInstance::mEndFrameThreaded, this, frame), this);
}
...
void SoundInstance::SetLoopStartTime(float seconds)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&SoundInstance::mLoopStartFrameThreaded, this, frame), this);
}
...
void SoundInstance::SetLoopEndTime(float seconds)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&SoundInstance::mLoopEndFrameThreaded, this, frame), this);
}
...
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&SoundInstance::SetBeatsPerMinuteThreaded, this, beats), this);
}

void SoundInstance::SetTimeSignature(float beats, float noteType)
{
  PL::gSound->Mixer.AddTask(CreateFunctor(&SoundInstance::SetTimeSignatureThreaded, this, beats, noteType), this);
}
...
bool SoundInstance::GetOutputForThisMixThreaded(BufferType* outputBuffer, const unsigned numberOfChannels)
{
  ...
  {
    // Set the mix version
    mSavedOutputVersionThreaded = PL::gSound->Mixer.mMixVersionThreaded;
    ...
    while (!sampleRange.Empty())
    {
      ...
      if (mInterpolatingVolumeThreaded)
      {
        ...
        if (!mInterpolatingVolumeThreaded)
        {
          ...
          PL::gSound->Mixer.AddTaskThreaded(CreateFunctor(&SoundInstance::DispatchEventFromMixThread,
                                                         (SoundNode*)this,
...
void SoundInstance::FinishedCleanUpThreaded()
{
  ...
  PL::gSound->Mixer.AddTaskThreaded(CreateFunctor(&SoundInstance::RemoveFromAllTagsThreaded, this), this);
  PL::gSound->Mixer.AddTaskThreaded(CreateFunctor(&SoundNode::RemoveAllOutputs, (SoundNode*)this), this);

  PL::gSound->Mixer.AddTaskThreaded(CreateFunctor(&SoundAsset::RemoveInstance, *mAssetObject, cNodeID), this);
}
...
bool SoundInstance::BelowMinimumVolumeThreaded(unsigned frames)
{
  ...
  if (volume1 < PL::gSound->Mixer.mMinimumVolumeThresholdThreaded &&
      volume2 < PL::gSound->Mixer.mMinimumVolumeThresholdThreaded)
    return true;
  else
    return false;
}
...
void SoundInstance::DisconnectThisAndAllInputs()
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&SoundInstance::FinishedCleanUpThreaded, this), this);
}
