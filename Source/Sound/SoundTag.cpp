...
LightningDefineType(TagObject, builder, type)
{
}
...
TagObject::TagObject() :
    ...
    mMixVersionThreaded(PL::gSound->Mixer.mMixVersionThreaded - 1),
    mVolume(1.0f),
...
void TagObject::SetVolume(float volume, float time)
{
  ...
  PL::gSound->Mixer.AddTask(CreateFunctor(&TagObject::SetVolumeThreaded, this, volume, time), nullptr);
}
...
void TagObject::SetEQBandGain(EqualizerBands::Enum whichBand, float gain)
{
  ...
  // TODO -- could this crash because it's not saving a handle?
  PL::gSound->Mixer.AddTask(CreateFunctor(&TagObject::SetEQBandGainThreaded, this, whichBand, gain), nullptr);
}
...
void TagObject::SetCompressorThresholdDb(float decibels)
{
  ...
  // TODO no handle
  PL::gSound->Mixer.AddTask(CreateFunctor(&DynamicsProcessor::SetThreshold, &CompressorObjectThreaded, decibels),
                           nullptr);
}
...
void TagObject::SetCompressorAttackMs(float milliseconds)
{
  ...
  // TODO no handle
  PL::gSound->Mixer.AddTask(CreateFunctor(&DynamicsProcessor::SetAttackMSec, &CompressorObjectThreaded, milliseconds),
                           nullptr);
}
...
void TagObject::SetCompressorReleaseMs(float milliseconds)
{
  ...
  // TODO no handle
  PL::gSound->Mixer.AddTask(CreateFunctor(&DynamicsProcessor::SetReleaseMSec, &CompressorObjectThreaded, milliseconds),
                           nullptr);
}
...
void TagObject::SetCompressorRatio(float ratio)
{
  ...
  // TODO no handle
  PL::gSound->Mixer.AddTask(CreateFunctor(&DynamicsProcessor::SetRatio, &CompressorObjectThreaded, ratio), nullptr);
}
...
void TagObject::SetCompressorKneeWidth(float knee)
{
  ...
  // TODO no handle
  PL::gSound->Mixer.AddTask(CreateFunctor(&DynamicsProcessor::SetKneeWidth, &CompressorObjectThreaded, knee), nullptr);
}
...
void TagObject::ProcessInstanceThreaded(BufferType* instanceOutput, unsigned channels, SoundInstance* instance)
{
  ...
  if (mMixVersionThreaded != PL::gSound->Mixer.mMixVersionThreaded)
...
void TagObject::UpdateForMixThreaded(unsigned howManyFrames, unsigned channels)
{
  ...
  mMixVersionThreaded = PL::gSound->Mixer.mMixVersionThreaded;
}
...
LightningDefineType(SoundTagDisplay, builder, type)
{
}
...
LightningDefineType(SoundTag, builder, type)
{
  ...
  LightningBindGetterSetter(UseEqualizer);
  LightningBindGetterSetter(EQLowPassGain);
  LightningBindGetterSetter(EQBand1Gain);
  LightningBindGetterSetter(EQBand2Gain);
  LightningBindGetterSetter(EQBand3Gain);
  LightningBindGetterSetter(EQHighPassGain);
  LightningBindGetterSetter(UseCompressor);
  LightningBindGetterSetter(CompressorThreshold);
  LightningBindGetterSetter(CompressorAttack);
  LightningBindGetterSetter(CompressorRelease);
  LightningBindGetterSetter(CompressorRatio);
  LightningBindGetterSetter(CompressorKneeWidth);
  LightningBindGetterSetter(InstanceLimit);
  LightningBindGetter(InstanceCount);
  LightningBindGetterSetter(Paused);
  LightningBindGetter(Instances);
  LightningBindMethod(TagSound);
  LightningBindMethod(UnTagSound);
  LightningBindMethod(StopSounds);
  LightningBindMethod(InterpolateVolume);
  LightningBindMethod(InterpolateDecibels);
  LightningBindMethod(EQSetAllBands);
  LightningBindGetterSetter(TagForDucking);

  PlasmaBindEvent(Events::AddedInstanceToTag, SoundEvent);
  PlasmaBindEvent(Events::TagHasNoInstances, SoundEvent);
}
...
SoundTag::~SoundTag()
{
  PL::gSound->mSoundTags.Erase(this);
...
void SoundTag::TagSound(HandleOf<SoundInstance>& instanceHandle)
{
  ...
  if (tag)
    PL::gSound->Mixer.AddTask(CreateFunctor(&TagObject::AddInstanceThreaded, tag, instance), nullptr);
...
void SoundTag::UnTagSound(HandleOf<SoundInstance>& instanceHandle)
{
  ...
  if (mTagObject)
    PL::gSound->Mixer.AddTask(CreateFunctor(&TagObject::RemoveInstanceThreaded, *mTagObject, instance), nullptr);
