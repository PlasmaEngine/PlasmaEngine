...
LightningDefineType(CastFilterEvent, builder, type)
{
  PlasmaBindDocumented();
  LightningBindGetterProperty(Object);
  LightningBindFieldProperty(mFilterState);
}
...
LightningDefineType(CastFilter, builder, type)
{
  ...
  LightningBindGetterSetterProperty(IgnoreCog);
  LightningBindGetterSetterProperty(CollisionGroup);

  PlasmaBindEvent(Events::CastFilterCallback, CastFilterEvent);
  LightningBindFieldProperty(mCallbackObject);
  LightningBindFieldProperty(mCallbackEventName);
}
...
LightningDefineType(CastResult, builder, type)
{
  LightningBindDefaultCopyDestructor();
  PlasmaBindDocumented();

  LightningBindGetterProperty(ObjectHit);
  LightningBindGetterProperty(Collider);
  LightningBindGetterProperty(WorldPosition);
  LightningBindGetterProperty(Normal);
  LightningBindGetterProperty(Distance);
  LightningBindMethod(GetLocalPosition);
}
...
LightningDefineType(CastResults, builder, type)
{
