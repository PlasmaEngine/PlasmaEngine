...
void ThreadedDrawFragment(ListType& jointList, ByteColor& color)
{
  ...
  while (!jointRange.Empty())
  {
    ...
    Plasma::Vec3 posA = joint.GetCollider(0)->GetWorldTranslation();
    Plasma::Vec3 posB = joint.GetCollider(1)->GetWorldTranslation();
    Plasma::gDebugDraw->Add(Plasma::Debug::Line(posA, posB).Color(color));
  }
