...
LightningDefineType(DragEffect, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDocumented();

  LightningBindGetterSetterProperty(LinearDamping)->PlasmaSerialize(real(0.2));
  LightningBindGetterSetterProperty(AngularDamping)->PlasmaSerialize(real(0.1));
  LightningBindGetterSetterProperty(LinearDrag)->PlasmaSerialize(real(0));
  LightningBindGetterSetterProperty(AngularDrag)->PlasmaSerialize(real(0));
}
