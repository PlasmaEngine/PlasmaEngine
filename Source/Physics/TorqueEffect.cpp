...
LightningDefineType(TorqueEffect, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDocumented();

  LightningBindGetterSetterProperty(LocalTorque)->PlasmaSerialize(true);
  LightningBindGetterSetterProperty(TorqueStrength)->PlasmaSerialize(real(1));
  LightningBindGetterSetterProperty(TorqueAxis)->PlasmaSerialize(Vec3::cYAxis);
  LightningBindGetterProperty(WorldTorqueAxis);
}
