...
{
public:
  LightningDeclareType(CustomJointEvent, TypeCopyMode::ReferenceType);
...
/// enforce that the relative velocities along the Jacobian are equal to plasma
/// (ignoring error correction or motors).
struct CustomConstraintInfo : public ReferenceCountedEventObject
{
  LightningDeclareType(CustomConstraintInfo, TypeCopyMode::ReferenceType);
