...
LightningDefineType(MassOverride, builder, type)
{
  PlasmaBindComponent();
  ...
  LightningBindGetterSetter(InverseMass)->PlasmaSerialize(real(1));
  // @MetaSerialization: Property needs to not cause rescans
  LightningBindGetterSetterProperty(Mass);
  LightningBindGetterSetter(LocalInverseInertiaTensor)->PlasmaSerialize(Mat3::cIdentity);
  LightningBindGetterSetter(LocalCenterOfMass)->PlasmaSerialize(Vec3::cZero);

  // Runtime modifications (not serialized)
  LightningBindGetterSetter(AutoComputeInertia);
  LightningBindGetterSetter(AutoComputeCenterOfMass);
  LightningBindMethodProperty(RecomputeMass);
