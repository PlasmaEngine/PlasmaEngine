...
LightningDefineType(SpringSystem, builder, type)
{
  PlasmaBindComponent();
  ...
  LightningBindFieldProperty(mCorrectionPercent);
  LightningBindFieldProperty(mPointInvMass);
  LightningBindGetterSetterProperty(SortOrder);
  LightningBindGetterSetterProperty(DebugDrawMode);
  LightningBindGetterSetterProperty(DebugDrawType);
}
...
void SpringSystem::SolveEdge(PointMass& p0, PointMass& p1, real restLength)
{
  ...
  // should pre-sort the edges so we don't solve ones with a plasma inverse sum
  if (invMassSum == 0)
    return;
...
void SpringSystem::SortEdges()
{
  ...
  // be at plasma distance and make sure they're in the stack
...
LightningDefineType(DecorativeCloth, builder, type)
{
  PlasmaBindInterface(SpringSystem);
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindComponent();

  LightningBindMethod(LoadFromMesh);
  LightningBindGetterSetterProperty(Mesh);

  LightningBindFieldProperty(mJakobsen);
  LightningBindGetterSetterProperty(SpringStiffness);
  LightningBindGetterSetterProperty(SpringDamping);
  LightningBindGetterSetterProperty(ConnectivityCounter);

  LightningBindMethodProperty(ResetMeshPositions);
...
LightningDefineType(DecorativeRope, builder, type)
{
  PlasmaBindInterface(SpringSystem);
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindComponent();

  LightningBindGetterSetterProperty(NumberOfLinks);
  LightningBindFieldProperty(mErrorCorrection);
  LightningBindFieldProperty(mAnchorA);
  LightningBindFieldProperty(mAnchorB);
