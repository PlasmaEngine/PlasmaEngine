...
namespace Plasma
{
LightningDefineType(PhysicsCar, builder, type)
{
  PlasmaBindComponent();
  ...
  LightningBindGetterSetterProperty(DebugDraw);

  LightningBindFieldProperty(mMaxSpeed);
  LightningBindFieldProperty(mMaxTorque);
  ...
  LightningBindGetterSetter(Brake);

  LightningBindFieldGetter(mWheelCogs);
  LightningBindMethod(NumberOfWheelsInContact);
...
LightningDefineType(PhysicsCar::CarWheelRef, builder, type)
{
...
LightningDefineType(PhysicsCar::CarWheelArray, builder, type)
{
  PlasmaBindDocumented();

  LightningBindMethod(Get);
  LightningBindGetter(Count);
}
...
{
  int count = static_cast<int>(mCarBody->mWheelRefs.Size());
  if (index >= count)
  {
...
{
  return static_cast<int>(mCarBody->mWheelRefs.Size());
}
