...
LightningDefineType(PhysicsEffect, builder, type)
{
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDocumented();

  LightningBindMethod(Toggle);
  LightningBindGetterSetterProperty(Active)->PlasmaSerialize(true);
  LightningBindGetterSetterProperty(WakeUpOnChange)->PlasmaSerialize(true);
  LightningBindGetterSetterProperty(DebugDrawEffect)->PlasmaSerialize(true);
  LightningBindGetter(EffectType);

  PlasmaBindTag(Tags::Physics);
  PlasmaBindTag(Tags::PhysicsEffect);
}
