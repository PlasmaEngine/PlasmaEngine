...
struct RelativeVelocityPolicy : public DefaultFragmentPolicy<RelativeVelocityJoint>
{
  ...
  real AxisFragment(MoleculeData& data, int atomIndex, RelativeVelocityJoint* joint, ConstraintMolecule& mol)
  {
    ...
    // nothing by setting the jacobian to the plasma axis (thus computing impulses
    // that are 0)
...
LightningDefineType(RelativeVelocityJoint, builder, type)
{
  PlasmaBindComponent();
  ...
  LightningBindMethod(GetAxis);
  LightningBindMethod(SetAxis);
  LightningBindMethod(GetSpeed);
  LightningBindMethod(SetSpeed);
  LightningBindMethod(GetMaxImpulse);
  LightningBindMethod(SetMaxImpulse);
  LightningBindMethod(GetAxisActive);
  LightningBindMethod(SetAxisActive);
}
