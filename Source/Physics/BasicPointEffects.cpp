...
LightningDefineType(BasicPointEffect, builder, type)
{
  ...
  LightningBindGetterSetterProperty(StrengthAtMin)->PlasmaSerialize(real(1));
  LightningBindGetterSetterProperty(StrengthAtMax)->PlasmaSerialize(real(5));
  LightningBindGetterSetterProperty(LocalPositionOffset)->PlasmaSerialize(Vec3::cZero);
...
LightningDefineType(PointForceEffect, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
...
LightningDefineType(PointGravityEffect, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
