...
LightningDefineType(RigidBody, builder, type)
{
  ...
  PlasmaBindEvent(Events::RigidBodySlept, ObjectEvent);
  PlasmaBindEvent(Events::RigidBodyAwoke, ObjectEvent);

  // Bind velocity methods
  LightningBindGetterSetterProperty(Velocity)->PlasmaSerialize(Vec3::cZero);
  LightningBindGetterSetterProperty(AngularVelocity)->PlasmaSerialize(Vec3::cZero);
  LightningBindMethod(ApplyLinearVelocity);
  LightningBindMethod(ApplyAngularVelocity);
  LightningBindMethod(ApplyVelocityAtPoint);
  LightningBindMethod(GetPointVelocity);
  // Bind force methods
  LightningBindGetterSetter(Force);
  LightningBindGetterSetter(Torque);
  LightningBindMethod(ApplyForceNoWakeUp);
  LightningBindMethod(ApplyTorqueNoWakeUp);
  LightningBindMethod(ApplyForceAtOffsetVectorNoWakeUp);
  LightningBindMethod(ApplyForceAtPointNoWakeUp);
  LightningBindMethod(ApplyForce);
  LightningBindMethod(ApplyTorque);
  ...
  LightningBindMethod(ApplyImpulseAtPoint);

  LightningBindGetterSetterProperty(DynamicState)->PlasmaSerialize(RigidBodyDynamicState::Dynamic);
  LightningBindGetterSetterProperty(AllowSleep)->PlasmaSerialize(true);
  LightningBindGetterSetter(Asleep)->PlasmaSerialize(true);
  LightningBindMethod(ForceAwake);
  LightningBindMethod(ForceAsleep);
  LightningBindGetterSetterProperty(RotationLocked)->PlasmaSerialize(false);
  LightningBindGetterSetterProperty(Mode2D)->PlasmaSerialize(Mode2DStates::InheritFromSpace);
  LightningBindGetterProperty(Mass);
  LightningBindGetter(LocalInverseInertiaTensor);
  LightningBindGetter(WorldInverseInertiaTensor);
  LightningBindGetter(WorldCenterOfMass);
  LightningBindGetter(ActiveBody);
}
