...
LightningDefineType(SweepResult, builder, type)
{
  ...
  LightningBindGetterProperty(Time);
  LightningBindGetterProperty(OtherCollider);
  LightningBindGetterProperty(OtherObject);
  LightningBindGetterProperty(Penetration);
  LightningBindGetterProperty(WorldPoint);
  LightningBindGetterProperty(WorldNormalTowardsSelf);
  LightningBindGetterProperty(WorldNormalTowardsOther);
}
...
LightningDefineType(PhysicsSpace, builder, type)
{
  ...
  PlasmaBindEvent(Events::GroupCollisionStarted, CollisionGroupEvent);
  PlasmaBindEvent(Events::GroupCollisionEnded, CollisionGroupEvent);
  PlasmaBindEvent(Events::PhysicsUpdateFinished, ObjectEvent);

  LightningBindGetterSetterProperty(SubStepCount);
  LightningBindGetterSetterProperty(AllowSleep);
  LightningBindGetterSetterProperty(Mode2D);
  LightningBindGetterSetterProperty(Deterministic);
  LightningBindGetterSetterProperty(CollisionTable);
  LightningBindGetterSetterProperty(PhysicsSolverConfig);
  ...
  LightningBindMethod(RemoveHierarchyPairFilter);

  LightningBindMethod(FlushPhysicsQueue);

  // Ray Cast
  LightningBindOverloadedMethod(CastRayFirst, LightningInstanceOverload(CastResult, const Ray&));
  LightningBindOverloadedMethod(CastRayFirst, LightningInstanceOverload(CastResult, const Ray&, CastFilter&));
  LightningBindOverloadedMethod(CastRay, LightningInstanceOverload(CastResultsRange, const Ray&, uint));
  LightningBindOverloadedMethod(CastRay, LightningInstanceOverload(CastResultsRange, const Ray&, uint, CastFilter&));
  // Segment Cast
  LightningBindOverloadedMethod(CastSegment, LightningInstanceOverload(CastResultsRange, const Segment&, uint));
  LightningBindOverloadedMethod(CastSegment, LightningInstanceOverload(CastResultsRange, const Segment&, uint, CastFilter&));
  // Volume Cast
  LightningBindOverloadedMethod(CastAabb, LightningInstanceOverload(CastResultsRange, const Aabb&, uint, CastFilter&));
  LightningBindOverloadedMethod(CastSphere, LightningInstanceOverload(CastResultsRange, const Sphere&, uint, CastFilter&));
  LightningBindOverloadedMethod(CastFrustum, LightningInstanceOverload(CastResultsRange, const Frustum&, uint, CastFilter&));
...
PhysicsSpace::~PhysicsSpace()
{
  ...
  while (!range.Empty())
  {
    ...
    range.PopFront();
    effect.PhysicsEffect::ComponentRemoved(LightningTypeId(PhysicsSpace), this);
  }
...
void PhysicsSpace::Serialize(Serializer& stream)
{
  ...
  SerializeNameDefault(mDynamicBroadphaseType, LightningTypeId(DynamicAabbTreeBroadPhase)->Name);
  SerializeNameDefault(mStaticBroadphaseType, LightningTypeId(StaticAabbTreeBroadPhase)->Name);
}
