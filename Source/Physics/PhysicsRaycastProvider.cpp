...
LightningDefineType(PhysicsRaycastProvider, builder, type)
{
  PlasmaBindDocumented();
  PlasmaBindExpanded();
  LightningBindConstructor();

  LightningBindFieldProperty(mDynamicColliders);
  LightningBindFieldProperty(mSelectGhosts);
  LightningBindFieldProperty(mStaticColliders);
  LightningBindFieldProperty(mMultiSelectStatic);
  LightningBindFieldProperty(mMultiSelectKinematic);
...
void PhysicsRaycastProvider::RayCast(Ray& ray, CastInfo& castInfo, RaycastResultList& results)
{
  ...
  CastResults physicsResults(static_cast<uint>(results.mCapacity), filter);
