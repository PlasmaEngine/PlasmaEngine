...
LightningDefineType(MeshDebug, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  LightningBindMethod(DrawTriangles);
  LightningBindMethod(DrawTriangleVoronoiRegion);
  LightningBindMethod(DrawAllTriangleVoronoiRegions);
  LightningBindMethod(GetTriangleIndexFromCast);
  LightningBindMethod(GetTriangleNormal);
  LightningBindMethod(GetPointFromCastOnTriangle);
  LightningBindMethod(DrawTriangleFromCast);
  LightningBindMethod(TestManifoldPoint);
  LightningBindMethod(DrawVectorAtPoint);
  LightningBindMethod(GenerateInfoForTriangles);
}
...
void MeshDebug::DrawAllTriangleVoronoiRegions()
{
  ...
  uint triCount = static_cast<uint>(mMesh->GetTriangleCount());
