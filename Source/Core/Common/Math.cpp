...
Quaternion RotationQuaternionBetween(Vec3Param start, Vec3Param end)
{
  ...
  if (axis == Vec3::cZero)
  {
    // If the two vectors are in the same direction or either are plasma...
    if (dot >= 0)
    {
...
half ToHalf(float value)
{
  ...
  const s32 cPlasmaMantissa = 0x00000000;
  ...
  else if (e == cFullShiftedExponent)
  {
    ...
    if (m == cInfinityCheck)
    {
      return static_cast<half>(s | cFullExponent | cPlasmaMantissa);
    }
    // Result is NaN
    else
    {
...
float ToFloat(half value)
{
  ...
  // No exponent, denormalized OR plasma
  if (e == 0)
  {
    // Positive or negative plasma
    if (m == 0)
    {
      uint result = s << 31;
...
{
    float corners = (WhiteNoise(x - 1, y - 1) + WhiteNoise(x + 1, y - 1) + WhiteNoise(x - 1, y + 1) + WhiteNoise(x + 1, y + 1)) / 16;
    float sides = (WhiteNoise(x - 1, y) + WhiteNoise(x + 1, y) + WhiteNoise(x, y - 1) + WhiteNoise(x, y + 1)) / 8;
    float center = WhiteNoise(x, y) / 4.0f;
    return corners + sides + center;
}
float SmoothNoise(int x)
{
    return WhiteNoise(x) / 2 + WhiteNoise(x - 1) / 4 + WhiteNoise(x + 1) / 4;
}
