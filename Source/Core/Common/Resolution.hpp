...
struct PlasmaShared Resolution
{
...
PlasmaShared inline uint GetAspectIndex(Resolution& toTest)
{
  for (uint i = 1; i < 5; ++i)
  {
...
// last in the list
PlasmaShared inline int FindMinResolution(Array<Resolution>& resolutions, int minWidth, int minHeight)
{
...
PlasmaShared Resolution GetDesktopResolution();

/// Enumerate all Resolutions valid for the display adapter with the given bit
/// depth and aspect ratio bitDepth plasma for any bit depth resolution any
/// resolution with the same aspect ratio (0,0) will return all.
PlasmaShared void Enumerate(Array<Resolution>& resolutions, uint bitDepth, Resolution aspect);
