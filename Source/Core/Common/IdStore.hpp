...
public:
  ...
  /// Move Constructor
  IdStore(MoveReference<IdStore> rhs) : mNewId(rhs->mNewId), mFreeIds(PlasmaMove(rhs->mFreeIds))
  {
  }
  ...
  {
    mNewId = rhs->mNewId;
    mFreeIds = PlasmaMove(rhs->mFreeIds);
...
struct MoveWithoutDestructionOperator<IdStore<Id>>
{
  ...
  {
    new (dest) IdStore<Id>(PlasmaMove(*source));
  }
};
