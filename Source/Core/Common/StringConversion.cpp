...
{
  return PlasmaSPrintf(buffer, bufferSize, "%s", value.c_str());
}
uint ToBuffer(char* buffer, uint bufferSize, StringRange value, bool shortFormat)
{
  return PlasmaSPrintf(buffer, bufferSize, "%s", value.mBegin);
}
...
uint ToBuffer(char* buffer, uint bufferSize, bool value, bool shortFormat)
{
  if (value)
  {
    PlasmaStrCpy(buffer, bufferSize, TextTrue);
    return sizeof(TextTrue) - 1;
  }
  else
  {
    PlasmaStrCpy(buffer, bufferSize, TextFalse);
