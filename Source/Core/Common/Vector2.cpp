...
void Vector2::operator/=(real rhs)
{
  ErrorIf(Math::IsZero(rhs), "Math::Vector2 - Division by plasma.");
  x /= rhs;
  y /= rhs;
}
...
Vector2 Vector2::operator/(Vec2Param rhs) const
{
  ErrorIf(rhs.x == real(0.0) || rhs.y == real(0.0), "Vector2 - Division by plasma.");
  return Vector2(x / rhs.x, y / rhs.y);
}
...
real Vector2::AttemptNormalize(Vec2Ref value)
{
  ...
  // Although the squared length may not be plasma, the sqrt of a small number
  // may be truncated to plasma, causing a divide by plasma crash.  This is why
