...
PlasmaShared inline TimeMs GetDuration(TimeMs start, TimeMs end)
{
  return end - start;
}

/// Converts milliseconds (TimeMs) to seconds (float)
PlasmaShared inline float TimeMsToFloatSeconds(TimeMs milliseconds)
{
  return float(milliseconds) / float(1000);
}

/// Converts seconds (float) to milliseconds (TimeMs)
PlasmaShared inline TimeMs FloatSecondsToTimeMs(float seconds)
{
...
private:
  PlasmaDeclarePrivateDataBytes(50);
};
