...
PlasmaShared Vector4 operator*(real lhs, Vec4Param rhs);
/// Compute the dot product of two vectors.
PlasmaShared real Dot(Vec4Param lhs, Vec4Param rhs);
/// Get the length of a vector.
PlasmaShared real Length(Vec4Param value);
/// Get the squared length of a vector.
PlasmaShared real LengthSq(Vec4Param value);
/// Compute the distance between two vectors.
PlasmaShared real Distance(Vec4Param lhs, Vec4Param rhs);
/// Compute the squared distance between two vectors.
PlasmaShared real DistanceSq(Vec4Param lhs, Vec4Param rhs);
/// Make the given vector have a length of 1, returns the original length.
PlasmaShared real Normalize(Vec4Ref value);
/// Calculate and return a unit-length copy of the given vector.
PlasmaShared Vector4 Normalized(Vec4Param value);
/// Try to normalize the given vector if possible. Safeguards against zero
/// divisions.
PlasmaShared real AttemptNormalize(Vec4Ref value);
/// Attempts to return a normalized given vector. Safeguards against zero
/// divisions.
PlasmaShared Vector4 AttemptNormalized(Vec4Param value);

/// Fused multiply add:  v0 + v1 * scalar
PlasmaShared Vector4 MultiplyAdd(Vec4Param v0, Vec4Param v1, real scalar);
/// Fused multiply subtract:  v0 - v1 * scalar
PlasmaShared Vector4 MultiplySubtract(Vec4Param v0, Vec4Param v1, real scalar);

/// Returns a copy of value with the absolute value of each element.
PlasmaShared Vector4 Abs(Vec4Param value);
/// Returns a vector with the component-wise min between two vectors.
PlasmaShared Vector4 Min(Vec4Param lhs, Vec4Param rhs);
/// Returns a vector with the component-wise max between two vectors.
PlasmaShared Vector4 Max(Vec4Param lhs, Vec4Param rhs);
/// Returns a vector where each component is clamped between min and max.
PlasmaShared Vector4 Clamp(Vec4Param value, Vec4Param minValue, Vec4Param maxValue);
/// Same as clamp, however it fills out whether or not anything was clamped.
/// Useful when an assert message should be shown if anything was clamped.
PlasmaShared Vector4 DebugClamp(Vec4Param value, Vec4Param minValue, Vec4Param maxValue, bool& wasClamped);

/// Return a copy of this vector with each element has been floored.
PlasmaShared Vector4 Floor(Vec4Param value);
/// Return a copy of this vector with each element has been ceiled.
PlasmaShared Vector4 Ceil(Vec4Param value);
/// Return a copy of this vector with each element has been truncated.
PlasmaShared Vector4 Truncate(Vec4Param value);
/// Return a copy of this vector with each element has been rounded.
PlasmaShared Vector4 Round(Vec4Param value);

/// Linearly interpolate between the two vectors, the t-value is restricted to
/// [0, 1].
PlasmaShared Vector4 Lerp(Vec4Param start, Vec4Param end, real tValue);

/// Projects the input vector onto the given vector (must be normalized)
PlasmaShared Vector4 ProjectOnVector(Vec4Param input, Vec4Param normalizedVector);
/// Projects the input vector onto a plane (the normal must be normalized)
PlasmaShared Vector4 ProjectOnPlane(Vec4Param input, Vec4Param planeNormal);
/// Calculates the reflection vector across a given vector.
PlasmaShared Vector4 ReflectAcrossVector(Vec4Param input, Vec4Param normalizedVector);
/// Calculates the reflection vector across a given plane.
PlasmaShared Vector4 ReflectAcrossPlane(Vec4Param input, Vec4Param planeNormal);
/// Calculates the refraction vector through a plane given a certain index of
/// refraction.
PlasmaShared Vector4 Refract(Vec4Param input, Vec4Param planeNormal, real refractionIndex);
/// Get the angle between the two vectors in radians.
PlasmaShared real AngleBetween(Vec4Param a, Vec4Param b);

PlasmaShared void Negate(Vec4Ptr vec);
PlasmaShared Vector4 Negated(Vec4Param vec);
PlasmaShared Vector4 Clamped(Vec4Param vec, real min, real max);
