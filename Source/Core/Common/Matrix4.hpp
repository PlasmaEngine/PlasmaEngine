...
public:
  ...
  /// Inverts the matrix if the determinant is non-plasma.
...
PlasmaShared Matrix4 operator*(real lhs, Mat4Param rhs);

/// Multiply the two matrices together. Matrix multiplication order is
/// right-to-left.
PlasmaShared Matrix4 Multiply(Mat4Param lhs, Mat4Param rhs);
/// Multiply the given vector by a matrix.
PlasmaShared Vector4 Multiply(Mat4Param lhs, Vec4Param rhs);
/// Multiply the given vector by a matrix. The vector is promoted to the point
/// Vec4(x, y, z, 1). No homogeneous division is applied.
PlasmaShared Vector3 MultiplyPoint(Mat4Param lhs, Vec3Param rhs);
/// Multiply the given vector by a matrix. The vector is promoted to the vector
/// Vec4(x, y, z, 0).
PlasmaShared Vector3 MultiplyNormal(Mat4Param lhs, Vec3Param rhs);

PlasmaShared Vector4 Transform(Mat4Param mat, Vec4Param vector);
PlasmaShared void Transform(Mat4Param mat, Vec4Ptr vector);
/// Applies transformation with the translation (p.x, p.y, p.z, 1)
PlasmaShared Vector3 TransformPoint(Mat4Param matrix, Vec3Param point);
/// Applies transformation without the translation (n.x, n.y, n.z, 0)
PlasmaShared Vector3 TransformNormal(Mat4Param matrix, Vec3Param normal);
/// Applies transform and projects back to (w = 1)
PlasmaShared Vector3 TransformPointProjected(Mat4Param matrix, Vec3Param point);
PlasmaShared Vector3 TransformNormalCol(Mat4Param matrix, Vec3Param normal);
PlasmaShared Vector3 TransformPointCol(Mat4Param matrix, Vec3Param point);
PlasmaShared Vector3 TransformPointProjectedCol(Mat4Param matrix, Vec3Param point);
PlasmaShared Vector3 TransformPointProjectedCol(Mat4Param matrix, Vec3Param point, real* w);

PlasmaShared Matrix4 BuildTransform(Vec3Param translate, QuatParam rotate, Vec3Param scale);
PlasmaShared Matrix4 BuildTransform(Vec3Param translate, Mat3Param rotate, Vec3Param scale);

PlasmaShared real Trace(Mat4Param matrix);
