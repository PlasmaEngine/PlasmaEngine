...
public:
  ...
  IdSequence(MoveReference<IdSequence> rhs) :
      mNext(rhs->mNext),
      mMissing(PlasmaMove(rhs->mMissing)),
  ...
  struct IdSequenceHistory
  {
    ...
    /// Constructor
    IdSequenceHistory(Id next, MoveReference<BitStream> history) : mNext(next), mHistory(PlasmaMove(history))
    {
    }

    /// Move Constructor
    IdSequenceHistory(MoveReference<IdSequenceHistory> rhs) : mNext(rhs->mNext), mHistory(PlasmaMove(rhs->mHistory))
    {
    }
  ...
  IdSequenceHistory GetSequenceHistory(uint range) const
  {
    ...
    return IdSequenceHistory(mNext, PlasmaMove(history));
  }
...
struct MoveWithoutDestructionOperator<IdSequence<Id>>
{
  ...
  {
    new (dest) IdSequence<Id>(PlasmaMove(*source));
  }
};
