...
struct PlasmaShared Quaternion
{
...
PlasmaShared Quaternion operator*(real lhs, QuatParam rhs);

PlasmaShared real Dot(QuatParam lhs, QuatParam rhs);
PlasmaShared real Length(QuatParam quaternion);
PlasmaShared real LengthSq(QuatParam quaternion);
PlasmaShared void Normalize(QuatRef quaternion);
PlasmaShared Quaternion Normalized(QuatParam quaternion);
PlasmaShared Quaternion Multiply(QuatParam lhs, QuatParam rhs);
PlasmaShared Vector3 Multiply(QuatParam lhs, Vec3Param rhs);
PlasmaShared Quaternion Lerp(QuatParam start, QuatParam end, real tValue);
PlasmaShared Quaternion Slerp(QuatParam start, QuatParam end, real tValue);
PlasmaShared Quaternion SlerpUnnormalized(QuatParam start, QuatParam end, real tValue);
PlasmaShared real AngleBetween(QuatParam a, QuatParam b);

PlasmaShared Quaternion CreateDiagonalizer(Mat3Param matrix);
