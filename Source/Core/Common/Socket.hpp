...
PlasmaShared u16 HostToNetworkShort(u16 hostShort);
PlasmaShared s16 HostToNetworkShort(s16 hostShort);
PlasmaShared u32 HostToNetworkLong(u32 hostLong);
PlasmaShared s32 HostToNetworkLong(s32 hostLong);
...
PlasmaShared u16 NetworkToHostShort(u16 networkShort);
PlasmaShared s16 NetworkToHostShort(s16 networkShort);
PlasmaShared u32 NetworkToHostLong(u32 networkLong);
PlasmaShared s32 NetworkToHostLong(s32 networkLong);

//                                SocketAddress //

/// Network host identifier
class PlasmaShared SocketAddress
{
public:
  ...
  /// Address data
  PlasmaDeclarePrivateDataBytes(SocketAddressStorageBytes);
};
...
/// successful, else 0
PlasmaShared Bits Serialize(SerializeDirection::Enum direction, BitStream& bitStream, SocketAddress& socketAddress);
...
PlasmaShared SocketAddress ResolveSocketAddress(Status& status,
                                              StringParam host,
                                              StringParam service,
...
PlasmaShared SocketAddress ResolveSocketAddress(Status& status,
                                              ...
                                              StringParam service,
                                              SocketAddressResolutionFlags::Enum addressResolutionFlags);
PlasmaShared SocketAddress ResolveSocketAddress(Status& status, StringParam host, StringParam service);
...
/// platforms)
PlasmaShared Array<SocketAddress> ResolveAllSocketAddresses(Status& status,
                                                          StringParam host,
                                                          StringParam service,
                                                          SocketAddressFamily::Enum addressFamily,
                                                          SocketProtocol::Enum protocol,
...
PlasmaShared Array<SocketAddress> ResolveAllSocketAddresses(Status& status,
                                                          ...
                                                          StringParam service,
                                                          SocketAddressResolutionFlags::Enum addressResolutionFlags);
PlasmaShared Array<SocketAddress> ResolveAllSocketAddresses(Status& status, StringParam host, StringParam service);
...
PlasmaShared Pair<String, String> ResolveHostAndServiceNames(Status& status,
                                                           const SocketAddress& address,
                                                           SocketNameResolutionFlags::Enum nameResolutionFlags);
PlasmaShared Pair<String, String> ResolveHostAndServiceNames(Status& status, const SocketAddress& address);
...
PlasmaShared String SocketAddressToString(Status& status,
                                        SocketAddressFamily::Enum addressFamily,
                                        const SocketAddress& address);
PlasmaShared String SocketAddressToString(SocketAddressFamily::Enum addressFamily, const SocketAddress& address);
...
PlasmaShared SocketAddress StringToSocketAddress(Status& status,
                                               SocketAddressFamily::Enum addressFamily,
                                               StringParam address);
PlasmaShared SocketAddress StringToSocketAddress(SocketAddressFamily::Enum addressFamily, StringParam address);
...
/// well formed
PlasmaShared bool IsValidIpv4Address(const SocketAddress& address);
/// Returns true if the socket address represents a valid IPv6 host, else false
/// This does not imply that the host exists, only that the socket address is
/// well formed
PlasmaShared bool IsValidIpv6Address(const SocketAddress& address);
...
PlasmaShared bool IsValidIpv4Address(StringParam address);
/// Returns true if the numeric address string represents a valid IPv6 host,
/// else false This does not imply that the host exists, only that the numeric
/// address string is well formed
PlasmaShared bool IsValidIpv6Address(StringParam address);

/// Converts a valid IPv4 socket address to a numeric address string, else
/// String()
PlasmaShared String Ipv4AddressToString(const SocketAddress& address);
/// Converts a valid IPv6 socket address to a numeric address string, else
/// String()
PlasmaShared String Ipv6AddressToString(const SocketAddress& address);

/// Returns a port as a numeric string, else String()
PlasmaShared String PortToString(uint port);

/// Converts a valid IPv4 socket address to a numeric address string with
/// appended port number, else String()
PlasmaShared String Ipv4AddressToStringWithPort(const SocketAddress& address);
/// Converts a valid IPv6 socket address to a numeric address string with
/// appended port number, else String()
PlasmaShared String Ipv6AddressToStringWithPort(const SocketAddress& address);

/// Converts a valid IPv4 numeric address string to a socket address, else
/// SocketAddress()
PlasmaShared SocketAddress StringToIpv4Address(StringParam address);
PlasmaShared SocketAddress StringToIpv4Address(StringParam address, ushort port);

/// Converts a valid IPv6 numeric address string to a socket address, else
/// SocketAddress()
PlasmaShared SocketAddress StringToIpv6Address(StringParam address);
PlasmaShared SocketAddress StringToIpv6Address(StringParam address, ushort port);

//                                    Socket //

/// Network host endpoint
/// Facilitates interprocess communication
class PlasmaShared Socket
{
public:
  //
  ...
  /// (reference count greater than plasma), else false
  ...
  /// counted If already initialized (reference count greater than plasma), only
  /// increments the reference count If not yet initialized (reference count of
  /// plasma), initializes the socket library and if successful, increments the
  ...
  /// counted If not initialized (reference count of plasma), does nothing If
...
PlasmaShared SocketAddress QueryLocalSocketAddress(Status& status, const Socket& socket);
...
/// most platforms)
PlasmaShared SocketAddress QueryRemoteSocketAddress(Status& status, const Socket& socket);
