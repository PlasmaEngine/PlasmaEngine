...
// Returns an invalid type that is plasmaed out (only used for not-crashing after
...
#define PlasmaInit ((Plasma::ContainerInitializerDummy*)nullptr)

/// Base class for containers that use allocators.
/// WARNING: This should be a PlasmaSharedTemplate, however Release is optimizing
...
class PlasmaExport AllocationContainer
{
public:
...
struct PlasmaSharedTemplate Pair
{
  ...
  Pair(const type0& value0, MoveReference<type1> value1) : first(value0), second(PlasmaMove(value1))
  {
  }
...
inline void ConstructWith(type* elem, MoveReference<initType> source)
{
  ...
  new (elem) type(PlasmaMove(source));
}
...
struct PlasmaSharedTemplate static_verify_function_signature
{
  typedef Fn type;
};
...
template <typename T>
struct PlasmaSharedTemplate has_equality_operator_helper
{
...
/// defined as false
template <typename T>
struct PlasmaSharedTemplate has_equality_operator : public integral_constant<bool, has_equality_operator_helper<T>::value>
{
};
...
struct PlasmaSharedTemplate ComparePolicy
{
...
/// operator
template <typename T>
struct PlasmaSharedTemplate ComparePolicy<T, TC_ENABLE_IF(has_equality_operator<T>::value)>
{
public:
  inline bool Equal(const T& left, const T& right) const
  {
...
template <>
struct PlasmaShared ComparePolicy<const char*>
{
...
template <typename T>
struct PlasmaSharedTemplate has_valid_compare_policy_helper
{
...
template <typename T>
struct PlasmaSharedTemplate has_valid_compare_policy
