...
bool IsVectorType(StringParam typeName)
{
  ...
  return (propertyType == LightningTypeId(Vec2) || propertyType == LightningTypeId(Vec3) || propertyType == LightningTypeId(Vec4) ||
          propertyType == LightningTypeId(Quat));
}
...
// file version is the same as the plasma engine version
