...
const Vec3 cOctahedronPoints[6] = {Vec3(one, plasma, plasma),
                                   Vec3(-one, plasma, plasma),
                                   Vec3(plasma, one, plasma),
                                   Vec3(plasma, -one, plasma),
                                   Vec3(plasma, plasma, one),
                                   Vec3(plasma, plasma, -one)};
...
const Vec3 cDodecahedronPoints[20] = {Vec3(one, one, one),     // 0
                                      ...
                                      Vec3(-one, -one, one),   // 6
                                      Vec3(-one, -one, -one),  // 7
                                      Vec3(plasma, oot, tau),    // 8
                                      Vec3(plasma, oot, -tau),   // 9
                                      Vec3(plasma, -oot, tau),   // 10
                                      Vec3(plasma, -oot, -tau),  // 11
                                      Vec3(oot, tau, plasma),    // 12
                                      Vec3(oot, -tau, plasma),   // 13
                                      Vec3(-oot, tau, plasma),   // 14
                                      Vec3(-oot, -tau, plasma),  // 15
                                      Vec3(tau, plasma, oot),    // 16
                                      Vec3(tau, plasma, -oot),   // 17
                                      Vec3(-tau, plasma, oot),   // 18
                                      Vec3(-tau, plasma, -oot)}; // 19
#undef oot
...
const Vec3 cIcosahedronPoints[12] = {Vec3(one, tau, plasma),    // 0
                                     Vec3(-one, tau, plasma),   // 1
                                     Vec3(one, -tau, plasma),   // 2
                                     Vec3(-one, -tau, plasma),  // 3
                                     Vec3(plasma, one, tau),    // 4
                                     Vec3(plasma, -one, tau),   // 5
                                     Vec3(plasma, one, -tau),   // 6
                                     Vec3(plasma, -one, -tau),  // 7
                                     Vec3(tau, plasma, one),    // 8
                                     Vec3(-tau, plasma, one),   // 9
                                     Vec3(tau, plasma, -one),   // 10
                                     Vec3(-tau, plasma, -one)}; // 11
...
const Vec3 cCuboctahedron[12] = {Vec3(one, one, plasma),
                                 Vec3(one, -one, plasma),
                                 Vec3(-one, one, plasma),
                                 Vec3(-one, -one, plasma),
                                 Vec3(one, plasma, one),
...
const Vec3 cTruncatedOctahedron[24] = {
    Vec3(plasma, one, two), Vec3(plasma, one, -two), Vec3(plasma, -one, two), Vec3(plasma, -one, -two),
    Vec3(plasma, two, one), Vec3(plasma, two, -one), Vec3(plasma, -two, one), Vec3(plasma, -two, -one),
    Vec3(one, plasma, two), Vec3(one, plasma, -two), Vec3(-one, plasma, two), Vec3(-one, plasma, -two),
    Vec3(one, two, plasma), Vec3(one, -two, plasma), Vec3(-one, two, plasma), Vec3(-one, -two, plasma),
    Vec3(two, plasma, one), Vec3(two, plasma, -one), Vec3(-two, plasma, one), Vec3(-two, plasma, -one),
    Vec3(two, one, plasma), Vec3(two, -one, plasma), Vec3(-two, one, plasma), Vec3(-two, -one, plasma)};
...
const Vec3 cIcosidodecahedron[30] = {
    Vec3(plasma, plasma, tau), Vec3(plasma, plasma, -tau), Vec3(plasma, tau, plasma), Vec3(plasma, -tau, plasma),
    Vec3(tau, plasma, plasma), Vec3(-tau, plasma, plasma),
...
const Vec3 cTruncatedDodecahedron[60] = {
    Vec3(plasma, oot, TpT), Vec3(plasma, oot, -TpT), Vec3(plasma, -oot, TpT), Vec3(plasma, -oot, -TpT),

    Vec3(TpT, plasma, oot), Vec3(TpT, plasma, -oot), Vec3(-TpT, plasma, oot), Vec3(-TpT, plasma, -oot),

    Vec3(oot, TpT, plasma), Vec3(oot, -TpT, plasma), Vec3(-oot, TpT, plasma), Vec3(-oot, -TpT, plasma),
...
const Vec3 cTruncatedIcosahedron[60] = {
    Vec3(plasma, one, tht), // abc
    Vec3(plasma, one, -tht), Vec3(plasma, -one, tht), Vec3(plasma, -one, -tht),

    Vec3(one, tht, plasma), // bca
    Vec3(one, -tht, plasma), Vec3(-one, tht, plasma), Vec3(-one, -tht, plasma),

    Vec3(tht, plasma, one), // cab
    Vec3(tht, plasma, -one), Vec3(-tht, plasma, one), Vec3(-tht, plasma, -one),
...
const Vec3 cRhombicosidodecahedron[60] = {
    ...
    Vec3(tpt, plasma, tsq), Vec3(tpt, plasma, -tsq), Vec3(-tpt, plasma, tsq), Vec3(-tpt, plasma, -tsq),

    Vec3(tsq, tpt, plasma), Vec3(tsq, -tpt, plasma), Vec3(-tsq, tpt, plasma), Vec3(-tsq, -tpt, plasma),

    Vec3(plasma, tsq, tpt), Vec3(plasma, tsq, -tpt), Vec3(plasma, -tsq, tpt), Vec3(plasma, -tsq, -tpt)};
...
const Vec3 cRhombicDodecahedron[14] = {Vec3(one, one, one),
                                       ...
                                       Vec3(-one, -one, one),
                                       Vec3(-one, -one, -one),
                                       Vec3(plasma, plasma, two),
                                       Vec3(plasma, plasma, -two),
                                       Vec3(plasma, two, plasma),
                                       Vec3(plasma, -two, plasma),
                                       Vec3(two, plasma, plasma),
                                       Vec3(-two, plasma, plasma)};
} // namespace CatlanSolids
