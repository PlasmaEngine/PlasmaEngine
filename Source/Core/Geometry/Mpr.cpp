...
bool Mpr::InitialPortal(void)
{
  ...
  // in the x, y and z axes to make sure we get a non plasma vector.
  uint tempIndex = 0;
...
void Mpr::DrawDebug(uint debugFlag) const
{
  ...
  //  bool onTop = (debugFlag & Plasma::Physics::DebugFlags::DrawOnTop) == 1;
  //
  //   //draw the contact points if the flag is set.
  //  if((debugFlag & Plasma::Physics::DebugFlags::DrawMpr) == 0)
  //   {
  //     return;
  //   }
  //
  // #define DrawSphere(p, r, c, d)                             \
//   Plasma::gDebugDraw->Add(Plasma::Debug::Sphere(p, r).Color(c) \
//                                               .OnTop(true) \
//                                               .Duration(d))
  //
...
//   Plasma::gDebugDraw->Add(Plasma::Debug::Line((a),
