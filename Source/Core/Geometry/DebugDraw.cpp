...
{
  typedef DebugObjectType LightningSelf;
  LightningBindFieldProperty(mColor);
  type->AddAttribute(ExportDocumentation);
  // LightningBindFieldProperty(mDuration); // Not implemented
  // LightningBindFieldProperty(mWidth); // Not implemented

  // LightningBindGetterSetterProperty(BackShade); // Not implemented
  // LightningBindGetterSetterProperty(Border); // Not implemented
  // LightningBindGetterSetterProperty(Filled); // Not implemented
  LightningBindGetterSetterProperty(OnTop);
  LightningBindGetterSetterProperty(ViewAligned);
  LightningBindGetterSetterProperty(ViewScaled);
  LightningBindFieldProperty(mViewScaleOffset);
}

LightningDefineType(Arc, builder, type)
{
  DebugDrawObject<Arc>::SetupTypeHelper(builder, type);

  LightningBindDestructor();
  LightningBindDefaultConstructor();
  LightningFullBindConstructor(builder, type, LightningSelf, "start, mid, end", Vec3, Vec3, Vec3);

  LightningBindFieldProperty(mStart);
  LightningBindFieldProperty(mMid);
  LightningBindFieldProperty(mEnd);
}

LightningDefineType(Box, builder, type)
{
  DebugDrawObject<Box>::SetupTypeHelper(builder, type);

  LightningBindDestructor();
  LightningBindDefaultConstructor();
  LightningFullBindConstructor(builder, type, LightningSelf, "position, halfExtents", Vec3, Vec2);
  LightningFullBindConstructor(builder, type, LightningSelf, "position, halfExtents", Vec3, float);
  LightningFullBindConstructor(builder, type, LightningSelf, "position, halfExtents, rotation", Vec3, Vec2, Quat);
  LightningFullBindConstructor(builder, type, LightningSelf, "position, halfExtents, rotation", Vec3, float, Quat);
  LightningFullBindConstructor(builder, type, LightningSelf, "aabb", Aabb);

  LightningBindFieldProperty(mPosition);
  LightningBindFieldProperty(mHalfExtents);
  LightningBindFieldProperty(mRotation);

  // LightningBindGetterSetterProperty(Corners); // Not implemented

  // Temporarily bound only on types that implement it
  LightningBindGetterSetterProperty(Filled);
}

LightningDefineType(Capsule, builder, type)
{
  DebugDrawObject<Capsule>::SetupTypeHelper(builder, type);

  LightningBindDestructor();
  LightningBindDefaultConstructor();
  LightningFullBindConstructor(builder, type, LightningSelf, "start, end, radius", Vec3, Vec3, float);
  LightningFullBindConstructor(builder, type, LightningSelf, "position, axis, height, radius", Vec3, Vec3, float, float);
...
LightningDefineType(Circle, builder, type)
{
  DebugDrawObject<Circle>::SetupTypeHelper(builder, type);

  LightningBindDestructor();
  LightningBindDefaultConstructor();
  LightningFullBindConstructor(builder, type, LightningSelf, "position, axis, radius", Vec3, Vec3, float);

  LightningBindFieldProperty(mPosition);
  LightningBindFieldProperty(mAxis);
  LightningBindFieldProperty(mRadius);
}

LightningDefineType(Cone, builder, type)
{
  DebugDrawObject<Cone>::SetupTypeHelper(builder, type);

  LightningBindDestructor();
  LightningBindDefaultConstructor();
  LightningFullBindConstructor(builder, type, LightningSelf, "position, direction, length, radius", Vec3, Vec3, float, float);

  LightningBindFieldProperty(mPosition);
  LightningBindFieldProperty(mDirection);
  LightningBindFieldProperty(mLength);
  LightningBindFieldProperty(mRadius);
}

LightningDefineType(Cylinder, builder, type)
{
...
LightningDefineType(Frustum, builder, type)
{
  DebugDrawObject<Frustum>::SetupTypeHelper(builder, type);

  LightningBindConstructor();
  LightningBindConstructor(Plasma::Frustum);
  LightningBindDestructor();
}

LightningDefineType(Line, builder, type)
{
  DebugDrawObject<Line>::SetupTypeHelper(builder, type);

  LightningBindDestructor();
  LightningBindDefaultConstructor();
  LightningFullBindConstructor(builder, type, LightningSelf, "start, end", Vec3, Vec3);
  LightningFullBindConstructor(builder, type, LightningSelf, "start, end, headSize", Vec3, Vec3, float);
  LightningFullBindConstructor(builder, type, LightningSelf, "ray", Ray);
  LightningFullBindConstructor(builder, type, LightningSelf, "ray, t", Ray, float);
  LightningFullBindConstructor(builder, type, LightningSelf, "segment", Segment);

  LightningBindFieldProperty(mStart);
  LightningBindFieldProperty(mEnd);
  LightningBindFieldProperty(mHeadSize);

  LightningBindGetterSetterProperty(DualHeads);
  LightningBindGetterSetterProperty(BoxHeads);

  // Temporarily bound only on types that implement it
  LightningBindGetterSetterProperty(Filled);
}

LightningDefineType(LineCross, builder, type)
{
...
LightningDefineType(Obb, builder, type)
{
  DebugDrawObject<Obb>::SetupTypeHelper(builder, type);

  LightningBindDestructor();
  LightningBindDefaultConstructor();
  LightningFullBindConstructor(builder, type, LightningSelf, "position, halfExtents", Vec3, Vec3);
  LightningFullBindConstructor(builder, type, LightningSelf, "position, halfExtents", Vec3, float);
  LightningFullBindConstructor(builder, type, LightningSelf, "position, halfExtents, rotation", Vec3, Vec3, Quat);
  LightningFullBindConstructor(builder, type, LightningSelf, "position, halfExtents, rotation", Vec3, float, Quat);
  LightningFullBindConstructor(builder, type, LightningSelf, "position, halfExtents, rotation", Vec3, Vec3, Mat3);
  // LightningFullBindConstructor(builder, type, LightningSelf, "obb", Obb);
  LightningFullBindConstructor(builder, type, LightningSelf, "aabb", Aabb);
  ...
  LightningBindGetterSetterProperty(Corners);

  // Temporarily bound only on types that implement it
  LightningBindGetterSetterProperty(Filled);
}

LightningDefineType(Sphere, builder, type)
{
  DebugDrawObject<Sphere>::SetupTypeHelper(builder, type);

  LightningBindDestructor();
  LightningBindDefaultConstructor();
  LightningFullBindConstructor(builder, type, LightningSelf, "position, radius", Vec3, float);
  LightningFullBindConstructor(builder, type, LightningSelf, "sphere", Sphere);

  LightningBindFieldProperty(mPosition);
  LightningBindFieldProperty(mRadius);

  LightningBindGetterSetterProperty(Colored);
}

LightningDefineType(Text, builder, type)
{
  DebugDrawObject<Text>::SetupTypeHelper(builder, type);

  LightningBindDestructor();
  LightningBindDefaultConstructor();
  LightningFullBindConstructor(builder, type, LightningSelf, "position, textHeight, text", Vec3, float, String);

  LightningBindFieldProperty(mPosition);
  LightningBindFieldProperty(mRotation);
  LightningBindFieldProperty(mTextHeight);
  LightningBindFieldProperty(mText);

  LightningBindGetterSetterProperty(Centered);
}

LightningDefineType(Triangle, builder, type)
{
  DebugDrawObject<Triangle>::SetupTypeHelper(builder, type);

  LightningBindDestructor();
  LightningBindDefaultConstructor();
  LightningBindConstructor(Vec3, Vec3, Vec3);

  LightningBindFieldProperty(mPoint0);
  LightningBindFieldProperty(mPoint1);
  LightningBindFieldProperty(mPoint2);
