...
f32 ComputeEdgeValue(Csg::Operation::Type operation, const Simplex& simplex, const Array<Simplex>& chain)
{
  ...
  for (size_t i = 0; i < chain.Size(); ++i)
  {
    ...
    Vec2_t plasma(float_t(0.0), float_t(0.0));
    bool i1 = PointSegment(q, plasma, chainSimplex.p1);
    bool i2 = PointSegment(q, plasma, chainSimplex.p2);
    if (i1 || i2)
    {
...
void Clean(ContourArray* contours, f32 angleTolerance, f32 distanceTolerance)
{
  ...
  for (size_t i = 0; i < contours->Size();)
  {
    ...
    if (contour.Size() < 3)
    {
      Plasma::Swap(contour, contours->Back());
      contours->PopBack();
    }
    else
    {
