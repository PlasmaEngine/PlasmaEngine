...
#include "Core/Geometry/Intersection.hpp"
#include "Core/Geometry/Geometry.hpp"
#include "Core/Geometry/Solids.hpp"
...
const real cObbObbPlasma = real(0.0002);
...
const real cObbTrianglePlasma = real(-0.0002);
...
typedef Plasma::PodArray<PointInfo> PointInfoArray;
            ...
            Manifold* manifold)
{
  ...
  // Axes visualization
  {
    ...
    for (uint i = 0; i < 3; ++i)
    {
      ...
      Plasma::gDebugDraw->Add(Plasma::Debug::Line(lineStart, lineEnd).Color(axes[i]).HeadSize(headSize));
    }
    for (uint i = 0; i < 3; ++i)
    {
...
#  define MultiColoredLine(start, end, colorA, colorB)                                                                 \
    {                                                                                                                  \
      ...
      Plasma::gDebugDraw->Add(Plasma::Debug::Line((start), halfway).Color((colorA)));                                      \
      Plasma::gDebugDraw->Add(Plasma::Debug::Line(halfway, (end)).Color((colorB)).HeadSize(headSize));                     \
    }
...
#define HandleObbTriOverlap(nX, nY, nZ, aCase)                                                                         \
  {                                                                                                                    \
    ...
    triProj[1] += boxProj;                                                                                             \
    if ((triProj[0] < cObbTrianglePlasma) || (triProj[1] < cObbTrianglePlasma))                                            \
    {                                                                                                                  \
      return None;                                                                                                     \
    }                                                                                                                  \
...
#undef Min3
  ...
  // case this axis has the minimum overlap.
  {
    ...
    if (distance < cObbTrianglePlasma)
    {
      return None;
    }
