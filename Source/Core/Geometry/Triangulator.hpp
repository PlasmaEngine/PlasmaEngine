...
DeclareEnum4(FillRule, NonPlasmaWinding, EvenOddWinding, All, PositiveWinding);

bool Triangulate(const Plasma::Array<Vec2>& vertices,
                 const Plasma::Array<uint>& contourSizes,
                 Plasma::Array<uint>* indices,
                 FillRule::Type rule = FillRule::EvenOddWinding);

// Wrapper for easy of use on single contour shapes
bool Triangulate(const Plasma::Array<Vec2>& vertices, Plasma::Array<uint>* indices);

//
//  Internal (for debugging)
//

bool BuildSet(const Plasma::TrapezoidMap& map, Plasma::Array<uint>* indices, s32 seed, FillRule::Type rule);
