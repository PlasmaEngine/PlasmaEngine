...
bool Epa::Expand(CSOVertex newPoint)
{
  ...
  Plasma::Array<unsigned> visibleFaces;
  // float maxDot = 0.0f;
  for (unsigned i = 0; i < mFaces.Size(); ++i)
  {
    Face& face = mFaces[i];
...
{
  Plasma::gDebugDraw->Add(Plasma::Debug::Sphere(mDebugPoint.cso, 0.01f).Color(Color::Red));

  for (unsigned i = 0; i < mEdges.Size(); ++i)
  {
    Edge& edge = mEdges[i];
    Plasma::gDebugDraw->Add(Plasma::Debug::Line(mVertices[edge.p0].cso, mVertices[edge.p1].cso).Color(Color::Orange));
  }
  ...
  for (unsigned i = 0; i < mFaces.Size(); ++i)
  {
    Face& face = mFaces[i];
    ByteColor color;
    if (mVisibleFaces.FindIndex(i) != Plasma::Array<unsigned>::InvalidIndex)
      color = Color::Orange;
    else if (i == mIndexClosest)
      color = Color::Red;
    else
      color = Color::Blue;
    Plasma::gDebugDraw->Add(Plasma::Debug::Triangle(mVertices[face.p0].cso, mVertices[face.p1].cso, mVertices[face.p2].cso)
                              .Color(color)
                              .Border(true)
                              .Alpha(50));
  }
