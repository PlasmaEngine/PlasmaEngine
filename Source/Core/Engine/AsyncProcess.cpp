...
LightningDefineType(AsyncProcessEvent, builder, type)
{
  LightningBindDestructor();
  LightningBindDefaultConstructor();
  PlasmaBindDocumented();

  LightningBindField(mBytes);
  LightningBindField(mStreamType);
}

LightningDefineType(AsyncProcess, builder, type)
{
  LightningBindDestructor();
  PlasmaBindDocumented();

  PlasmaBindEvent(Events::PartialStandardOutputResponse, AsyncProcessEvent);
  PlasmaBindEvent(Events::PartialStandardErrorResponse, AsyncProcessEvent);
  PlasmaBindEvent(Events::StandardOutputFinished, Event);
  PlasmaBindEvent(Events::StandardErrorFinished, Event);

  LightningBindMethod(Create);
  LightningFullBindMethod(
      builder, type, &AsyncProcess::Start, LightningInstanceOverload(bool, ProcessStartInfo&), "Start", "startInfo");
  LightningBindMethod(IsRunning);
  LightningFullBindMethod(builder, type, &AsyncProcess::WaitForClose, LightningInstanceOverload(int), "WaitForClose", "");
  LightningFullBindMethod(
      builder, type, &AsyncProcess::WaitForClose, LightningInstanceOverload(int, int), "WaitForClose", "milliseconds");
  LightningBindMethod(Close);
  LightningBindMethod(Terminate);
  LightningBindGetterSetter(StoreStandardOutputData);
  LightningBindGetterSetter(StoreStandardErrorData);
  LightningBindField(mStandardInput);
  LightningBindGetter(StandardOutput);
  LightningBindGetter(StandardError);
}
...
void AsyncProcess::Start(Status& status, ProcessStartInfo& startInfo)
{
  ...
  {
    mStandardInput = LightningAllocate(FileStream);
