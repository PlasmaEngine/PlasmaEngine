...
LightningDefineType(SavingEvent, builder, type)
{
}
...
bool ICodeInspector::SupportsPlasmaConnect()
{
  return false;
}
...
void ICodeInspector::AddCompletionsFromMetaType(Array<Completion>& completions, BoundType* type)
{
  ...
  if (ClassDoc* classDoc = PL::gDocumentation->mClassMap.FindValue(type->Name, nullptr))
  {
...
void ICodeInspector::AddCallTipFromMetaMethod(Array<CallTip>& tips, BoundType* owner, Function* function)
{
  ...
  if (ClassDoc* classDoc = PL::gDocumentation->mClassMap.FindValue(owner->Name, NULL))
  {
...
LightningDefineType(DocumentResource, builder, type)
{
...
DocumentResource::~DocumentResource()
{
  ...
  PL::gResources->TextResources.Erase(this->LoadPath);
}
...
void DocumentResource::DocumentSetup(ResourceEntry& entry, bool searchable)
{
  ...
  if (searchable)
    PL::gResources->TextResources.InsertNoOverwrite(LoadPath, mResourceId);
  UpdatePossibleProxiedClasses();
}

void DocumentResource::UpdatePossibleProxiedClasses()
{
  // This should probably look through only LightningScripts for "class" and
  // LightningFragments for "struct" This can possibly point a proxied fragment to a
  // LightningScript
  static Regex sClassStructRegex("(class|struct)\\s+([A-Z][a-zA-Z0-9_]*)");

  Matches matches;
  Plasma::StringRange unparsedContent = LoadTextData();
...
void DocumentResource::UpdateContentItem(ContentItem* contentItem)
{
  ...
  ResourceSystem::TextResourceMap& textResources = PL::gResources->TextResources;
...
{
  if (mContentItem && PL::gContentSystem->mHistoryEnabled)
  {
    String backUpPath = PL::gContentSystem->GetHistoryPath(mContentItem->mLibrary);
