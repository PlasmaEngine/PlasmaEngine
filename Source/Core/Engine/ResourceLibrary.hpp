...
DeclareEvent(CompileLightningFragments);
// Sent out on the PL::gResources in the constructor of ResourceLibrary (no
// resources added/loaded yet).
DeclareEvent(ResourceLibraryConstructed);
} // namespace Events

// Status of Lightning Script Compile
DeclareEnum2(LightningCompileStatus,
...
// Lightning Compiled Event
class LightningCompiledEvent : public Event
{
public:
  LightningDeclareType(LightningCompiledEvent, TypeCopyMode::ReferenceType);
  LibraryRef mLibrary;
};

// Compile Fragment Event
class LightningCompileFragmentEvent : public Event
{
public:
  LightningDeclareType(LightningCompileFragmentEvent, TypeCopyMode::ReferenceType);
  LightningCompileFragmentEvent(Module& dependencies,
                            Array<LightningDocumentResource*>& fragments,
                            ResourceLibrary* owningLibrary);

  Module& mDependencies;
  Array<LightningDocumentResource*>& mFragments;
...
public:
  ResourcePackage();
  LightningDeclareType(ResourcePackage, TypeCopyMode::ReferenceType);
...
{
  LightningDeclareType(ResourcePackageDisplay, TypeCopyMode::ReferenceType);
...
public:
  ...
  /// call this function in response to the LightningManager compile events before
  ...
  // Only called once all libraries within Plasma are fully compiled
  ...
  Array<LightningDocumentResource*> mScripts;
  Array<LightningDocumentResource*> mFragments;
  Array<LightningLibraryResource*> mPlugins;

  // The plugins that this resource library has built (may be empty if none
  // exist or haven't been compiled yet)
  OrderedHashMap<LightningLibraryResource*, SwapLibrary> mSwapPlugins;
  ...
  // We need this to stick around for the Lightning debugger
  ...
  // These should be set during the Graphics and LightningScript project
  // initialization.
