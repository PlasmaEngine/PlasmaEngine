...
DefineEvent(CompileLightningFragments);
DefineEvent(ResourceLibraryConstructed);
} // namespace Events

LightningDefineType(LightningCompiledEvent, builder, type)
{
}

LightningDefineType(LightningCompileFragmentEvent, builder, type)
{
}

// Resource Entry
LightningCompileFragmentEvent::LightningCompileFragmentEvent(Module& dependencies,
...
{
  String idString = Plasma::ToString(this->mResourceId, shortFormat);
...
Plasma::ResourceTemplate* ResourceEntry::GetResourceTemplate()
{
  if (mLibrarySource)
...
LightningDefineType(ResourcePackageDisplay, builder, type)
{
}
...
LightningDefineType(ResourcePackage, builder, type)
{
...
// Compiled Library
SwapLibrary::SwapLibrary() : mCompileStatus(LightningCompileStatus::Modified)
{
}
...
void SwapLibrary::Commit()
{
  if (mPendingLibrary)
  {
    ErrorIf(mCompileStatus != LightningCompileStatus::Compiled,
...
LightningDefineType(ResourceLibrary, builder, type)
{
}
...
ResourceLibrary::ResourceLibrary()
{
  ...
  EventConnect(&mScriptProject, Lightning::Events::PreParser, &ResourceLibrary::OnScriptProjectPreParser, this);
  EventConnect(&mScriptProject, Lightning::Events::PostSyntaxer, &ResourceLibrary::OnScriptProjectPostSyntaxer, this);
  EventConnect(&mScriptProject, Lightning::Events::TypeParsed, &EngineLibraryExtensions::TypeParsedCallback);

  ObjectEvent toSend(this);
  PL::gResources->DispatchEvent(Events::ResourceLibraryConstructed, &toSend);
}
...
void ResourceLibrary::Add(Resource* resource, bool isNew)
{
  ...
  // Filter lightning resources into their appropriate containers
  ...
  {
    mScripts.PushBack((LightningDocumentResource*)resource);
    ScriptsModified();
  }
  else if (resourceType->IsA(sFragmentType))
  {
    mFragments.PushBack((LightningDocumentResource*)resource);
    FragmentsModified();
  }
  else if (resourceType->IsA(LightningTypeId(LightningLibraryResource)))
  {
    mPlugins.PushBack((LightningLibraryResource*)resource);
    PluginsModified();
  }
...
void ResourceLibrary::Remove(Resource* resource)
{
  ...
  // Remove lightning resources from their containers
  BoundType* resourceType = LightningVirtualTypeId(resource);
  if (resourceType->IsA(sScriptType))
  {
    mScripts.EraseValue((LightningDocumentResource*)resource);
    ScriptsModified();
  }
  else if (resourceType->IsA(sFragmentType))
  {
    mFragments.EraseValue((LightningDocumentResource*)resource);
    FragmentsModified();
  }
  else if (resourceType->IsA(LightningTypeId(LightningLibraryResource)))
  {
    LightningLibraryResource* libraryResource = (LightningLibraryResource*)resource;
...
void ResourceLibrary::Unload()
{
  ...
  forRange (Resource* resource, Resources.All())
  {
    if (resource != nullptr && resource->GetReferenceCount() != 1)
    {
      ...
      String resourceType = resource->LightningGetDerivedType()->Name;
  ...
  PL::gResources->LoadedResourceLibraries.Erase(Name);
  ...
  ResourceEvent event;
  PL::gResources->DispatchEvent(Events::ResourcesUnloaded, &event);
...
{
  mSwapScript.mCompileStatus = LightningCompileStatus::Modified;
  LightningManager::GetInstance()->mShouldAttemptCompile = true;
...
{
  mSwapScript.mCompileStatus = LightningCompileStatus::Modified;
  mSwapFragment.mCompileStatus = LightningCompileStatus::Modified;
  LightningManager::GetInstance()->mShouldAttemptCompile = true;
...
bool ResourceLibrary::CompileScripts(HashSet<ResourceLibrary*>& modifiedLibrariesOut)
{
  ...
  if (mSwapScript.mCompileStatus == LightningCompileStatus::Compiled)
    return true;
  ...
  ++LightningManager::GetInstance()->mVersion;
  ...
  PlasmaPrint("  Compiling %s Scripts\n", this->Name.c_str());

  // Clear the project out since it may have files from before
  mScriptProject.Clear();

  // Add all scripts
  forRange (LightningDocumentResource* script, mScripts)
  {
  ...
  if (mSwapScript.mPendingLibrary != nullptr)
  {
    ...
    mSwapScript.mCompileStatus = LightningCompileStatus::Compiled;
    return true;
  }
...
bool ResourceLibrary::CompileFragments(HashSet<ResourceLibrary*>& modifiedLibraries)
{
  // If we already compiled, then we know that all dependent libraries must have
  // been good
  if (mSwapFragment.mCompileStatus == LightningCompileStatus::Compiled)
    return true;
  ...
  // By this point, we've already compiled all our dependencies
  PlasmaPrint("  Compiling %s Fragments\n", this->Name.c_str());

  LightningCompileFragmentEvent e(dependencies, mFragments, this);
  LightningManager::GetInstance()->DispatchEvent(Events::CompileLightningFragments, &e);
  mSwapFragment.mPendingLibrary = e.mReturnedLibrary;

  if (mSwapFragment.mPendingLibrary != nullptr)
  {
    modifiedLibraries.Insert(this);
    LightningManager::GetInstance()->mPendingFragmentProjectLibrary = mSwapFragment.mPendingLibrary;
    mSwapFragment.mCompileStatus = LightningCompileStatus::Compiled;
    return true;
  }
...
{
  // Plugins should only depend on the Core library and native Plasma libraries
  ...
  forRange (LightningLibraryResource* libraryResource, mPlugins.All())
  {
    if (libraryResource->GetResourceTemplate() != nullptr)
      continue;

    SwapLibrary& swapPlugin = mSwapPlugins[libraryResource];
    if (swapPlugin.mCompileStatus == LightningCompileStatus::Modified)
    {
      Status status;
      ...
      {
        modifiedLibrariesOut.Insert(this);
        swapPlugin.mCompileStatus = LightningCompileStatus::Compiled;
      }
      else
      {
