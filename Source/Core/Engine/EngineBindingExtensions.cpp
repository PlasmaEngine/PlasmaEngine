...
// Meta Resource
LightningDefineType(MetaResource, builder, type)
{
}
...
LightningDefineType(MetaEditorScriptObject, builder, type)
{
  LightningBindField(mAutoRegister)->AddAttribute(PropertyAttributes::cOptional);
}
...
// Meta Dependency
LightningDefineType(MetaDependency, builder, type)
{
}
...
void MetaDependency::PostProcess(Status& status, ReflectionObject* owner)
{
  ...
  if (!classType->IsA(LightningTypeId(Component)))
  {
  ...
  CogComponentMeta* componentMeta = classType->HasOrAdd<::Plasma::CogComponentMeta>(classType);
  componentMeta->mDependencies.Insert(propertyType);
  componentMeta->mSetupMode = SetupMode::DefaultConstructor;
}

// Meta Interface
LightningDefineType(MetaInterface, builder, type)
{
}
