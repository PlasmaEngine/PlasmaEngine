...
LightningDefineType(CogReplaceEvent, builder, type)
{
}
...
{
  PL::gUndoMap->UpdateHandleIfExists(oldCog, newCog);

  // Update Component handles
  forRange (Component* newComponent, newCog->GetComponents())
  {
    BoundType* componentType = LightningVirtualTypeId(newComponent);
    if (Component* oldComponent = oldCog->QueryComponentType(componentType))
      PL::gUndoMap->UpdateHandleIfExists(oldComponent, newComponent);
  }
...
Cog* ArchetypeRebuilder::RebuildCog(Cog* oldCog, HashSet<MetaSelection*>* modifiedSelections)
{
  ...
  Cog* updatedCog = PL::gFactory->BuildFromStream(&context, loader);
