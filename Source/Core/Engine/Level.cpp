...
LightningDefineType(Level, builder, type)
{
...
void Level::SaveSpace(Space* space)
{
  ...
  BackUpFile(PL::gContentSystem->GetHistoryPath(mContentItem->mLibrary), mContentItem->GetFullPath());

  // Save level to file
  space->SaveLevelFile(this->LoadPath);

  PlasmaPrintFilter(Filter::ResourceFilter, "Saved level file '%s'.\n", Name.c_str());
