...
PlasmaThreadLocal ErrorContextStack* ActiveErrorContextStack = nullptr;
...
String ErrorContextObject::GetDescription()
{
  ...
  if (ContextObject)
  {
    BoundType* metaType = LightningVirtualTypeId(ContextObject);
...
void DoNotifyErrorWithContext(StringParam message, NotifyException::Enum notifyException)
{
  ...
  if (PL::gNotifyCallbackStack.Back() == IgnoreDoNotify)
    return;
