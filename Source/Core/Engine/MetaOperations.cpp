...
LightningDefineType(PropertyOperation, builder, type)
{
  LightningBindFieldGetter(mValueBefore);
  LightningBindFieldGetter(mValueAfter);
}
    ...
    mPropertyPath(property)
{
  ...
  ConnectThisTo(LightningManager::GetInstance(), Events::ScriptsCompiledPrePatch, OnScriptsCompiled);
}
...
void PropertyOperation::OnScriptsCompiled(LightningCompileEvent* e)
{
