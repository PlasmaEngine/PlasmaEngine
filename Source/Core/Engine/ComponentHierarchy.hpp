...
{
public:
  LightningDeclareType(ComponentHierarchy, TypeCopyMode::ReferenceType);
...
template <typename ComponentType>
LightningDefineType(ComponentHierarchy<ComponentType>, builder, type)
{
  PlasmaBindDocumented();
  LightningBindGetter(PreviousSibling);
  LightningBindGetter(NextSibling);
  LightningBindGetter(LastDirectChild);
  LightningBindGetter(LastDeepestChild);
  LightningBindGetter(NextInHierarchyOrder);
  LightningBindGetter(PreviousInHierarchyOrder);
  LightningBindFieldGetter(mParent);
  LightningBindGetter(Root);
  LightningBindGetter(ChildCount);

  LightningBindMethod(GetChildren);
