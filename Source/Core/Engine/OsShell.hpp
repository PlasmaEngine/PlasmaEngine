...
{
public:
  LightningDeclareType(OsShell, TypeCopyMode::ReferenceType);
  ...
  /// Scan for new input devices and register them with Plasma.
  ...
  static void ShellOnPaste(ClipboardData& data, Shell* shell);
};

class ClipboardEvent : public Event, public ClipboardData
{
public:
  LightningDeclareType(ClipboardEvent, TypeCopyMode::ReferenceType);
  void Clear();
...
{
public:
  LightningDeclareType(OsFileSelection, TypeCopyMode::ReferenceType);
  bool Success;
  Array<String> Files;
};
...
{
public:
  typedef SimpleSaveFileDialog LightningSelf;
