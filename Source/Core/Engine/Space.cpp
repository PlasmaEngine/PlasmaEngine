...
LightningDefineType(Space, builder, type)
{
  ...
  LightningBindMethod(CreateAtPosition);
  LightningBindMethod(CreateLink);

  LightningBindMethod(LoadLevel);
  LightningBindMethod(ReloadLevel);
  LightningBindGetter(CurrentLevel);
  LightningBindMethod(AddObjectsFromLevel);

  LightningBindMethod(FindObjectByName);
  LightningBindMethod(FindFirstObjectByName);
  LightningBindMethod(FindLastObjectByName);
  LightningBindMethod(FindFirstRootObjectByName);
  LightningBindMethod(FindLastRootObjectByName);
  LightningBindMethod(FindAllObjectsByName);
  LightningBindMethod(FindAllRootObjectsByName);

  LightningBindMethod(DestroyAll);
  LightningBindMethod(DestroyAllFromLevel);

  LightningBindCustomGetter(IsEditorMode);
  LightningBindMethod(MarkModified);
  LightningBindMethod(GetModified);
  LightningBindMethod(MarkNotModified);

  LightningBindCustomGetter(AllObjects);
  LightningBindCustomGetter(AllRootObjects);
}
...
Space::~Space()
{
  ...
  PL::gEngine->mSpaceList.Erase(this);
...
void Space::Initialize(CogInitializer& initializer)
{
  ...
  PL::gEngine->mSpaceList.PushBack(this);
...
Cog* Space::CreateAt(StringParam source, Vec3Param position, QuatParam rotation, Vec3Param scale)
{
  ...
  Cog* cog = PL::gFactory->BuildAndSerialize(LightningTypeId(Cog), source, &context);
...
Cog* Space::CreateNamedLink(StringParam archetypeName, Cog* objectA, Cog* objectB)
{
  ...
  CogInitializer initializer(this);
  initializer.Context = &context;
  Cog* cog = PL::gFactory->BuildAndSerialize(LightningTypeId(Cog), archetypeName, &context);
...
void Space::SerializeObjectsToSpace(CogInitializer& initializer, CogCreationContext& context, Serializer& loader)
{
  ...
  {
    Cog* cog = PL::gFactory->BuildFromStream(&context, loader);
    if (cog == nullptr)
    {
...
void Space::LoadLevelFile(StringParam filePath)
{
  ...
  if (stream)
  {
    ...
    MarkNotModified();
    PlasmaPrintFilter(Filter::DefaultFilter, "Level file '%s' was loaded.\n", fileName.c_str());
...
Level* Space::AddObjectsFromLevel(Level* level)
{
  ...
  if (level)
  {
    ...
    if (status.Succeeded())
    {
      ...
      MarkNotModified();
      PlasmaPrint("Level '%s' was loaded.\n", level->Name.c_str());
    }
    else
    {
...
void Space::LoadLevel(Level* level)
{
  ...
  if (Level* pendingLoad = mPendingLevel)
  {
    PlasmaPrintFilter(Filter::DefaultFilter, "Already loading level '%s'\n", pendingLoad->ResourceIdName.c_str());
    return;
  }

  if (level == nullptr)
  {
    PlasmaPrintFilter(Filter::DefaultFilter, "Failed to find level.\n");
    return;
  }

  if (!mLevelLoaded)
  {
    PlasmaPrintFilter(Filter::DefaultFilter, "Loading level '%s' directly.\n", level->Name.c_str());
  ...
  {
    PlasmaPrintFilter(Filter::DefaultFilter, "Loading level '%s' delayed.\n", level->Name.c_str());
