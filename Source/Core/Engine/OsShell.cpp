...
void OsShell::ShellOnPaste(ClipboardData& data, Shell* shell)
{
  OsShell* self = (OsShell*)shell->mUserData;
  ClipboardEvent toSend;
  toSend.mHasImage = data.mHasImage;
  toSend.mHasText = data.mHasText;
  toSend.mImage = data.mImage;
  toSend.mText = data.mText;
...
void OsShell::Update(bool debugger)
{
  ...
  // Plasma the cursor movement before the windows message pump to clear last
  // frames movement
...
LightningDefineType(ClipboardEvent, builder, type)
{
  LightningBindMethodProperty(Clear);
  LightningBindGetterSetterProperty(Text);
  LightningBindMemberProperty(mHandled);
}
