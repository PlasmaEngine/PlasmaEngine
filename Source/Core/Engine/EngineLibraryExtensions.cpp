...
void EngineLibraryExtensions::TypeParsedCallback(Lightning::ParseEvent* e, void* userData)
{
  ...
  // is defined
  LightningDocumentResource* resource = (LightningDocumentResource*)e->Location->CodeUserData;
...
void EngineLibraryExtensions::FindProxiedTypeOrigin(BoundType* proxiedType)
{
  ...
  DocumentResource* document = static_cast<DocumentResource*>(PL::gResources->GetResource(id));
  if (document)
...
bool FindResourceMode(Call& call, ExceptionReport& report, ResourceNotFound::Enum mode)
{
  ...
  ResourceManager* manager = PL::gResources->Managers.FindValue(currentFunction->Owner->Name, nullptr);
...
void AddResourceFind(LibraryBuilder& builder, BoundType* resourceType)
{
  ...
  p0.ParameterType = LightningTypeId(String);
  p0.Name = "name";
