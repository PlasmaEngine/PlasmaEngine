...
// (such as errors) This is primarily used when Lightning fully compiles and informs
...
public:
  ...
  /// Gets whether or not we support completion of Plasma.Connect(...
  virtual bool SupportsPlasmaConnect();
  ...
  /// When generating the Plasma.Connect function, where should we put it?
  ...
  /// a DocMethod first)
  static void AddCallTipFromMetaMethod(Array<CallTip>& tips, BoundType* owner, Lightning::Function* method);
...
{
public:
  LightningDeclareType(DocumentResource, TypeCopyMode::ReferenceType);
...
{
public:
  LightningDeclareType(SavingEvent, TypeCopyMode::ReferenceType);
