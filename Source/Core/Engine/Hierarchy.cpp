...
// Hierarchy Event
LightningDefineType(HierarchyEvent, builder, type)
{
  PlasmaBindDocumented();
  PlasmaBindEvent(Events::Attached, HierarchyEvent);
  PlasmaBindEvent(Events::Detached, HierarchyEvent);
  PlasmaBindEvent(Events::ChildAttached, HierarchyEvent);
  PlasmaBindEvent(Events::ChildDetached, HierarchyEvent);
  PlasmaBindEvent(Events::ChildrenOrderChanged, Event);
  LightningBindFieldProperty(Parent);
  LightningBindFieldProperty(Child);
}

LightningDefineType(HierarchyComposition, builder, type)
{
}

// Hierarchy
LightningDefineType(Hierarchy, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindDocumented();
...
HierarchyComposition::HierarchyComposition() : MetaComposition(LightningTypeId(Cog))
{
}
...
{
  if (typeToAdd == LightningTypeId(Cog))
    return true;
