...
#define RegisterBroadPhase(type, canBeUsedAs)                                                                          \
  PL::gBroadPhaseLibrary->RegisterBroadPhaseCreator(LightningTypeId(type), new BroadPhaseCreatorType<type>(), canBeUsedAs);
...
{
public:
  LightningDeclareType(BroadphasePropertyExtension, TypeCopyMode::ReferenceType);
  static void EnumerateBroadphases(HandleParam instance, Property* property, Array<String>& strings)
  {
    PL::gBroadPhaseLibrary->EnumerateNamesOfType(BroadphaseType, strings);
  }
