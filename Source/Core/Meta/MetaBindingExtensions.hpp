...
/// For LightningComponent's, Initialize isn't called in an Editor Space. However,
...
#define PlasmaAdvancedGroup()                                                                                            \
  AddAttribute(::Plasma::PropertyAttributes::cGroup)->AddParameter(::Plasma::CommonGroups::cAdvanced)
...
#define PlasmaBindExternalEvent(EventName, EventType, SenderType)                                                        \
  BindEventSent(builder, LightningTypeId(SenderType), (EventName), LightningTypeId(EventType))
