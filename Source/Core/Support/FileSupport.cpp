...
void Download(StringParam filePath)
{
  Download(FilePath::GetFileNameWithoutExtension(filePath),
           ...
           Array<String>(PlasmaInit, filePath));
}
...
void Download(StringParam suggestedNameWithoutExtension, StringParam workingDirectory, const Array<String>& filePaths)
{
  ...
  if (filePaths.Size() == 1 && FileExists(filePaths.Front()))
  {
    ...
    PlasmaPrint("Downloading single file %s\n", filePath.c_str());
  ...
  else
  {
    ...
    forRange (StringParam filePath, filePaths)
    {
      ...
      if (normalized.StartsWith(workingDirectoryNormalized))
      {
        ...
        {
          PlasmaPrint("Adding file to download archive (in working directory) %s\n", filePath.c_str());
          archive.AddFile(filePath, relativePath);
        }
        else
        {
          PlasmaPrint("Adding directory to download archive (in working directory) %s\n", filePath.c_str());
      ...
      else
      {
        if (FileExists(filePath))
        {
          PlasmaPrint("Adding file to download archive (out of working directory) %s\n", filePath.c_str());
          archive.AddFile(filePath, filePath);
        }
        else
        {
          PlasmaPrint("Adding directory to download archive (out of working directory) %s\n", filePath.c_str());
