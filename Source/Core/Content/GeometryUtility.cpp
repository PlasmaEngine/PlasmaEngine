...
{
  plasmaArray.Resize(numElements);
  for (uint i = 0; i < numElements; ++i)
  {
    aiVector3D aiVec = aiArray[i];
    plasmaArray.PushBack(Vec2(aiVec.x, aiVec.y));
  }
}
...
void ConvertAndFillArrayVec3(aiVector3D* aiArray, uint numElements, Array<Vec3>& plasmaArray)
{
  ...
  {
    aiVector3D aiVec = aiArray[i];
    plasmaArray.PushBack(Vec3(aiVec.x, aiVec.y, aiVec.z));
  }
}
...
void ConvertAndFillArrayVec4(aiColor4D* aiArray, uint numElements, Array<Vec4>& plasmaArray)
{
  ...
  for (uint i = 0; i < numElements; ++i)
  {
    ...
    plasmaArray.PushBack(Vec4(aiVec.r, aiVec.g, aiVec.b, aiVec.a));
  }
}
