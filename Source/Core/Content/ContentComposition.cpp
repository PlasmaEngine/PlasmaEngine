...
LightningDefineType(ContentComponent, builder, type)
{
  type->HandleManager = LightningManagerId(PointerManager);
  type->Add(new MetaSerialization());
}

LightningDefineType(BuilderComponent, builder, type)
{
  type->AddAttribute(ObjectAttributes::cCore);
}

DefineSafeIdHandle(ContentComposition);
LightningDefineType(ContentComposition, builder, type)
{
  PlasmaBindHandle();
  type->Add(new ContentMetaComposition());
}

LightningDefineType(ContentMetaComposition, builder, type)
{
}

ContentMetaComposition::ContentMetaComposition() : MetaComposition(LightningTypeId(ContentComponent))
{
}
...
{
  return LightningAllocate(ContentComponent, typeToCreate);
}
...
void SerializeComponents(Serializer& stream, ContentComposition* contentItem)
{
  ...
  if (stream.GetMode() == SerializerMode::Loading)
  {
    ...
    while (stream.GetPolymorphic(node))
    {
      ...
      ContentComponent* rc = PL::gContentSystem->ComponentFactory.CreateFromName(node.TypeName);
      if (rc)
      {
        rc->Serialize(stream);
      ...
      else
      {
        ...
        PlasmaPrintFilter(Filter::DefaultFilter,
...
void ContentComposition::AddComponent(ContentComponent* cc)
{
  ...
  mComponentMap.Insert(LightningVirtualTypeId(cc), cc);
  cc->mOwner = this;
