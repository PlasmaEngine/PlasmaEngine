...
LightningDefineType(KeyFrameEvent, builder, type)
{
}

LightningDefineType(TrackEvent, builder, type)
{
}
...
Vec2 KeyFrame::GetGraphPosition()
{
  ...
  float y = 0.0f;
  if (keyType->IsA(LightningTypeId(float)))
    y = mValue.Get<float>();
  else if (keyType->IsA(LightningTypeId(int)))
    y = (float)mValue.Get<int>();
  else if (keyType->IsA(LightningTypeId(bool)))
...
LightningDefineType(TrackNode, builder, type)
{
...
TrackNode::TrackNode()
{
  ...
  mDisabled = false;
  mPropertyType = LightningTypeId(void);
                     ...
                     RichAnimation* richAnim)
{
  ...
  {
    BoundType* boundType = LightningTypeId(float);
    mPropertyType = boundType;
  }

  /// Add sub tracks if we're a vector type
  if (mPropertyType == LightningTypeId(Vec2))
    CreateSubTracks(this, "XY", richAnim);
  else if (mPropertyType == LightningTypeId(Vec3))
    CreateSubTracks(this, "XYZ", richAnim);
  else if (mPropertyType == LightningTypeId(Vec4))
    CreateSubTracks(this, "RGBA", richAnim);
  else if (mPropertyType == LightningTypeId(Quat))
...
void TrackNode::Serialize(Serializer& stream)
{
  ...
  if (stream.GetMode() == SerializerMode::Loading)
  {
    if (mPropertyType.mName == "float")
      mPropertyType = LightningTypeId(float);
    else if (mPropertyType.mName == "bool")
      mPropertyType = LightningTypeId(bool);
    else if (mPropertyType.mName == "uint")
      mPropertyType = LightningTypeId(int);
    else if (mPropertyType.mName == "Vec2")
      mPropertyType = LightningTypeId(Vec2);
    else if (mPropertyType.mName == "Vec3")
      mPropertyType = LightningTypeId(Vec3);
    else if (mPropertyType.mName == "Vec4")
      mPropertyType = LightningTypeId(Vec4);
    else if (mPropertyType.mName == "Quat")
      mPropertyType = LightningTypeId(Quat);
  }
...
void TrackNode::Initialize(RichAnimation* richAnimation, TrackNode* parent)
{
  ...
  if (Type == TrackType::Property)
  {
    ...
    if (mPropertyType == LightningTypeId(void) && mTargetMeta)
    {
...
Any TrackNode::SampleTrack(float t)
{
  ...
  if (mPropertyType == LightningTypeId(float))
  {
    BakePiecewiseFunction();
    return mBakedCurve.Sample(t);
  }
  else if (mPropertyType == LightningTypeId(Vec2))
...
Any TrackNode::SampleObject(Cog* animGraphObject)
{
  ...
  if (Type == TrackType::SubProperty)
  {
    ...
    if (parentType == LightningTypeId(Vec2))
      return SubSample<Vec2>(sample, element);
    else if (parentType == LightningTypeId(Vec3))
      return SubSample<Vec3>(sample, element);
    else if (parentType == LightningTypeId(Vec4))
      return SubSample<Vec4>(sample, element);
    else if (parentType == LightningTypeId(Quat))
...
Property* TrackNode::GetProperty(Cog* animGraphObject)
{
  ...
  if (Component* component = GetComponent(animGraphObject))
  {
    ...
    BoundType* componentMeta = LightningVirtualTypeId(component);
...
void TrackNode::BakeKeyFrames(Array<KeyEntry>& keyFrames)
{
  ...
  // on the component
  if (mPropertyType == LightningTypeId(void))
    return;
  // For float types, we want to use the baked points on an adaptive curve
  else if (mPropertyType == LightningTypeId(float))
  {
    BakePiecewiseFunction();
...
LightningDefineType(RichAnimation, builder, type)
{
  type->HandleManager = LightningManagerId(PointerManager);
  LightningBindGetterSetterProperty(SampleTolerance);
}
    ...
    Cog* object, Cog* animGraphObject, BoundType* componentType, StringParam propertyName, bool createNew)
{
  ...
  BoundType* virtualComponentType = LightningVirtualTypeId(component);
...
LightningDefineType(RichAnimationBuilder, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::CallSetDefaults);

  LightningBindGetterSetterProperty(PreviewArchetype)->Add(new MetaEditorResource(false, true));
}
