...
GeometryProcessorCodes::Enum GeometryImporter::ProcessModelFiles()
{
  ...
  {
    PlasmaPrint("Missing model file '%s'\n", mInputFile.c_str());
    return Plasma::GeometryProcessorCodes::Failed;
  }

  if (!FileExists(mMetaFile))
  {
    PlasmaPrint("Missing meta file '%s'\n", mMetaFile.c_str());
    return Plasma::GeometryProcessorCodes::Failed;
  }
  ...
  if (!mScene)
  {
    ...
    PlasmaPrint("Geometry Processor Error: %s\n", error.c_str());
    return Plasma::GeometryProcessorCodes::Failed;
  }
...
String GeometryImporter::ExtractDataFromNodesRescursive(aiNode* node, String parentName)
{
  ...
  hierarchyData.mLocalTransform = AiMat4ToPlasmaMat4(node->mTransformation);
