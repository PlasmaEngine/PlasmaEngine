...
namespace Plasma
{
LightningDefineType(AudioContent, builder, type)
{
}
...
// SoundBuilder
LightningDefineType(SoundBuilder, builder, type)
{
  PlasmaBindDependency(AudioContent);
  PlasmaBindDocumented();
  PlasmaBindExpanded();

  LightningBindFieldProperty(Name);
  LightningBindFieldProperty(mFileLoadType);
  LightningBindFieldProperty(mNormalize)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindFieldProperty(mMaxVolume)->Add(new EditorSlider(0.0f, 1.0f, 0.1f))->PlasmaFilterBool(mNormalize);
}
...
void CreateAudioContent(ContentSystem* system)
{
  ...
  ContentTypeEntry audioContent(LightningTypeId(AudioContent), MakeAudioContent);
