...
LightningDefineType(CameraViewport, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDocumented();

  LightningBindGetterSetterProperty(RenderInEditor);
  LightningBindGetterSetterProperty(RenderInGame);
  LightningBindFieldProperty(mRenderOrder);
  LightningBindFieldProperty(mCameraPath)->PlasmaBindPropertyRename("Camera");
  LightningBindFieldProperty(mRendererPath)->PlasmaBindPropertyRename("Renderer");
  LightningBindGetterSetterProperty(ResolutionOrAspect);
  LightningBindGetterSetterProperty(RenderToViewport)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindFieldProperty(mForwardViewportEvents)->PlasmaFilterEquality(mRenderToViewport, bool, true);

  LightningBindFieldProperty(mViewportScaling)->PlasmaFilterEquality(mRenderToViewport, bool, true);
  LightningBindFieldProperty(mMarginColor)->PlasmaFilterEquality(mRenderToViewport, bool, true);
  LightningBindFieldProperty(mNormalizedSize)->PlasmaFilterEquality(mRenderToViewport, bool, true);
  LightningBindFieldProperty(mNormalizedOffset)->PlasmaFilterEquality(mRenderToViewport, bool, true);

  LightningBindMethod(ViewportTakeFocus);
  LightningBindGetter(ViewportHasFocus);

  LightningBindGetter(WorldToView);
  LightningBindGetter(ViewToPerspective);
  LightningBindGetter(WorldToPerspective);
  LightningBindGetter(FinalTexture);

  LightningBindGetter(MouseWorldRay);
  LightningBindMethod(ScreenToWorldRay);
  LightningBindMethod(ScreenToWorldZPlane);
  LightningBindMethod(ScreenToWorldViewPlane);
  LightningBindMethod(ScreenToWorldPlane);
  LightningBindMethod(WorldToScreen);
  LightningBindMethod(ScreenToViewport);
  LightningBindMethod(ViewportToScreen);
  LightningBindMethod(ViewPlaneSize);

  LightningBindGetter(ViewportResolution);
  LightningBindGetter(ViewportResolutionWithMargin);
  LightningBindGetter(ViewportOffset);
  LightningBindGetter(ViewportOffsetWithMargin);

  LightningBindGetter(Camera);
  LightningBindGetter(Frustum);
}
