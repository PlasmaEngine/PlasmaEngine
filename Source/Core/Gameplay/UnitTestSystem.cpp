...
static const String cPlasmaProjWithoutDot("plasmaproj");
static const String cPlasmaProjWithDot(".plasmaoproj");
static const IntVec2 cWindowSize(1024, 768);

LightningDefineType(UnitTestEvent, builder, type)
{
...
LightningDefineType(UnitTestEndFrameEvent, builder, type)
{
...
LightningDefineType(UnitTestBaseMouseEvent, builder, type)
{
  LightningBindDestructor();
}
...
LightningDefineType(UnitTestMouseEvent, builder, type)
{
  LightningBindConstructor();
  LightningBindDestructor();
}
...
LightningDefineType(UnitTestMouseDropEvent, builder, type)
{
  LightningBindConstructor();
  LightningBindDestructor();
}
...
LightningDefineType(UnitTestKeyboardEvent, builder, type)
{
  LightningBindConstructor();
  LightningBindDestructor();
}
...
LightningDefineType(UnitTestKeyboardTextEvent, builder, type)
{
  LightningBindConstructor();
  LightningBindDestructor();
}
...
LightningDefineType(UnitTestWindowEvent, builder, type)
{
  LightningBindConstructor();
  LightningBindDestructor();
}
...
// Unit Test System
LightningDefineType(UnitTestSystem, builder, type)
{
}
...
void UnitTestSystem::Initialize(SystemInitializer& initializer)
{
  ...
  if (unitTestSystemActive)
  {
    ...
    PL::gEngine->has(OsShell)->mOsShellHook = this;
...
void UnitTestSystem::HookUpdate()
{
  ...
  else if (mMode == UnitTestMode::Playing)
  {
    while (mEventIndex < mEvents.Size())
    {
      ...
      if (LightningVirtualTypeId(event) != LightningTypeId(UnitTestEndFrameEvent))
        event->Execute(this);
      else
        break;
    }
...
void UnitTestSystem::RecordToPlasmaTestFile()
{
  ...
  config->AddFilter("Unit Test (*.plasmatest)", "*.plasmatest");
  config->mDefaultSaveExtension = "plasmatest";
  config->DefaultFileName = BuildString(PL::gEngine->GetProjectSettings()->ProjectName, ".plasmatest");
...
void UnitTestSystem::OnUnitTestRecordFileSelected(OsFileSelection* event)
{
  ...
  RecordToPlasmaTestFile(filePath);
  Download(filePath);
}

void UnitTestSystem::RecordToPlasmaTestFile(StringParam plasmaTestFile)
{
  String name = FilePath::GetFileNameWithoutExtension(plasmaTestFile);
  String directory = FilePath::Combine(FilePath::GetDirectoryPath(plasmaTestFile), name);
  ...
  ProjectSettings* settings = PL::gEngine->GetProjectSettings();
  ...
  String newProjectFileName = BuildString(name, cPlasmaProjWithDot);
  ...
  // Rename the project files to the same name as the plasmatest file
  ...
  archive.WriteZipFile(plasmaTestFile);
  DeleteDirectory(directory);
}

void UnitTestSystem::PlayFromPlasmaTestFile()
{
  FileDialogConfig* config = FileDialogConfig::Create();
...
void UnitTestSystem::OnUnitTestPlayFileSelected(OsFileSelection* event)
{
  ...
  PlayFromPlasmaTestFile(event->Files.Front());
}

void UnitTestSystem::PlayFromPlasmaTestFile(StringParam plasmaTestFile)
{
  String name = FilePath::GetFileNameWithoutExtension(plasmaTestFile);
  String directory = FilePath::Combine(FilePath::GetDirectoryPath(plasmaTestFile), name);
  if (DirectoryExists(directory))
  ...
  archive.ReadZipFile(ArchiveReadFlags::All, plasmaTestFile);
  ...
  String projectFileName = BuildString(name, cPlasmaProjWithDot);
...
void UnitTestSystem::SubProcessPlay()
{
  ...
  // time)
  Cog* project = PL::gEngine->GetProjectSettings()->GetOwner();
  project->AddComponentByType(LightningTypeId(FrameRateSettings));
...
OsWindow* UnitTestSystem::SubProcessSetupWindow()
{
  ...
  IntVec2 monitorSize = PL::gEngine->has(OsShell)->GetPrimaryMonitorSize();
...
void UnitTestSystem::DiffDirectories(StringParam dir1, StringParam dir2, StringParam diffProgram)
{
  ...
  forRange (StringParam relativePath, relativePaths1.All())
  {
    ...
    if (relativePaths2.Contains(relativePath))
    {
      ...
      // Don't diff the plasmaproj file because the verison will
      // often be different (and frame rate settings may change)
      if (FilePath::GetExtension(relativePath) == cPlasmaProjWithoutDot)
        continue;
      ...
      String hash1 = Lightning::Sha1Builder::GetHashStringFromFile(status, path1);
      if (status.Failed())
      {
      ...
      String hash2 = Lightning::Sha1Builder::GetHashStringFromFile(status, path2);
      if (status.Failed())
      {
        DoNotifyWarning("UnitTestSystem", String::Format("Failed to open file: %s", status.Message.c_str()));
        continue;
...
void UnitTestSystem::RecordEvent(UnitTestEvent* e)
{
  ...
  BoundType* type = LightningVirtualTypeId(e);
...
void UnitTestSystem::LoadRecordedEvents()
{
  ...
  while (loader.GetPolymorphic(eventNode))
  {
    ...
    UnitTestEvent* e = LightningAllocate(UnitTestEvent, type);
    e->Serialize(loader);
