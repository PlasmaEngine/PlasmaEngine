...
LightningDefineType(TileMap, builder, type)
{
  PlasmaBindComponent();
...
{
  return (Archetype*)PL::gResources->GetResourceManager(LightningTypeId(Archetype))
...
{
  return (PhysicsMesh*)PL::gResources->GetResourceManager(LightningTypeId(PhysicsMesh))
...
void TileMap::SaveToTileMapSource(Serializer& stream)
{
  ...
  if (PL::gRuntimeEditor)
    mSource = (TileMapSource*)PL::gRuntimeEditor->NewResourceOnWrite(
        TileMapSourceManager::GetInstance(), LightningTypeId(TileMap), "Source", GetSpace(), mSource, archetype, mModified);
...
TileStatus::Enum TileMap::ValidConfiguration(Cog* cog, bool tilemapCollision, bool tilemapSprites)
{
  forRange (Component* component, cog->GetComponents())
  {
    ...
    if (componentType->IsA(LightningTypeId(Collider)))
    {
      // Using tilemap collision and archetype has wrong component.
      if (tilemapCollision && !componentType->IsA(LightningTypeId(MeshCollider)))
