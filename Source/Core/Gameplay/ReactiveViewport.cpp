...
LightningDefineType(ViewportMouseEvent, builder, type)
{
  PlasmaBindDocumented();

  LightningBindFieldProperty(mWorldRay);
  LightningBindFieldProperty(mRayStart);
  LightningBindFieldProperty(mRayDirection);
  LightningBindFieldProperty(mHitPosition);
  LightningBindFieldProperty(mHitNormal);
  LightningBindFieldProperty(mHitUv);
  LightningBindFieldProperty(mHitDistance);
  LightningBindGetterProperty(HitObject);
  LightningBindGetterProperty(CameraViewport);

  LightningBindMethod(ToWorldZPlane);
  LightningBindMethod(ToWorldViewPlane);
  LightningBindMethod(ToWorldPlane);
}
...
LightningDefineType(ReactiveViewport, builder, type)
{
}
...
LightningDefineType(GameWidget, builder, type)
{
}
...
{
  CommandManager::GetInstance()->GetContext()->Remove(LightningTypeId(Space));
