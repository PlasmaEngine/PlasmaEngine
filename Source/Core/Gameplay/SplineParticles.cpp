...
LightningDefineType(SplineParticleEmitter, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindInterface(ParticleEmitterShared);

  LightningBindFieldProperty(mEmitRadius);
  LightningBindFieldProperty(mSpawnT)->Add(new EditorSlider(0, 1, 0.001f));
  LightningBindFieldProperty(mSpawnTVariance)->Add(new EditorSlider(0, 0.5f, 0.001f));
  LightningBindFieldProperty(mClampT);
  LightningBindFieldProperty(mTargetSplineCog);
...
LightningDefineType(SplineParticleAnimator, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDependency(Transform);
  PlasmaBindDependency(ParticleSystem);
  PlasmaBindDependency(SplineParticleEmitter);
  PlasmaBindDocumented();

  LightningBindGetterSetterProperty(Speed);
  LightningBindFieldProperty(mAutoCalculateLifetime);

  LightningBindFieldProperty(mHelix)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindFieldProperty(mHelixRadius)->PlasmaFilterBool(mHelix);
  LightningBindFieldProperty(mHelixWaveLength)->PlasmaFilterBool(mHelix);
  LightningBindFieldProperty(mHelixOffset)->PlasmaFilterBool(mHelix);

  LightningBindFieldProperty(mMode)->AddAttribute(PropertyAttributes::cInvalidatesObject);
  LightningBindFieldProperty(mSpringFrequencyHz)
      ->PlasmaFilterEquality(mMode, SplineAnimatorMode::Enum, SplineAnimatorMode::Spring);
  LightningBindFieldProperty(mSpringDampingRatio)
      ->PlasmaFilterEquality(mMode, SplineAnimatorMode::Enum, SplineAnimatorMode::Spring);
}
