...
LightningDefineType(MouseCapture, builder, type)
{
  PlasmaBindComponent();
  PlasmaBindSetup(SetupMode::DefaultSerialization);
  PlasmaBindDocumented();

  LightningBindMethod(Capture);
  LightningBindOverloadedMethod(ReleaseCapture, LightningInstanceOverload(void));
  LightningBindOverloadedMethod(ReleaseCapture, LightningInstanceOverload(void, ViewportMouseEvent*));
  LightningBindGetter(IsCaptured);

  PlasmaBindEvent(Events::MouseDragStart, ViewportMouseEvent);
  PlasmaBindEvent(Events::MouseDragMove, ViewportMouseEvent);
  PlasmaBindEvent(Events::MouseDragUpdate, ViewportMouseEvent);
  PlasmaBindEvent(Events::MouseDragEnd, ViewportMouseEvent);
}
